...
Функция ПолучитьИтогиЗаПериодКнигаПродаж(СтруктураПараметров) Экспорт
	...
	|		КОНЕЦ КАК НДС0,
	|		ВЫБОР
	|			КОГДА НДСЗаписиКнигиПродажОбороты.СтавкаНДС В (&СтавкаБезНДС)
	|				ТОГДА НДСЗаписиКнигиПродажОбороты.СуммаБезНДСОборот + НДСЗаписиКнигиПродажОбороты.НДСОборот
	|			ИНАЧЕ 0
	...
	СтавкиНДС18.Добавить(Справочники.СтавкиНДС.НДС12);
	...
	СтавкаНДС0 = Справочники.СтавкиНДС.НДС0;
	СтавкаБезНДС = Новый Массив();
	СтавкаБезНДС.Добавить(Справочники.СтавкиНДС.БезНДС);
	СтавкаБезНДС.Добавить(Справочники.СтавкиНДС.БезНДСНеРК);
...
Процедура СформироватьОсновнойРаздел(СтруктураПараметров, Результат);
	...
	Макет = ПолучитьМакет("КнигиПродажРК");
...
Функция ДополнительноеУсловиеЗапроса(СтруктураПараметров, ЗаписьДополнительногоЛиста = Ложь, ИмяТаблицы = "")
	...
	Если СтруктураПараметров.ПрименяетсяОсвобождениеОтУплатыНДС Тогда
		...
		СтрокаУсловия = 
		"([ИмяТаблицы]СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС) И НЕ [ИмяТаблицы]ВидЦенности В (&ВидыЦенностей_НалоговыйАгент)
		|ИЛИ [ИмяТаблицы]СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС) И [ИмяТаблицы]ВидЦенности В (&ВидыЦенностей_НалоговыйАгент))";
	ИначеЕсли НЕ СтруктураПараметров.ПлательщикНДС Тогда
		// Только начисления в качестве налогового агента.
		// Начисления по ставке 18% и 10% в разделе 12.
		СтрокаУсловия = "[ИмяТаблицы]ВидЦенности В (&ВидыЦенностей_НалоговыйАгент) И [ИмяТаблицы]СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)";
	Иначе
...
Функция ТекстЗапросаЗаписиРегистраНДСПродажи(СтруктураПараметров)
	...
	|		КОНЕЦ) КАК НДС0,
	|	СУММА(ВЫБОР
	|			КОГДА НДСЗаписиКнигиПродаж.СтавкаНДС В (&СтавкаБезНДС)
	|				ТОГДА НДСЗаписиКнигиПродаж.СуммаБезНДС + НДСЗаписиКнигиПродаж.НДС
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК СуммаСовсемБезНДС,
