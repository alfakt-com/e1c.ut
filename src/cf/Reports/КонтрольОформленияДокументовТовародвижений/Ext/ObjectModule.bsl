...
Функция ТекстЗапросаПродажи()
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	ТоварыКОтгрузке.ДокументОтгрузки.ЖелаемаяДатаОтгрузки КАК ЖелаемаяДатаОтгрузки,
	               |	ТоварыКОтгрузке.ДокументОтгрузки.СпособДоставки КАК СпособДоставки,
	               |	ТоварыКОтгрузке.ДокументОтгрузки.Комментарий КАК Комментарий,
	               |	ТоварыКОтгрузке.ДокументОтгрузки.Партнер КАК Контрагент,
	               |	ВЫБОР
	               |		КОГДА ТоварыКОтгрузке.ДокументОтгрузки ССЫЛКА Документ.ПеремещениеТоваров
	               |			ТОГДА ТоварыКОтгрузке.ДокументОтгрузки.Ответственный
	               |		ИНАЧЕ ТоварыКОтгрузке.ДокументОтгрузки.Менеджер
	               |	КОНЕЦ КАК Менеджер,
	               |	ЕСТЬNULL(РеализацияТоваровУслуг.Ссылка, """") КАК Реализация
	               |{ВЫБРАТЬ
	               |	Склад.*,
	               |	Номенклатура.*,
	               |	Характеристика.*,
	               ...
	               |	Назначение.*}
	               |ИЗ
	               |	РегистрНакопления.ТоварыКОтгрузке.Остатки(&ПериодГраница, ТИПЗНАЧЕНИЯ(ДокументОтгрузки) В (ТИП(Документ.ЗаказКлиента), ТИП(Документ.РеализацияТоваровУслуг), ТИП(Документ.ЗаявкаНаВозвратТоваровОтКлиента)) {(ДокументОтгрузки).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}) КАК ТоварыКОтгрузке
	               ...
	               |				
	               |				СГРУППИРОВАТЬ ПО
	               |					Infor_ИсторияСтатусовВМС.Объект) КАК ВложенныйЗапрос
	               |				ПО Infor_ИсторияСтатусовВМС.Объект = ВложенныйЗапрос.Объект
	               |					И Infor_ИсторияСтатусовВМС.ДатаИзменения = ВложенныйЗапрос.ДатаИзменения) КАК Infor_СтатусыОбъектов
	               |		ПО ТоварыКОтгрузке.ДокументОтгрузки = Infor_СтатусыОбъектов.Объект
...
Функция ТекстЗапросаВозвратыТоваровОтКлиентов()
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	Infor_СтатусыОбъектов.СтатусОбъекта КАК СтатусОбъекта,
	               |	ТоварыКОформлениюПоступления.ДокументПоступления.ЖелаемаяДатаОтгрузки КАК ЖелаемаяДатаОтгрузки,
	               |	ТоварыКОформлениюПоступления.ДокументПоступления.СпособДоставки КАК СпособДоставки,
	               |	ТоварыКОформлениюПоступления.ДокументПоступления.Комментарий КАК Комментарий,
	               |	ТоварыКОформлениюПоступления.ДокументПоступления.Партнер КАК Контрагент,
	               |	ВЫБОР
	               |		КОГДА ТоварыКОформлениюПоступления.ДокументПоступления ССЫЛКА Документ.ПеремещениеТоваров
	               |			ТОГДА ТоварыКОформлениюПоступления.ДокументПоступления.Ответственный
	               |		ИНАЧЕ ТоварыКОформлениюПоступления.ДокументПоступления.Менеджер
	               |	КОНЕЦ КАК Менеджер,
	               |	ЕСТЬNULL(РеализацияТоваровУслуг.Ссылка, """") КАК Реализация
	               |{ВЫБРАТЬ
	               |	Склад.*,
	               ...
	               |	Назначение.*}
	               |ИЗ
	               |	РегистрНакопления.ТоварыКПоступлению.Остатки(&ПериодГраница, ТИПЗНАЧЕНИЯ(ДокументПоступления) В (ТИП(Документ.ЗаявкаНаВозвратТоваровОтКлиента), ТИП(Документ.ВозвратТоваровОтКлиента)) {(ДокументПоступления).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}) КАК ТоварыКОформлениюПоступления
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыРаспоряжения
	               ...
	               |				ПО Infor_ИсторияСтатусовВМС.Объект = ВложенныйЗапрос.Объект
	               |					И Infor_ИсторияСтатусовВМС.ДатаИзменения = ВложенныйЗапрос.ДатаИзменения) КАК Infor_СтатусыОбъектов
	               |		ПО ТоварыКОформлениюПоступления.ДокументПоступления = Infor_СтатусыОбъектов.Объект
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
	               |		ПО ТоварыКОформлениюПоступления.ДокументПоступления = РеализацияТоваровУслуг.ЗаказКлиента
	               |{ГДЕ
	               |	(&СтрокаВидОперацииВозвратыТоваровОтКлиентов) КАК ВидОперации,
	               |	(ВЫБОР
	               |			КОГДА ТоварыКОформлениюПоступления.КОформлениюПоступленийПоОрдерамОстаток > 0
...
Функция ТекстЗапросаВозвратыТоваровПоставщикам()
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	Назначение.*}
	               |ИЗ
	               |	РегистрНакопления.ТоварыКОтгрузке.Остатки(&ПериодГраница, ТИПЗНАЧЕНИЯ(ДокументОтгрузки) В (ТИП(Документ.ВозвратТоваровПоставщику)) {(ДокументОтгрузки).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}) КАК ТоварыКОтгрузке
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыРаспоряжения
	               |		ПО ТоварыКОтгрузке.ДокументОтгрузки = РеквизитыРаспоряжения.Ссылка
	               |			И (НЕ РеквизитыРаспоряжения.ДополнительнаяЗапись)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
...
Функция ТекстЗапросаВнутреннееПотребление()
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	Назначение.*}
	               |ИЗ
	               |	РегистрНакопления.ТоварыКОтгрузке.Остатки(&ПериодГраница, ТИПЗНАЧЕНИЯ(ДокументОтгрузки) В (ТИП(Документ.ЗаказНаВнутреннееПотребление), ТИП(Документ.ВнутреннееПотреблениеТоваров)) {(ДокументОтгрузки).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}) КАК ТоварыКОтгрузке
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыРаспоряжения
	               |		ПО ТоварыКОтгрузке.ДокументОтгрузки = РеквизитыРаспоряжения.Ссылка
	               |			И (НЕ РеквизитыРаспоряжения.ДополнительнаяЗапись)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
...
Функция ТекстЗапросаПеремещениеТоваров()
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	ЕСТЬNULL(РеализацияТоваровУслуг.Ссылка, """") КАК Реализация
	               |ИЗ
	               |	РегистрНакопления.ТоварыКОтгрузке.Остатки(&ПериодГраница, ТИПЗНАЧЕНИЯ(ДокументОтгрузки) В (ТИП(Документ.ЗаказНаПеремещение), ТИП(Документ.ПеремещениеТоваров)) {(ДокументОтгрузки).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}) КАК ТоварыКОтгрузке
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыРаспоряжения
	               |		ПО ТоварыКОтгрузке.ДокументОтгрузки = РеквизитыРаспоряжения.Ссылка
	               |			И (НЕ РеквизитыРаспоряжения.ДополнительнаяЗапись)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	               ...
	               |	ТоварыКОформлениюПоступления.ДокументПоступления.ЖелаемаяДатаОтгрузки,
	               |	ТоварыКОформлениюПоступления.ДокументПоступления.СпособДоставки,
	               |	ТоварыКОформлениюПоступления.ДокументПоступления.Комментарий,
	               |	ТоварыКОформлениюПоступления.ДокументПоступления.Партнер,
	               |	ВЫБОР
	               |		КОГДА ТоварыКОформлениюПоступления.ДокументПоступления ССЫЛКА Документ.ПеремещениеТоваров
	               |			ТОГДА ТоварыКОформлениюПоступления.ДокументПоступления.Ответственный
	               |		ИНАЧЕ ТоварыКОформлениюПоступления.ДокументПоступления.Менеджер
	               ...
	               |ИЗ
	               |	РегистрНакопления.ТоварыКПоступлению.Остатки(&ПериодГраница, ТИПЗНАЧЕНИЯ(ДокументПоступления) В (ТИП(Документ.ЗаказНаПеремещение), ТИП(Документ.ПеремещениеТоваров)) {(ДокументПоступления).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}) КАК ТоварыКОформлениюПоступления
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыРаспоряжения
	               |		ПО ТоварыКОформлениюПоступления.ДокументПоступления = РеквизитыРаспоряжения.Ссылка
	               |			И (НЕ РеквизитыРаспоряжения.ДополнительнаяЗапись)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
...
Функция ТекстЗапросаСборкаРазборка()
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	Назначение.*}
	               |ИЗ
	               |	РегистрНакопления.ТоварыКОтгрузке.Остатки(&ПериодГраница, ТИПЗНАЧЕНИЯ(ДокументОтгрузки) В (ТИП(Документ.ЗаказНаСборку), ТИП(Документ.СборкаТоваров)) {(ДокументОтгрузки).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}) КАК ТоварыКОтгрузке
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыРаспоряжения
	               |		ПО ТоварыКОтгрузке.ДокументОтгрузки = РеквизитыРаспоряжения.Ссылка
	               |			И (НЕ РеквизитыРаспоряжения.ДополнительнаяЗапись)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	               ...
	               |	Назначение.*}
	               |ИЗ
	               |	РегистрНакопления.ТоварыКПоступлению.Остатки(&ПериодГраница, ТИПЗНАЧЕНИЯ(ДокументПоступления) В (ТИП(Документ.ЗаказНаСборку), ТИП(Документ.СборкаТоваров)) {(ДокументПоступления).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}) КАК ТоварыКОформлениюПоступления
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыРаспоряжения
	               |		ПО ТоварыКОформлениюПоступления.ДокументПоступления = РеквизитыРаспоряжения.Ссылка
	               |			И (НЕ РеквизитыРаспоряжения.ДополнительнаяЗапись)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
...
Функция ТекстЗапросаПрочиеОприходования()
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	Назначение.*}
	               |ИЗ
	               |	РегистрНакопления.ТоварыКПоступлению.Остатки(&ПериодГраница, ТИПЗНАЧЕНИЯ(ДокументПоступления) В (ТИП(Документ.ПрочееОприходованиеТоваров)) {(ДокументПоступления).* КАК Распоряжение, (Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}) КАК ТоварыКОформлениюПоступления
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыРаспоряжения
	               |		ПО ТоварыКОформлениюПоступления.ДокументПоступления = РеквизитыРаспоряжения.Ссылка
	               |			И (НЕ РеквизитыРаспоряжения.ДополнительнаяЗапись)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
...
Функция ТекстЗапросаОформлениеИзлишковНедостач()
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	НедоофомленныеИзлишкиИНедостачи.Серия КАК Серия,
	               |	НедоофомленныеИзлишкиИНедостачи.Назначение КАК Назначение,
	               |	"""" КАК СтатусОбъекта,
	               |	ДАТАВРЕМЯ(1, 1, 1) КАК ЖелаемаяДатаОтгрузки,
	               |	ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПустаяСсылка) КАК СпособДоставки,
	               |	"""" КАК Комментарий,
	               |	ЗНАЧЕНИЕ(Справочник.Партнеры.пустаяСсылка) КАК Контрагент,
	               ...
	               |	Назначение.*}
	               |ИЗ
	               |	РегистрНакопления.ТоварыКОформлениюИзлишковНедостач.Остатки(&ПериодГраница, {(Склад).*, (Номенклатура).*, (Характеристика).*, (Серия).*, (Назначение).*}) КАК НедоофомленныеИзлишкиИНедостачи
	               |{ГДЕ
	               |	(&СтрокаВидОперацииОформлениеИзлишковИНедостач) КАК ВидОперации,
	               |	(&СтрокаТипОтклоненияОформлениеФинансовыхДокументов) КАК ТипОтклонения,
	               |	(ВЫБОР
