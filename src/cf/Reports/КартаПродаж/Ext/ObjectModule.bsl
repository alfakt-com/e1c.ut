...
Процедура ВывестиСхему(ДокументРезультат, ТаблицаДанных)
	...
	ВыбиратьГорода = Ложь;
	Если (Схема.Слои.Найти("Регионы России") <> Неопределено ИЛИ Схема.Слои.Найти("Регионы Казахстана") <> Неопределено)
...
Процедура ЗаполнитьСхему(Схема, ТаблицаДанных)
	...
	Если ПараметрВариантАнализа.Значение = 0 Тогда // Выручка за период отчета
		...
		ЭлементЛегенды.ТекстПодписи = НСтр("ru = 'Выручка, тенге'");
