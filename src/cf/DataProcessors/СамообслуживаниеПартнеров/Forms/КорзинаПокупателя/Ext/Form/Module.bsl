...
Функция ТекстЗапросаДанныеКорзиныПоРегистру()
	...
	|		КОГДА &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|			ТОГДА ВЫБОР
	|					КОГДА ЕстьNULL(СпрНоменклатура.СтавкаНДС, ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
...
Процедура УстановитьРеквизитыФормыПоВыборке(Выборка)
	...
	Если НЕ Выборка = Неопределено Тогда
		...
		НалогообложениеНДС = ЗначениеНастроекПовтИсп.НалогообложениеНДС(Выборка.Организация, Выборка.Склад, , , ТекущаяДатаСеанса());
	Иначе
...
Процедура ПриИзмененииСоглашенияСервер(РезультатВыбора)
	...
	НалогообложениеНДС = ЗначениеНастроекПовтИсп.НалогообложениеНДС(РезультатВыбора.Организация, РезультатВыбора.Склад, , , ТекущаяДатаСеанса());
...
Процедура ОбработатьДобавлениеТоваров(ТаблицаТоваров)
	...
	Запрос.Текст = Обработки.СамообслуживаниеПартнеров.ВременныеТаблицыДляРасчетаДоступныхОстатков("ВтВсеТоварыИСклады")
		...
		|		КОГДА &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
		|			ТОГДА ВЫБОР
		|					КОГДА СпрНоменклатура.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС12)
		|						ТОГДА 0.12
