...
Функция СформироватьПечатнуюФормуИНВ19(СтруктураТипов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		Если СтруктураОбъектов.Ключ <> "Документ.ИнвентаризационнаяОпись" Тогда 
			ТекстСообщения = НСтр("ru = 'Формирование печатной формы ""ИНВ-18"" доступно только для документов ""%ТипДокумента%"".'");
...
Процедура ЗаполнитьТабличныйДокументИНВ19(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ИнвентаризационнаяОпись) //Айнура 210422
	...
	Если ТаблицаТоваров.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='В рамках периода документа ""%Опись%"" не были оформлены складские акты. Нет данных для формирования печатной формы ""ИНВ-18""'");
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Обработка.ПечатьИНВ19.ПФ_MXL_ИНВ20");
	...
	ОбластьЗаголовок.Параметры.ДатаСнятияОстатков = Формат(Шапка.ДатаСнятияОстатков, "ДФ='dd.MM.yyyy'' года'''");
	ОбластьЗаголовок.Параметры.ОснованиеДата  = Формат(Шапка.ОснованиеДата, "ДФ='dd.MM.yyyy'' года'''");
