...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ТОРГ2") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"ТОРГ2",
			НСтр("ru='Акт о расхождениях при приемке товаров'"),
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "А1") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"А1",
			НСтр("ru='Акт о приемке активов (А-1)'"),
			СформироватьПечатнуюФормуА1(СтруктураТипов, ОбъектыПечати, ПараметрыПечати));
...
Процедура ЗаполнитьТабличныйДокументТорг2(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПечатьВНоменклатуреПоставщкика)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Область.Параметры.Заполнить(ДанныеПечати);
		Область.Параметры.НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.НомерДокумента, Ложь, Истина);
		Область.Параметры.ПредставлениеОрганизации = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование,ЮридическийАдрес,Телефоны,");
		Область.Параметры.Поставщик = ФормированиеПечатныхФорм.ОписаниеОрганизации(ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Поставщик, ДанныеПечати.ДатаДокумента), "ПолноеНаименование,ЮридическийАдрес,Телефоны,");
		Область.Параметры.ОрганизацияБИН = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "ИНН,", Ложь);
...
Функция СформироватьПечатнуюФормуА1(СтруктураТипов, ОбъектыПечати, ПараметрыПечати)
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.АвтоМасштаб = Истина;
	УстановитьПривилегированныйРежим(Истина);
	
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_А1";
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		ДанныеДляПечати = МенеджерОбъекта.ПолучитьДанныеДляПечатнойФормыА1(ПараметрыПечати, СтруктураОбъектов.Значение);
		
		ЗаполнитьТабличныйДокументА1(
			ТабличныйДокумент,
			ДанныеДляПечати,
			ОбъектыПечати);
...
Процедура ЗаполнитьТабличныйДокументА1(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати)
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Обработка.ПечатьАктыОРасхождениях.ПФ_MXL_А1");
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		СведенияОбОрганизации = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Поставщик, ДанныеПечати.ДатаДокумента);
		СведенияОПолучателе  = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Организация, ДанныеПечати.ДатаДокумента);
		
		Область = Макет.ПолучитьОбласть("Шапка");
		Область.Параметры.Заполнить(ДанныеПечати);
		
		ПараметрыШапки.Вставить("ОрганизацияБИН", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПолучателе, "ИНН,", Ложь));
		ПараметрыШапки.Вставить("НомерДокумента", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеПечати.НомерДокумента, Ложь, Истина));
		ПараметрыШапки.Вставить("ПредставлениеОрганизации", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПолучателе, "ПолноеНаименование,"));
		ПараметрыШапки.Вставить("ПредставлениеОрганизацииПолучатель", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПолучателе, "ПолноеНаименование,ЮридическийАдрес,Телефоны,"));
		ПараметрыШапки.Вставить("Производитель", ДанныеПечати.Производитель);
		ПараметрыШапки.Вставить("Поставщик", ФормированиеПечатныхФорм.ОписаниеОрганизации(ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Поставщик, ДанныеПечати.ДатаДокумента), "ПолноеНаименование,ЮридическийАдрес,Телефоны,"));
		Область.Параметры.Заполнить(ПараметрыПоДате("ДатаДоставкиТоваров", ДанныеПечати.ДатаДоставкиТоваров));
		Область.Параметры.Заполнить(ПараметрыПоДате("ДатаОтправленияТоваров", ДанныеПечати.ДатаОтправленияТоваров));
		Область.Параметры.Заполнить(ПараметрыПоДате("ДатаДоговораПоставки", ДанныеПечати.ДатаДоговораПоставки));
		ДанныеСчетовФактур.Сбросить();
		Если ДанныеСчетовФактур.НайтиСледующий(Новый Структура("ДокументОснование", ДанныеПечати.Ссылка)) Тогда
			Область.Параметры.Заполнить(ДанныеСчетовФактур);
			ПараметрыШапки.Вставить("НомерСчетаФактуры", ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ДанныеСчетовФактур.НомерСчетаФактуры));
			Область.Параметры.Заполнить(ПараметрыПоДате("ДатаСчетаФактуры", ДанныеСчетовФактур.ДатаСчетаФактуры));
		КонецЕсли;
		ПараметрыШапки.Вставить("РуководительФИО", ДанныеПечати.Руководитель);
		...
		// Вывод таблицы приемки товаров
		Область = Макет.ПолучитьОбласть("ЗаголовокТаблицыПоДокументам");
		ТабличныйДокумент.Вывести(Область);
		ТаблицаПриемкиТоваров = ДанныеПечати.ТаблицаПриемкиТоваров.Выгрузить();
		
		Область = Макет.ПолучитьОбласть("СтрокаТаблицыДокументы");
		...
		// Вывод таблицы приемки товаров
		Область = Макет.ПолучитьОбласть("ЗаголовокТаблицыДатаВремя");
		ТабличныйДокумент.Вывести(Область);

		Область = Макет.ПолучитьОбласть("СтрокаТаблицыДатаВремя");
		Для Каждого Строка Из ТаблицаПриемкиТоваров Цикл
			Область.Параметры.Заполнить(Строка);
			ТабличныйДокумент.Вывести(Область);
		КонецЦикла;
		
		Область = Макет.ПолучитьОбласть("ДополнительныеДанные");
		Область.Параметры.Заполнить(ДанныеПечати);
		ТабличныйДокумент.Вывести(Область);
		...
		Область = Макет.ПолучитьОбласть("ЗаголовокТаблицы3");
		ТабличныйДокумент.Вывести(Область);
		
		Область = Макет.ПолучитьОбласть("СтрокаТаблицы3");
		ПараметрыТаблицы3 = Новый Структура;
		ПараметрыТаблицы3.Вставить("Поставщик", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование,"));
		ПараметрыТаблицы3.Вставить("СкладОтправителяТовара", ДанныеПечати.СкладОтправителяТовара);
		ПараметрыТаблицы3.Вставить("СтруктурнаяЕдиница", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПолучателе, "ПолноеНаименование,"));
		Область.Параметры.Заполнить(ПараметрыТаблицы3);
		ТабличныйДокумент.Вывести(Область);
		
		Область = Макет.ПолучитьОбласть("ЗаголовокТаблицыНоменклатура");
		ПараметрыЗаголовкаТаблицаНом = Новый Структура("Валюта", Константы.ВалютаРегламентированногоУчета.Получить());
		Область.Параметры.Заполнить(ПараметрыЗаголовкаТаблицаНом);
		ТабличныйДокумент.Вывести(Область);
		
		Область = Макет.ПолучитьОбласть("СтрокаТаблицыНоменклатура");
		ПараметрыСтрокиТовары = Новый Структура;
		Для Каждого Товар Из Товары Цикл
			...
			Область.Параметры.Заполнить(Товар);
			ПараметрыСтрокиТовары.Вставить("Товар", НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(
				Товар.НоменклатураНаименование,
				Товар.ХарактеристикаНаименование));
			ПараметрыСтрокиТовары.Вставить("ЗначениеКода", Товар[КолонкаКодов]);
			
			Если Товар.КоличествоОтклонение > 0 Тогда 
				ПараметрыСтрокиТовары.Вставить("ИзлишкиКоличество", Товар.КоличествоОтклонение);
			ИначеЕсли Товар.КоличествоОтклонение < 0 Тогда 
				ПараметрыСтрокиТовары.Вставить("НедостачаКоличество", Товар.КоличествоОтклонение * -1);
			КонецЕсли;
			
			Если Товар.СуммаОтклонение > 0 Тогда 
				ПараметрыСтрокиТовары.Вставить("ИзлишкиСумма", Товар.СуммаОтклонение);
			ИначеЕсли Товар.КоличествоОтклонение < 0 Тогда 
				ПараметрыСтрокиТовары.Вставить("НедостачаСумма", Товар.СуммаОтклонение * -1);
			КонецЕсли;
			
			Область.Параметры.Заполнить(ПараметрыСтрокиТовары);
			ТабличныйДокумент.Вывести(Область);
			
			ПараметрыСтрокиТовары.Очистить();
		...
		Область.Параметры.Заполнить(ДанныеПечати);
		Область.Параметры.Заполнить(ПараметрыПоДате("ДатаПринятияИОприходования", ДанныеПечати.ДатаДокумента));
		
		// Вывод кладовщика
		ПараметрыПодвал = Новый Структура;
		ПараметрыПодвал.Вставить("КладовщикПринявшийТоварФИО", ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(ДанныеПечати.КладовщикПринявшийТовар));
		Область.Параметры.Заполнить(ПараметрыПодвал);
		ТабличныйДокумент.Вывести(Область);
