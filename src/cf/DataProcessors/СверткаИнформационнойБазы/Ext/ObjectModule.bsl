...
Процедура СформироватьДокументыВводаОстатков(ПовторныйЗапуск = Ложь) Экспорт
	...
	// 2.11 Планирование
	ОбработатьДокументыПланирования("ПланЗакупок");
	ОбработатьДокументыПланирования("ПланОстатков");
	ОбработатьДокументыПланирования("ПланПродаж");
	ОбработатьДокументыПланирования("ПланПродажПоКатегориям");
	ОбработатьДокументыПланирования("ПланСборкиРазборки");
						...
						Пока Выборка.Следующий() Цикл
							...
							Если НЕ Выборка.ИспользоватьСкладскиеПомещения ИЛИ ВыбранныеВариантыПомещений.Количество() = 0 Тогда
								...
								Если НоваяСтрока.СуммаНДС > 0 Тогда
									НоваяСтрока.СтавкаНДС = Справочники.СтавкиНДС.НДС12;
...
Процедура ОбработатьДокументыПланирования(ИмяДокументаПланирования)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	                |	ДокументПланирования.Ссылка КАК ДокументСсылка
	                |ИЗ
	                |	&ДокументПланирования КАК ДокументПланирования
	                |ГДЕ
	                |	ДокументПланирования.Проведен
	                |	И ДокументПланирования.Дата <= &ДатаОст
	                |	И ДокументПланирования.ОкончаниеПериода > &ДатаОст";
		
	Запрос.Текст = СтрЗаменить(Запрос.Текст , "&ДокументПланирования", "Документ." + ИмяДокументаПланирования);
	...
	Выборка = Результат.Выбрать();
	Пока Выборка.Следующий() Цикл
		ДокОст = Выборка.ДокументСсылка.ПолучитьОбъект();
		Если ДокОст = Неопределено тогда
			Продолжить;
		КонецЕсли;

		ТекстКомментария = НСтр("ru='[Перенесен обработкой свертки базы][Был %1]'");
		ТекстКомментария = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстКомментария, СокрЛП(Выборка.ДокументСсылка));
		ДокОст.Комментарий = ТекстКомментария;
		ДокОст.Дата        = КонецДня(ДатаСверткиИБ) + 1;
		ДокОст.Записать(РежимЗаписиДокумента.Запись);
	КонецЦикла;
	
	СообщениеСвертки(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Обработаны документы %1'"), Метаданные.Документы[ИмяДокументаПланирования].Синоним));		
