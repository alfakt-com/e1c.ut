...
Функция КоличествоКЗаказуДляПоддержанияЗапаса(Товар) Экспорт
	...
	ВосполнениеСтраховогоЗапаса = Мин(Товар.СтраховойЗапас, Макс(0, -ДоТочкиЗаказа - Товар.ЗаказыКпоступлению));
...
Функция СформироватьТекстЗапросаСтатистикиПотребления(СоставРазделовЗапроса = Неопределено) Экспорт
	...
	Если ВыполнятьРасчетПоКоличествуПродаж Тогда
		...
		// (движение по складу в этот день быть обязано, из него и берем признак наличия остатков на начало и конец дня
		// получения выручки).
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Функция ТаблицаПотребностейПоЗаказамИОстатков(Отбор, ТипОтбора, ТаблицаСпособовОбеспечения = Неопределено, ТаблицаЗапасы = Неопределено) Экспорт
	...
	Если ТипОтбора = "ОтборПоЗначениям" И Отбор.Заказ.Метаданные().Реквизиты.Найти("Приоритет") = Неопределено Тогда
		ТекстПриоритет = "NULL";
	КонецЕсли;
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПриоритетУсловие", ТекстПриоритет);
...
Функция ТекстПолейУсловияЗакупок()
	...
	Текст =
		...
		|								ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК),
		|								ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)) ТОГДА
		|							1
		|						ИНАЧЕ
		|							1 + Т.Номенклатура.СтавкаНДС.Ставка / 100
		|					КОНЕЦ
		|			ИНАЧЕ
		|				1
		|		КОНЕЦ
