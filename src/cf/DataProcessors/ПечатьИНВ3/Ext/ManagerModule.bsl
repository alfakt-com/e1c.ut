...
Функция СформироватьПечатнуюФормуИНВ3(СтруктураТипов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		Если СтруктураОбъектов.Ключ <> "Документ.ИнвентаризационнаяОпись" Тогда 
			ТекстСообщения = НСтр("ru = 'Формирование печатной формы ""ИНВ-10"" доступно только для документов ""%ТипДокумента%"".'");
...
Процедура ЗаполнитьТабличныйДокументИНВ3(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ИнвентаризационнаяОпись)
	...
	Если ТабличнаяЧасть.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='В рамках периода документа ""%Опись%"" не были оформлены складские акты или не было проведено ни одного пересчета товаров. Нет данных для формирования печатной формы ""ИНВ-10""'");
	...
	ОбластьПерваяСтрока  = Макет.ПолучитьОбласть("ПерваяСтрокаОборотнойСтороны");
	...
	ОбластьЗаголовок.Параметры.СписокСчетов       = "______________";
	//ОбластьЗаголовок.Параметры.ОснованиеДата      = ?(ЗначениеЗаполнено(Шапка.ОснованиеДата),      Формат(Шапка.ОснованиеДата,     "ДФ=""дд ММММ гггг 'года'"""), """____""_______________20____года");
	ОбластьЗаголовок.Параметры.ДатаНачала         = ?(ЗначениеЗаполнено(Шапка.ДатаНачала),         Формат(Шапка.ДатаНачала,        "ДФ=""дд ММММ гггг 'года'"""), """____""_______________20____года");
	ОбластьЗаголовок.Параметры.ДатаОкончания      = ?(ЗначениеЗаполнено(Шапка.ДатаОкончания),      Формат(Шапка.ДатаОкончания,     "ДФ=""дд ММММ гггг 'года'"""), """____""_______________20____года");
	ОбластьЗаголовок.Параметры.ДатаСнятияОстатков = ?(ЗначениеЗаполнено(Шапка.ДатаСнятияОстатков), Формат(Шапка.ДатаСнятияОстатков,"ДФ=""дд ММММ гггг 'года'"""), """____""_______________20____года");
	...
	Пока ТабличнаяЧасть.Следующий() Цикл
		...
		Если НомерСтроки = 1 Тогда // первая строка
			...
			ТекстСтраница = НСтр("ru='Оборотная сторона формы Инв-10" + Символы.ПС + "Страница %НомерСтраницы%'");
			ТекстСтраница = СтрЗаменить(ТекстСтраница,"%НомерСтраницы%",НомерСтраницы);
			ОбластьНомерСтраницы.Параметры.НомерСтраницы = ТекстСтраница;
			ТабличныйДокумент.Вывести(ОбластьНомерСтраницы);
			ТабличныйДокумент.Вывести(ОбластьПерваяСтрока);

			ОбластьШапки.Параметры.Валюта = СокрЛП(ВалютаРеглУчета);
	...
	МассивВыводимыхОбластей.Очистить();
	МассивВыводимыхОбластей.Добавить(ОбластьПодвалОписи);
	...
	ОбластьПодвалОписи.Параметры.Валюта                                  = ВалютаРеглУчета;
