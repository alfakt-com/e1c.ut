...
Функция ЗагрузитьКурсыВалютПоПараметрамРК(Знач Валюты, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки, ПриЗагрузкеВозниклиОшибки = Ложь)
	...
	Пока ТекущийПериодЗагрузки <= ОкончаниеПериодаЗагрузки Цикл
		...
		ФайлНаВебСервере = "http://" + СерверИсточник + "/" + Адрес + ТМП;  ВалютыЗагружаемыеИзИнтернета = ВалютыЗагружаемыеИзИнтернета();
	...
	Если ОбъектXDTO.Свойства().Получить("item") <> Неопределено Тогда
		...
		Для Каждого Валюта Из Валюты Цикл
			...
			Если НайденнаяСтрокаКурса <> Неопределено Тогда
				//Отбор для загрузки регламентным заданием
				Если ТипЗнч(Валюта) <> Тип("Структура") Тогда
