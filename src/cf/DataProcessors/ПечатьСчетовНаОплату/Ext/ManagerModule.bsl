...
Процедура ЗаполнитьРеквизитыШапкиСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, ТаблицаТовары)
	...
	Если ДанныеПечати.ПлатежЗаРубеж Тогда
		...
		КонецЕсли;
		СтруктураДанныхШапки.Вставить("КодНазначенияПлатежа", ДанныеПечати.КодНазначенияПлатежа);
		СтруктураДанныхШапки.Вставить("ПолучательКБЕ", СведенияОПоставщике.КБЕ);
		
		//Начало: МК Баубериков А - 05.08.2020
		ПредставлениеПоставщикаДляПлатПоручения = ПредставлениеПоставщикаДляПлатПоручения + ", Договор: "+ Строка(ДанныеПечати.Ссылка.Договор);
		//Конец: МК Баубериков А - 05.08.2020
	...
	Иначе
		...
		СтруктураДанныхШапки.Вставить("ПоставщикКБЕ", СведенияОПоставщике.КБЕ);
		СтруктураДанныхШапки.Вставить("КодНазначенияПлатежа", "");
		
		СтруктураКодНазначенияПлатежа = Новый Структура("КодНазначенияПлатежа");
		ЗаполнитьЗначенияСвойств(СтруктураКодНазначенияПлатежа, ДанныеПечати);
		ЗаполнитьЗначенияСвойств(СтруктураДанныхШапки, СтруктураКодНазначенияПлатежа);
	...
	//Начало: МК Баубериков А - 05.08.2020
	ПредставлениеПоставщикаДляПлатПоручения = ФормированиеПечатныхФорм.ОписаниеОрганизации(ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.ОрганизацияПоставщик, ДанныеПечати.Дата),
			"ПолноеНаименование,ИНН,ЮридическийАдрес,Телефоны,");
	ПредставлениеПоставщикаДляПлатПоручения = ПредставлениеПоставщикаДляПлатПоручения + ", Договор: "+Строка(ДанныеПечати.Ссылка.Договор);
	СтруктураДанныхПоставщик.Вставить("ПредставлениеПоставщика", ПредставлениеПоставщикаДляПлатПоручения);
	//Конец: МК Баубериков А - 05.08.2020
...
Функция ПечатьПредварительныйПросмотрСчетаНаОплату(Организация, ОбъектыПечати, ИмяМакета)
	...
	СтруктураДанныхПокупатель = Новый Структура;
	СтруктураДанныхПокупатель.Вставить("ПредставлениеПолучателя", 
		НСтр("ru = 'Поле содержит информацию покупателя: полное наименование, ИИН/БИН, юридический адрес, телефоны.'"));
	ОбластьМакета.Параметры.Заполнить(СтруктураДанныхПокупатель);
	ТабличныйДокумент.Вывести(ОбластьМакета);
	...
	Товары.Вставить("Цена",             112.00);
	Товары.Вставить("СтавкаНДС",        "12%");
	Товары.Вставить("СуммаНДС",         12.00);
	Товары.Вставить("Сумма",            112.00);
	...
	ОбластьМакета.Параметры.Заполнить(Новый Структура("НДС, ВсегоНДС", НСтр("ru = 'В т.ч. НДС (12%)'"), Товары.СуммаНДС));
	ТабличныйДокумент.Вывести(ОбластьМакета);
...
Функция ОписаниеОрганизации(ДанныеПечати, ИмяОрганизации)
	...
	РеквизитыОрганизации = "ПолноеНаименование,ИНН,ЮридическийАдрес,Телефоны,";
