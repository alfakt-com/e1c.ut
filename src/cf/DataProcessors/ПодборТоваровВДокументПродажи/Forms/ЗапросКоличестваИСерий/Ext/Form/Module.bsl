...
Процедура УстановитьУсловноеОформление()
	...
	//++Аян_К++ №5974 010922 старт
	Если ТипЗнч(Параметры.Регистратор) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		Если НЕ Альфа_КТ.ПроверитьДоступ("ИсключениеНаОтгрузку", ПараметрыСеанса.ТекущийПользователь, ТекущаяДата(), ТекущаяДата()) Тогда
			Если ПроверитьСкладОтгрузки(Параметры.Склад) Тогда 
				...
				ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТаблицаДоступныхДействийТекущая.ВариантОбеспечения");
				ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
				ОтборЭлемента.ПравоеЗначение = ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.Отгрузить");
				
				Элемент.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	//++Аян_К++ №5974 010922 финиш
	
КонецПроцедуры

Функция ПроверитьСкладОтгрузки(пСклад)  //++Аян_К++ №5974 010922
