...
Процедура КорзинаНоменклатураПриИзменении(Элемент)
	...
	Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(ДокументСсылка, "swУсловияОплаты") Тогда
		Если ПолучитьЗначениеРеквизита(ДокументСсылка, "swУсловияОплаты") = ПредопределенноеЗначение("Перечисление.swУсловияОплаты.НаличныйРасчет") тогда   
...
Функция ЗаполнитьЦеныПоСоглашениюСервер(ПоВсемСтрокам = Ложь)
	...
	ПараметрыЗаполненияЦен.Вставить("НалогообложениеНДС", НалогообложениеНДС);
