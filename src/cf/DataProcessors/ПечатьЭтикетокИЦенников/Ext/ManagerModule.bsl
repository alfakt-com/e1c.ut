...
Функция ПодготовитьСтруктуруДанных(СтруктураНастроек, Режим) Экспорт
	...
	Если Режим = "Товары" Тогда
		...
		Если ИсходныеДанные <> Неопределено Тогда
			...
			Запрос.Текст = СтрЗаменить(Запрос.Текст, """QRКод""", "ИсходныеДанные.QRКод");
...
#Область ФормированиеТабличногоДокумента
	...
	КоллекцияПечатныхФорм.Колонки.Добавить("ПроцентСкидки");
		...
		Для Каждого СтрокаТовары Из СтруктураРезультата.Таблица Цикл
			...
			Если СтрокаТовары[ПечатнаяФорма.ИмяКолонкиКоличество] > 0 И СтрокаТовары[ПечатнаяФорма.ИмяКолонкиШаблон] = ПечатнаяФорма.Шаблон Тогда
				...
				Для каждого Рисунок Из Область.Рисунки Цикл
					...
					Если СтрНайти(Рисунок.Имя, Справочники.ШаблоныЭтикетокИЦенников.ИмяПараметраQRкод()) = 1 Тогда
						ЗначениеQRКода = СтрокаТовары[СтруктураРезультата.СоответствиеПолейСКДКолонкамТаблицы.Получить(Справочники.ШаблоныЭтикетокИЦенников.ИмяПараметраQRкод())];
						Если ЗначениеЗаполнено(ЗначениеQRКода) Тогда
													
							Рисунок.Картинка = МенеджерОборудованияВызовСервера.ПолучитьКартинкуQRкода(ЗначениеQRКода);
...
Процедура ЗаполнитьСвойстваСтрокиШтрихкодовУпаковокПоШтрихкоду(Строка, Штрихкод) ЭКспорт
	...
	Если НЕ ПараметрыШтрихкода.Результат = Неопределено Тогда
		...
		Если ПараметрыШтрихкода.ТипШтрихкода = Перечисления.ТипыШтрихкодов.SSCC Тогда
			...
			Строка.СерийныйНомер            = Формат(СвойстваШтрихкода.СерийныйСсылочныйНомер, "ЧГ=0");
		...
		ИначеЕсли ПараметрыШтрихкода.ТипШтрихкода = Перечисления.ТипыШтрихкодов.Code128 Тогда
			...
			Строка.ИдентификаторОрганизации = Формат(СвойстваШтрихкода.КодФСРАР, "ЧГ=0");
			Строка.ДатаМаркировки           = СвойстваШтрихкода.ДатаМаркировки;
			Строка.СерийныйНомер            = Формат(СвойстваШтрихкода.ПорядковыйНомерВТеченииДня, "ЧГ=0");
