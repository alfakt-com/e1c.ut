...
Процедура ТоварыУпаковкаПриИзменении(Элемент)
	...
	ТекущиеДанные = Новый Структура("Номенклатура, Характеристика, Упаковка, СтараяУпаковка, СтароеКоличествоВДокументе, КоличествоЭтикеток, ПроцентСкидки");
	...
	Если ЗначениеЗаполнено(ТекДанные.Цена) И ЗначениеЗаполнено(ТекДанные.ЦенаДополнительно) Тогда //++Аян_К++ 281021
		ТекущиеДанные.ПроцентСкидки 					= (ТекДанные.Цена * 100) / ТекДанные.ЦенаДополнительно;
	КонецЕсли;
	
	ЗаполнитьЗначенияСвойств(ТекДанные, ПриИзмененииНоменклатуры(ТекущиеДанные));
...
Функция ПолучитьПараметры()
	...
	ТЗТовары.Свернуть("ШаблонЦенника");
	НуженQR = Ложь;
	Для Каждого СтрТовар Из ТЗТовары Цикл
		ПараметрыШаблона = СтрТовар.ШаблонЦенника.Шаблон.Получить().ПараметрыШаблона;
		Для Каждого Элемент Из ПараметрыШаблона Цикл
			Если Элемент.Ключ = "QRКод" Тогда
				НуженQR = Истина;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	...
	Если НуженQR Тогда
		ДанныеДляQR = Новый ТаблицаЗначений;
		ДанныеДляQR = ПолучитьДанныеДляQR();
	КонецЕсли;
	...
	Товары.Колонки.Добавить("QRКод", Новый ОписаниеТипов("Строка"));
	...
	Для Каждого СтрокаТЧ Из Объект.Товары Цикл
		...
		Если ДанныеДляQR <> Неопределено Тогда
			НайдСсылка = ДанныеДляQR.НайтиСтроки(Новый Структура("НоменклатураСсылка", СтрокаТЧ.Номенклатура));
			Если НайдСсылка.Количество() <> 0 Тогда
				НоваяСтрока.QRКод = НайдСсылка[0].СсылкаQR;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьТаблицуТоваровНаСервере(СохранятьШаблоныИКоличество, ОбновлениеДанныхПоВыбраннымТоварам = Ложь)
	...
	Для Каждого СтрокаТЧ Из СтруктураРезультата.Таблица Цикл
		...
		//Если Объект.ВидЦены <> Справочники.ВидыЦен.НайтиПоНаименованию("Розничная") ИЛИ Объект.ВидЦены <> Справочники.ВидыЦен.НайтиПоНаименованию(" излишки") 
		//	Или Объект.ВидЦены <> Справочники.ВидыЦен.НайтиПоНаименованию(" излишки") Тогда //++Аян_К++ 281021
		//	Если ЗначениеЗаполнено(СтрокаТЧ.Цена) И ЗначениеЗаполнено(СтрокаТЧ.ЦенаДополнительно) Тогда  
		//		НоваяСтрока.ПроцентСкидки                            = (СтрокаТЧ.Цена * 100) / СтрокаТЧ.ЦенаДополнительно;
		//	КонецЕсли;
		//КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого Параметр Из ПараметрыМакета.ПараметрыШаблона Цикл
			...
			Если СтрНайти(Параметр.Ключ, "ВидЦеныДополнительно") > 0 Тогда
				...
				КонецЕсли;
			КонецЕсли;
			Если СтрНайти(Параметр.Ключ, "ПроцентСкидки") > 0 Тогда
				Если ШаблоныВлияющиеНаВидимостьЭлементов.ВидЦеныДополнительно.Найти(Выборка.Ссылка) = Неопределено Тогда
					ШаблоныВлияющиеНаВидимостьЭлементов.ВидЦеныДополнительно.Добавить(Выборка.Ссылка);
				КонецЕсли;
			КонецЕсли;
...
///Сергей 09.11.2020г +++ простановка новых цен согластно акций
#Область Заполнение_цен_по_акциям

&НаКлиенте
Процедура ЗаполнитьЦеныПоАкциям(Команда)
	
	///+++Проверка на заполнение нужных пораметров+++
	Если Объект.Товары.Количество() = 0 Тогда 
		Сообщить("Табличная часть Товаров пустая!!!");
		Возврат;	
	КонецЕсли;
	
	Если Объект.РежимПолученияЦен = Число(1) Тогда 
		Если НЕ ЗначениеЗаполнено(Объект.ВидЦены) Тогда
			Сообщить("Не заполнен вид Цены!!!");
			Возврат;
		КонецЕсли;
	Иначе
		Сообщить("Спеццены и скидки простовляются только по основанию виду Цен!!!");
		Возврат;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.new_Подразделение) тогда
		Сообщить("Не заполнено Подразделение для расчета!!!");
		Возврат;	
	КонецЕсли;	
		
	Если НЕ ЗначениеЗаполнено(Объект.new_ДатаРасчетаАкций) Тогда
		Сообщить("Не заполнена 'Дата расчета по Акциям' на которую необходимо проставить спеццену, скидку!!!");
		Возврат;
	КонецЕсли;
	...
	Объект.ВидЦеныДополнительно = "";
	Элементы.ВидЦеныДополнительно.Видимость = Ложь;
	Объект.ДатаДополнительно = "";
	Элементы.ДатаВидЦеныДополнительно.Видимость = Ложь;
	
	Подразд 			= Объект.new_Подразделение;
	ДатаРасчета 		= Объект.new_ДатаРасчетаАкций;
	ВыбрВидЦен			= Объект.ВидЦены;
	
	Для Каждого Стр Из Объект.Товары Цикл 
		Стр.ШаблонЦенника = ПроставитьНужныйШаблон();	
		УстановитьШаблонЦеников();
		
		Стр.ЦенаДополнительно 	= Стр.Цена;
		Стр.Цена 				= Число(0);	
		
		Ном = Стр.Номенклатура;
		
		ЗначСпец = ВозвРатСпеццены(ДатаРасчета, Подразд, Ном, ВыбрВидЦен);
		Если ЗначСпец <> Неопределено Тогда
			Стр.Цена = ЗначСпец;
			Стр.new_ЭтоСпецЦена = Истина;
		Иначе 
			Стр.new_ЭтоСпецЦена = Ложь;	
			ЗначПроц = ВозвРатПроцентаСкидки(Подразд, ДатаРасчета, Ном, ВыбрВидЦен);
			Если ЗначПроц <> Неопределено Тогда
				Стр.Цена = Стр.ЦенаДополнительно - Окр((Стр.ЦенаДополнительно * (ЗначПроц/100)),0,0);
			КонецЕсли;
		КонецЕсли;		
	КонецЦикла;
...
&НаСервере
Функция ВозвРатСпецЦены(ДатаРасчета, Подразд, Ном, ВыбрВидЦен)
	
	Возврат new_МаркетинговыеАктивности.ПолучитьСпецЦенуДляЦеников(ДатаРасчета, Подразд, Ном, ВыбрВидЦен);

КонецФункции

&НаСервере
Функция ВозвРатПроцентаСкидки(Подразд, ДатаРасчета, Ном, ВыбрВидЦен)
	
	Возврат new_МаркетинговыеАктивности.ПолучитьДействующиеСкидкиДляЦенников(Подразд, ДатаРасчета, Ном, ВыбрВидЦен);

КонецФункции

&НаСервере
Функция ПроставитьНужныйШаблон()
	Возврат Справочники.ШаблоныЭтикетокИЦенников.НайтиПоНаименованию("Ценник со старой ценой");	
КонецФункции

&НаСервере
Процедура УстановитьШаблонЦеников()
	
	Шаблон = ПроставитьНужныйШаблон();
...
&НаКлиенте
Процедура ЗаполнитьСкидку(Команда)
	
	Для Каждого Стр Из Объект.Товары Цикл
		Если ЗначениеЗаполнено(Стр.Цена) И ЗначениеЗаполнено(Стр.ЦенаДополнительно) Тогда
			Стр.ПроцентСкидки = 100-((Стр.Цена * 100) / Стр.ЦенаДополнительно);
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьДанныеДляQR()
	ПараметрыПодключения   = Справочники.НастройкиПоПодключениям.НайтиПоНаименованию("HTTP-MirKeramiki.kz", Истина);
	Если ПараметрыПодключения.Пустая() Тогда
		ЗаписьЖурналаРегистрации(УправлениеЭлектроннойПочтой.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Ошибка, , , "Не найдены параметры подключения к ""HTTP-MirKeramiki.kz""");
	КонецЕсли;
	имяВыходногоФайла = ПолучитьимяВременногоФайла("xml");
    //Соединение = Новый HTTPСоединение("mirceramiki.kz",,"a.kuzhanbayeva@alfakt.com","s~Zf*4",, 30, Новый ЗащищенноеСоединениеOpenSSL());
    ...
    Запрос = Новый HTTPЗапрос("/qr-xml");
	Попытка
	...
	Если Ответ.КодСостояния > 300 Тогда 
		Сообщить("Ошибка запроса, код статуса: " + Ответ.КодСостояния);	
    КонецЕсли;
    		
	ДанныеДляQR = Новый ТаблицаЗначений;
	ДанныеДляQR.Колонки.Добавить("ГУИД", Новый ОписаниеТипов("Строка"));
	ДанныеДляQR.Колонки.Добавить("СсылкаQR", Новый ОписаниеТипов("Строка"));
	ДанныеДляQR.Колонки.Добавить("НоменклатураСсылка", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	
	
	//объявление механизмов 
    МеханизмЧтения = Новый ЧтениеXML;
    МеханизмЧтения.ОткрытьФайл(имяВыходногоФайла);
    ...
    Документ = Построитель.Прочитать(МеханизмЧтения);
        ...
        Для каждого элемента Из Документ.ЭлементДокумента.ДочерниеУзлы Цикл
           ...
           //считывание данных
           Если элемента.ИмяУзла = "product" Тогда
               товар = элемента;
			   НовСтрока = ДанныеДляQR.Добавить();
				Для каждого значения Из товар.ДочерниеУзлы Цикл
					Если значения.ИмяУзла = "guid" тогда
						Если СокрЛП(значения.ТекстовоеСодержимое) <> "" Тогда
	                   		НовСтрока.ГУИД = СокрЛП(значения.ТекстовоеСодержимое);
							НовСтрока.НоменклатураСсылка  = Справочники.Номенклатура.ПолучитьСсылку(Новый УникальныйИдентификатор(НовСтрока.ГУИД));
						КонецЕсли;
					ИначеЕсли значения.ИмяУзла = "link" тогда
                        НовСтрока.СсылкаQR = СокрЛП(значения.ТекстовоеСодержимое);
                   КонецЕсли;
			   	КонецЦикла;
           КонецЕсли;
         КонецЦикла; 	
