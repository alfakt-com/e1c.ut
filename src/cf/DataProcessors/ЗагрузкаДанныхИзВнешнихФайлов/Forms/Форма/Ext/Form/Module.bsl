...
Процедура СопоставитьДанныеТабличногоДокументаСДаннымиИБ()
	...
	НомерКолонки = НомерКолонки +1;
	НомераКолонок.Вставить("КолонкаВидЦены", "C" + Формат(НомерКолонки, "ЧН=0; ЧГ=0")); //++Аян_К++ 061022 №6174
	НомерКолонки = НомерКолонки +1;                                                     //++Аян_К++ 061022 №6174
	...
	Пока ЗаполненыНоменклатураКодАртикулШтрихкод Цикл
		...
		Попытка
			...
			НоваяСтрока.ВидЦены = ТабличныйДокумент.Область("R" + СтроковыйНомер + НомераКолонок.Получить("КолонкаВидЦены")).Текст; //++Аян_К++ 061022 №6174
...
Процедура ИнициализироватьТабличныйДокумент()
	...
	ТабличныйДокумент.Присоединить(ОбластьТовары);
	ОбластьТовары = МакетЗаполненияПоВнешнемуИсточнику.ПолучитьОбласть("ВидЦены"); //++Аян_К++ 061022 №6174
	ТабличныйДокумент.Присоединить(ОбластьТовары);
