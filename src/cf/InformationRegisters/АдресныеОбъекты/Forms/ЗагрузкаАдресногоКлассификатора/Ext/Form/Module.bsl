...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей") 
		И ДоступнаЗагрузкаАдресныхСведенийИзИнтернет И ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") Тогда
	...
	Иначе
		...
		Если ДоступнаЗагрузкаАдресныхСведенийИзИнтернет И ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") Тогда
...
Процедура ЗавершениеЗагрузки()
	...
	// Для сброса признака АдресныйКлассификаторУстарел в параметрах работы клиента.
		...
		Если ЕстьУстаревшиеАдресныеСведенияКЛАДРДляУдаления() Тогда
			...
			ТекстВопроса = НСтр("ru = 'Найдены устаревшие сведения о регионах, которые не используются в программе.'");
...
Процедура ОчиститьУстаревшиеАдресныеСведенияКЛАДР()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Очистка устаревших сведений адресного классификатора'");
	...
	Если ФоновоеЗадание.Статус = "Ошибка" Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Очистка устаревших сведений адресного классификатора'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,
