...
Процедура ПодтверждениеСтавкиНДССервер(ПараметрыПодтверждения)
	...
	НомерОтметкиНО        = ПараметрыПодтверждения.НомерОтметкиНалоговогоОргана;
	ДатаОтметкиНО         = ПараметрыПодтверждения.ДатаОтметкиНалоговогоОргана;
	...
	ИначеЕсли ТекущееСостояние = Перечисления.НДССостоянияРеализация0.НеПодтвержденаРеализация0 Тогда
		СтавкаНДС = ?(НЕ ЗначениеЗаполнено(СтавкаНДС),Справочники.СтавкиНДС.НДС12,СтавкаНДС);
		ДатаПодтверждения = КонецДня(КонецПериода);;
		НомерОтметкиНО = "";
		ДатаОтметкиНО  = '00010101000000';
	Иначе
		СтавкаНДС = Справочники.СтавкиНДС.ПустаяСсылка();
	...
	Для Каждого	СтрокаНабора Из НаборЗаписей Цикл
		...
		Если СтрокаНабора.НомерОтметкиНалоговогоОргана <> НомерОтметкиНО Тогда
			СтрокаНабора.НомерОтметкиНалоговогоОргана = НомерОтметкиНО;
		КонецЕсли;
		
		Если СтрокаНабора.ДатаОтметкиНалоговогоОргана <> ДатаОтметкиНО Тогда
			СтрокаНабора.ДатаОтметкиНалоговогоОргана = ДатаОтметкиНО;
		КонецЕсли;
		
	КонецЦикла;	
	
	Если (ПерезаписатьДокумент И ТипЗнч(ДокументРеализации) = Тип("ДокументСсылка.ПервичныйДокумент"))
...
Процедура ЗаполнитьИзФормыРучногоВвода(ВыбранноеЗначение)
	...
	ПараметрыПодтверждения.НомерОтметкиНалоговогоОргана = ВыбранноеЗначение.НомерОтметкиНалоговогоОргана;
	ПараметрыПодтверждения.ДатаОтметкиНалоговогоОргана  = ВыбранноеЗначение.ДатаОтметкиНалоговогоОргана;
...
Процедура ОткрытьТекущуюФормуЗаписи()
	...
	ПараметрыПодбора.Вставить("НомерОтметкиНалоговогоОргана", ТекущиеДанные.НомерОтметкиНалоговогоОргана);
	ПараметрыПодбора.Вставить("ДатаОтметкиНалоговогоОргана",  ТекущиеДанные.ДатаОтметкиНалоговогоОргана);
...
Функция ПараметрыПодтвержденияСтавкиНДС()
	...
	ПараметрыПодтверждения.Вставить("НомерОтметкиНалоговогоОргана");
	ПараметрыПодтверждения.Вставить("ДатаОтметкиНалоговогоОргана");
