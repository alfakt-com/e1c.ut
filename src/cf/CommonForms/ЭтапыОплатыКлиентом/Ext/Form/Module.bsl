...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	БанковскийСчет = Справочники.БанковскиеСчетаОрганизаций.НайтиПоНаименованию("0EXX в Сбербанк АО ДБ, ТОО ""Альфа-КТ""");
...
&НаКлиенте
Процедура ЭтапыГрафикаОплатыПриАктивизацииПоля(Элемент)
	
	///Сергей 31.05.2021 +++
	Попытка
		Если ЭтапыГрафикаОплаты.Количество() > 0 Тогда
			Если Элемент.ТекущиеДанные.Блок = Истина Тогда
				//Элементы.ЭтапыГрафикаОплаты.ПодчиненныеЭлементы.ЭтапыОплатыДатаПлатежа.ТолькоПросмотр = Истина;
				Элемент.ПодчиненныеЭлементы.ЭтапыОплатыДатаПлатежа.ТолькоПросмотр = Истина;
				//СтрДан.Ячейки.ДатаПлатежа.ТолькоПросмотр = Истина;
			Иначе 
				Элемент.ПодчиненныеЭлементы.ЭтапыОплатыДатаПлатежа.ТолькоПросмотр = Ложь;	
			КонецЕсли;
		КонецЕсли;		
	Исключение	
	КонецПопытки;
...
&НаКлиенте
Процедура ЭтапыОплатыДатаПлатежаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	///Сергей 31.05.2021 +++
	Попытка
		Дан = Элементы.ЭтапыГрафикаОплаты.ТекущиеДанные;
		ДатаПл = Дан.ДатаПлатежа;
		Если Дан.Блок = Истина Тогда
			Элемент.ТолькоПросмотр = Истина;
			ВыбранноеЗначение = ДатаПл;
			Возврат;
		Иначе 
