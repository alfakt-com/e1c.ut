...
Процедура ЗаполнитьТаблицуКОформлению()
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ВозвратТоваровОтКлиента) Тогда
		ТекстОснований = ТекстОснований + "
			|ОБЪЕДИНИТЬ ВСЕ" +
			ТекстЗапросаДанныхДокумента("ВозвратТоваровОтКлиента");
	КонецЕсли;
	...
	КонецЕсли;
	Если ПравоДоступа("Чтение", Метаданные.Документы.ОтчетОРозничныхПродажах) Тогда
		ТекстОснований = ТекстОснований + "
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
...
Функция ТекстЗапросаДанныхДокумента(ИмяОбъекта)
	...
	|	ДанныеДокумента.Валюта              КАК Валюта,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДанныеДокумента.Ссылка) = ТИП(Документ.КорректировкаРеализации)
	|			ИЛИ ТИПЗНАЧЕНИЯ(ДанныеДокумента.Ссылка) = ТИП(Документ.ВозвратТоваровОтКлиента)
	|			ИЛИ ТИПЗНАЧЕНИЯ(ДанныеДокумента.Ссылка) = ТИП(Документ.ВозвратТоваровМеждуОрганизациями) ТОГДА
	|			ИСТИНА
