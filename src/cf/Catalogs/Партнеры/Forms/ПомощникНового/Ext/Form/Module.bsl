...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастройкиПодключенияКСервисуИППЗаданы = ПартнерыИКонтрагенты.НастройкиПодключенияКСервисуИППЗаданы() И ПолучитьФункциональнуюОпцию("ОтключенныйФункционал");
	...
	Если Параметры.Свойство("Основание") И ТипЗнч(Параметры.Основание) = Тип("Структура") Тогда
		...
		Если Параметры.Основание.Свойство("ИНН") Тогда
			...
			Если СтрДлина(ИНН) = 12 Тогда
				ТипЮрЛица = Сред(ИНН, 5, 1);
				Если ТипЮрЛица = "4" Тогда
					ВидКомпании = 0;
					Если СтрНайти("12", Сред(ИНН, 6, 1)) > 0 Тогда
						ВидКомпании = 3;
					КонецЕсли;
				ИначеЕсли ТипЮрЛица = "5" Тогда
					ВидКомпании = 1;
				ИначеЕсли ТипЮрЛица = "6" Тогда
					ВидКомпании = 2;
				КонецЕсли;
			КонецЕсли;
		...
		Иначе
			Если ЭтоКомпания = 0 Тогда
				...
				ИначеЕсли УказыватьЮридическиеРеквизиты И (ВидКомпании = 0 Или ВидКомпании = 3) Тогда
					Если ЗначениеЗаполнено(НаименованиеКонтрагента) Или ЗначениеЗаполнено(ИНН) Или ЗначениеЗаполнено(КБЕ) Или ЗначениеЗаполнено(КодПоОКПО) Тогда
						НеобходимостьВопроса = Истина;
					КонецЕсли;
				ИначеЕсли УказыватьЮридическиеРеквизиты И ВидКомпании = 1 Тогда
					Если ЗначениеЗаполнено(НаименованиеКонтрагента) ИЛИ ЗначениеЗаполнено(СтранаРегистрации) ИЛИ ЗначениеЗаполнено(РегистрационныйНомер) ИЛИ ЗначениеЗаполнено(КБЕНерезидент) Тогда
						НеобходимостьВопроса = Истина;
					КонецЕсли
				ИначеЕсли УказыватьЮридическиеРеквизиты И ВидКомпании = 2 Тогда
					Если ЗначениеЗаполнено(ИНН) Или ЗначениеЗаполнено(КодПоОКПО) ИЛИ ЗначениеЗаполнено(КБЕ) Тогда
						НеобходимостьВопроса = Истина;
					КонецЕсли;
			...
			Иначе
				Если ЗначениеЗаполнено(ФамилияПартнера) Или ЗначениеЗаполнено(ИмяПартнера) Или ЗначениеЗаполнено(ОтчествоПартнера)
					Или ЗначениеЗаполнено(АдресЭППартнераФизЛицо) Или ЗначениеЗаполнено(ТелефонПартнераФизЛицо) Или ЗначениеЗаполнено(ИННПартнерФизЛицо) Или ЗначениеЗаполнено(КБЕПартнерФизЛицо) Тогда
					НеобходимостьВопроса = Истина;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьИтоговуюИнформацию()
	...
	Если (ИспользуютсяТолькоПартнеры Или ДействиеСКонтрагентом = 1) И УказыватьЮридическиеРеквизиты Тогда
		...
		Иначе
			...
			Если ЭтоКомпания = 0 И ВидКомпании <> 1 Тогда
				...
				ИтоговаяИнформация = СтрЗаменить(ИтоговаяИнформация, "#КБЕ#", КБЕ);
				ИтоговаяИнформация = СтрЗаменить(ИтоговаяИнформация, "#РДК#", "");
			ИначеЕсли ЭтоКомпания = 0 И ВидКомпании = 1 Тогда
				ИтоговаяИнформация = СтрЗаменить(ИтоговаяИнформация, "#КБЕ#", КБЕНерезидент);
				ОбщегоНазначенияУТКлиентСервер.УдалитьИзСтрокиПодстроку(ИтоговаяИнформация, "#РДК#");
			ИначеЕсли ЭтоКомпания = 1 Тогда
				ИтоговаяИнформация = СтрЗаменить(ИтоговаяИнформация, "#КБЕ#", КБЕПартнерФизЛицо);
				ОбщегоНазначенияУТКлиентСервер.УдалитьИзСтрокиПодстроку(ИтоговаяИнформация, "#РДК#");
			Иначе
				ОбщегоНазначенияУТКлиентСервер.УдалитьИзСтрокиПодстроку(ИтоговаяИнформация, "#РДК#");
			КонецЕсли;
	...
	Если ВидКомпании = 3 И Не ИспользуютсяТолькоПартнеры И ВыбратьГоловногоКонтрагента = 1 Тогда
		...
		ИтоговаяИнформация = СтрЗаменить(ИтоговаяИнформация, "#ИННГоловногоКонтрагента#",
		                     ИННГоловногоКонтрагента + ?(ПустаяСтрока(КППГоловногоКонтрагента), "", "\" + КППГоловногоКонтрагента));
		ИтоговаяИнформация = СтрЗаменить(ИтоговаяИнформация, "#КБЕГоловногоКонтрагента#", КБЕНерезидент);
		...
		Если УказатьБанковскийСчетГоловногоКонтрагента Тогда
			...
			Иначе
				...
				КонецЕсли;
			КонецЕсли;
			ИтоговаяИнформация = СтрЗаменить(ИтоговаяИнформация, "#БГК#", "")
		Иначе
			ОбщегоНазначенияУТКлиентСервер.УдалитьИзСтрокиПодстроку(ИтоговаяИнформация, "#БГК#");
		КонецЕсли;
	...
	ВидимостьСтраныРегистрации = (ИспользуютсяТолькоПартнеры ИЛИ УказыватьЮридическиеРеквизиты) И ВидКомпании = 1 И ЭтоКомпания = 0;
...
Функция СообщениеДубльИНН(ПредставлениеИНН_КПП, НайденныйДубльПоИНН)
	...
	СтрСообщение = НСтр("ru='Контрагент создан не будет. БИН / ИИН - %1 уже указаны для контрагента %2 с кодом %3, ответственный - %4.'");
	Если Не ИспользуютсяТолькоПартнеры Тогда
		СтрСообщение = СтрСообщение + " " + НСтр("ru='Измените БИН / ИИН или используйте уже имеющегося партнера.'");
	КонецЕсли;
...
Функция ИННКППГоловногоКонтрагентаИспользуютсяВИнформационнойБазе()
	...
	Если НайденныйДубльПоИНН <> Неопределено Тогда
		СтрСообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='БИН / ИИН - %1 уже указаны для контрагента %2 с кодом %3, ответственный - %4.'"),
			ИНН + ?(ПустаяСтрока(КППГоловногоКонтрагента), "", "\" + КППГоловногоКонтрагента),
			НайденныйДубльПоИНН.Ссылка,
			НайденныйДубльПоИНН.Код,
			НайденныйДубльПоИНН.ОсновнойМенеджер);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрСообщение,, "ИНН");
		Возврат Истина;
	КонецЕсли;
...
Функция ПредставлениеТипаЮрФизЛица()
	...
	Если ЭтоКомпания = 0 Тогда
		...
		ИначеЕсли ВидКомпании = 1 Тогда
			Возврат НСтр("ru='Юридическое лицо(за пределами РК)'");
		ИначеЕсли ВидКомпании = 2 Тогда
			Возврат НСтр("ru='Индивидуальный предприниматель'");
		Иначе
...
Процедура ВыполнитьПоискПоПервичнойИнформации()
	...
	СтруктураПоиска.Вставить("РегистрационныйНомер",    (ИспользуютсяТолькоПартнеры Или УказыватьЮридическиеРеквизиты) И ВидКомпании = 1 И Не ПустаяСтрока(РегистрационныйНомер) И ЭтоКомпания = 0);
	...
	ПредставленияРеквизитов.Вставить("СтрокаИНН", НСтр("ru='БИН / ИИН'"));
...
Процедура ПоместитьВременныеТаблицыРеквизитовДанных(МенеджерВременныхТаблиц,
	...
	Если СтруктураПоиска.ИНН Тогда
		...
		Если ЭтоКомпания = 0 И (ВидКомпании = 0 Или ВидКомпании = 3) И ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") Тогда
...
Функция ПервичнаяИнформацияЗаполненаНеправильно()
	...
	Если УказыватьЮридическиеРеквизиты Тогда
		...
		Если ЭтоКомпания = 0 И НЕ ВидКомпании = 1 И НЕ УпрощенныйВводДоступен Тогда
			Если ПустаяСтрока(ИНН) Тогда
				ВывестиСообщениеПользователю(НСтр("ru = 'Не указано значение БИН / ИИН'"),"ИНН");
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
...
Функция СведенияГоловногоКонтрагентаЗаполненыНеправильно()
	...
	Если ВыбратьГоловногоКонтрагента = 1 Тогда
		...
		Если ЗначениеЗаполнено(ИНН) Тогда
			Если ИННКППГоловногоКонтрагентаИспользуютсяВИнформационнойБазе() Тогда
				Отказ = Истина;
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаписатьПартнераИСвязанныеДанные(СписокСозданных)
	...
	Если ИспользуютсяТолькоПартнеры Или (ДействиеСКонтрагентом = 1 И НЕ НаОснованииКонтрагента И УказыватьЮридическиеРеквизиты) Тогда
		...
		ОбъектКонтрагент.КБЕ                   = ?(ЭтоКомпания = 0,?(ВидКомпании <> 1,КБЕ,КБЕНерезидент),КБЕПартнерФизЛицо);
		...
		ОбъектКонтрагент.СтранаРегистрации     = ?(ОбъектКонтрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент, СтранаРегистрации, Справочники.СтраныМира.Казахстан);
		...
		Если ЭтоКомпания = 0 И ВидКомпании = 3 Тогда
			...
			ИначеЕсли ВыбратьГоловногоКонтрагента = 1 Тогда
				...
				ОбъектГоловнойКонтрагент.КБЕ                = КБЕНерезидент;
...
Процедура ЗаполнитьНаОснованииКонтрагента(ДанныеКонтрагента,Отказ)
	...
	Если ДанныеКонтрагента.Свойство("Ссылка") Тогда
		...
		Если ВыборкаШапка.Следующий() Тогда
			...
			ДанныеКонтрагента.Вставить("КБЕ", ВыборкаШапка.КБЕ);
	...
	Если ДанныеКонтрагента.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
		...
		КБЕПартнерФизЛицо = ДанныеКонтрагента.КБЕ;
...
Функция ТекстПредупрежденияГоловногоКонтрагента(СписокВыбораГоловногоКонтрагента, ИНН)
	...
	Если НЕ ЗначениеЗаполнено(ИНН) Тогда
		ТекстПредупреждения = НСтр("ru='Не задан БИН / ИИН: невозможно установить головного контрагента.'");
	ИначеЕсли СписокВыбораГоловногоКонтрагента.Количество() = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Не найдены головные контрагенты по указанному БИН / ИИН.'")
	КонецЕсли;
