...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ВыводитьПроизвольныйВидЦен", ЗначениеПараметра) И ЗначениеПараметра = Истина Тогда
		...
		Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
		                      ...
		                      |ИЗ
		                      |	Справочник.ВидыЦен КАК ВидыЦен
		                      |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЦен.ДополнительныеРеквизиты КАК ДопРеквизиты
		                      |		ПО ВидыЦен.Ссылка = ДопРеквизиты.Ссылка
		                      |ГДЕ
		                      |	НЕ ВидыЦен.ПометкаУдаления
							  |	И ДопРеквизиты.Значение = ЗНАЧЕНИЕ(Перечисление.ВидыСоглашенийСКлиентамиДляОграниченияЧтения.Типовые)");
		...
		//Для менеджеров магазинов не доступны Оптовые цены
		//ДолжностьМен =  Справочники.ДолжностиОрганизаций.НайтиПоНаименованию("менеджер по продажам",Истина);
		//Если ПользТек.Должность = ДолжностьМен Тогда
		//	ДанныеВыбора.Удалить(Справочники.ВидыЦен.НайтиПоНаименованию("ОПТ",Истина));
		//КонецЕсли;
		...
		// Если в параметрах передан отбор по дополнительному реквизиту "Вид соглашения"
		Если Параметры.Свойство("Партнер", ЗначениеПараметра) И ЗначениеЗаполнено(ЗначениеПараметра) Тогда
			
			СвойствоПВХ = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя", "ВидЦеныИндивидуальноеДляКонтрагента");
			
			Если СвойствоПВХ.Пустая() Тогда
				Возврат	
			КонецЕсли;
			
			ВидЦены = ЗначениеПараметра.ДополнительныеРеквизиты.Найти(СвойствоПВХ);
			
			Если ВидЦены = Неопределено Тогда 
				Возврат
			КонецЕсли;
			
			ДанныеВыбора.Добавить(ВидЦены.Значение);
		КонецЕсли;
	ИначеЕсли Параметры.Свойство("Контрагент", ЗначениеПараметра) И ЗначениеПараметра Тогда 
		...
		Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
		                      ...
		                      |	НЕ ВидыЦен.ПометкаУдаления
		                      |	И ВидыЦен.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыДействияВидовЦен.Действует)
		                      |	И ДопРеквизиты.Значение = ЗНАЧЕНИЕ(Перечисление.ВидыСоглашенийСКлиентамиДляОграниченияЧтения.Индивидуальные)");
