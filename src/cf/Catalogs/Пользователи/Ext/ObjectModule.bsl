...
Процедура ПередЗаписью(Отказ)
	...
	//Начало: МК Баубериков А - 08.10.2020
	Если Ссылка.ПометкаУдаления = Ложь И ПометкаУдаления = Истина Тогда//пометили на удаление
 
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ЦС_TelegramID.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.ЦС_TelegramID КАК ЦС_TelegramID
		|ГДЕ
		|	ЦС_TelegramID.Пользователь = &Пользователь";
    ...
    КонецЕсли;	
	//Конец: МК Баубериков А - 08.10.2020
