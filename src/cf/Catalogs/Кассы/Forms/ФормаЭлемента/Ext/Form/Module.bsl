...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьПараметрыВыбораВалютаДенежныхСредств();
...
Процедура ПризнакКассовойКнигиОбособленногоПодразделенияПриИзменении(Элемент)
	...
	Если Не Объект.ЭтоКассаОбособленногоПодразделения Тогда
		...
		Объект.ВалютаДенежныхСредств = ВалютаРеглУчета;
		СформироватьАвтоНаименование();
	КонецЕсли;
...
Процедура ПризнакКассовойКнигиОбособленногоПодразделенияПриИзмененииСервер()
	...
	УстановитьПараметрыВыбораВалютаДенежныхСредств();
	УстановитьПараметрыВыбораКассоваяКнига();
	УправлениеЭлементамиФормы();
...
Процедура КассоваяКнигаПриИзменении(Элемент)
	...
	КассоваяКнигаПриИзмененииСервер();
...
&НаСервере
Процедура КассоваяКнигаПриИзмененииСервер()
	
	Объект.ВалютаДенежныхСредств = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.КассоваяКнига, "ВалютаДенежныхСредств");
...
&НаСервере
Процедура УстановитьПараметрыВыбораВалютаДенежныхСредств()
	
	МассивПараметров = Новый Массив;
	Если ПризнакКассовойКнигиОбособленногоПодразделения = 0 Тогда
		МассивПараметров.Добавить(Новый ПараметрВыбора("Отбор.Ссылка", ВалютаРеглУчета));
	КонецЕсли;
	
	Элементы.ВалютаДенежныхСредств.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметров);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьПараметрыВыбораКассоваяКнига()
	
	МассивПараметров = Новый Массив;
	Если ЗначениеЗаполнено(Объект.ВалютаДенежныхСредств) Тогда
		МассивПараметров.Добавить(Новый ПараметрВыбора("Отбор.ВалютаДенежныхСредств", Объект.ВалютаДенежныхСредств));
	КонецЕсли;
	
	Элементы.КассоваяКнига.ПараметрыВыбора = Новый ФиксированныйМассив(МассивПараметров);
