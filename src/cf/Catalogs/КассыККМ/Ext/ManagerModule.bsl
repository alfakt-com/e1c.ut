...
Функция ПараметрыКассыККМ(КассаККМ) Экспорт
	...
	|	ВЫБОР КОГДА
	|		НастройкиРМККассыККМ.ПодключаемоеОборудование.ТипОборудования = ЗНАЧЕНИЕ(Перечисление.ТипыПодключаемогоОборудования.ККТ)
	|		ИЛИ (НастройкиРМККассыККМ.ПодключаемоеОборудование.ТипОборудования = ЗНАЧЕНИЕ(Перечисление.ТипыПодключаемогоОборудования.ФискальныйРегистратор) И НастройкиРМККассыККМ.ПодключаемоеОборудование.ДрайверОборудования.ОбработчикДрайвера = ЗНАЧЕНИЕ(Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикWebkassaФискальныеРегистраторы)) ТОГДА
	|		ИСТИНА
	|	ИНАЧЕ
	|		ЛОЖЬ
	...
	|	ВЫБОР КОГДА НастройкиРМККассыККМ.ПодключаемоеОборудование.ДрайверОборудования.ОбработчикДрайвера = ЗНАЧЕНИЕ(Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикWebkassaФискальныеРегистраторы) ТОГДА
	|		ИСТИНА
	|	ИНАЧЕ
	|		ЛОЖЬ
	|	КОНЕЦ КАК ОтправлятьТолькоEmail
	|ИЗ
	|	Справочник.КассыККМ КАК КассыККМ
	...
	ВозвращаемоеЗначение.Вставить("ОтправлятьТолькоEmail",                  Ложь);
	...
	Если Выборка.Следующий() Тогда
		...
		ВозвращаемоеЗначение.ОтправлятьТолькоEmail                  = Выборка.ОтправлятьТолькоEmail;
