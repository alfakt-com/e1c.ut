		...
		Если ЗаполнятьПоПроизводственномуКалендарю И УчитыватьПраздники Тогда
			...
			Иначе
				Если ДанныеДня.ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.Праздник 
					ИЛИ ДанныеДня.ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.ДополнительныйВыходной Тогда
...
Функция ДниВключенныеВГрафикПроизвольнойДлины(Год, ПроизводственныйКалендарь, ШаблонЗаполнения, УчитыватьПраздники, ДатаОтсчета, Знач ДатаНачала = Неопределено, Знач ДатаОкончания = Неопределено)
	...
	// Дополним таблицу также дополнительными выходными днями
	ОтборСтрок.ВидДня = Перечисления.ВидыДнейПроизводственногоКалендаря.ДополнительныйВыходной;
	ДанныеДополнительныхВыходных = ДанныеПроизводственногоКалендаря.НайтиСтроки(ОтборСтрок);
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ДанныеДополнительныхВыходных, ДанныеПраздничныхДней);
