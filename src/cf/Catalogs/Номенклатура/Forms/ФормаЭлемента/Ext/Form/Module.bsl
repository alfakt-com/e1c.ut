...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РаботаСНоменклатурой.ПриСозданииНаСервереФормаНоменклатуры(ЭтотОбъект, Элементы.ГруппаРаботаСНоменклатурой, "Гиперссылка");
...
Процедура ПриСозданииЧтенииНаСервере()
	...
	ОбновитьВидимостьПредупрежденийТНВЭД();
...
Процедура ЗаполнитьПоПараметрам()
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		// ЭлектронноеВзаимодействие.РаботаСНоменклатурой
		Если Параметры.Свойство("АдресДанныхЗаполнения") и ЭтоАдресВременногоХранилища(Параметры.АдресДанныхЗаполнения) Тогда
		
			ТаблицаСНоменклатурой = ПолучитьИзВременногоХранилища(Параметры.АдресДанныхЗаполнения);
			ЕстьДопРеквизиты = ТаблицаСНоменклатурой.Колонки.Найти("ДополнительныеРеквизиты") <> Неопределено;
			ЕстьКлассификаторы  = ТаблицаСНоменклатурой.Колонки.Найти("Классификаторы") <> Неопределено;
			
			Для каждого ДанныеЗаполнения Из ТаблицаСНоменклатурой Цикл
				...
				Объект.Артикул = ДанныеЗаполнения.Артикул;
				Объект.НаименованиеПолное = НаименованиеПолное;
				Объект.ЕдиницаИзмерения = СсылкаНаЕдиницуИзмерения;
				...
				Если ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоВидовНоменклатуры") Тогда
					Объект.ВидНоменклатуры = ДанныеЗаполнения.ВидНоменклатурыПоУмолчанию.ВидНоменклатуры;
				Иначе
					МассивВидов = Справочники.ВидыНоменклатуры.ПолучитьПредустановленныеВидыНоменклатуры();
					Для каждого ТекВид Из МассивВидов Цикл
						Если ТекВид.Наименование = ДанныеЗаполнения.Тип Тогда
							Объект.ВидНоменклатуры = ТекВид;
						КонецЕсли;
					КонецЦикла; 
				КонецЕсли;;
				Объект.ГруппаДоступа = Константы.ГруппаДоступаНоменклатурыИзСервиса.Получить();
				
				Если ЗначениеЗаполнено(Объект.ВидНоменклатуры) Тогда
					ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ВидНоменклатуры, "ИспользованиеХарактеристик");
					Объект.ИспользованиеХарактеристик = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ВидНоменклатуры, "ИспользованиеХарактеристик");
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(ДанныеЗаполнения.Изображения) Тогда
					ИзображенияИзСервиса.ЗагрузитьЗначения(ДанныеЗаполнения.Изображения.ВыгрузитьКолонку("ИзображениеURL"));
				КонецЕсли;
				...
				Если ДанныеЗаполнения.ШтрихКоды.Количество() > 0 Тогда
					...
					КонецЦикла;
					ШтрихкодыСервис = ПоместитьВоВременноеХранилище(Штрихкоды, УникальныйИдентификатор);
...
&НаСервере
Процедура ОбновитьВидимостьПредупрежденийТНВЭД()
	
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы, "ГруппаПредупреждениеТНВЭД",
		"Видимость", ЭСФСерверУТ.КодТНВЭДВключенВПеречень(Объект.КодТНВЭД));
	
КонецПроцедуры

&НаКлиенте
Процедура ПлощадьОднойПлиткиПриИзменении(Элемент)
	Объект.ВесЕдиницы 	= 	?(Объект.ВесЗнаменатель = 0 , 0, Объект.ПлощадьЕдиницы * Объект.ВесЧислитель / Объект.ВесЗнаменатель);
	Объект.ОбъемЕдиницы = 	?(Объект.ОбъемЗнаменатель = 0 , 0, Объект.ПлощадьЕдиницы * Объект.ОбъемЧислитель / Объект.ОбъемЗнаменатель);
КонецПроцедуры
