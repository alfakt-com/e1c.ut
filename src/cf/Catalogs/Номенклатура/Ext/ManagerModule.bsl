...
Процедура ЗаполнитьРеквизитыПоВидуНоменклатуры(Номенклатура,
	...
	+"ВладелецСерий,ВладелецХарактеристик,ВладелецТоварныхКатегорий,ОсобенностьУчета,ПродукцияМаркируемаяДляГИСМ,КиЗГИСМ,ТабачнаяПродукция";
...
Функция РеквизитыОтключенныеПоФО(ЭтоРеквизитыХарактеристик = Ложь) Экспорт
	...
	ИначеЕсли Не ИспользоватьСтарыеКлассификаторы И ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") Тогда
		Результат.Добавить("КодОКВЭД");
	КонецЕсли;
