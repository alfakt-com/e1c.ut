...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
    ...
    Если ИспользоватьЭлектронныеАВР Тогда
        Элементы.СпособВыпискиАктовВыполненныхРабот.СписокВыбора.Добавить(
		Перечисления.СпособыВыпискиАктовВыполненныхРабот.НаПорталеИСЭСФ);
    КонецЕсли;
	...
	//++Тишбаев О.М.
	Если Параметры.Ключ.Пустая() И Объект.БанковскийСчет.Пустая() Тогда 
		Объект.БанковскийСчет = Справочники.БанковскиеСчетаОрганизаций.ПолучитьСсылку(Новый УникальныйИдентификатор("252aa197-2666-11ec-8326-def4291cfcda"))
	КонецЕсли;
	
	СписокПорядокРасчетов = Элементы.ПорядокРасчетов.СписокВыбора;
	
	НайденныйЭлемент      = СписокПорядокРасчетов.НайтиПоЗначению(Перечисления.ПорядокРасчетов.ПоНакладным);
	Если НайденныйЭлемент <> Неопределено Тогда
		СписокПорядокРасчетов.Удалить(НайденныйЭлемент);
	КонецЕсли;
	
	НайденныйЭлемент      = СписокПорядокРасчетов.НайтиПоЗначению(Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов);
	Если НайденныйЭлемент <> Неопределено Тогда
		Объект.ПорядокРасчетов = НайденныйЭлемент.Значение;
	КонецЕсли; 
	//++
...
Процедура ТипДоговораПриИзмененииСервер()
	...
	Если Объект.ТипДоговора = Перечисления.ТипыДоговоров.Импорт Тогда
    ...
    УстановитьВидимостьСпособаВыпискиАктов();
	
	ЗаполнитьСписокВыбораНалогообложенияИВидаДеятельностиНДС();
...
Процедура НалогообложениеНДСПриИзмененииСервер()
	...
	Если Объект.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаНаЭкспорт  Тогда 
		Объект.СтавкаНДС = Справочники.СтавкиНДС.НДС0;
	КонецЕсли;
...
Процедура НаправлениеДеятельностиПриИзмененииСервер()
	...
	РеквизитыНазначения = "УчетЗатрат, НалогообложениеНДС, НалогообложениеНДСОпределяетсяВДокументе";
	...
	Если ЗначенияРеквизитовНазначения.УчетЗатрат Тогда
		Если Объект.ТипДоговора = Перечисления.ТипыДоговоров.СКомиссионером Или
			Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПокупателем Или
			Объект.ТипДОговора = Перечисления.ТипыДоговоров.СДавальцем Или
			Объект.ТипДОговора = Перечисления.ТипыДоговоров.СПоставщиком Или
			Объект.ТипДОговора = Перечисления.ТипыДоговоров.СПереработчиком Или
			Объект.ТипДОговора = Перечисления.ТипыДоговоров.ВвозИзЕАЭС Или
			Объект.ТипДОговора = Перечисления.ТипыДоговоров.СКомитентом Тогда
			...
			Иначе
				Объект.НалогообложениеНДС = ЗначенияРеквизитовНазначения.НалогообложениеНДС;
			КонецЕсли;
			КонецЕсли;
			
		КонецЕсли;
		
		Если Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПоставщиком Или
			Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПереработчиком Тогда
			...
			Иначе
				Объект.ЗакупкаПодДеятельность = ЗначенияРеквизитовНазначения.НалогообложениеНДС;
			КонецЕсли;
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	ЗаполнитьСписокВыбораНалогообложенияИВидаДеятельностиНДС(Ложь);
...
&НаСервере
Процедура УстановитьВидимостьСпособаВыпискиАктов()
    
    Элементы.ГруппаСпособВыпискиДокументов.Видимость = (Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПокупателем);
...
&НаСервере
Процедура ЗаполнитьСписокВыбораНалогообложенияИВидаДеятельностиНДС(ПроверитьЗаполнение = Истина)
	
	Если Объект.ТипДоговора = Перечисления.ТипыДоговоров.СПокупателем
		ИЛИ Объект.ТипДоговора = Перечисления.ТипыДоговоров.СДавальцем
		ИЛИ Объект.ТипДоговора = Перечисления.ТипыДоговоров.СКомиссионером Тогда
	...
	Иначе
		...
		УчетНДСУТ.ЗаполнитьСписокВыбораНалогообложенияНДСЗакупки(
			Элементы.НалогообложениеНДС, 
			Объект.ХозяйственнаяОперация);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не Объект.кт_БезДоговора Тогда
		Отказ = ПроверитьПериод(); //++Аян_К++ 011122
	КонецЕсли;
	
КонецПроцедуры

Функция ПроверитьПериод() //++Аян_К++ 011122
	
	Если Объект.Партнер.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
		Возврат Ложь;
	Иначе
		Если НЕ ЗначениеЗаполнено(объект.Номер) Или НЕ ЗначениеЗаполнено(объект.Дата) Тогда
			Сообщить("Необходимо указать ""Номер"" и ""Дату"" договора!");
			Возврат Истина;
		Иначе
			Возврат Ложь;
		КонецЕсли;
