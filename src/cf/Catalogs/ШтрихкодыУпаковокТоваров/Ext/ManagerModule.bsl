...
Функция РезультатСозданияШтрихкодовУпаковокПоДеревуМаркируемойПродукции(ДеревоМаркированнойПродукции,
	...
	Если ШтрихКодыУпаковок = Неопределено Тогда
		...
		ШтрихКодыУпаковок = ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ШтрихкодыУпаковок(
				ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ЗначенияШтрихкодовИзДереваМаркируемойПродукции(ДеревоМаркированнойПродукции, Неопределено));
	...
	Для Каждого СтрокаДерева Из ДеревоМаркированнойПродукции.Строки Цикл
		...
		КонецЕсли;
			...
			Иначе
				...
				ДанныеНовойУпаковки = Справочники.ШтрихкодыУпаковокТоваров.СтруктураСозданияШтрихкодаУпаковки();
				Если ДеревоМаркированнойПродукции.Колонки.Найти("ЗначениеШтрихкода") <> Неопределено Тогда
					ДанныеНовойУпаковки.ЗначениеШтрихкода = СтрокаДерева.ЗначениеШтрихкода;
				Иначе
					ДанныеНовойУпаковки.ЗначениеШтрихкода = СтрокаДерева.Штрихкод;
				КонецЕсли;
				ДанныеНовойУпаковки.ДатаУпаковки      = ТекущаяДатаСеанса();
				ДанныеНовойУпаковки.ТипУпаковки       = СтрокаДерева.ТипУпаковки;
				...
				НоваяСтрока = ШтрихкодыУпаковок.Добавить();
				Если ШтрихкодыУпаковок.Колонки.Найти("ЗначениеШтрихкода") <> Неопределено Тогда
					НоваяСтрока.ЗначениеШтрихкода = ДанныеНовойУпаковки.ЗначениеШтрихкода;
				Иначе
					НоваяСтрока.Штрихкод = ДанныеНовойУпаковки.ЗначениеШтрихкода;
				КонецЕсли;
				НоваяСтрока.ХешСумма          = ДанныеНовойУпаковки.ХешСумма;
				НоваяСтрока.Ссылка            = ШтрихкодУпаковки;
		...
		Если ТипЗнч(ДеревоМаркированнойПродукции) = Тип("ДеревоЗначений")
			Или ДеревоМаркированнойПродукции.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаПродукцииИСМП.ПачкиБезБлока
			Или ДеревоМаркированнойПродукции.ТипУпаковки = Перечисления.ПрочиеЗоныПересчетаПродукцииИСМП.БлокиБезКоробки Тогда	
...
Процедура ИзменитьШтрихкодУпаковки(ШтрихкодУпаковки, ЗначенияРеквизитов) Экспорт
	...
	Попытка
		...
		Если ЗначенияРеквизитов.Свойство("Серия") Тогда
			ПараметрыУказанияСерий = ГосударственныеИнформационныеСистемыПереопределяемый.ПараметрыУказанияСерийФормыОбъекта(ШтрихкодУпаковкиОбъект, Справочники.ШтрихкодыУпаковокТоваров);
			НоменклатураСервер.ЗаполнитьСтатусыУказанияСерий(ШтрихкодУпаковкиОбъект, ПараметрыУказанияСерий);
	...
	Исключение
		...
		ИмяСобытияЖурналРегистрации = НСтр("ru = 'ГосИС: Запись штрихкода упаковки'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
