...
Функция ПолучитьРеквизитыОрганизации(Организация) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		КонецЕсли;
		КБЕ = Выборка.КБЕ;
		СерияСвидетельстваПоНДС = Выборка.СерияСвидетельстваПоНДС;
		НомерСвидетельстваПоНДС = Выборка.НомерСвидетельстваПоНДС;
		ДатаСвидетельстваПоНДС  = Выборка.ДатаСвидетельстваПоНДС;
		СвидетельствоПоНДС = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	СтруктураРеквизитов.Вставить("КБЕ", 						КБЕ);
	СтруктураРеквизитов.Вставить("СерияСвидетельстваПоНДС", 	СерияСвидетельстваПоНДС);
	СтруктураРеквизитов.Вставить("НомерСвидетельстваПоНДС", 	НомерСвидетельстваПоНДС);
	СтруктураРеквизитов.Вставить("ДатаСвидетельстваПоНДС", 		ДатаСвидетельстваПоНДС);
	СтруктураРеквизитов.Вставить("СвидетельствоПоНДС", 			СвидетельствоПоНДС);
	СтруктураРеквизитов.Вставить("ОбособленноеПодразделение", 	ОбособленноеПодразделение);
	СтруктураРеквизитов.Вставить("ГоловнаяОрганизация", 		ГоловнаяОрганизация);
	СтруктураРеквизитов.Вставить("УчетПоМестнымБюджетам", 		УчетПоМестнымБюджетам);
...
Функция ЗакупкаПодДеятельность(Организация, Склад = Неопределено, Знач Дата = Неопределено) Экспорт
	...
	|	ПО РегУчетнаяПолитика.УчетнаяПолитика = СпрУчетнаяПолитика.Ссылка
	|	И НЕ СпрУчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС
