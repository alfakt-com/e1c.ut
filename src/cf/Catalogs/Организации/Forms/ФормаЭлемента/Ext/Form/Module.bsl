...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если (НЕ УправлениеПредприятием 
		...
		И ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") // в РК код налоговой льготы не проверяется
		Тогда
...
&НаКлиенте
Процедура СоответствиеРеквизитовПечатиНаРусскомИГосударственномЯзыках(Команда)
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ОрганизацияСсылка", Объект.Ссылка);
	
	ОткрытьФорму("РегистрСведений.СоответствиеРеквизитовНаРусскомИГосударственномЯзыках.Форма.ВводСоответствияРеквизитовПечатиНаРусскомИГосударственномЯзыках", ПараметрыОткрытия, ЭтаФорма,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура УправлениеФормойНаСервере()
	...
	Если ВидОрганизации = "ЮридическоеЛицо" Тогда
		...
		Элементы.Наименование.ПодсказкаВвода = НСтр("ru = 'например, Стройснаб ТОО'");
		Элементы.НаименованиеСокращенное.ПодсказкаВвода = НСтр("ru = 'например, ТОО ""Стройснаб""'");
		Элементы.НаименованиеПолное.ПодсказкаВвода = НСтр("ru = 'например, Товарищество с ограниченной ответственностью Стройснаб'");
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(Элементы, "УказыватьРеквизитыГоловнойОрганизацииВСчетеФактуре",
																	"Видимость", (ВидОрганизации = "ОбособленноеПодразделение"));
...
Процедура ИзменитьВидОрганизацииНаСервере()
	...
	Если ВидОрганизации = "ЮридическоеЛицо" Тогда
			...
			Объект.УказыватьРеквизитыГоловнойОрганизацииВСчетеФактуре = Ложь;
...
Процедура УстановитьСвойстваУчетнойПолитикиИНалогов()
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьРеглУчет") Тогда
		...
		Элементы.ПолучитьНастройкуТиповНалогов.Видимость = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьОписаниеКлассификаторов(Форма)
	...
	СтрокаОписанияКлассификаторов = НСтр("ru = 'КФС: %КодОКФС%. Форма собственности: %НаименованиеОКФС%. ОКЭД: %КодОКВЭД%. Вид деятельности: %НаименованиеОКВЭД%.'");
...
&НаКлиенте
Процедура ПолучитьНастройкуТиповНалогов(Команда)
	Возврат;
КонецПроцедуры
