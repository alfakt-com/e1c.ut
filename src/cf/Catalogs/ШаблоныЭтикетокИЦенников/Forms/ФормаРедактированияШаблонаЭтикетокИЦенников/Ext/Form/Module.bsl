...
Процедура НазначениеПриИзмененииСервер(Знач ОчищатьНазначение = Истина)
	...
	Для Каждого Область Из ПолеТабличногоДокумента.Области Цикл
		...
		Если СтрНачинаетсяС(ВРег(Имя), ВРег("QRКод")) Тогда
			Индекс = ЗаменитьРисунокQRкода(Имя);
			ПолеТабличногоДокумента.Рисунки[Индекс].Имя = Имя;
		КонецЕсли;
...
&НаСервере
Функция ВставитьРисунокQRкода(ИмяТекущейОбласти)
	
	РисунокШтрихкода = ПолеТабличногоДокумента.Рисунки.Добавить(ТипРисункаТабличногоДокумента.Картинка);
	Индекс = ПолеТабличногоДокумента.Рисунки.Индекс(РисунокШтрихкода);
	РисунокШтрихкода.Расположить(ПолеТабличногоДокумента.Область(ИмяТекущейОбласти));
	РисунокШтрихкода.Имя = ИмяПараметраQRкод()+СтрЗаменить(Новый УникальныйИдентификатор,"-","_");
	РисунокШтрихкода.РазмерКартинки = РазмерКартинки.Пропорционально;
	РисунокШтрихкода.Линия          = Новый Линия(ТипЛинииРисункаТабличногоДокумента.НетЛинии, 0);
	
	//УстановитьКартинкуQRкода(РисунокШтрихкода);//ПолеТабличногоДокумента.Рисунки[Индекс]);
...
&НаСервере
Функция ЗаменитьРисунокQRкода(ИмяРисунка)
	...
	Индекс = ПолеТабличногоДокумента.Рисунки.Индекс(РисунокШтрихкода);
	//УстановитьКартинкуQRкода(ИмяРисунка);//ПолеТабличногоДокумента.Рисунки[Индекс]);
...
Процедура ПодставитьПолеШаблона(Область, ВыбраннаяСтрока)
	...
	Если ИмяПоляВШаблоне = ИмяПараметраШтрихкод() ИЛИ ИмяПоляВШаблоне = ИмяПараметраКодАкцизнойМарки() ИЛИ ИмяПоляВШаблоне = ИмяПараметраQRкод() Тогда
...
Процедура ПодставитьПолеШаблонаЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		ИначеЕсли ИмяПоляВШаблоне = ИмяПараметраQRкод() Тогда
			ВставитьРисунокQRкода(ТекущаяОбласть.Имя);
		КонецЕсли;
