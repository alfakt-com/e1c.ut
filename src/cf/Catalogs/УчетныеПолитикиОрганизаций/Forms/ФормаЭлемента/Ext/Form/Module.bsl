...
Процедура СистемаНалогообложенияПриИзменении(Элемент)
	...
	Если ПрименяетсяУСН Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.ОбъектНалогообложенияУСН) И ПолучитьФункциональнуюОпциюФормы("ОтключенныйФункционал") Тогда
...
Процедура ПрименяетсяОсвобождениеОтУплатыНДСПриИзменении(Элемент)
	...
	Если Объект.ПрименяетсяОсвобождениеОтУплатыНДС Тогда
		Объект.ПрименяетсяУчетНДСПоФактическомуИспользованию = Ложь;
		Объект.Учитывать5ПроцентныйПорог = Ложь;
		Объект.РаспределятьНДСМетодомЗачета = Ложь;
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементов()
	...
	Элементы.ГруппаНДС.Видимость = ?(ПолучитьФункциональнуюОпцию("ОтключенныйФункционал"), Не ПрименяетсяУСН, Истина);
	Элементы.ГруппаУСН.Видимость = ПрименяетсяУСН и ПолучитьФункциональнуюОпцию("ОтключенныйФункционал");
	Элементы.ГруппаНалоговыйУчет.Видимость = Не ПрименяетсяУСН;
	Элементы.ГруппаФиксированныеАктивы.Видимость = Не ПрименяетсяУСН;
	
	ИспользоватьРаздельныйУчет = ПолучитьФункциональнуюОпцию("ИспользоватьРаздельныйУчетПоНалогообложению");// И НЕ Объект.ПрименяетсяУчетНДСПоФактическомуИспользованию;
	...
	Элементы.ГруппаПравилаСписанияНДСПоТоварам.Видимость = ИспользоватьРаздельныйУчет;
	Элементы.ГруппаВариантУчетаНДСПриИзмененииВидаДеятельности.Видимость = ИспользоватьРаздельныйУчет;
	Элементы.ГруппаСтатьяРасходовЕНВД.Видимость = ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") И (ИспользоватьРаздельныйУчет И НЕ  Объект.ПрименяетсяУчетНДСПоФактическомуИспользованию);
...
Процедура УстановитьДоступностьЭлементов()
	...
	Элементы.РаспределятьНДСМетодомЗачета.Доступность = НЕ Объект.ПрименяетсяОсвобождениеОтУплатыНДС;
...
Процедура УстановитьСтавкуНалогаУСН()
	...
	Если ПрименяетсяУСН И ПолучитьФункциональнуюОпциюФормы("ОтключенныйФункционал") Тогда
