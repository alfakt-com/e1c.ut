...
// Проверяет необходимость регистрации объекта на узлах.
//
// Параметры:
//	Источник - СправочникОбъект, ДокументОбъект, РегистрСведенийНаборЗаписей и т.п. - источник события "ПередЗаписью".
//
Функция ПропуститьРегистрациюОбъекта(Источник) Экспорт
	
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьСкрытиеПерсональныхДанныхСубъектов") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если Источник.ДополнительныеСвойства.Свойство("СкрытиеКонфиденциальнойИнформации") Тогда
		Возврат Истина;
	КонецЕсли;
		...
		Если ЗначениеЗаполнено(ПолныеИменаРеквизитов) Тогда
			...
			Если ОбщегоНазначения.ЭтоРегистрСведений(ОбъектМетаданных) Тогда
				...
				Если РежимЗаписиНезависимый Тогда
					...
					Субъект = Источник.Отбор[ПолныеИменаРеквизитов[0]].Значение;
				...
				Если Не ЗначениеЗаполнено(Субъекты) Тогда
					Субъекты = Источник.ВыгрузитьКолонку(ПолныеИменаРеквизитов[0]);
				КонецЕсли;
			...
			Иначе
				...
				Возврат ЕстьСкрытыеПерсональныеДанные(Источник, ПолныеИменаРеквизитов);
	...
	Если МетаданныеТЧ <> Неопределено Тогда
		...
		Для Каждого Реквизит Из МетаданныеТЧ.Реквизиты Цикл
			...
			Если Реквизит.Тип.СодержитТип(Тип) Тогда
				...
				Если Полные Тогда
					ИменаРеквизитов.Добавить(СтрШаблон("%1.%2", ИмяТабличнойЧасти, Реквизит.Имя));
				Иначе
					ИменаРеквизитов.Добавить(Реквизит.Имя);
				КонецЕсли;
...
Функция ЕстьСубъектыСоСкрытымиПерсональнымиДанными(Знач Субъекты)
	...
	Если Не ЗначениеЗаполнено(Субъекты) 
		Или Не ПолучитьФункциональнуюОпцию("ИспользоватьСкрытиеПерсональныхДанныхСубъектов") Тогда
		Возврат ЕстьСкрытые;
	КонецЕсли;
	...
	Если Не Метаданные.ОпределяемыеТипы.СубъектПерсональныхДанных.Тип.СодержитТип(ТипЗнч(Субъекты[0])) Тогда
		Возврат ЕстьСкрытые;
	КонецЕсли;
