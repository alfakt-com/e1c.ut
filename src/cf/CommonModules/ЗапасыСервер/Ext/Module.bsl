...
Процедура СформироватьВТДоступныеВидыЗапасов(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов, ИмяВТИсточникаТекущихВидовЗапасов) Экспорт
	...
	|					ИНАЧЕ ЕСТЬNULL(НалогообложениеВладельца.НалогообложениеНДС, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС))
	|				КОНЕЦ
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
	|	КОНЕЦ
	|ИЗ
	...
	|			И (&ПоВсемТипамЗапасов ИЛИ ВЫБОР
	|									КОГДА Настройка.СпособПередачиТоваров = ЗНАЧЕНИЕ(Перечисление.СпособыПередачиТоваров.Продажа)
	|										ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.Товар)
	|									КОГДА Настройка.СпособПередачиТоваров = ЗНАЧЕНИЕ(Перечисление.СпособыПередачиТоваров.ПередачаНаКомиссию)
	|										ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.КомиссионныйТовар)
	|									ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ПустаяСсылка)
	|								КОНЕЦ В (&ТипЗапасов))
	|		ЛЕВОЕ СОЕДИНЕНИЕ Налогообложение КАК НалогообложениеВладельца
	|		ПО (Настройка.ОрганизацияВладелец = НалогообложениеВладельца.Организация)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УстаревшиеВидыЗапасовСОстатками КАК УстаревшиеВидыЗапасовСОстатками
	|		ПО ВидыЗапасов.Ссылка = УстаревшиеВидыЗапасовСОстатками.ВидЗапасов
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВидыЗапасовДокумента КАК ВидыЗапасовДокумента
	|		ПО ВидыЗапасов.Ссылка = ВидыЗапасовДокумента.ВидЗапасов
			...
			Пока ВыборкаВидЗапасов.Следующий() Цикл
				...
				Пока ВыборкаНомерГТД.Следующий() Цикл
					...
					Пока ТекущиеИтоги.Следующий() Цикл
						...
						// Проверка лишнего резерва между соседними периодами.
						Если ПредыдущиеИтоги <> Неопределено Тогда
							
							РазницаРезервов = ТекущиеИтоги.Резерв - ПредыдущиеИтоги.Резерв;
							РазницаПоложительныхРезервов = Макс(ТекущиеИтоги.Резерв, 0) - Макс(ПредыдущиеИтоги.Резерв, 0);
							РазницаОтрицательныхРезервов = Мин(ТекущиеИтоги.Резерв, 0) - Мин(ПредыдущиеИтоги.Резерв, 0);
							Дельта = 0;
							
							Если РазницаОтрицательныхРезервов > 0 Или РазницаПоложительныхРезервов < 0 Тогда
								Дельта = РазницаРезервов;
							КонецЕсли;
							
							Если Дельта И ТекущиеИтоги.КоличествоИзменение < 0 Тогда
			...
			ИначеЕсли (КоличествоТовара > 0 И КоличествоОстаток > 0)
			 Или (КоличествоТовара < 0 И КоличествоОстаток < 0) Тогда
				...
				Если Выборка.ВидЗапасов <> ВидЗапасов Тогда
					...
					ДвижениеРезерваВладельца.Период                     = КонецМесяца(ВыборкаДанныеДокумента.Период);
...
Процедура РаспределитьНовыеРезервыПоТаблицеОстатков(
	...
	Для Каждого СтрокаРаспределения Из ТаблицаРаспределения Цикл
		...
		Для Каждого СтрокаОстатка Из СтрокиОстатка Цикл
			...
			РезервВладельца.ЭтоСторно = Ложь;
...
Процедура СформироватьВТСторноРезервов(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|				ИЗ
	|					ВТОтборыРезервовДляСторно КАК ВТОтборыРезервовДляСторно)
	|		И СформированныеРезервы.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И (КорВидыЗапасов.ТипЗапасов <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварТребующийПодтвержденияВыпуска)
...
Функция ЗапросРаспределенияСУчетомПриоритетовТаблицыТоваровНаТаблицуОстатков(МенеджерВременныхТаблиц, ДополнительныеСвойства)
	...
	|		ИЛИ ВидыЗапасов.РеализацияЗапасовДругойОрганизации
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	АналитикаУчетаНоменклатуры,
	|	НомерГТД;
...
Функция РаспределитьРезервыТоваровОрганизацийПоПериодам(КоличествоПериодовПриФормированииРезервовОрганизаций, ТаблицаНовыеРезервыИсходная, МенеджерВременныхТаблиц) Экспорт
	...
	Если ЕстьИзмененияВТаблице Тогда
		...
		Для Индекс = 0 По КоличествоСтрок - 1 Цикл
			...
			СтрокаРасхода = ТаблицаНовыеРезервы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаРасхода, СтрокаПрихода);
			
			СтрокаПрихода.ВидДвижения    = ВидДвиженияНакопления.Приход;
			
			СтрокаРасхода.ВидДвижения    = ВидДвиженияНакопления.Расход;
			СтрокаРасхода.Организация    = СтрокаПрихода.КорОрганизация;
			СтрокаРасхода.ВидЗапасов     = СтрокаПрихода.КорВидЗапасов;
			СтрокаРасхода.КорОрганизация = СтрокаПрихода.Организация;
			СтрокаРасхода.КорВидЗапасов  = СтрокаПрихода.ВидЗапасов;
		КонецЦикла;
...
Процедура ТаблицаОстатковТоваровПереданныхНаКомиссию(ДокументОбъект, ПараметрыЗаполненияВидовЗапасов, МенеджерВременныхТаблиц, ДатаОстатков) 
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ // ВидыЗапасов, переданные на комиссию
...
Функция ДополнитьЗапросКонтролемОстатковНаКаждыйМесяц(Запрос, ТекстЗапроса, ПараметрыДополнения, МинимальнаяДатаОстатков, ДатыОстатков = Неопределено) Экспорт
	...
	Если ПараметрыДополнения.ОкончаниеПериодаКонтроля = Неопределено Тогда
		
		КонецТекущегоМесяца = КонецМесяца(ТекущаяДатаСеанса());
		КонечныйМесяц = Макс(КонецМесяцаДатыПоследнегоДвижения, КонецТекущегоМесяца);
		МесяцыКонтроля.Вставить(КонечныйМесяц);
		
	Иначе
		Если ПараметрыДополнения.ОкончаниеПериодаКонтроля >= КонецМесяцаДатыПоследнегоДвижения Тогда
			МесяцыКонтроля.Вставить(КонецМесяцаДатыПоследнегоДвижения);
			КонечныйМесяц = КонецМесяцаДатыПоследнегоДвижения;
		Иначе
			КонечныйМесяц = ПараметрыДополнения.ОкончаниеПериодаКонтроля;
		КонецЕсли;
	КонецЕсли;	
			
	// За последние три года возможна налоговая проверка.
	ТриГодаНазад = КонецМесяца(ДобавитьМесяц(КонецМесяцаДатыПоследнегоДвижения, -36)); 
	
	Если МинимальнаяДатаОстатков < ТриГодаНазад Тогда
		НачальныйМесяц = ТриГодаНазад;
	Иначе
		НачальныйМесяц = КонецМесяца(МинимальнаяДатаОстатков);
	КонецЕсли;
	...
	Для Каждого МесяцКонтроля из МесяцыКонтроля Цикл
		...
		Если ПараметрыДополнения.ПодбиратьЗапасыРазныхПериодов Тогда
			...
			Для Каждого СтрМас из ДатыОстатков Цикл
				...
				Если ТекущийМесяц >= КонецМесяцаПолучаемыхОстатков Тогда
