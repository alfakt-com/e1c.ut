...
// Возвращаемое значение:
//   Структура - структура с ключами:
//     * ИмяКонфигураци - Строка - имя конфигурации.
...
//     * ТребуетсяПовторнаяОтправка - Булево - Истина если требуется отправить повторно данные, клоторые находятся сейчас в очереди.
//
...
//     * Ключ - СправочникСсылка.Пользователи - пользователь, настройки которого нужно выгрузить.
//     * Значение - Строка - новый логин пользователя.
//   УстановитьМонопольныйРежимАвтоматически - Булево - признак того, что в случае необходимости будет выполнена 
...
// Проверяет состав плана обмена, в случае неккоректного состава вызывается исключение.
// Проверки:
...
//     * Пользователь - СправочникСсылка.Пользователи - пользователь к которму будет привязан пользователь сервиса.
...
Функция ФильтрИзмененийНабораЗаписей(ОбъектМетаданных, Узел, ОсталосьВыгрузить)
	...
	Если Метаданные.Последовательности.Содержит(ОбъектМетаданных) 
		Или Метаданные.РегистрыРасчета.Содержит(ОбъектМетаданных.Родитель())
		Или ЭтоРегистрСведенийСИзмерениемСоставногоТипа(ОбъектМетаданных) Тогда
...
Функция ЭтоРегистрСведенийСИзмерениемСоставногоТипа(ОбъектМетаданных)
	
	Если Не Метаданные.РегистрыСведений.Содержит(ОбъектМетаданных) Тогда
		Возврат Ложь;
	ИначеЕсли ОбъектМетаданных.РежимЗаписи <> Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.Независимый Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Для Каждого Измерение Из ОбъектМетаданных.Измерения Цикл
		Если Измерение.ОсновнойОтбор И Измерение.Тип.Типы().Количество() > 1 Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла; 
...
Процедура ЗавершитьЗагрузкуСОшибкой(СостояниеЗагрузки, ПодробноеПредставлениеОшибки, КраткоеПредставлениеОшибки)
	...
	СообщенияВМоделиСервиса.ОтправитьСообщение(Сообщение, РаботаВМоделиСервисаБТСПовтИсп.КонечнаяТочкаМенеджераСервиса());
...
Процедура ВызватьИсключениеНеизвестныйОбъектМетаданных(Знач ОбъектМетаданных)
	...
	ВызватьИсключение СтрШаблон(НСтр("ru = 'Неизвестный объект метаданных: %1'"), ОбъектМетаданных);
