...
Процедура УстановитьГраницыРасчетов(РасчитаныйПериод, ОкончаниеПериодаРасчета, ИмяРегистраЗаданий, АналитикиРасчета, ВременныеТаблицы, НомерЗаданияДоРасчета, КоличествоОбработанныхЭлементов = 0)
	УстановитьПривилегированныйРежим(Истина);
	
	Если НачалоРасчета(ИмяРегистраЗаданий, ОкончаниеПериодаРасчета, АналитикиРасчета, НомерЗаданияДоРасчета) < РасчитаныйПериод Тогда
		Возврат;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("СледующийМесяц", КонецМесяца(РасчитаныйПериод) + 1);
...
Функция ТекстДвиженияРасчетовСПоставщиками()
	Возврат "
		...
		|		И ВЫБОР 
        |           КОГДА ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.АвансовыйОтчет) И ВЫРАЗИТЬ(ДД.Регистратор КАК Документ.АвансовыйОтчет).Мультивалютный
        |               ТОГДА ДД.Валюта = КРасчету.ВалютаДокумента
        |           ИНАЧЕ ДД.Валюта = КРасчету.Валюта
        |       КОНЕЦ
		|ГДЕ
		|	ДД.Период МЕЖДУ &НачалоПериода И &ОкончаниеПериода
		|	И ДД.Сумма <> 0 
...
Функция СуммыПоставщиковВВалютеРегл()
	...
	|		КОГДА ДанныеРегистра.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		ТОГДА ДанныеРегистра.Предоплата
	|		КОГДА ДанныеРегистра.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) И ДанныеРегистра.Предоплата < 0
	|		ТОГДА -ДанныеРегистра.Предоплата
	|		ИНАЧЕ 0
	|	КОНЕЦ
	|	) КАК Предоплата,
	...
	|		КОГДА ДанныеРегистра.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		ТОГДА ДанныеРегистра.ДолгРегл
	|		КОГДА ДанныеРегистра.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) И ДанныеРегистра.ДолгРегл<0
	|		ТОГДА -ДанныеРегистра.ДолгРегл
	|		ИНАЧЕ 0
	|	КОНЕЦ
	|	) КАК ДолгРегл,
	...
	|		КОГДА ДанныеРегистра.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		ТОГДА ДанныеРегистра.ДолгУпр
	|		КОГДА ДанныеРегистра.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) И ДанныеРегистра.ДолгУпр<0
	|		ТОГДА -ДанныеРегистра.ДолгУпр
	|		ИНАЧЕ 0
	|	КОНЕЦ
	|	) КАК ДолгУпр,
...
Процедура СуммыДокументовВВалютеРегл(ВариантРасчета, МассивДокументов = Неопределено)
	...
	Пока ВыборкаПоДокументу.Следующий() Цикл
		...
		Пока ВыборкаПоТипуЗаписи.Следующий() Цикл
			...
			СуммаВзаиморасчетов = ВыборкаПоТипуЗаписи.СуммаВзаиморасчетов;
			БазаНДСРеглПоДокументу = 0;
			...
			Пока ВыборкаПоРасчетам.НайтиСледующий(Новый Структура("Регистратор", ВыборкаПоДокументу.Ссылка)) Цикл
				...
				Предоплата     = Предоплата + ВыборкаПоРасчетам.Предоплата;
				ПредоплатаРегл = ПредоплатаРегл + ВыборкаПоРасчетам.ПредоплатаРегл;
				ПредоплатаУпр  = ПредоплатаУпр + ВыборкаПоРасчетам.ПредоплатаУпр;
				
				Долг           = Долг + ВыборкаПоРасчетам.Долг;
				ДолгРегл       = ДолгРегл + ВыборкаПоРасчетам.ДолгРегл;
				ДолгУпр        = ДолгУпр + ВыборкаПоРасчетам.ДолгУпр;
				
				СуммаДокументаРегл = СуммаДокументаРегл + ВыборкаПоРасчетам.ПредоплатаРегл + ВыборкаПоРасчетам.ДолгРегл;
				СуммаДокументаУпр  = СуммаДокументаУпр + ВыборкаПоРасчетам.ПредоплатаУпр + ВыборкаПоРасчетам.ДолгУпр;
			...
			КурсВалютыВзаиморасчетов = РаботаСКурсамиВалютУТ.ПолучитьКурсВалютыИзКэша(
					ВыборкаПоТипуЗаписи.ВалютаВзаиморасчетов,
					ВыборкаПоТипуЗаписи.Период,
					Валюты.КэшКурсовВалют);
			
			БазаСНДСРеглПоДокументу = ПредоплатаРегл
				+ (СуммаВзаиморасчетов - Предоплата) * КурсВалютыВзаиморасчетов / КурсВалютыРегл;
			
			Если Предоплата + Долг < СуммаВзаиморасчетов Тогда
				СуммаДокументаРегл = СуммаДокументаРегл
				                       + (СуммаВзаиморасчетов - Предоплата - Долг) * КурсВалютыВзаиморасчетов / КурсВалютыРегл;
				СуммаДокументаУпр = СуммаДокументаУпр 
				                       + (СуммаВзаиморасчетов - Предоплата - Долг) * КурсВалютыВзаиморасчетов / КурсВалютыУпр;
			КонецЕсли;
			
			УчтеноБазыРаспределения = 0;
			УжеРаспределено = 0;
			УжеРаспределеноУпр = 0;
			УжеРаспределеноБазаСНДСРегл = 0;
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Выборка.ПересчитатьСуммыРегл И СуммаДокумента <> 0 Тогда
					...
					БазаСНДСРегл = Окр(БазаСНДСРеглПоДокументу * (УчтеноБазыРаспределения + СуммаСНДС) / СуммаДокумента, 2) - УжеРаспределеноБазаСНДСРегл;
					...
					УжеРаспределеноБазаСНДСРегл = УжеРаспределеноБазаСНДСРегл + БазаСНДСРегл;
					...
					Если ПорядокОплаты = Перечисления.ПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте 
						...
						Если ПересчитываемУпр Тогда
							НоваяЗапись.БазаНДСУпр = НоваяЗапись.БазаНДСРегл * КурсВалютыРегл / КурсВалютыУпр;
							Если НоваяЗапись.СуммаНДС = 0 Тогда 
								НоваяЗапись.СуммаНДСУпр = 0;
							Иначе 
								НоваяЗапись.СуммаНДСУпр = НоваяЗапись.СуммаНДСРегл * КурсВалютыРегл / КурсВалютыУпр;
							КонецЕсли;
					...
					Иначе
						...
						Если ПересчитываемРегл Тогда
							Если НоваяЗапись.СуммаНДС = 0 Тогда 
								НоваяЗапись.СуммаНДСРегл = 0;
							Иначе 
								НоваяЗапись.СуммаНДСРегл = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(БазаСНДСРегл, ТекПроцентНДС);
							КонецЕсли;
							НоваяЗапись.СуммаБезНДСРегл = СуммаСНДСРегл - НоваяЗапись.СуммаНДСРегл;
							НоваяЗапись.БазаНДСРегл = БазаСНДСРегл - НоваяЗапись.СуммаНДСРегл;
						КонецЕсли;
						Если ПересчитываемУпр Тогда
							Если НоваяЗапись.СуммаНДС = 0 Тогда 
								НоваяЗапись.СуммаНДСУпр = 0;
