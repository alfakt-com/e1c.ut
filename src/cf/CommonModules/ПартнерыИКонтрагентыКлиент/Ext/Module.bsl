...
Процедура ЗаполнитьИсториюАдресов(Форма, РеквизитыКомпании) Экспорт
	...
	Если НЕ ХранитьИсториюИзменений Тогда
		ПартнерыИКонтрагентыВызовСервера.ВключитьХранениеИсторииИзменений("ЮрАдресКонтрагента");
	КонецЕсли;
	...
	Для Каждого СтрокаКонтактнойИнформации Из ИсторияАдресов Цикл
		...
		ДанныеСтроки.Вид = ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ЮрАдресКонтрагента");
		ДанныеСтроки.Тип = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Адрес");
		ДанныеСтроки.ЗначенияПолей = СтрокаКонтактнойИнформации.КонтактнаяИнформация;
