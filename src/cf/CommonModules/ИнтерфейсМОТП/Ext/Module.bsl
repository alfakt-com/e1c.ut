...
Функция ДеревоУпаковок(Значение, Детализация = Неопределено, ВариантПолученияМРЦ = "ВычислениеИЗапрос") Экспорт
	...
	Если ТипЗнч(Значение) = Тип("Строка") Тогда
		...
		Иначе
			...
			КодМаркировкиБезСкобок   = ИСМПТВыбытиеКодовМаркировкиКлиентСервер.ШтрихкодированиеИСМПКлиентСервер_КодМаркировкиБезСкобок(Значение);
	...
	ИначеЕсли ТипЗнч(Значение) = Тип("Массив") Тогда
		...
		Для Каждого КодМаркировки Из Значение Цикл
			...
			Иначе
				ЭтоЛогистическаяУпаковка = Истина;
				КодМаркировкиБезСкобок = ИСМПТВыбытиеКодовМаркировкиКлиентСервер.ШтрихкодированиеИСМПКлиентСервер_КодМаркировкиБезСкобок(КодМаркировки);
...
Функция ДанныеПродукцииПоШтрихкодуEAN(Значение) Экспорт
	...
	Если РезультатЗапросаСпискаПродукции.ПродукцияОрганизации <> Неопределено Тогда
		...
		Для Каждого КлючИЗначение Из ДанныеПродукцииПоШтрихкодуEAN Цикл
			...
			НайденнаяСтрока = ТаблицаПродукции.Найти(ИСМПТВыбытиеКодовМаркировкиКлиентСервер.ШтрихкодированиеИСКлиентСервер_GTINПоШтрихкодуEAN(ШтрихкодEAN), "GTIN");
...
Функция ЗапроситьСтатусыКодовМаркировки(КодыМаркировки, СтатусыКодовМаркировкиКеш = Неопределено) Экспорт
	...
	КлючСессии = ИСМПТВыбытиеКодовМаркировкиСервер.ИнтерфейсАвторизацииИСМПСлужебный_ПроверитьОбновитьКлючСессии(
	...
	РезультатЗапроса = ИСМПТВыбытиеКодовМаркировкиСервер.ИнтеграцияИСМП_ОтправитьДанныеВСервис(URLЗапроса, ТелоЗапроса, КлючСессии, "POST", ИнтерфейсМОТПКлиентСервер.ПараметрыОтправкиHTTPЗапросов());
...
Функция ЗапроситьДанныеОбАгрегацииКодовМаркировки(КодМаркировки) Экспорт
	...
	РезультатЗапроса = ИСМПТВыбытиеКодовМаркировкиСервер.ИнтеграцияИСМП_ПолучитьДанныеИзСервиса(URLЗапроса, КлючСессии, ИнтерфейсМОТПКлиентСервер.ПараметрыОтправкиHTTPЗапросов());
	...
	Для Каждого КлючИЗначение Из КодыМаркировкиУпаковок Цикл
		...
		Если ТипЗнч(КлючИЗначение) <> Тип("Строка") Тогда // Логистическая упаковка или блок
			...
			Если Детализация = Неопределено
				...
				ИСМПТВыбытиеКодовМаркировкиСервер.ИнтерфейсИСМПСлужебный_ПроверитьИсправитьДублиСтрокДереваУпаковок(КодМаркировкиУпаковки, Кеш.КодыМаркировки);
				...
				ИСМПТВыбытиеКодовМаркировкиСервер.ИнтерфейсИСМПСлужебный_ПроверитьДанныеСтроки(ДанныеСтроки, СтрокаДерева, "GTIN");
				ИСМПТВыбытиеКодовМаркировкиСервер.ИнтерфейсИСМПСлужебный_ПроверитьДанныеСтроки(ДанныеСтроки, СтрокаДерева, "ИННВладельца");
				ИСМПТВыбытиеКодовМаркировкиСервер.ИнтерфейсИСМПСлужебный_ПроверитьДанныеСтроки(ДанныеСтроки, СтрокаДерева, "МРЦ");
		...
		Иначе // Табачная пачка
			...
			ИСМПТВыбытиеКодовМаркировкиСервер.ИнтерфейсИСМПСлужебный_ПроверитьИсправитьДублиСтрокДереваУпаковок(КлючИЗначение, Кеш.КодыМаркировки);
			...
			Иначе
				...
				ИСМПТВыбытиеКодовМаркировкиСервер.ИнтерфейсИСМПСлужебный_ПроверитьДанныеСтроки(ДанныеСтроки, СтрокаДереваВременная, "ИННВладельца");
				ИСМПТВыбытиеКодовМаркировкиСервер.ИнтерфейсИСМПСлужебный_ПроверитьДанныеСтроки(ДанныеСтроки, СтрокаДереваВременная, "GTIN");
				ИСМПТВыбытиеКодовМаркировкиСервер.ИнтерфейсИСМПСлужебный_ПроверитьДанныеСтроки(ДанныеСтроки, СтрокаДереваВременная, "МРЦ");
