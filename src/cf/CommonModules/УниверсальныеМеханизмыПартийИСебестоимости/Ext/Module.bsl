...
Функция ДокументыСРазнымиПериодамиИлиОрганизациямиВДвижениях(РазныеПериоды, РазныеОрганизации, ИмяРегистра = "") Экспорт
	...
	// если Ложь, то есть только расчетные движения, которые формируются при расчете периода самого документа - при расчете
	// других периодов их не нужно затирать. Для движений других организаций Значение должно быть только Истина.
	Значение = Истина;
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка
			...
			ИначеЕсли ТипЗнч(Регистратор) = Тип("ДокументСсылка.КорректировкаРегистров")
			 ИЛИ ТипЗнч(Регистратор) = Тип("ДокументСсылка.РасчетСебестоимостиТоваров")
			 ИЛИ (ТипЗнч(Регистратор) = Тип("ДокументСсылка.ЗаявлениеОВвозеТоваров") И ИмяРегистраНакопления = "ПартииТоваровОрганизаций") Тогда
...
Функция ЭтоСуммируемыйРеквизитРегистра(МетаРеквизит)
	...
	Если СтрНайти(МетаРеквизит.Имя, "НДС") = 0 Тогда // в некоторых регистрах НДС хранится в реквизитах
...
Процедура ПроверкаЗатратНеРаспределенныхНаСебестоимость(ПараметрыПроверки) Экспорт
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеОтбораПоДатеДокументаАналитики", ТекстОтбора);
