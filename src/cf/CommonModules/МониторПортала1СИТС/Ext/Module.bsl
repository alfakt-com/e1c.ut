...
Процедура ИнтернетПоддержкаИСервисы_ПриСозданииНаСервере(Форма) Экспорт
	...
	Если Элементы.БИПМониторИнтернетПоддержки.Видимость Тогда
		КомандаФормы = Форма.Команды.БИПМониторИнтернетПоддержки;
		ОбщаяКомандаМониторПортала1СИТСМетаданные = Метаданные.ОбщиеКоманды.МониторПортала1СИТС;
		КомандаФормы.Заголовок = ОбщаяКомандаМониторПортала1СИТСМетаданные.Синоним;
		КомандаФормы.Подсказка = ОбщаяКомандаМониторПортала1СИТСМетаданные.Подсказка;
	КонецЕсли;
