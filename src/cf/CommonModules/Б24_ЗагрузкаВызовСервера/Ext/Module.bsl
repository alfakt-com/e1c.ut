...
#Область ПоискСозданиеСделки		
		...
		Если ЗначениеЗаполнено(ВнешнийКод) И НЕ ЗначениеЗаполнено(Сделка) тогда
			Сделка = Б24_СинхронизацияВызовСервера.ПолучитьЗначениеПоXML(Тип("ДокументСсылка.ПредзаказКлиента"), ВнешнийКод);
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(Сделка) тогда 
			Сделка = Документы.ПредзаказКлиента.СоздатьДокумент();
...
#КонецОбласти	
		...
		Сделка.НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДСУПокупателя;
