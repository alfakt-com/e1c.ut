...
//
Функция ПолучитьОтчетОРозничныхПродажахПоЧекуККМ(ЧекККМ, ОдинДокумент = Истина, ДокументВозврата = Неопределено) Экспорт
	...
	|	И ТаблицаПроверяемыеТовары.Ссылка.ЧекККМ = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ТоварыЧека.Серия КАК Серия,
	|	ТоварыЧека.Количество КАК Количество,
	|	ТоварыЧека.КоличествоУпаковок КАК КоличествоУпаковок
	|ПОМЕСТИТЬ ТоварыОтбора
	|ИЗ
	|	ТоварыЧека КАК ТоварыЧека
	|	ЛЕВОЕ СОЕДИНЕНИЕ ДанныеВозврата КАК ДанныеВозврата
	|	По ТоварыЧека.Номенклатура = ДанныеВозврата.Номенклатура
	|	И ТоварыЧека.Характеристика = ДанныеВозврата.Характеристика
	|ГДЕ
	|	ТоварыЧека.Количество - ЕСТЬNULL(ДанныеВозврата.Количество, 0) > 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ОтчетОРозничныхПродажахТовары.Серия КАК Серия
	...
	|		ПО ЧекККМ.КассоваяСмена = ОтчетОРозничныхПродажахТовары.Ссылка.КассоваяСмена
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтчетыОРозничныхпродажах.Ссылка КАК Ссылка,
	|	СУММА(1) КАК Поле1
	|ИЗ
	|	ОтчетыОРозничныхпродажах КАК ОтчетыОРозничныхпродажах
	|ГДЕ
	|	(ОтчетыОРозничныхпродажах.Номенклатура, ОтчетыОРозничныхпродажах.Характеристика, ОтчетыОРозничныхпродажах.Серия) В
	|			(ВЫБРАТЬ
	|				ТоварыОтбора.Номенклатура,
	|				ТоварыОтбора.Характеристика,
	|				ТоварыОтбора.Серия
	|			ИЗ
	|				ТоварыОтбора КАК ТоварыОтбора)
	|
	|СГРУППИРОВАТЬ ПО
	|	ОтчетыОРозничныхпродажах.Ссылка";
	
	Запрос.УстановитьПараметр("Ссылка", ЧекККМ);
	Запрос.УстановитьПараметр("ДокументВозврата", ?(ДокументВозврата = Неопределено, Документы.ВозвратТоваровОтКлиента.ПустаяСсылка(), ДокументВозврата));
	
	Выборка = Запрос.Выполнить().Выбрать();
	Если Не Выборка.Количество() = 0 И ОдинДокумент Тогда
		Выборка.Следующий();
		Возврат Выборка.Ссылка;
	ИначеЕсли Не Выборка.Количество() = 0 Тогда 
		Возврат Выборка;
	Иначе
...
Функция ВыполнитьОперациюЗакрытияКассовойСмены(Объект, ОшибкаПриСнятииZОтчета, ОписаниеОшибки = "") Экспорт
	...
	Если ЗначениеЗаполнено(КассаККМ) Тогда
		...
		ОтчетОРозничныхПродажах = РозничныеПродажиДляКазахстана.ЗаполнитьОтчетОРозничныхПродажахПоКассовойСмене(КассоваяСмена, ОписаниеОшибки, СтандартнаяОбработка);
		...
		Иначе
		Для Каждого СтрокаТаблицы Из ОтчетОРозничныхПродажах Цикл
