...
Процедура ОткрытьФормуПроверкиИПодбораМаркируемойПродукции(Форма, Знач ПараметрыОткрытияФормыПроверки) Экспорт
	...
	Если ПустаяСтрока(ПараметрыОткрытияФормыПроверки.ИмяРеквизитаФормыОбъект) Тогда
		...
		ПараметрыФормы.Вставить("НачальныйСтатусПроверки", ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияАлкогольнойПродукции.ВНаличии"));
	...
	Иначе
		...
		Иначе
			...
			ПараметрыФормы.Вставить("НачальныйСтатусПроверки", ПредопределенноеЗначение("Перечисление.СтатусыПроверкиНаличияАлкогольнойПродукции.НеПроверялась"));
		...
		Для Каждого СтрокаПродукции Из ТаблицаПродукции Цикл
			...
			Если КлючевыеЗначенияСовпадают Тогда
				...
				Иначе
					...
					ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормИСМПКлиентПереопределяемый_ПриИзмененииСерии(Форма,
...
Процедура ОткрытьФормуПроверкиИПодбораПриОтветеНаВопрос(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	ИСМПТВыбытиеКодовМаркировкиКлиент.ИнтеграцияИСКлиентПереопределяемый_ВыполнитьЗаписьОбъектаВФорме(Форма, Объект, ДействиеПослеЗаписи, СтандартнаяОбработка);
