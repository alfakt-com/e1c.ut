...
Процедура ПроверкаКорректностьРеализацииУказаннойВВозврате(ПараметрыПроверки) Экспорт
	...
	|	ВТВозвратТоваров.ВидЗапасовОтгрузки 				  КАК ВидЗапасовОтгрузки,
	|	СУММА(ЕСТЬNULL(ВТДвиженияРеализаций.НомерЗапроса, 0)) КАК НомерЗапроса
	|ПОМЕСТИТЬ ВТРасхожденияВозвратов
	|ИЗ
	|	ВТВозвратТоваров КАК ВТВозвратТоваров
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТДвиженияРеализаций КАК ВТДвиженияРеализаций
	|		ПО ВТВозвратТоваров.Организация = ВТДвиженияРеализаций.Организация
	|			И ВТВозвратТоваров.Реализация = ВТДвиженияРеализаций.Реализация
	|			И ВТВозвратТоваров.АналитикаУчетаНоменклатуры = ВТДвиженияРеализаций.АналитикаУчетаНоменклатуры
	|			И ВТВозвратТоваров.ВидЗапасовОтгрузки = ВТДвиженияРеализаций.ВидЗапасов
	|СГРУППИРОВАТЬ ПО
	...
	|ИМЕЮЩИЕ
	|	СУММА(ЕСТЬNULL(ВТДвиженияРеализаций.НомерЗапроса, 0)) = 0";
	...
	СписокПолей.Добавить("ВидЗапасовОтгрузки",			НСтр("ru='Вид запасов'"));
...
Процедура Использование_РаспределениеНДС(ПараметрыОбработчика) Экспорт
	...
	Если НЕ (МесяцРасчета%3 = 0) Тогда
...
Функция ПроверитьНаличиеЗаданийКЗакрытиюМесяца(ПараметрыОбработчика, ВыполнятьССамогоРаннего = Истина,
			ИмяТаблицыОтбораОрганизаций = "", ИмяРегистраЗаданий = "", ДействиеПодробнее = Неопределено, ТекстОшибки = Неопределено)
	...
	Если ЗначениеЗаполнено(Выборка.Месяц) Тогда
		...
		УстановитьСостояниеНеВыполнен(
			ПараметрыОбработчика,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				?(ТекстОшибки = Неопределено, НСтр("ru='Требуется пересчет операции %1 %2'"), ТекстОшибки),
...
Процедура ЗаполнитьПроверкиДляРегистрации(ТаблицаПроверок) Экспорт
	...
	ОтключенныеПроверки = ОтключенныеПроверки();
	...
	Для Каждого МетаЗначение Из Метаданные.Перечисления.ОперацииЗакрытияМесяца.ЗначенияПеречисления Цикл
		...
		Если ОтключенныеПроверки.Найти(Перечисления.ОперацииЗакрытияМесяца[МетаЗначение.Имя]) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
// Возвращает список отключенных проверок этапа закрытия месяца.
//
// Параметры:
//	ЭтапРасчета - ПеречислениеСсылка.ОперацииЗакрытияМесяца - этап закрытия месяца.
//
...
//	Строка - идентификатор служебной проверки.
//
Функция ОтключенныеПроверки()
	
	Результат = Новый Массив;
	Результат.Добавить(Перечисления.ОперацииЗакрытияМесяца.ПризнаниеВНалоговомУчетеЛизинговыхПлатежей);
...
Функция КоллекцияКонтрольныхРегистров()
	...
	Коллекция.Добавить("НДСЗаданияКЗакрытиюМесяца");
	Коллекция.Добавить("НДСКВозмещениюЗаданияКЗакрытиюМесяца");
