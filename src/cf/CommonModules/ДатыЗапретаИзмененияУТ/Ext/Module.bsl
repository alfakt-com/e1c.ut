...
Процедура ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения(Знач ИсточникиДанных) Экспорт
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "РегистрНакопления.ТМЦВЭксплуатации",            "Период", "СписанияОприходованияТоваров");
	
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "Документ.АктВыполненныхРабот",                      "ДатаПодписанияГЗ", "ПродажиВозвратыОтКлиентов", "Организация");
	...
	ДатыЗапретаИзменения.ДобавитьСтроку(ИсточникиДанных, "Документ.РеализацияУслугПрочихАктивов",             "ДатаПодписанияГЗ", "ПродажиВозвратыОтКлиентов", "Организация");
...
Процедура ПередПроверкойЗапретаИзменения(Объект, ВыполнятьПроверку, УзелПроверки, ВерсияОбъекта) Экспорт
	...
	Если ТипОбъекта = Тип("ДокументОбъект.АктВыполненныхРабот")
		 ИЛИ ТипОбъекта = Тип("ДокументОбъект.РеализацияУслугПрочихАктивов")
		 Тогда
		
		// Отказ от проверки с учетом того, что ДатаПодписанияГЗ, используемая в проверке, может быть указана позже.
		АктПодписанНоваяВерсия  = ЗначениеЗаполнено(Объект.ДатаПодписанияГЗ);
		АктПодписанСтараяВерсия = ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
		Объект.Ссылка, "ДатаПодписанияГЗ")) = Истина;
		
		Если Не АктПодписанНоваяВерсия И Не АктПодписанСтараяВерсия Тогда
			ВыполнятьПроверку = Ложь;
			УзелПроверки = Неопределено;
			
		ИначеЕсли Не АктПодписанНоваяВерсия Тогда
			ВерсияОбъекта = "СтараяВерсия"; // Проверить только новую версию объекта.
			
		ИначеЕсли Не АктПодписанСтараяВерсия Тогда
			ВерсияОбъекта = "НоваяВерсия"; // Проверить только старую версию объекта.
		КонецЕсли;
