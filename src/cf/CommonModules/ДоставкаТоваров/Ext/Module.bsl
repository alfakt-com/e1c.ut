...
Процедура ОбновитьСписокРаспоряженийНаДоставку(Форма, ОбновляемыеРаспоряжения = Неопределено) Экспорт
	...
	Зона 						= Форма.Зона;
	Склад 						= Форма.Склад;
	Город                       = Форма.кт_Города;//++Аян_К++ Заявка №5996 финиш 010922
	...
	Если ИсходящаяДоставка Тогда
		...
		//++Аян_К++ 070922 по заявке №5996 старт
		ТекстЗапроса = ПолучитьТекстЗапросаДоставка();
		//++Аян_К++ 070922 по заявке №5996 финиш
	...
	//+ТТ Оптимизация кода. Ускорение
	ТЗРаспоряжения = Пакет[Пакет.ВГраница()].Выгрузить();
	ТЗРаспоряжения.Свернуть("Распоряжение");
	ТЗРаспоряжения = ПолучитьСтатусыВМС(ТЗРаспоряжения.ВыгрузитьКолонку("Распоряжение"));
	...
	Пока Выборка.СледующийПоЗначениюПоля("Зона") Цикл
		...
		Пока Выборка.СледующийПоЗначениюПоля("Распоряжение") Цикл
			//Начало: МК Баубериков А - 20.01.2021
			СтатусВМС = ПолучитьСтатусВМС(Выборка.Распоряжение,ТЗРаспоряжения);
			Если ПустаяСтрока(СтатусВМС) ИЛИ СтатусВМС = "Отгрузка завершена" тогда
				Продолжить;
			КонецЕсли;
			ДатаОтгрузки = ПолучитьЖелаемуюДатуОтгрузки(Выборка.Распоряжение,ТЗРаспоряжения);
			Если НЕ ЗначениеЗаполнено(ДатаОтгрузки) ИЛИ ДатаОтгрузки < ТекущаяДата()-24*60*60*90 тогда
				Продолжить;
			КонецЕсли;
			//Конец: МК Баубериков А - 20.01.2021
			Если ВыборочноеОбновление Тогда
			...
			Пока Выборка.СледующийПоЗначениюПоля("Склад") Цикл
				...
				Пока Выборка.Следующий() Цикл
					...
					Если ИсходящаяДоставка Тогда
						...
						Если ОсталосьРаспределить > 0 Тогда
							...
							//ТекущаяСтрокаВесОбъем.статусВМС = ПолучитьСтатусВМС(стр.Ссылка)
				...
				Для Каждого СтрокаВесОбъемДата Из ВесОбъемРаспоряженийПоДатам Цикл
					...
					//Начало: МК Баубериков А - 25.08.2020
					СтрокаРаспоряжение.СтатусВМС = ПолучитьСтатусВМС(СтрокаРаспоряжение.Распоряжение,ТЗРаспоряжения);
					//Конец: МК Баубериков А - 25.08.2020
					//Начало: МК Баубериков А - 05.10.2020
					СтрокаРаспоряжение.ДатаОтгрузкиЗаказа = ПолучитьЖелаемуюДатуОтгрузки(СтрокаРаспоряжение.Распоряжение,ТЗРаспоряжения);
					//Конец: МК Баубериков А - 05.10.2020
					//Начало: МК Баубериков А - 19.10.2020
					СтрокаРаспоряжение.КлиентГотовПринять = ПолучитьКлиентГотовПринять(СтрокаРаспоряжение.Распоряжение);
					//Конец: МК Баубериков А - 19.10.2020 
...
Функция ПолучитьТекстЗапросаДоставка()
	...
	ТекстЗапроса = 
		...
		|ПОМЕСТИТЬ ДоставленныеОбъекты
		|ИЗ
		|	РегистрСведений.Infor_ИсторияСтатусовВМС КАК Infor_ИсторияСтатусовВМС
		|ГДЕ
		|	ВЫБОР
		|			КОГДА Infor_ИсторияСтатусовВМС.Объект ССЫЛКА Документ.ЗаказКлиента
		|				ТОГДА ВЫРАЗИТЬ(Infor_ИсторияСтатусовВМС.Объект КАК Документ.ЗаказКлиента).Дата >= ДОБАВИТЬКДАТЕ(&ТекущаяДатаСеанса, МЕСЯЦ, -3)
		|			КОГДА Infor_ИсторияСтатусовВМС.Объект ССЫЛКА Документ.ПеремещениеТоваров
		|				ТОГДА ВЫРАЗИТЬ(Infor_ИсторияСтатусовВМС.Объект КАК Документ.ПеремещениеТоваров).Дата >= ДОБАВИТЬКДАТЕ(&ТекущаяДатаСеанса, МЕСЯЦ, -3)
		|			ИНАЧЕ ЛОЖЬ
		|		КОНЕЦ
		|	И Infor_ИсторияСтатусовВМС.СтатусОбъекта В (""Часть отгружена"", ""Отгрузка завершена"")
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	РОТоварыПоРаспоряжениям.Распоряжение
		|ИЗ
		|	Документ.РасходныйОрдерНаТовары.ТоварыПоРаспоряжениям КАК РОТоварыПоРаспоряжениям
		|ГДЕ
		|	ВЫБОР
		|			КОГДА РОТоварыПоРаспоряжениям.Распоряжение ССЫЛКА Документ.ЗаказКлиента
		|				ТОГДА ВЫРАЗИТЬ(РОТоварыПоРаспоряжениям.Распоряжение КАК Документ.ЗаказКлиента).Дата >= ДОБАВИТЬКДАТЕ(&ТекущаяДатаСеанса, МЕСЯЦ, -3)
		|			КОГДА РОТоварыПоРаспоряжениям.Распоряжение ССЫЛКА Документ.ПеремещениеТоваров
		|				ТОГДА ВЫРАЗИТЬ(РОТоварыПоРаспоряжениям.Распоряжение КАК Документ.ПеремещениеТоваров).Дата >= ДОБАВИТЬКДАТЕ(&ТекущаяДатаСеанса, МЕСЯЦ, -3)
		|			ИНАЧЕ ЛОЖЬ
		|		КОНЕЦ
		|
		|ИНДЕКСИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДоставленныеОбъекты.Объект КАК Объект
		|ПОМЕСТИТЬ ДоставленныеОбъектыБезПовторов
		|ИЗ
		|	ДоставленныеОбъекты КАК ДоставленныеОбъекты
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|ПОМЕСТИТЬ РаспоряженияНаДоставку2
		|ИЗ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ОбновляемыеРаспоряжения КАК ОбновляемыеРаспоряжения
		...
		|	И РаспоряженияНаДоставку.Дата >= ДОБАВИТЬКДАТЕ(&ТекущаяДатаСеанса, МЕСЯЦ, -3)
		|	И ВЫБОР
		...
		|	РаспоряженияНаДоставку.Склад.ИспользоватьОрдернуюСхемуПриОтгрузке
		|		И РаспоряженияНаДоставку.Склад.ДатаНачалаОрдернойСхемыПриОтгрузке <= &ТекущаяДатаСеанса,
		...
		|	И (РаспоряженияНаДоставку.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПоручениеЭкспедиторуВПункте)
		|			ИЛИ РаспоряженияНаДоставку.Склад В ИЕРАРХИИ (&Склад))
		|	И НЕ РаспоряженияНаДоставку.Распоряжение В
		|				(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|					ДоставленныеОбъекты.Объект
		|				ИЗ
		|					ДоставленныеОбъекты)
		|	И РаспоряженияНаДоставку.Дата >= ДОБАВИТЬКДАТЕ(&ТекущаяДатаСеанса, МЕСЯЦ, -3)
		|
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	РаспоряженияНаДоставку2.Распоряжение КАК Распоряжение,
		|	РаспоряженияНаДоставку2.Склад КАК Склад,
		|	РаспоряженияНаДоставку2.ЭтоОрдерныйСклад КАК ЭтоОрдерныйСклад,
		|	РаспоряженияНаДоставку2.ПолучательОтправитель КАК ПолучательОтправитель,
		|	РаспоряженияНаДоставку2.СпособДоставки КАК СпособДоставки,
		|	РаспоряженияНаДоставку2.Зона КАК Зона,
		|	РаспоряженияНаДоставку2.Перевозчик КАК Перевозчик,
		|	РаспоряженияНаДоставку2.Дата КАК Дата,
		|	РаспоряженияНаДоставку2.ОсобыеУсловияПеревозки КАК ОсобыеУсловияПеревозки
		|ПОМЕСТИТЬ РаспоряженияНаДоставку
		|ИЗ
		|	РаспоряженияНаДоставку2 КАК РаспоряженияНаДоставку2
		|		ЛЕВОЕ СОЕДИНЕНИЕ ДоставленныеОбъектыБезПовторов КАК ДоставленныеОбъектыБезПовторов
		|		ПО РаспоряженияНаДоставку2.Распоряжение = ДоставленныеОбъектыБезПовторов.Объект
		|ГДЕ
		|	ДоставленныеОбъектыБезПовторов.Объект ЕСТЬ NULL
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|				ИЛИ РаспоряженияНаДоставку.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПоручениеЭкспедиторуВПункте))
		|ГДЕ
		...
		|	(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|		МАКСИМУМ(ВЫБОР
		|				КОГДА НАЧАЛОПЕРИОДА(ТоварыСУчетомДоставленных.Период, ДЕНЬ) < НАЧАЛОПЕРИОДА(&ТекущаяДатаСеанса, ДЕНЬ)
		...
		|		РаспоряженияНаДоставку.Зона КАК Зона,
		|		РаспоряженияНаДоставку.Перевозчик КАК Перевозчик,
		|		СУММА(ТоварыСУчетомДоставленных.КОтгрузке) КАК Количество,
		|		СУММА(ТоварыСУчетомДоставленных.КОтгрузке * (ВЫРАЗИТЬ(ВЫБОР
		...
		|						ТОГДА ВЫРАЗИТЬ(СправочникНоменклатура.ЕдиницаИзмерения.Числитель / СправочникНоменклатура.ЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))
		...
		|						ТОГДА ВЫРАЗИТЬ((ВЫРАЗИТЬ((ВЫРАЗИТЬ(СправочникНоменклатура.ЕдиницаИзмерения.Числитель / СправочникНоменклатура.ЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))) / (ВЫРАЗИТЬ(СправочникНоменклатура.ОбъемЕдиницаИзмерения.Числитель / СправочникНоменклатура.ОбъемЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))) КАК ЧИСЛО(15, 7))) / (ВЫРАЗИТЬ(СправочникНоменклатура.ОбъемЧислитель / СправочникНоменклатура.ОбъемЗнаменатель КАК ЧИСЛО(15, 7))) * (СправочникНоменклатура.ВесЧислитель / СправочникНоменклатура.ВесЗнаменатель) * (СправочникНоменклатура.ВесЕдиницаИзмерения.Числитель / СправочникНоменклатура.ВесЕдиницаИзмерения.Знаменатель) КАК ЧИСЛО(15, 7))
		...
		|							И ЕСТЬNULL(СправочникНоменклатура.ВесЗнаменатель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ВесЕдиницаИзмерения.Знаменатель, 0) <> 0
		|						ТОГДА ВЫРАЗИТЬ((ВЫРАЗИТЬ((ВЫРАЗИТЬ(СправочникНоменклатура.ЕдиницаИзмерения.Числитель / СправочникНоменклатура.ЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))) / (ВЫРАЗИТЬ(СправочникНоменклатура.ПлощадьЕдиницаИзмерения.Числитель / СправочникНоменклатура.ПлощадьЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))) КАК ЧИСЛО(15, 7))) / (ВЫРАЗИТЬ(СправочникНоменклатура.ПлощадьЧислитель / СправочникНоменклатура.ПлощадьЗнаменатель КАК ЧИСЛО(15, 7))) * (СправочникНоменклатура.ВесЧислитель / СправочникНоменклатура.ВесЗнаменатель) * (СправочникНоменклатура.ВесЕдиницаИзмерения.Числитель / СправочникНоменклатура.ВесЕдиницаИзмерения.Знаменатель) КАК ЧИСЛО(15, 7))
		...
		|							И ЕСТЬNULL(СправочникНоменклатура.ВесЗнаменатель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ВесЕдиницаИзмерения.Знаменатель, 0) <> 0
		|						ТОГДА ВЫРАЗИТЬ((ВЫРАЗИТЬ((ВЫРАЗИТЬ(СправочникНоменклатура.ЕдиницаИзмерения.Числитель / СправочникНоменклатура.ЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))) / (ВЫРАЗИТЬ(СправочникНоменклатура.ДлинаЕдиницаИзмерения.Числитель / СправочникНоменклатура.ДлинаЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))) КАК ЧИСЛО(15, 7))) / (ВЫРАЗИТЬ(СправочникНоменклатура.ДлинаЧислитель / СправочникНоменклатура.ДлинаЗнаменатель КАК ЧИСЛО(15, 7))) * (СправочникНоменклатура.ВесЧислитель / СправочникНоменклатура.ВесЗнаменатель) * (СправочникНоменклатура.ВесЕдиницаИзмерения.Числитель / СправочникНоменклатура.ВесЕдиницаИзмерения.Знаменатель) КАК ЧИСЛО(15, 7))
		...
		|						ТОГДА ВЫРАЗИТЬ(СправочникНоменклатура.ЕдиницаИзмерения.Вес * СправочникНоменклатура.ЕдиницаИзмерения.ВесЕдиницаИзмерения.Числитель / СправочникНоменклатура.ЕдиницаИзмерения.ВесЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))
		...
		|							И ЕСТЬNULL(СправочникНоменклатура.ВесЗнаменатель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ВесЕдиницаИзмерения.Знаменатель, 0) <> 0
		|						ТОГДА ВЫРАЗИТЬ(СправочникНоменклатура.ВесЧислитель / СправочникНоменклатура.ВесЗнаменатель * (СправочникНоменклатура.ВесЕдиницаИзмерения.Числитель / СправочникНоменклатура.ВесЕдиницаИзмерения.Знаменатель) КАК ЧИСЛО(15, 7))
		|					ИНАЧЕ 0
		|				КОНЕЦ / (ВЫРАЗИТЬ(1 КАК ЧИСЛО(15, 7))) КАК ЧИСЛО(15, 7)))) КАК Вес,
		|		СУММА(ТоварыСУчетомДоставленных.КОтгрузке * (ВЫРАЗИТЬ(ВЫБОР
		|					КОГДА СправочникНоменклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Объем)
		|							И СправочникНоменклатура.ОбъемИспользовать
		|							И ЕСТЬNULL(СправочникНоменклатура.ЕдиницаИзмерения.Знаменатель, 0) <> 0
		|						ТОГДА ВЫРАЗИТЬ(СправочникНоменклатура.ЕдиницаИзмерения.Числитель / СправочникНоменклатура.ЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))
		|					КОГДА СправочникНоменклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Вес)
		|							И СправочникНоменклатура.ОбъемИспользовать
		|							И ЕСТЬNULL(СправочникНоменклатура.ЕдиницаИзмерения.Знаменатель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ВесЕдиницаИзмерения.Числитель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ВесЕдиницаИзмерения.Знаменатель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ВесЧислитель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ВесЗнаменатель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ОбъемЗнаменатель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ОбъемЕдиницаИзмерения.Знаменатель, 0) <> 0
		|						ТОГДА ВЫРАЗИТЬ((ВЫРАЗИТЬ((ВЫРАЗИТЬ(СправочникНоменклатура.ЕдиницаИзмерения.Числитель / СправочникНоменклатура.ЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))) / (ВЫРАЗИТЬ(СправочникНоменклатура.ВесЕдиницаИзмерения.Числитель / СправочникНоменклатура.ВесЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))) КАК ЧИСЛО(15, 7))) / (ВЫРАЗИТЬ(СправочникНоменклатура.ВесЧислитель / СправочникНоменклатура.ВесЗнаменатель КАК ЧИСЛО(15, 7))) * (СправочникНоменклатура.ОбъемЧислитель / СправочникНоменклатура.ОбъемЗнаменатель) * (СправочникНоменклатура.ОбъемЕдиницаИзмерения.Числитель / СправочникНоменклатура.ОбъемЕдиницаИзмерения.Знаменатель) КАК ЧИСЛО(15, 7))
		|					КОГДА СправочникНоменклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Площадь)
		|							И СправочникНоменклатура.ОбъемИспользовать
		|							И ЕСТЬNULL(СправочникНоменклатура.ЕдиницаИзмерения.Знаменатель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ПлощадьЕдиницаИзмерения.Числитель, 0) <> 0
		...
		|							И ЕСТЬNULL(СправочникНоменклатура.ОбъемЗнаменатель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ОбъемЕдиницаИзмерения.Знаменатель, 0) <> 0
		|						ТОГДА ВЫРАЗИТЬ((ВЫРАЗИТЬ((ВЫРАЗИТЬ(СправочникНоменклатура.ЕдиницаИзмерения.Числитель / СправочникНоменклатура.ЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))) / (ВЫРАЗИТЬ(СправочникНоменклатура.ПлощадьЕдиницаИзмерения.Числитель / СправочникНоменклатура.ПлощадьЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))) КАК ЧИСЛО(15, 7))) / (ВЫРАЗИТЬ(СправочникНоменклатура.ПлощадьЧислитель / СправочникНоменклатура.ПлощадьЗнаменатель КАК ЧИСЛО(15, 7))) * (СправочникНоменклатура.ОбъемЧислитель / СправочникНоменклатура.ОбъемЗнаменатель) * (СправочникНоменклатура.ОбъемЕдиницаИзмерения.Числитель / СправочникНоменклатура.ОбъемЕдиницаИзмерения.Знаменатель) КАК ЧИСЛО(15, 7))
		|					КОГДА СправочникНоменклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Длина)
		|							И СправочникНоменклатура.ОбъемИспользовать
		|							И ЕСТЬNULL(СправочникНоменклатура.ЕдиницаИзмерения.Знаменатель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ДлинаЕдиницаИзмерения.Числитель, 0) <> 0
		...
		|							И ЕСТЬNULL(СправочникНоменклатура.ОбъемЗнаменатель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ОбъемЕдиницаИзмерения.Знаменатель, 0) <> 0
		|						ТОГДА ВЫРАЗИТЬ((ВЫРАЗИТЬ((ВЫРАЗИТЬ(СправочникНоменклатура.ЕдиницаИзмерения.Числитель / СправочникНоменклатура.ЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))) / (ВЫРАЗИТЬ(СправочникНоменклатура.ДлинаЕдиницаИзмерения.Числитель / СправочникНоменклатура.ДлинаЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))) КАК ЧИСЛО(15, 7))) / (ВЫРАЗИТЬ(СправочникНоменклатура.ДлинаЧислитель / СправочникНоменклатура.ДлинаЗнаменатель КАК ЧИСЛО(15, 7))) * (СправочникНоменклатура.ОбъемЧислитель / СправочникНоменклатура.ОбъемЗнаменатель) * (СправочникНоменклатура.ОбъемЕдиницаИзмерения.Числитель / СправочникНоменклатура.ОбъемЕдиницаИзмерения.Знаменатель) КАК ЧИСЛО(15, 7))
		|					КОГДА СправочникНоменклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Упаковка)
		|							И ЕСТЬNULL(СправочникНоменклатура.ЕдиницаИзмерения.ОбъемЕдиницаИзмерения.Знаменатель, 0) <> 0
		|						ТОГДА ВЫРАЗИТЬ(СправочникНоменклатура.ЕдиницаИзмерения.Объем * СправочникНоменклатура.ЕдиницаИзмерения.ОбъемЕдиницаИзмерения.Числитель / СправочникНоменклатура.ЕдиницаИзмерения.ОбъемЕдиницаИзмерения.Знаменатель КАК ЧИСЛО(15, 7))
		|					КОГДА (СправочникНоменклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.КоличествоШтук)
		|							ИЛИ СправочникНоменклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.ПустаяСсылка)
		|							ИЛИ ЕСТЬNULL(СправочникНоменклатура.ЕдиницаИзмерения.Ссылка, ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка))
		|							И СправочникНоменклатура.ОбъемИспользовать
		|							И ЕСТЬNULL(СправочникНоменклатура.ОбъемЗнаменатель, 0) <> 0
		|							И ЕСТЬNULL(СправочникНоменклатура.ОбъемЕдиницаИзмерения.Знаменатель, 0) <> 0
		|						ТОГДА ВЫРАЗИТЬ(СправочникНоменклатура.ОбъемЧислитель / СправочникНоменклатура.ОбъемЗнаменатель * (СправочникНоменклатура.ОбъемЕдиницаИзмерения.Числитель / СправочникНоменклатура.ОбъемЕдиницаИзмерения.Знаменатель) КАК ЧИСЛО(15, 7))
		|					ИНАЧЕ 0
		...
		|		ТаблицыИсточника.Количество КАК Количество
		|	ИЗ
		|		ВтДвиженияКРаспределениюПоОрдернойСхеме КАК ТаблицыИсточника) КАК ТаблицыИсточника
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ТаблицыИсточника.Распоряжение.Проведен КАК ПризнакОплаты
		|ИЗ
		|	(ВЫБРАТЬ
		|		ТаблицыИсточника.Дата КАК Дата,
		|		ТаблицыИсточника.Номенклатура КАК Номенклатура,
		...
		|		ТаблицыИсточника.ПолеУпорядочивания КАК ПолеУпорядочивания
		|	ИЗ
		|		ВтРаспоряженияНаДоставкуПолучателямПоОрдернойСхеме КАК ТаблицыИсточника) КАК ТаблицыИсточника
		|ГДЕ
		|	ТаблицыИсточника.Дата >= &ДатаНачала
		|	И ТаблицыИсточника.Дата <= &ДатаОкончания
		|
		|УПОРЯДОЧИТЬ ПО
...
//Начало: МК Баубериков А - 19.10.2020
Функция ПолучитьКлиентГотовПринять(ТекРаспоряжение)
	
	КлиентГотовПринять = Ложь;
	
	Если ТипЗнч(ТекРаспоряжение) = Тип("ДокументСсылка.ЗаказКлиента") тогда
		КлиентГотовПринять = ТекРаспоряжение.КлиентГотовПринять;
	КонецЕсли;
	
	Возврат КлиентГотовПринять;	
...
//Начало: МК Баубериков А - 05.10.2020
Функция ПолучитьЖелаемуюДатуОтгрузки(ЗаказКлиента, ТЗ)
	
	ДатаОтгрузки = Дата('00010101');
	
	Нашел = ТЗ.Найти(ЗаказКлиента);
	Если НЕ Нашел = Неопределено Тогда
		ДатаОтгрузки = Нашел.ЖелаемаяДатаОтгрузки;	
	КонецЕсли;
...
//Начало: МК Баубериков А - 25.08.2020
Функция ПолучитьСтатусВМС(ЗаказКлиента, ТЗ)
...
Функция ПолучитьСтатусыВМС(ЗаказКлиента)
	
	Запрос = Новый Запрос;
	Запрос.Текст=
	"ВЫБРАТЬ
	|	Infor_ИсторияСтатусовВМС.Объект КАК Объект,
	|	МАКСИМУМ(Infor_ИсторияСтатусовВМС.ДатаИзменения) КАК ДатаИзменения
	|ПОМЕСТИТЬ ВременнаяТаблица
	|ИЗ
	|	РегистрСведений.Infor_ИсторияСтатусовВМС КАК Infor_ИсторияСтатусовВМС
	|ГДЕ
	|	Infor_ИсторияСтатусовВМС.Объект В(&ЗаказКлиента)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	МАКСИМУМ(Infor_ИсторияСтатусовВМС.СтатусОбъекта) КАК Статус,
	|	ЕСТЬNULL(ПеремещениеТоваровДополнительныеРеквизиты.Значение, ВременнаяТаблица.Объект.ЖелаемаяДатаОтгрузки) КАК ЖелаемаяДатаОтгрузки
	|ИЗ
	|	ВременнаяТаблица КАК ВременнаяТаблица
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Infor_ИсторияСтатусовВМС КАК Infor_ИсторияСтатусовВМС
	|		ПО ВременнаяТаблица.Объект = Infor_ИсторияСтатусовВМС.Объект
	|			И (Infor_ИсторияСтатусовВМС.ДатаИзменения = ВременнаяТаблица.ДатаИзменения)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПеремещениеТоваров.ДополнительныеРеквизиты КАК ПеремещениеТоваровДополнительныеРеквизиты
	|		ПО ВременнаяТаблица.Объект = ПеремещениеТоваровДополнительныеРеквизиты.Ссылка
	|			И (ПеремещениеТоваровДополнительныеРеквизиты.Свойство.Имя = ""ЖелаемаяДатаОтгрузки_Доп"")
	|
	|СГРУППИРОВАТЬ ПО
	|	ВременнаяТаблица.Объект,
	|	ЕСТЬNULL(ПеремещениеТоваровДополнительныеРеквизиты.Значение, ВременнаяТаблица.Объект.ЖелаемаяДатаОтгрузки)";
...
Функция ТекстЗапросаВтРаспоряженияНаДоставку(ДоставкаПолучателям)
	...
	Иначе
		...
		|			ИЛИ НЕ ТоварыКДоставке.ЗаданиеНаПеревозку.Проведен)
		|	И РаспоряженияНаДоставку.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияДоставки.ТребуетсяДоставка)
		|	И РаспоряженияНаДоставку.Дата >= ДобавитьКДате(&ТекущаяДатаСеанса,Месяц,-3) // +ТТ. Оптимизация. Ускорение
		|	И ВЫБОР
		|			КОГДА &ОтборПоТипуИсполнителей = ЗНАЧЕНИЕ(Перечисление.ТипыИсполнителейЗаданийНаПеревозку.НашаТранспортнаяСлужба)
