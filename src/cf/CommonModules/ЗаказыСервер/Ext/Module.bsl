...
Функция ОтменитьНеотработанныеСтрокиПоОтгрузке(ДокументОбъект, ПараметрыЗаполнения, ПараметрыОтмены) Экспорт
	...
	Запрос.УстановитьПараметр("Товары", ТаблицаВыбранныхСтрок);
	...
	ОформленоПоОрдерам = РегистрыНакопления.ТоварыКОтгрузке.ТаблицаОформлено(Запрос.Выполнить().Выгрузить(), Корректировка);
	...
	// Для взаимосвязного распределения количества ордеров учтем предшедствующее распределение количества накладных по кодам строк
...
Функция ОтменитьНеотработанныеСтрокиПоПриемке(ДокументОбъект, ПараметрыЗаполнения, ПараметрыОтмены) Экспорт
	...
	Запрос.Текст = 
		...
		|	И Склады.ДатаНачалаОрдернойСхемыПриПоступлении <= &ТекущаяДата";
	...
	ОформленоПоНакладным = ПараметрыЗаполнения.МенеджерРегистра.ТаблицаОформлено(ТаблицаВыбранныхСтрок, ПараметрыЗаполнения.ОтборПоИзмерениям);
	ОформленоПоОрдерам = РегистрыНакопления.ТоварыКПоступлению.ТаблицаОформлено(Запрос.Выполнить().Выгрузить(), Корректировка);
