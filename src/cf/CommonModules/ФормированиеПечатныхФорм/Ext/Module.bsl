...
//  Строка - Имя дополнительной колонки.
//
Функция ИмяДополнительнойКолонкиПоИнвентаризациям() Экспорт
	
	ДопКолонка =  Константы.ДополнительнаяКолонкаПечатныхФормДокументовИнентаризации.Получить();
...
Функция ОписаниеОрганизации(СписокСведений, Список = "", СПрефиксом = Истина) Экспорт
	...
	ПрефиксыРеквизитов.Вставить("ИНН",			НСтр("ru='БИН / ИИН %1'"));
...
Функция СведенияОЮрФизЛице(ЮрФизЛицо, ДатаПериода, ДляФизЛицаТолькоИнициалы = Истина, Знач БанковскийСчет = Неопределено) Экспорт
	...
	Сведения = Новый Структура("Представление, СокращенноеНаименование, ПолноеНаименование, НаименованиеДляПечатныхФорм,
		|СерияСвидетельстваПоНДС, НомерСвидетельстваПоНДС, ДатаСвидетельстваПоНДС,
		|КодПоОКПО, КодОКВЭД, ИНН, КПП, КБЕ, Телефоны, ЮридическийАдрес, Банк, БИК, КоррСчет, НомерСчета");
	...
	Если ЗначениеЗаполнено(ЮрФизЛицо)
	   И (ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Организации")
		...
		Если ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Контрагенты") И ЮрФизЛицо.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицоНеРезидент Тогда
			Сведения.Вставить("ИНН", 					 Реквизиты.РегистрационныйНомер);
		Иначе
			Сведения.Вставить("ИНН", 					 Реквизиты.ИНН);
		КонецЕсли;
		Сведения.Вставить("КБЕ", 						 Реквизиты.КБЕ);
		...
		Если ТипЗнч(ЮрФизЛицо) = Тип("СправочникСсылка.Организации") Тогда
			...
			Сведения.Вставить("СерияСвидетельстваПоНДС", Реквизиты.СерияСвидетельстваПоНДС);
			Сведения.Вставить("НомерСвидетельстваПоНДС", Реквизиты.НомерСвидетельстваПоНДС);
			Сведения.Вставить("ДатаСвидетельстваПоНДС",  Реквизиты.ДатаСвидетельстваПоНДС);
			Сведения.Вставить("ГоловнаяОрганизация", 	 Реквизиты.ГоловнаяОрганизация);
			Сведения.Вставить("ОбособленноеПодразделение",Реквизиты.ОбособленноеПодразделение);
		Иначе
			Сведения.Вставить("ОфициальноеНаименование", Реквизиты.Наименование);
			Сведения.Вставить("Свидетельство", 			 "");
			Сведения.Вставить("Свидетельство", 			 "");
			Сведения.Вставить("СерияСвидетельстваПоНДС", "");
			Сведения.Вставить("НомерСвидетельстваПоНДС", "");
			Сведения.Вставить("ДатаСвидетельстваПоНДС",  "");
			Сведения.Вставить("ГоловнаяОрганизация", 	 Справочники.Организации.ПустаяСсылка());
			Сведения.Вставить("ОбособленноеПодразделение",Ложь);
		КонецЕсли;
...
Функция СоответствиеСтавокИСуммНДС() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		СоответствиеСтавокНДС.Вставить(Выборка.Ссылка, 0);
	КонецЦикла;
...
Функция СодержитсяВПодсистеме(ИмяПодсистемы, ОбъектМетаданных)
	...
	Для Каждого ПодчиненнаяПодстистема Из МетаданныеПодсистемы.Подсистемы Цикл
		
		Если ПодчиненнаяПодстистема.Состав.Содержит(ОбъектМетаданных) Тогда
