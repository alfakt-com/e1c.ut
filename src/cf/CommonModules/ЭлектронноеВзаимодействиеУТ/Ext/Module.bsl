...
Функция НайтиСоздатьНоменклатуруПоставщика(Контрагент, Идентификатор, Наименование) Экспорт
...
Процедура ЗаполнитьДанныеСчетаФактурыУПД(СтруктураДанных, СтруктураЭД, ДеревоДанных) Экспорт
...
Функция ПодготовитьСтруктуруДляСчетаФактурыУПД(ДеревоДанных) Экспорт
	...
	Если ВидСчетаФактуры = "Авансовый" Тогда
		...
		Для Каждого СведенияОТоваре Из СведенияОТоварах.Строки Цикл
			...
			НоваяСтрока.СтавкаНДС = ?(СтавкаНДС = "НДС исчисляется налоговым агентом", Перечисления.СтавкиНДС.НДС18_118, СтавкаНДС);
