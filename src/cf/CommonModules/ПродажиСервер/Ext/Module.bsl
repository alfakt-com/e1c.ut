...
Функция ПерезаполнитьНакладнуюПоОрдерам(Объект) Экспорт
	...
	Если Объект.Контрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо тогда
		СтруктураДействийСИзмененнымиСтроками.Вставить("swОкруглятьСуммуДоЦелого");
	КонецЕсли;
	// sw end
...
Функция ПредставлениеСчетаФактурыВДокументеПродажи(Основание, Организация, НеТребуется = Ложь) Экспорт
	...
	Если СчетаФактуры.Количество() > 0 Тогда
		...
		Если Не РеквизитыСчетаФактуры.Корректировочный 
			...
			СтрокаГиперссылки = Новый ФорматированнаяСтрока(
				НСтр("ru='Требуется дополнительный/исправительный счет-фактура'"), , 
				WebЦвета.Кирпичный, ,
...
Процедура ЗаполнитьВозвращаемыеТоварыПоЧеку (ДокументВозврата, ИмяТаблицы) Экспорт
	...
	|			И ЧекиККМ.Характеристика = ДанныеВозврата.Характеристика
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах КАК ОтчетОРозничныхПродажах
	|		ПО ОтчетОРозничныхПродажах.КассоваяСмена = &ДокументРеализации
	|ГДЕ
	|	ЧекиККМ.Количество - ЕСТЬNULL(ДанныеВозврата.Количество, 0) - ЕСТЬNULL(ЧекиККМВозврат.Количество, 0) > 0
	|	И ЧекиККМ.Номенклатура.ТипНоменклатуры В
	|		(ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
...
Процедура СформироватьЗапросКорректностьУказанияСтавкиНДС(ТекстЗапроса)
	...
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС0)
	...
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС12)
	...
	|				ТОГДА ЛОЖЬ
	|			КОГДА ВременнаяТаблицаДокументПродажи.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт)
	|					И ВременнаяТаблицаТовары.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС0)
	|				ТОГДА ИСТИНА
	|			КОГДА ВременнаяТаблицаДокументПродажи.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОблагаетсяНДСУПокупателя)
	|					И ВременнаяТаблицаТовары.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС12)
	|				ТОГДА ИСТИНА
	|			КОГДА (ВременнаяТаблицаДокументПродажи.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|					ИЛИ ВременнаяТаблицаДокументПродажи.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД))
...
Процедура СформироватьЗапросКорректностьСуммыНДС(ТекстЗапроса)
	...
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС12) ТОГДА
	|			ВЫБОР КОГДА ВременнаяТаблицаДокументПродажи.ЦенаВключаетНДС ТОГДА
	|				ВременнаяТаблицаТовары.СуммаНДС <> ВЫРАЗИТЬ((ВременнаяТаблицаТовары.Сумма * 0.12 /1.12) КАК ЧИСЛО(15,2))
	|			ИНАЧЕ
	|				ВременнаяТаблицаТовары.СуммаНДС <> ВЫРАЗИТЬ((ВременнаяТаблицаТовары.Сумма * 0.12) КАК ЧИСЛО(15,2))
	|			КОНЕЦ
	...
	|		КОГДА ВременнаяТаблицаТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС12) ТОГДА
	|			ВЫБОР КОГДА ВременнаяТаблицаДокументПродажи.ЦенаВключаетНДС ТОГДА
	|				ВЫРАЗИТЬ((ВременнаяТаблицаТовары.Сумма * 0.12 /1.12) КАК ЧИСЛО(15,2))
	|			ИНАЧЕ
	|				ВЫРАЗИТЬ((ВременнаяТаблицаТовары.Сумма * 0.12) КАК ЧИСЛО(15,2))
	|			КОНЕЦ
	|		КОГДА ВременнаяТаблицаТовары.СтавкаНДС В (ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18), ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18_118)) ТОГДА
	|			ВЫБОР КОГДА ВременнаяТаблицаДокументПродажи.ЦенаВключаетНДС ТОГДА
