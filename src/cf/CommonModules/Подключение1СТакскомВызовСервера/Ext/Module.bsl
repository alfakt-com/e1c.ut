...
Функция НовыйКонтекстВзаимодействия(
	...
	СтруктураЗапуска = Подключение1СТакскомКлиентСервер.ОпределитьВозможностьЗапускаПоМестуИПараметрам(
		МестоЗапуска,
		ПараметрыИПП);
	...
	ОсновныеПараметры.ИмяWSОпределения      = Подключение1СТакскомКлиентСервер.АдресWSDLСервисаБизнесПроцессов();
	ОсновныеПараметры.URIСервиса            = Подключение1СТакскомКлиентСервер.URIСервисаСервисаБизнесПроцессов();
...
Функция КонфигурацияЗарегистрированаВСервисеИПП(ОшибкаОбращенияКВебСервису = Ложь) Экспорт
	...
	Возврат Подключение1СТакскомКлиентСервер.КонфигурацияЗарегистрированаВСервисеИПП(
...
Процедура ОбработкаКомандСервиса(
	...
	Если НЕ Продолжение Тогда
		...
		Подключение1СТакскомКлиентСервер.ДобавитьСессионныеПараметрыКЗапросу(
			КСКонтекст,
			ПараметрыЗапроса,
			МассивСессионныхПараметровЗапроса);
		
		Если СоздаваемыйКонтекстВзаимодействия <> Неопределено Тогда
			Подключение1СТакскомКлиентСервер.ДобавитьДополнительныеСтартовыеПараметрыЗапроса(
				КСКонтекст,
...
Процедура ДобавитьКомандыСервиса(
	...
	Попытка
		...
		Подключение1СТакскомКлиентСервер.ДобавитьКомандыСервиса(
			ОписаниеСервисаИППНаСервере,
			ДополнительныеПараметры,
			КонтекстОбработчика,
			ОсновныеПараметры,
...
Процедура ВыполнитьКомандуСервиса(КСКонтекст, СтруктураКоманды, КонтекстОбработчика)
	...
	ИначеЕсли ИмяКоманды = "store.putorganizations" Тогда
		Подключение1СТакскомКлиентСервер.ЗаписатьСписокОрганизаций(КСКонтекст, СтруктураКоманды);
		
	ИначеЕсли ИмяКоманды = "store.putadressclassifier" Тогда
		Подключение1СТакскомКлиентСервер.ЗаписатьАдресныйКлассификатор(КСКонтекст, СтруктураКоманды);
...
Процедура ВыполнениеКомандыПрочитатьПараметры(
	...
	Для Каждого Параметр Из МассивПараметров Цикл
		...
		ЗначениеПараметра = Подключение1СТакскомКлиентСервер.ЗначениеСессионногоПараметра(
			КСКонтекст,
			Параметр.Имя);
	...
	Подключение1СТакскомКлиентСервер.ДобавитьСессионныеПараметрыКЗапросу(КСКонтекст, ПараметрыЗапроса);
...
Процедура ПодготовитьДанныеОбОрганизации(КСКонтекст, КонтекстОбработчика)
	...
	Организация = Подключение1СТакскомКлиентСервер.ЗначениеСессионногоПараметра(КСКонтекст,
		"IDOrganizationED");
...
Процедура ПодготовитьДанныеСертификатаПоОтпечатку(КСКонтекст, КонтекстОбработчика)
	...
	ОтпечатокСертификата = Подключение1СТакскомКлиентСервер.ЗначениеСессионногоПараметра(
		КСКонтекст,
