...
Функция ОпределитьНаличиеСчетовФактурПолученных(ПараметрыПоискаСчетовФактур) Экспорт
	...
	|ГДЕ
	|	ЛОЖЬ И ВозвратТоваровОтКлиента.Проведен
	...
	|	И ВозвратТоваровПоставщику.Дата МЕЖДУ &ПериодНачало И &ПериодОкончание
	|	И ВозвратТоваровПоставщику.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|	И ВозвратТоваровПоставщику.Организация В (&МассивОрганизаций)
	|	И (ВЫБОР
	|			КОГДА &Фильтр И &ВсеКромеФильтра
	|				ТОГДА ВозвратТоваровПоставщику.Ссылка НЕ В (&СписокФильтр)
	|			КОГДА &Фильтр И НЕ &ВсеКромеФильтра
	|				ТОГДА ВозвратТоваровПоставщику.Ссылка В (&СписокФильтр)
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	И ОтчетКомиссионера.СтавкаНДСВознаграждения <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|	И ОтчетКомиссионера.СтавкаНДСВознаграждения <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК)
	...
	|	И ОтчетПоКомиссииМеждуОрганизациями.СтавкаНДСВознаграждения <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|	И ОтчетПоКомиссииМеждуОрганизациями.СтавкаНДСВознаграждения <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК)
	...
	|ГДЕ
	|	ЛОЖЬ И КорректировкаРеализации.ВидКорректировки = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратНедопоставленногоТовара)
	...
	|ПОМЕСТИТЬ ВтОснованияСФПолученныеИсправления
	|ИЗ
	|	Документ.КорректировкаПриобретения КАК КорректировкаПриобретения
	|ГДЕ
	...
	|	ЕСТЬNULL(СчетФактураПолученный.Ссылка,ЕСТЬNULL(СчетФактураПолученныйНА.Ссылка,ЕСТЬNULL(ЗаписьКнигиПокупок.Ссылка,НЕОПРЕДЕЛЕНО))) КАК СчетФактура,
	|	ЕСТЬNULL(СчетФактураПолученный.Ссылка.Проведен,ЕСТЬNULL(СчетФактураПолученныйНА.Ссылка.Проведен,ЕСТЬNULL(ЗаписьКнигиПокупок.Проведен,ЛОЖЬ))) КАК СчетФактураПроведен,
	|	ЕСТЬNULL(СчетФактураПолученный.Ссылка.Дата,ЕСТЬNULL(СчетФактураПолученныйНА.Ссылка.Дата,ЕСТЬNULL(ЗаписьКнигиПокупок.ДатаПолучения,ДАТАВРЕМЯ(1, 1, 1)))) КАК СчетФактураДата,
	|	ЕСТЬNULL(СчетФактураПолученный.Ссылка.Номер,ЕСТЬNULL(СчетФактураПолученныйНА.Ссылка.Номер,ЕСТЬNULL(ЗаписьКнигиПокупок.НомерСчетаФактуры,""""))) КАК СчетФактураНомер,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаписьКнигиПокупок КАК ЗаписьКнигиПокупок
	|		ПО ОснованияСФПолученные.Основание = ЗаписьКнигиПокупок.ДокументРасчетов
	...
	|			ТОГДА НЕ СчетФактураПолученный.Ссылка ЕСТЬ NULL ИЛИ НЕ СчетФактураПолученныйНА.Ссылка ЕСТЬ NULL ИЛИ НЕ ЗаписьКнигиПокупок.Ссылка ЕСТЬ NULL
	|		ИНАЧЕ
	|			СчетФактураПолученный.Ссылка ЕСТЬ NULL И СчетФактураПолученныйНА.Ссылка ЕСТЬ NULL И ЗаписьКнигиПокупок.Ссылка ЕСТЬ NULL
	|	КОНЕЦ)
	|	И (ВЫБОР
	|			КОГДА &СчетФактураПроведен = НЕОПРЕДЕЛЕНО
	|				ТОГДА ИСТИНА
	|			КОГДА &СчетФактураПроведен
	|				ТОГДА ЕСТЬNULL(СчетФактураПолученный.Ссылка.Проведен,ЕСТЬNULL(СчетФактураПолученныйНА.Ссылка.Проведен,ЕСТЬNULL(ЗаписьКнигиПокупок.Проведен,НЕОПРЕДЕЛЕНО))) = ИСТИНА
	|			ИНАЧЕ
	|				ЕСТЬNULL(СчетФактураПолученный.Ссылка.Проведен,ЕСТЬNULL(СчетФактураПолученныйНА.Ссылка.Проведен,ЕСТЬNULL(ЗаписьКнигиПокупок.Проведен,НЕОПРЕДЕЛЕНО))) = ЛОЖЬ
	|		КОНЕЦ)
	|	И (ВЫБОР
	...
	|			КОГДА НЕ ЗаписьКнигиПокупок.Ссылка ЕСТЬ NULL
	|				ТОГДА ЗаписьКнигиПокупок.ДатаСчетаФактуры <= &ДатаСФНеБолее
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ЛОЖЬ КАК ЭтоСчетФактураПолученный // признак используется в отчете о наличии с/ф для специального форматирования в БП представления документа, если ЛОЖЬ, то общий порядок
	|ИЗ
	|	ВтОснованияСФПолученныеИсправления КАК ОснованияСФПолученныеИсправления
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураПолученный.ДокументыОснования КАК СчетФактураПолученный
...
Функция ВерсияПостановленияНДС1137(Период) Экспорт
	...
	// изменения в Постановление № 981 не актуальный для ведения учета РК (определение ПериодЗаписиЖурнала)
	Возврат 3;
...
Функция ТекстЗапросаНомераГТДпоСчетамФактурам(Запрос) Экспорт
	...
	|ИЗ
	|	Документ.СчетфактураПолученный.ДокументыОснования КАК СФОснования
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровПоставщику.ВидыЗапасов КАК Строки
	|			ПО СФОснования.ДокументОснование = Строки.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетфактураПолученный КАК Счетфактура
	|			ПО СФОснования.Ссылка = Счетфактура.Ссылка
	|ГДЕ
...
Функция ТекстЗапросаИнформацияПоТНВЭД() Экспорт
	...
	|	ВЫБОР
	|		КОГДА РеализацияТоваровУслугТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС0)
	...
	|	СчетаФактурыВЕАЭС.СчетФактура,
	|	ВЫБОР
	|		КОГДА ЗаписьКнигиПродажЦенности.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС0)
	...
	|	СчетаФактурыВЕАЭС.СчетФактура,
	|	ВЫБОР
	|		КОГДА КорректировкаРеализацииТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС0)
