...
Процедура ПроверитьПолучитьВариантКомплектации(ТекущаяСтрока, СтруктураДействий)
	...
	Если ЗначениеЗаполнено(ТекущаяСтрока.Номенклатура) Тогда
		...
		Если Не СтрутураРезультата.ПринадлежитНоменклатуреХарактеристике Тогда
...
Процедура ЗаполнитьСтавкуНДСВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) 
	...
	Если СтруктураДействий.Свойство("ЗаполнитьСтавкуНДС", НалогообложениеНДС) Тогда
		...
		Если СтруктураДействий.Свойство("СтавкаНДС0") Тогда 
			Договор    = СтруктураДействий.СтавкаНДС0.Договор;
			Контрагент = СтруктураДействий.СтавкаНДС0.Контрагент;
			Если Не Договор.Пустая() И Не Контрагент.Пустая() 
				И (Контрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ИндивидуальныйПредприниматель ИЛИ Контрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо) 
				И Договор.СтавкаНДС = Справочники.СтавкиНДС.НДС0 Тогда
...
Функция ПроверитьGrohe(Номенклатура)
	...
	Запрос.УстановитьПараметр("Производитель", Справочники.Производители.НайтиПоНаименованию("Grohe"));
...
Процедура ЗаполнитьЦенуПродажиВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения)
	...
	//++Аян_К++ 140322 Украинский конфликт границы закрыты для Grohe
	Если НЕ Альфа_КТ.ПроверитьДоступ("ГроеОпт", ПараметрыСеанса.ТекущийПользователь, ТекущаяДата(), ТекущаяДата()) Тогда 
		Если ТекущаяСтрока.ВидЦены = Справочники.ВидыЦен.НайтиПоНаименованию("ОПТ") Тогда 
			Если ПроверитьGrohe(ТекущаяСтрока.Номенклатура) Тогда
				СтруктураПараметровОтбора.ВидЦены = Справочники.ВидыЦен.НайтиПоНаименованию("Розничная");
				ТекущаяСтрока.ВидЦены             = Справочники.ВидыЦен.НайтиПоНаименованию("Розничная");
				Сообщить("ОПТ для Grohe продавать нельзя!", СтатусСообщения.ОченьВажное);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	///Сергей 07.10.2020 +++ Расчет "Скидок", "Спеччен"     
	Если СтруктураДействий.Свойство("ЭтоЗаказ") Тогда 
		СпецЦена = new_МаркетинговыеАктивности.ПолучитьДействующиеСпецЦену(СтруктураПараметровОтбора, СтруктураДействий.ПодразделениеЗаказ);
		Если СпецЦена <> Неопределено и СпецЦена > 0  Тогда
			ТекущаяСтрока[ИмяПоляЦена] = СпецЦена;
			new_ЭтоСпецЦена = Истина;
		Иначе 
			new_ЭтоСпецЦена = Ложь;	
		КонецЕсли;
		Попытка
			ТекущаяСтрока.new_ЭтоСпецЦена = new_ЭтоСпецЦена;
		Исключение
		КонецПопытки;
	КонецЕсли;                                                                                                                                            
	///Сергей 07.10.2020 --- Расчет "Скидок", "Спеччен" 
...
Функция ШаблонПоляВыборкиПоКлючуДействия(КлючДействия, ТолькоПроверитьПоддерживаемыеКлючи = Ложь)
	...
	Если КлючДействия = "ЗаполнитьПризнакМаркируемаяПродукция" Тогда
		...
		Шаблон =  ", " + ИСМПТВыбытиеКодовМаркировкиСервер.ИнтеграцияИСУТ_ОпределениеПризнакаМаркируемаяПродукцияТекстаЗапроса("втТаблицаНоменклатуры.%Ключ%");
	...
	Если КлючДействия = "ЗаполнитьВидПродукцииИС" Тогда
		...
		Возврат ",
		|	ВЫБОР 
		|		КОГДА втТаблицаНоменклатуры.%Ключ%.АлкогольнаяПродукция ТОГДА
		...
		|		КОГДА втТаблицаНоменклатуры.%Ключ%.ОбувнаяПродукция
