...
// Кэш для для фукнции Справочники.ВидыЗапасов.ВидЗапасовПоОтборам
//
// Параметры:
//  Организация				 - 	 - 
//  ТипЗапасов				 - 	 - 
...
Функция ВидЗапасовПоОтборам(Организация,
	...
	ГруппаФинансовогоУчета,
	ГруппаПродукции,
	ВидЦены) Экспорт // Количество параметров певышает описанное в стандарте, но тут это осознанное решение для работы кэша
...
//
Функция НалогообложениеНДС(Знач Организация, Знач Склад = Неопределено, Знач Договор = Неопределено, НаправлениеДеятельности = Неопределено, Знач Дата = Неопределено) Экспорт
...
// Возвращает деятельность НДС закупки по договору, направления деятельности с учетом действующей учетной политики организации
//
// Параметры:
//	Организация - СправочникСсылка.Организации - Организация, для которой определяется система налогообложения
//	Склад - СправочникСсылка.Склады - Склад-получатель товаров, для которого определяется система налогообложения
//	Договор - СправочникСсылка.ДоговорыКонтрагентов, СправочникСсылка.ДоговорМеждуОрганизациями - Договор закупки, выбранный в документе
//	НаправлениеДеятельности - СправочникСсылка.НаправленияДеятельности - Направление деятельности, по которое выполняется закупка
//	Дата - Дата определения системы налогообложения по организации и складу
//
// Возвращаемое значение:
//  ПеречислениеСсылка.ТипыНалогообложенияНДС
//
Функция ЗакупкаПодДеятельность(Знач Организация, Знач Склад = Неопределено, Знач Договор = Неопределено, Знач НаправлениеДеятельности = Неопределено, Знач Дата = Неопределено, Знач ХозяйственнаяОперация = Неопределено) Экспорт
	
	ЗакупкаПодДеятельность = Неопределено;
	
	Если ЗначениеЗаполнено(Договор) Или ЗначениеЗаполнено(НаправлениеДеятельности) Тогда
		
		// Получим вид деятельности из НСИ
		...
		|ВЫБРАТЬ
		|	1 КАК Порядок,
		|	ДоговорыМеждуОрганизациями.ЗакупкаПодДеятельность КАК ЗакупкаПодДеятельность
		|ИЗ
		|	Справочник.ДоговорыМеждуОрганизациями КАК ДоговорыМеждуОрганизациями
		|ГДЕ
		|	ДоговорыМеждуОрганизациями.Ссылка = &Договор
		...
		|ВЫБРАТЬ
		|	2 КАК Порядок,
		|	НаправленияДеятельности.НалогообложениеНДС КАК ЗакупкаПодДеятельность
		|ИЗ
		|	Справочник.НаправленияДеятельности КАК НаправленияДеятельности
		|ГДЕ
		|	НаправленияДеятельности.Ссылка = &НаправлениеДеятельности
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			Выборка = РезультатЗапроса.Выбрать();
			Выборка.Следующий();
			ЗакупкаПодДеятельность = Выборка.ЗакупкаПодДеятельность;
		КонецЕсли;
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ЗакупкаПодДеятельность) Тогда
		// Проверим значение соотвествие текущей учетной политике и хоз. операции
	...
	Если НЕ ЗначениеЗаполнено(ЗакупкаПодДеятельность) Тогда
		// Получим значение по-умолчанию из учетной политики
		ЗакупкаПодДеятельность = Справочники.Организации.ЗакупкаПодДеятельность(Организация, Склад, Дата);
	КонецЕсли;
	
	Возврат ЗакупкаПодДеятельность;
	
КонецФункции

// Возвращает деятельность НДС закупки с одновременным потреблением по договору, направления деятельности с учетом действующей учетной политики организации
//
// Параметры:
//	Организация - СправочникСсылка.Организации - Организация, для которой определяется система налогообложения
//	Склад - СправочникСсылка.Склады - Склад-получатель товаров, для которого определяется система налогообложения
...
//  ПеречислениеСсылка.ТипыНалогообложенияНДС
//
Функция ЗакупкаСПотреблениемПодДеятельность(Знач Организация, Знач Склад = Неопределено, Знач Договор = Неопределено, Знач НаправлениеДеятельности = Неопределено, Знач Дата = Неопределено, Знач ХозяйственнаяОперация = Неопределено) Экспорт
...
// Возвращает вид деятельности НДС потребления ценностей по направлению деятельности, организации и складу на дату
//
// Параметры:
//	Организация - СправочникСсылка.Организации - Организация, для которой определяется система налогообложения
//	НаправлениеДеятельности - СправочникСсылка.НаправленияДеятельности - Направление деятельности, выбранное в документе
...
//  ПеречислениеСсылка.ТипыНалогообложенияНДС
//
Функция ПередачаПодДеятельность(Знач Организация, Знач НаправлениеДеятельности = Неопределено, Знач Склад = Неопределено, Знач Дата = Неопределено, Знач ХозяйственнаяОперация = Неопределено) Экспорт
	
	ПотреблениеПодДеятельность = Неопределено;
	...
	Если ЗначениеЗаполнено(НаправлениеДеятельности) Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	НаправленияДеятельности.НалогообложениеНДС КАК ПотреблениеПодДеятельность
		|ИЗ
		|	Справочник.НаправленияДеятельности КАК НаправленияДеятельности
		|ГДЕ
		|	НаправленияДеятельности.Ссылка = &НаправлениеДеятельности
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			Выборка = РезультатЗапроса.Выбрать();
			Выборка.Следующий();
			ПотреблениеПодДеятельность = Выборка.ПотреблениеПодДеятельность;
		КонецЕсли;
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПотреблениеПодДеятельность) Тогда
		// Проверим значение соотвествие текущей учетной политике и хоз. операции
		УчетНДСУТ.ПроверитьКорректностьДеятельностиНДСПотребления(
			ПотреблениеПодДеятельность, 
			Организация,
			Дата,
			ХозяйственнаяОперация);
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(ПотреблениеПодДеятельность) Тогда
		// Получим значение по умолчанию из учетной политики
		ПотреблениеПодДеятельность = Справочники.Организации.НалогообложениеНДС(Организация, Склад, Дата);
	КонецЕсли;
	
	Возврат ПотреблениеПодДеятельность;
