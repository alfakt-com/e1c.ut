...
Функция СтавкаНДСПоЗначениюПриЗагрузке(СтавкаНалогаСтрока) Экспорт
	...
	Если ЗначениеЗаполнено(СтавкаНалогаСтрока) И Не СтавкаНалогаСтрока = "Без НДС" Тогда
		Ставка = 0 + СтавкаНалогаСтрока;
	Иначе
		Ставка = 0;
	КонецЕсли;
	
	Возврат Справочники.СтавкиНДС.СтавкаНДСПоЗначению(Ставка);
