...
Функция ДобавитьИзменитьСтрокуВТЧ(Объект, СтруктураПараметров, ДобавляемыеДанные, РежимОбработки,
	...
	Если Не ДанныеЕстьВТЧ Тогда
		...
		Если ДобавлятьТовар Тогда
			...
			///Сергей 24.08.2020(Установка ключа строки) +++
			Если ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ЗаказКлиента") или ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.ПредзаказКлиента") Тогда
				bon_БонуснаяПрограммаСервер.УстановитьКодСтрокиСерв(Объект, "Товары");		
			КонецЕсли;
			///Сергей 24.08.2020(Установка ключа строки) ---
