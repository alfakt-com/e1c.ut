...
Функция ПереоформитьРасходныеОрдера(Параметры) Экспорт
	...
	//++Аян_К++ на начало дня нет остатков по этому делаю на конец дня
	Для Каждого Стр Из ТаблицаНоменклатурыДляЗапроса Цикл
		Стр.Период = КонецДня(Стр.Период);
	КонецЦикла;
...
Функция СформироватьТаблицуПотребностейВУпаковках(Склад, Помещение, МенеджерВременныхТаблиц, ПараметрыФормированияЗаданий)
	...
	// попытка их разупаковки и увелечении потребности в меньших упаковках).
	ТекстЗапроса = "";
	Если Не ПараметрыФормированияЗаданий.Свойство("ПоРаспоряжениямНаОтбор") Тогда
		ТекстЗапроса = 
		"ВЫБРАТЬ
...
Процедура РазместитьПозициюТовара(МенеджерВременныхТаблиц, СтруктураТаблиц, ВыборкаПоСериям, МассивУчастков, КоличествоУпаковок)
	...
	// размещаемый товар. - по возрастанию(по убыванию со знаком "минус") для ячеек из области обособленого хранения;
...
Функция ВычислитьТоварыДляОформленияРасходныхОрдеров(Параметры,
	...
	Иначе
		...
		|ВЫБРАТЬ
		|	ТоварыКОтгрузке.ДокументОтгрузки КАК ДокументОтгрузки,
		|	КОНЕЦПЕРИОДА(ТоварыКОтгрузке.Период, ДЕНЬ) КАК Период,
		|	ВЫБОР 
		...
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ,
		|	КОНЕЦПЕРИОДА(ТоварыКОтгрузке.Период, ДЕНЬ)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
...
// Функция возращает признак использования на складе неордерной схемы при поступлении
//	Параметры
//		Склады - Массив, СправочникСсылка.Склад - в параметре передаются ссылки на элементы справочника Склады.
//
// Возвращаемое значение:
