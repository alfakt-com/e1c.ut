			...
			Иначе
				...
				Иначе
					...
					Если Источник.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыПоЗарплатномуПроекту Тогда
						ПараметрыЭД.ВидОперацииЭД = Перечисления.ВидыОперацийЭДОбменСБанками.ПлатежноеПоручениеНаПеречислениеЗаработнойПлаты;
					ИначеЕсли Источник.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВзносов Тогда
						Если Источник.ТипНалога = Перечисления.ТипыНалогов.СО Тогда
							ПараметрыЭД.ВидОперацииЭД = Перечисления.ВидыОперацийЭДОбменСБанками.ПлатежноеПоручениеНаПеречислениеСоциальныхОтчислений;
						ИначеЕсли Источник.ТипНалога = Перечисления.ТипыНалогов.ОПВ ИЛИ Источник.ТипНалога = Перечисления.ТипыНалогов.ОППВ Тогда
							ПараметрыЭД.ВидОперацииЭД = Перечисления.ВидыОперацийЭДОбменСБанками.ПлатежноеПоручениеНаПеречислениеПенсионныхВзносов;
						Иначе
							ПараметрыЭД.ВидОперацииЭД = Неопределено;
						КонецЕсли;
					Иначе
						ПараметрыЭД.ВидОперацииЭД = Перечисления.ВидыОперацийЭДОбменСБанками.ПлатежноеПоручение;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Процедура ЗаполнитьДанныеПлатежныхПоручений(МассивСсылок, ДанныеДляЗаполнения) Экспорт
	...
	ТаблицаСотрудников = ЗаполнитьСписокСотрудников(Запрос);
	...
	Пока Выборка.Следующий() Цикл
		...
		Для каждого КлючИЗначение Из ПоляПлатежа Цикл
			...
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДанныеДляЗаполнения[ИндексДокумента], Ключ, ПоляПлатежа[КлючИЗначение.Ключ]);
		КонецЦикла;
		
		ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДанныеДляЗаполнения[ИндексДокумента], "ПереданСписокСотрудников", Ложь);
		Если Выборка.Операция = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыПоЗарплатномуПроекту
			ИЛИ Выборка.Операция = Перечисления.ХозяйственныеОперации.ПеречислениеВзносов Тогда
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(ДанныеДляЗаполнения[ИндексДокумента], "ПереданСписокСотрудников", Истина);
			
			ПараметрыОтбора = Новый Структура("Ссылка",Выборка.Ссылка);
			ТаблицаРезультат = ТаблицаСотрудников.Скопировать(ПараметрыОтбора);
			ЭлектронноеВзаимодействие.ЗагрузитьТаблицуВДерево(ДанныеДляЗаполнения[ИндексДокумента], ТаблицаРезультат, "СписокСотрудников");
...
Процедура ЗаполнитьДанныеПорученийНаПереводВалюты(МассивСсылок, ДанныеДляЗаполнения) Экспорт
	...
	|;"
		...
		+ ";" + "
	|ВЫБРАТЬ
	|	ИнструкцииБанку.Инструкция   КАК Инструкция,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ОрганизацияАдресМеждународныйЗначенияПолей) Тогда
			
			СведенияОбАдресе = РаботаСАдресами.СведенияОбАдресе(Выборка.ОрганизацияАдресМеждународныйЗначенияПолей);
			ПоляПлатежа["РеквизитыПлательщика_АдресСтруктурированный_Индекс"] = СведенияОбАдресе.Индекс;
			ПоляПлатежа["РеквизитыПлательщика_АдресСтруктурированный_Город"]  = СведенияОбАдресе.НаселенныйПункт
				+ ?(ЗначениеЗаполнено(СведенияОбАдресе.НаселенныйПунктСокращение), " " + СведенияОбАдресе.НаселенныйПунктСокращение, "");
			ПоляПлатежа["РеквизитыПлательщика_АдресСтруктурированный_Адрес"]  = СведенияОбАдресе.Улица
				+ ?(ЗначениеЗаполнено(СведенияОбАдресе.УлицаСокращение), " " + СведенияОбАдресе.УлицаСокращение, "")
				+ ?(СведенияОбАдресе.Здание.Количество(), ", " + СведенияОбАдресе.Здание.Номер, "");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Выборка.КонтрагентАдресМеждународныйЗначенияПолей) Тогда
			
			СведенияОбАдресе = РаботаСАдресами.СведенияОбАдресе(Выборка.КонтрагентАдресМеждународныйЗначенияПолей);
			ПоляПлатежа["РеквизитыПолучателя_АдресСтруктурированный_Индекс"] = СведенияОбАдресе.Индекс;
			ПоляПлатежа["РеквизитыПолучателя_АдресСтруктурированный_Город"]  = СведенияОбАдресе.НаселенныйПункт
				+ ?(ЗначениеЗаполнено(СведенияОбАдресе.НаселенныйПунктСокращение), " " + СведенияОбАдресе.НаселенныйПунктСокращение, "");
			ПоляПлатежа["РеквизитыПолучателя_АдресСтруктурированный_Адрес"]  = СведенияОбАдресе.Улица
				+ ?(ЗначениеЗаполнено(СведенияОбАдресе.УлицаСокращение), " " + СведенияОбАдресе.УлицаСокращение, "")
				+ ?(СведенияОбАдресе.Здание.Количество(), ", " + СведенияОбАдресе.Здание.Номер, "");
		КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Для каждого КлючИЗначение Из ПоляПлатежа Цикл
			...
			ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
				ДанныеДляЗаполнения[ИндексДокумента], Ключ, ПоляПлатежа[КлючИЗначение.Ключ]);
		КонецЦикла;
	КонецЦикла;
...
Функция ИнициализироватьПоляПлатежногоПоручения()
	...
	Поля.Вставить("РеквизитыПлательщика_КБЕ",               "ОрганизацияКБЕ");
	Поля.Вставить("РеквизитыПолучателя_КБЕ",                "КонтрагентКБЕ");
	Поля.Вставить("РеквизитыПлатежа_КодНазначенияПлатежа",  "КодНазначенияПлатежа");
	Поля.Вставить("РеквизитыПлатежа_ДатаПолученияТовара",   "ДатаПолученияТовара");
	Поля.Вставить("РеквизитыПлатежа_ДатаВалютирования",     "ДатаВалютирования");
...
Функция ЗаполнитьСписокСотрудников(Запрос)
	...
	|		ПО РасшифровкаПлатежа.Ссылка = ПлатежныеПоручения.Ссылка
	|ГДЕ
	|	РасшифровкаПлатежа.Ведомость ССЫЛКА Документ.ВедомостьНаВыплатуЗарплатыВБанк
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ПО РасшифровкаПлатежа.Ссылка = ПлатежныеПоручения.Ссылка
	|ГДЕ
	|	РасшифровкаПлатежа.Ведомость ССЫЛКА Документ.ВедомостьНаПеречислениеВзносовОтчислений
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ ВТ_ФИОСрез
	|ИЗ
	|	РегистрСведений.ФИОФизическихЛиц КАК ФИОФизическихЛиц
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ФИОМаксимум КАК ВТ_ФИОМаксимум
	|		ПО ФИОФизическихЛиц.ФизическоеЛицо = ВТ_ФИОМаксимум.ФизическоеЛицо
	|			И ФИОФизическихЛиц.Период = ВТ_ФИОМаксимум.Период
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВедомостьНаВыплатуЗарплатыВБанк.ФизическоеЛицо.ИНН КАК ИНН,
	|	ВедомостьНаВыплатуЗарплатыВБанк.ФизическоеЛицо.ДатаРождения КАК ДатаРождения,
	|	ВедомостьНаВыплатуЗарплатыВБанк.НомерЛицевогоСчета КАК РасчСчет,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ПериодПлатежа,
	|	ВедомостьНаВыплатуЗарплатыВБанк.КВыплате КАК Сумма,
	|	Ведомости.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ВедомостьНаВыплатуЗарплатыВБанк.Зарплата КАК ВедомостьНаВыплатуЗарплатыВБанк
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Ведомости КАК Ведомости
	|		ПО ВедомостьНаВыплатуЗарплатыВБанк.Ссылка = Ведомости.Ведомость
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ФИОСрез КАК ФИОФизическихЛицСрезПоследних
	|		ПО ВедомостьНаВыплатуЗарплатыВБанк.ФизическоеЛицо = ФИОФизическихЛицСрезПоследних.ФизическоеЛицо
	|			И ВедомостьНаВыплатуЗарплатыВБанк.Ссылка.ПериодРегистрации >= ФИОФизическихЛицСрезПоследних.Период
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	"""",
	|	ТЧНалогиВзносыОтчисления.Ссылка.ПериодРегистрации,
	|	ТЧНалогиВзносыОтчисления.НалогВзнос,
	|	Ведомости.Ссылка
	|ИЗ
	|	Документ.ВедомостьНаПеречислениеВзносовОтчислений.НалогиВзносыОтчисления КАК ТЧНалогиВзносыОтчисления
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Ведомости КАК Ведомости
	|		ПО (ТЧНалогиВзносыОтчисления.Ссылка = Ведомости.Ведомость)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ФИОСрез КАК ФИОФизическихЛицСрезПоследних
	|		ПО (ТЧНалогиВзносыОтчисления.ФизическоеЛицо = ФИОФизическихЛицСрезПоследних.ФизическоеЛицо)
	|			И (ТЧНалогиВзносыОтчисления.Ссылка.ПериодРегистрации >= ФИОФизическихЛицСрезПоследних.Период)";
