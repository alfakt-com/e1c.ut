...
// Перед началом расчета заполняет все необходимые параметры и инициализирует все структуры данных, используемые
// алгоритма расчета. Внимание: если какая-то сущность используется более чем в одном этапе расчета, то ее стоит занести
// в ПараметрыРасчета.
//
...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета)
	...
	|	И ((&УправленческийУчетОрганизаций И Т.Стоимость <> 0
...
Процедура ПолучитьДанныеДляПартийТоваров(ПараметрыРасчета)
	...
	УниверсальныеМеханизмыПартийИСебестоимости.УничтожитьВременныеТаблицы(ПараметрыРасчета,
		"Декларации, ПрошлыеРеализации, ОстаткиИмпорт, ЕстьДекларации,
...
Функция ТекстИнициализацияПартийТоваров() // вт АналитикаДавальца, Декларации, ДекларацииПоПоступлениям, ПрошлыеРеализации, ПродукцияДавальца, ЕстьДекларации, Выпуски, НеотфактурованныеПоставки
	Возврат "
		|ВЫБРАТЬ
...
Функция ТекстТекущиеОборотыПартийТоваров() // использует ЕстьДекларации
	Возврат 
		...
		|			ТОГДА
		|				ВЫБОР КОГДА ВнутреннееПотребление.ПотреблениеДляДеятельности <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)
...
Функция ТекстНачальныеОстаткиЗакупокПоИмпорту()
	Возврат 
		...
		|ГДЕ
		|	Поступление.ХозяйственнаяОперация В (
...
Функция ПолучитьДанныеДляДопРасходов(ПараметрыРасчета)
	...
	КонтекстДвижений = ОписаниеДвижений(
		...
		, // ПоляСортировки
		, // СортировкаПоУсловию
		"Количество", // БазисПриходаРегл
		"Количество"  // БазисРасходаРегл 
		); 
	...
	Запрос.Текст = ""
		...
		+ ТекстОбъединениеЗапросов() + ТекстБазисПеремещенияПоступленияСборкаСебестоимость()
		+ ТекстОбъединениеЗапросов() + ТекстБазисПрошлыеПеремещенияПоступленияСборкаСебестоимость()
...
Функция СформироватьПартииПрочихРасходов(ПараметрыРасчета, ДанныеДляРасчета)
	...
	Для Каждого Движение Из ДанныеДляРасчета Цикл
		...
		СтоимостьРегл  = ?(Движение.СтоимостьРегл < 0, -Движение.СтоимостьРегл, Движение.СтоимостьРегл);
		...
		Для Каждого Значение Из АналитикиОбсчетаПартийПрочих(Движение, ПустыеСсылки) Цикл
			...
			Для Каждого ИндексСтроки Из ИндексыСтрок Цикл
				...
				ОписаниеБазы = ?(Налоги[Ключи[3]] = Неопределено, Новый Структура("Базис, БазисРегл", 0, 0), Налоги[Ключи[3]]);
...
Функция ПолучитьДанныеПриходовПартийРасходов(ПараметрыРасчета, ДанныеДляРасчета)
	...
	КонтекстДвижений = ОписаниеДвижений(
		...
		ПоляРасчета,
		"Организация",
		"Базис, БазисРегл, Количество, Вес, Объем, Стоимость, СтоимостьБезНДС, СтоимостьРегл, НДСРегл, НДСУпр, ПостояннаяРазница, ВременнаяРазница",
		"Базис",
		"<БАЗИС_РАСХОДА>",
		"ДокументРеализации",
		"Период",,,
...
Функция ТекстБазисПриходыСебестоимость()
	...
	ТекстЗапроса = 
		...
		|	И НЕ (ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИП(Документ.ПоступлениеТоваров)
...
Функция ТекстБазисПеремещенияПоступленияСборкаСебестоимость()
	...
	ТекстЗапроса = 
		...
		|									ТИП(Документ.ПоступлениеТоваров))
...
Функция ОписаниеЦепочекПартийНДС(ПараметрыРасчета)
	...
	// Потребление <- (Остаток, Партия, ДопРасходы, ДопРасходыБезПартии, ДопРасходыСПартией,
	// КорректировкаПриобретенияПрошлогоПериода, Перемещение, ПеремещениеОбособленно, Сторно, Выпуск).
	ДобавитьОписаниеПриемника(ОписаниеЦепочек,
...
Функция ТекстПриходыСебестоимостиТоваров() // использует ПриходыТоваровНДС
	Возврат
		...
		|	ВЫБОР
		|		КОГДА ДД.Регистратор ССЫЛКА Документ.ВводОстатков
		|			И НЕ ВЫРАЗИТЬ(Регистратор КАК Документ.ВводОстатков).ВводОстатков22
		|		ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ КАК РасчетЗавершен,
	 ...
	 И Расход.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиентуРеглУчет) Тогда
		...
		КонецЕсли;
	 	...
	 	Если КорректироватьБазуВРасходе Тогда
			Приход.Знаменатель = Приход.Знаменатель - Знак * Мин(Расход.Знаменатель, Приход.Знаменатель);
		Иначе 
	    	Приход.Знаменатель = Приход.Знаменатель - Знак * КоличествоСписания;
		КонецЕсли;
...
Функция ОписаниеДвижений(Контекст, ИмяРегистра, ПоляРасчета, КлючиСравнения, Показатели,
						 БазисПрихода, БазисРасхода, КлючРасхода, ПолеПорядка,
						 ПоляСортировки = "", СортировкаПоУсловию = "",
						 БазисПриходаРегл = "", БазисРасходаРегл = "")
	...
	Результат.Вставить("БазисПриходаРегл", 		  	БазисПриходаРегл);
	Результат.Вставить("БазисРасходаРегл", 		  	БазисРасходаРегл);
...
Процедура ЗапуститьРасчетПартийВФоне(ПараметрыРасчета, ПараметрыФоновогоРасчета,
	...
	Пока СледующийЭлементКоллекции(ВыборкаГруппаПодграфов) Цикл
		...
		Пока ПродолжатьВыгрузку Цикл
			...
			УниверсальныеМеханизмыПартийИСебестоимости.УничтожитьВременныеТаблицы(ПараметрыРасчета,
				"ПорцияИсточников, ПорцияПриемников");
...
Процедура ВыгрузитьШапкуТаблицыПромежуточныхДанныхРасчета(ПараметрыРасчета, ИмяТаблицы, ИсточникДанных = Неопределено)
	...
	Если ТипЗнч(ИсточникДанных) = Тип("Строка") Тогда
		...
		ТаблицаДанных = УниверсальныеМеханизмыПартийИСебестоимости.ВыгрузитьВременнуюТаблицу(ПараметрыРасчета, ИсточникДанных, 0);
	Иначе
...
Процедура ВыгрузитьТаблицуПромежуточныхДанныхРасчета(ПараметрыРасчета, ИмяТаблицы, ИсточникДанных = Неопределено) Экспорт
	...
	Если ТипЗнч(ИсточникДанных) = Тип("Строка") Тогда
		...
		ТаблицаДанных = УниверсальныеМеханизмыПартийИСебестоимости.ВыгрузитьВременнуюТаблицу(ПараметрыРасчета, ИсточникДанных);
