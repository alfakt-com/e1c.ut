...
Процедура ПересчитатьСуммуРучнойСкидкиВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуРучнойСкидки", ИмяКоличества) Тогда
		...
		Если СтруктураДействий.Свойство("swОкруглятьСуммуДоЦелого") Тогда
			ТекущаяСтрока.СуммаРучнойСкидки = Окр(ТекущаяСтрока[ИмяКоличества] *ТекущаяСтрока.Цена * ТекущаяСтрока.ПроцентРучнойСкидки / 100, 0);
		КонецЕсли;
		// sw end
...
Процедура ПересчитатьСуммуВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПересчитатьСумму", ИмяКоличества) Тогда
		...
		Если ТекущаяСтрока <> Неопределено Тогда
			...
			// Швецов Тимур, 05.02.2019
			Если СтруктураДействий.Свойство("swОкруглятьСуммуДоЦелого") Тогда
				ТекущаяСтрока.Сумма = Окр(ТекущаяСтрока.Цена * ТекущаяСтрока[ИмяКоличества], 0);
			КонецЕсли;
			// sw end
...
Процедура ПересчитатьЦенуСкидкуПоСуммеВПродажахВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	#КонецЕсли
			...
			Иначе
				...
				ИначеЕсли ТекущаяСтрока.Цена = 0 Тогда
					Если ЗначениеЗаполнено(ТекущаяСтрока.ПроцентРучнойСкидки) Тогда
						...
						// Швецов Тимур, 05.02.2019
						Если СтруктураДействий.Свойство("swОкруглятьСуммуДоЦелого") Тогда
							ТекущаяСтрока.СуммаРучнойСкидки = Окр((ТекущаяСтрока.Сумма * ТекущаяСтрока.ПроцентРучнойСкидки)  / (100 - ТекущаяСтрока.ПроцентРучнойСкидки), 0);
						КонецЕсли;
						// sw end	
					КонецЕсли;
				...
				Иначе
					...
					// Швецов Тимур, 05.02.2019
					Если СтруктураДействий.Свойство("swОкруглятьСуммуДоЦелого") Тогда
						СуммаБезСкидки = Окр(ТекущаяСтрока.Цена * ТекущаяСтрока[ИмяКоличества], 0);
					КонецЕсли;
					// sw end
		...
		Иначе
			...
			Иначе
				Если СтруктураДействий.Свойство("swОкруглятьСуммуДоЦелого") Тогда
					ТекущаяСтрока.Цена = Окр(ТекущаяСтрока.Сумма / ТекущаяСтрока[ИмяКоличества], 0, РежимОкругления.Окр15как10);
				Иначе
					ТекущаяСтрока.Цена = Окр(ТекущаяСтрока.Сумма / ТекущаяСтрока[ИмяКоличества], 2, РежимОкругления.Окр15как10);	
				КонецЕсли;				
			КонецЕсли;
...
Процедура ПересчитатьСуммуПродажиНДСВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуПродажиНДС", СтруктураПараметровДействия) Тогда
		...
		Если НЕ КэшированныеЗначения.Свойство("ПроцентыСтавокНДС") Тогда
			КэшированныеЗначения.Вставить("ПроцентыСтавокНДС", Новый Соответствие);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтавкаНДС) Тогда
			ТекущийПроцентНДС = КэшированныеЗначения.ПроцентыСтавокНДС.Получить(СтавкаНДС);
			Если ТекущийПроцентНДС = Неопределено Тогда
				ТекущийПроцентНДС = ЦенообразованиеКлиентСервер.ПолучитьСтавкуНДСЧислом(СтавкаНДС, НДСпоСтавкам4и2);
				КэшированныеЗначения.ПроцентыСтавокНДС.Вставить(СтавкаНДС, ТекущийПроцентНДС);
			КонецЕсли;
		Иначе
			ТекущийПроцентНДС = 0;
		КонецЕсли;
...
Процедура ПересчитатьСуммуАвтоматическойСкидкиВСтрокеТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения) Экспорт
	...
	Если СтруктураДействий.Свойство("ПересчитатьСуммуАвтоматическойСкидки", ИмяКоличества) Тогда
		...
		Если СтруктураДействий.Свойство("swОкруглятьСуммуДоЦелого") Тогда
			ТекущаяСтрока.СуммаАвтоматическойСкидки = Окр(ТекущаяСтрока[ИмяКоличества] *ТекущаяСтрока.Цена * ТекущаяСтрока.ПроцентАвтоматическойСкидки / 100, 0);
		КонецЕсли;
		///Сергей 07.10.2020 --- Расчет "Скидок", "Спеччен"
