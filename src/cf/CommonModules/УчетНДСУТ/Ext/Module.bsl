...
Процедура ОтразитьДокументыВУчетеНДС(Период, МассивСсылок) Экспорт
	...
	//В УПК движения по регистрам НДС не актуализируем при проведении документа РаспределениеНДС
	|	И Ложь
	|";
...
Процедура ОтразитьВвозТоваровИзТаможенногоСоюза(Запрос, ТекстыЗапроса, Регистры, ТекстТовары) Экспорт
	...
	//++ Локализация
	Запрос.УстановитьПараметр("ТипыНоменклатурыТовар", ТипыНоменклатурыТовар());
...
Функция ДатаНачалаДействия150ФЗ() Экспорт
	...
	Возврат ?(ПолучитьФункциональнуюОпцию("ОтключенныйФункционал"),Дата("20160701"), Дата("21160701"));
...
Функция УпрощенныйПорядокВычетаНДСПоНесырьевомуЭкспорту(Дата) Экспорт
	...
	Возврат ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") И ДатаНачалаДействия150ФЗ() <= Дата;
...
Функция СтранаЯвляетсяЧленомЕАЭС(Страна, Период = Неопределено) Экспорт
	...
	Если Период >= '2015-01-01' 
			И (КодСтраны = "112" ИЛИ КодСтраны = "643") Тогда // Россия, Беларусь
		Возврат Истина;
	КонецЕсли;
...
//
Процедура ЗаполнитьСписокВыбораДеятельностиНДСПоступления(Поле, Знач Организация, Знач Дата, Знач ХозяйственнаяОперация = Неопределено) Экспорт
	
	Поле.СписокВыбора.Очистить();
	СписокДоступных = СписокДоступныхВидовДеятельностиНДСПоступления(Организация, Дата, ХозяйственнаяОперация);
...
//
Процедура ЗаполнитьСписокВыбораДеятельностиНДСПотребления(Поле, Знач Организация, Знач Дата, Знач ХозяйственнаяОперация = Неопределено) Экспорт
...
// 	ХозяйственнаяОперация - ПеречислениеСсылка.ХозяйственныеОперации - операция документа.
//
Процедура ПроверитьКорректностьДеятельностиНДСПотребления(ВидДеятельностиНДС, Знач Организация, Знач Дата, Знач ХозяйственнаяОперация = Неопределено) Экспорт
...
//
Процедура ПроверитьКорректностьДеятельностиНДСПоступления(ВидДеятельностиНДС, Знач Организация, Знач Дата, Знач ХозяйственнаяОперация = Неопределено) Экспорт
...
//	ХозяйственнаяОперация - ПеречислениеСсылка.ХозяйственныеОперации - Хозяйственная операция документа
//	ВыбранноеЗначение - ПеречислениеСсылка.ТипыНалогообложенияНДС - Текущее выбранное значение налогообложения
//	ЭтоНакладная - Булево - Признак того, что запонение списка выполняется в накладной.
//
Процедура ЗаполнитьСписокВыбораНалогообложенияПродажи(Поле, ХозяйственнаяОперация, ВыбранноеЗначение, ЭтоНакладная = Истина) Экспорт
	
	Поле.СписокВыбора.Очистить();
	СписокДоступных = СписокДоступныхВидовНалогообложенияПродажи(ХозяйственнаяОперация, ЭтоНакладная);
	Для каждого Элемент Из СписокДоступных Цикл
		Поле.СписокВыбора.Добавить(Элемент.Значение, Элемент.Представление);
	КонецЦикла;
	
	Если ЗначениеЗаполнено(ВыбранноеЗначение)
		И Поле.СписокВыбора.НайтиПоЗначению(ВыбранноеЗначение) = Неопределено Тогда 
		Поле.СписокВыбора.Добавить(ВыбранноеЗначение);
	КонецЕсли;
	
	Поле.Видимость = Поле.СписокВыбора.Количество() > 1;
...
//	ЭтоНакладная - Булево - Признак того, что запонение списка выполняется в накладной.
//
Процедура ПроверитьКорректностьНалогообложенияНДСПродажи(НалогообложениеНДС, ХозяйственнаяОперация, ЭтоНакладная = Истина) Экспорт
...
//
Функция ТипыНалогообложенияНДСОблагаемые() Экспорт
...
// Определяет вид деятельности для раздельного учета НДС по налогообложению НДС продажи.
//
// Параметры:
...
Процедура ПараметрыЗаполненияВидовЗапасовПоНалогообложению(ОтборыВидовЗапасов, Организация, Дата, НалогообложениеНДС = Неопределено) Экспорт
	...
	НалогообложениеОрганизации = Справочники.Организации.НалогообложениеНДС(Организация, , Дата);
...
Процедура УстановитьУсловноеОформлениеСуммНДС(Форма,
	...
	СписокЗначений.Добавить(Справочники.СтавкиНДС.БезНДС);
	СписокЗначений.Добавить(Справочники.СтавкиНДС.БезНДСНеРК);
	СписокЗначений.Добавить(Справочники.СтавкиНДС.НДС0);
...
Процедура ПолучитьПартии(ПараметрыРасчета)
	...
	|					КОГДА ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС12) ТОГДА Партии.НДСРегл * 100 / 12
...
Функция СформироватьНДСПредъявленный(ПараметрыРасчета)
	...
	ПолучитьНДСПоФактическомуИспользованию(ПараметрыРасчета);
...
Процедура ПолучитьНДСПоФактическомуИспользованию(ПараметрыРасчета)
	...
	|ПОМЕСТИТЬ УчетныеПолитикиМаксимумы
	|ИЗ
	...
	|		ПО СчетаФактуры.Организация = УчетнаяПолитикаОрганизаций.Организация
	|			И СчетаФактуры.Дата >= УчетнаяПолитикаОрганизаций.Период
	|
	|СГРУППИРОВАТЬ ПО
	...
	|///////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	УчетныеПолитикиМаксимумы.СчетФактура КАК СчетФактура,
	|	ЕСТЬNULL(УчетнаяПолитикаОрганизаций.ПрименяетсяУчетНДСПоФактическомуИспользованию, ЛОЖЬ) КАК ПрименяетсяУчетНДСПоФактическомуИспользованию
	|ПОМЕСТИТЬ ТаблицаНДСПоФактическомуИспользованию
	|ИЗ
	|	УчетныеПолитикиМаксимумы КАК УчетныеПолитикиМаксимумы
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УчетнаяПолитикаОрганизаций КАК УчетныеПолитикаОрганизаций
	|		ПО УчетныеПолитикиМаксимумы.Период = УчетныеПолитикаОрганизаций.Период
	|			И УчетныеПолитикиМаксимумы.Организация = УчетныеПолитикаОрганизаций.Организация
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.УчетныеПолитикиОрганизаций КАК УчетнаяПолитикаОрганизаций
	|		ПО УчетныеПолитикаОрганизаций.УчетнаяПолитика = УчетнаяПолитикаОрганизаций.Ссылка
	|
	|";
...
Процедура ПолучитьИсправительныеСчетаФактурыДокументы(ПараметрыРасчета)
	...
	|	МАКСИМУМ(ВЫБОР 
	|		КОГДА НЕ ПартииПрочихРасходов.Регистратор ЕСТЬ NULL 
	//УПК Проводки формируются в счет-фактуре, а не в распределении НДС, поэтому период не приводится к концу квартала
	|			ТОГДА ИсправительныеСчетаФактуры.ДатаЗаписиКнигиПокупок
	|		ИНАЧЕ ИсправительныеСчетаФактуры.ДатаЗаписиКнигиПокупок
	|	КОНЕЦ) <= &КонецПериода
	|
	|ИНДЕКСИРОВАТЬ ПО
...
Процедура РассчитатьЗаписиНДСПредъявленныйИзменениеВидаДеятельности(ПараметрыРасчета)
	...
	//УПК при ведении раздельного учета НДС и установке в учетной политике организации "Применяется пропорциональный метод отнесения НДС в зачет"
	//движения по РН НДСПредъяявленный по изменению вида деятельности не должны формироваться, кроме документа списания
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		ТаблицаНДСПоФактическомуИспользованию КАК НДСПоФактическомуИспользованию
	|	ПО
	|		Партии.СчетФактура = НДСПоФактическомуИспользованию.СчетФактура
	|ГДЕ
	...
	|			"+?(ПолучитьФункциональнуюОпцию("ОтключенныйФункционал"), "И Партии.ДатаПоступления >= &ДатаНачалаДействия150ФЗ)", ")")+"
	|	И (Партии.НалогообложениеФинПартии <> Партии.НалогообложениеПартии 
	|			ИЛИ Партии.ДокументРеализации <> НЕОПРЕДЕЛЕНО)
	|	И СписанияНДСНаРасходы.СчетФактура ЕСТЬ NULL
	//УПК при ведении раздельного учета НДС и установке в учетной политике организации "Применяется пропорциональный метод отнесения НДС в зачет"
	//движения по РН НДСПредъяявленный по изменению вида деятельности не должны формироваться
	|	И ((&ИспользоватьРаздельныйУчетПоНалогообложению И НЕ ЕСТЬNULL(НДСПоФактическомуИспользованию.ПрименяетсяУчетНДСПоФактическомуИспользованию, ЛОЖЬ))
	|	ИЛИ (&ИспользоватьРаздельныйУчетПоНалогообложению И ЕСТЬNULL(НДСПоФактическомуИспользованию.ПрименяетсяУчетНДСПоФактическомуИспользованию, ЛОЖЬ)
	|	И ТИПЗНАЧЕНИЯ(Партии.Регистратор) В (	ТИП(Документ.ВнутреннееПотреблениеТоваров), 
	...
	|	И Партии.НалогообложениеФинПартии <> Партии.НалогообложениеПартии
	//УПК Если исходное налогооблажение равно ОпределяетсяРаспределением, то движение по РН НДСПредъявленный не должно формироваться,
	//т.к. формирование движений происходит в счет-фактуре
	|	И Партии.НалогообложениеФинПартии <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОпределяетсяРаспределением)
	...
	|											ТИП(Документ.СписаниеНедостачТоваров),
	|											ТИП(Документ.ПорчаТоваров))))
	|	И ((Партии.НалогообложениеФинПартии = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|		И ТИПЗНАЧЕНИЯ(Партии.Регистратор) = ТИП(Документ.КорректировкаВидаДеятельностиНДС)) ИЛИ НЕ Партии.НалогообложениеФинПартии = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	//УПК при распределении НДС отключено формирование движений, если было списание НДС на расходы
	|	И Партии.НалогообложениеФинПартии <> ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОпределяетсяРаспределением)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
...
Процедура РассчитатьЗаписиНДСПредъявленныйВосстановлениеПодВидДеятельности(ПараметрыРасчета)
	...
	//УПК Восстановление НДС отражаем вне зависимости от периода счета-фактуры остатков НДС
	|		И ЛОЖЬ
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
...
Процедура РассчитатьЗаписиНДСПредъявленныйПринятиеКВычету(ПараметрыРасчета)
	...
	|		ИЛИ ДД.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОпределяетсяРаспределением)
	|		ИЛИ (ДД.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортНесырьевыхТоваров))
	|				"+?(ПолучитьФункциональнуюОпцию("ОтключенныйФункционал"),"И (ЕСТЬNULL(ДанныеПервичныхДокументов.ДатаРегистратора, ДАТАВРЕМЯ(1,1,1)) >= &ДатаНачалаДействия150ФЗ))",")")+"
	|	И ВЫБОР 
	...
	|		ИЛИ ДД.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОпределяетсяРаспределением)
	|		ИЛИ (ДД.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортНесырьевыхТоваров))
	|				"+?(ПолучитьФункциональнуюОпцию("ОтключенныйФункционал"), "И ДанныеПартий.ДатаПоступления >= &ДатаНачалаДействия150ФЗ))", ")")+"
	|	И БлокировкаВычетаНДС.СчетФактура ЕСТЬ NULL
	|
	|СГРУППИРОВАТЬ ПО
	|	ДанныеПартий.Период,
...
Процедура УстановитьПараметрТипыНалогообложенияНДСПоступления(Запрос) Экспорт
	...
	Запрос.УстановитьПараметр("ТипыНалогообложенияНДСПоступления", ТипыНалогообложенияНДСОблагаемые());
...
Процедура РассчитатьНДСЗаписиКнигиПродажПоНДСПредъявленный(ПараметрыРасчета)
	...
	|	ВЫБОР 
	|		КОГДА НДСПредъявленный.СуммаБезНДС = 0 И НДСПредъявленный.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС12)
	|			ТОГДА НДСПредъявленный.НДС * 100 / 12
...
//	ХозяйственнаяОперация - ПеречислениеСсылка.ХозяйственныеОперации - Хозяйственная операция документа.
//
Процедура ЗаполнитьСписокВыбораНалогообложенияНДСЗакупки(Поле, ХозяйственнаяОперация) Экспорт
	
	Поле.СписокВыбора.Очистить();
	СписокДоступных = СписокДоступныхВидовНалогообложенияНДСЗакупки(ХозяйственнаяОперация);
	Поле.Видимость = СписокДоступных.Количество() > 1;
	Для каждого Элемент Из СписокДоступных Цикл
		Поле.СписокВыбора.Добавить(Элемент.Значение, Элемент.Представление);
	КонецЦикла;
...
Функция СписокДоступныхВидовДеятельностиНДСПоступления(Организация, Дата, ХозяйственнаяОперация)
	
	Если Не ЗначениеЗаполнено(Дата) Тогда
		Дата = ТекущаяДатаСеанса();
	КонецЕсли;
	
	ВидДеятельностиНДС = Справочники.Организации.ЗакупкаПодДеятельность(Организация, Неопределено, Дата);
	...
	ПрименяетсяУчетНДСПоФактическомуИспользованию = ?(ПараметрыУчетнойПолитики = Неопределено, Ложь, ПараметрыУчетнойПолитики.ПрименяетсяУчетНДСПоФактическомуИспользованию);
	...
	Иначе
		...
		Если ДоступнаПродажаНаЭкспорт И ИспользоватьРаздельныйУчет И ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") Тогда
	...
	Если ИспользоватьРаздельныйУчет И НЕ ПрименяетсяУчетНДСПоФактическомуИспользованию
...
Функция СписокДоступныхВидовДеятельностиНДСПотребления(Организация, Знач Дата, ХозяйственнаяОперация)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеремещениеТоваров 
			И ИспользоватьРаздельныйУчет И ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") Тогда
	...
	Если ИспользоватьРаздельныйУчет
		И (ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика
			...
			ИЛИ ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика) Тогда
		СписокНалогообложений.Добавить(Перечисления.ТипыНалогообложенияНДС.ОпределяетсяРаспределением, НСтр("ru = 'Определяется распределением'"));
	КонецЕсли;
...
Функция СписокДоступныхВидовНалогообложенияПродажи(ХозяйственнаяОперация, ЭтоНакладная)
	
	СписокНалогообложений = Новый СписокЗначений;
	
	СписокНалогообложений.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС, НСтр("ru = 'Облагается НДС'"));
	СписокНалогообложений.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС, НСтр("ru = 'Не облагается НДС'"));
	...
	Если ДоступнаПродажаЕНВД Тогда
		СписокНалогообложений.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД, НСтр("ru = 'Облагается ЕНВД'"));
	КонецЕсли;
	КонецЕсли;
	...
	Если ДоступнаПродажаНаЭкспорт Тогда
		Если ЭтоНакладная ИЛИ НЕ ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") Тогда
			СписокНалогообложений.Добавить(Перечисления.ТипыНалогообложенияНДС.ПродажаНаЭкспорт, НСтр("ru = 'Экспорт (0%)'"));
		Иначе
			Если ПолучитьФункциональнуюОпцию("ИспользоватьПродажиНаЭкспортНесырьевыхТоваров") Тогда
				СписокНалогообложений.Добавить(Перечисления.ТипыНалогообложенияНДС.ЭкспортНесырьевыхТоваров, НСтр("ru = 'Экспорт несырьевых товаров'"));
			КонецЕсли;
...
Функция СписокДоступныхВидовНалогообложенияНДСЗакупки(ХозяйственнаяОперация)
	
	СписокНалогообложений = Новый СписокЗначений;
	
	ПриемНаКомиссию = (ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОказаниеАгентскихУслуг
	...
	Если ДоступноНалоговыйАгентПоНДС Тогда
		СписокНалогообложений.Добавить(Перечисления.ТипыНалогообложенияНДС.НалоговыйАгентПоНДС, НСтр("ru = 'Налоговый агент по НДС'"));
	КонецЕсли;
	...
	Если ДоступноОблагаетсяНДСУПокупателя Тогда
		СписокНалогообложений.Добавить(Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДСУПокупателя, НСтр("ru = 'Облагается НДС у покупателя'"));
	КонецЕсли;
