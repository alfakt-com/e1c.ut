...
Функция ПроверитьКорректностьНомераСчета(Знач Номер, ВалютныйСчет = Ложь, ТекстОшибки = "") Экспорт
	...
	Если Не ВалютныйСчет И СтрДлина(Номер) <> 20 Тогда
		ТекстОшибки = ТекстОшибки + ?(ПустаяСтрока(ТекстОшибки), "", " ") +
			СтрШаблон(НСтр("ru = 'Номер счета должен состоять из 20 символов. Введено %1 символов'"), ДлинаНомера);
		Результат = Ложь;
	КонецЕсли;
...
Функция ИмеютсяНедопустимыеСимволы(Строка) Экспорт
...
Процедура ЗагрузитьСекциюДокумента(СтрокаДокумента, ТекущаяСтрока, КоличествоСтрок, МассивСтрок)
	...
	СтрокаРазбора = ПолучитьСтрокуИмпорта(ТекущаяСтрока, КоличествоСтрок, МассивСтрок);
	СоответствиеТеговДокументов = СоответствиеТеговДокументовРазныхВерсийФормата();
	ЭтоСписаниеСРасчетногоСчета = Ложь;
	Пока Лев(Врег(СокрЛП(СтрокаРазбора)), 14) <> "КОНЕЦДОКУМЕНТА" Цикл
		...
		Если РазобратьТеговуюСтроку(СтрокаРазбора, Тег, Значение) Тогда
			...
			Если ИмеютсяНедопустимыеСимволы(Тег) = ЛОЖЬ И СтрокаДокумента.Свойство(Тег) Тогда
				
				Если ЗначениеЗаполнено(Значение) И НЕ ЗначениеЗаполнено(СтрокаДокумента[Тег]) Тогда
					СтрокаДокумента[Тег] = Значение;
				КонецЕсли;
				
			ИначеЕсли СоответствиеТеговДокументов.Получить(Тег) <> Неопределено Тогда
				
				Если ЗначениеЗаполнено(Значение) И НЕ ЗначениеЗаполнено(СтрокаДокумента[СоответствиеТеговДокументов.Получить(Тег)]) Тогда
					СтрокаДокумента[СоответствиеТеговДокументов.Получить(Тег)] = Значение;
				КонецЕсли;
				
			ИначеЕсли Врег(Тег) = "ВИДДОКУМЕНТА" Тогда
				
				СтрокаДокумента.Операция = Значение;
				
			ИначеЕсли Врег(Тег) = "ДАТАОПЕРАЦИИ" Тогда
				
				СтрокаДокумента.ДатаПоступило = Значение;
				СтрокаДокумента.ДатаСписано = Значение;
				
			ИначеЕсли Врег(Тег) = "СУММАРАСХОД" Тогда
				
				Сумма = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Значение);
				Если Сумма <> Неопределено И Сумма > 0 Тогда
					ЭтоСписаниеСРасчетногоСчета = Истина;
					СтрокаДокумента.Сумма = Значение;
				КонецЕсли;
				
			ИначеЕсли Врег(Тег) = "СУММАПРИХОД" Тогда
	...
	СтрокаДокумента.ДатаПоступило = ?(ЭтоСписаниеСРасчетногоСчета, Неопределено, СтрокаДокумента.ДатаПоступило);
	СтрокаДокумента.ДатаСписано   = ?(ЭтоСписаниеСРасчетногоСчета, СтрокаДокумента.ДатаСписано, Неопределено);
...
Функция СоответствиеТеговДокументовРазныхВерсийФормата()
	
	Результат = Новый Соответствие;
	Результат.Вставить("НОМЕРДОКУМЕНТА",             "Номер");
	Результат.Вставить("ДАТАДОКУМЕНТА",              "Дата");
	Результат.Вставить("ПЛАТЕЛЬЩИКНАИМЕНОВАНИЕ",     "Плательщик");
	Результат.Вставить("ПЛАТЕЛЬЩИКБИН_ИИН",          "ПлательщикИНН");
	Результат.Вставить("ПЛАТЕЛЬЩИКИИК",              "ПлательщикСчет");
	Результат.Вставить("ПЛАТЕЛЬЩИКБАНКНАИМЕНОВАНИЕ", "ПлательщикБанк1");
	Результат.Вставить("ПЛАТЕЛЬЩИКБАНКБИК",          "ПлательщикБИК");
	Результат.Вставить("ПЛАТЕЛЬЩИКБАНКИИК",          "ПлательщикКорсчет");
	Результат.Вставить("ПОЛУЧАТЕЛЬНАИМЕНОВАНИЕ",     "Получатель");
	Результат.Вставить("ПОЛУЧАТЕЛЬБИН_ИИН",          "ПолучательИНН");
	Результат.Вставить("ПОЛУЧАТЕЛЬИИК",              "ПолучательСчет");
	Результат.Вставить("ПОЛУЧАТЕЛЬБАНКНАИМЕНОВАНИЕ", "ПолучательБанк1");
	Результат.Вставить("ПОЛУЧАТЕЛЬБАНКБИК",          "ПолучательБИК");
	Результат.Вставить("ПОЛУЧАТЕЛЬБАНКИИК",          "ПолучательКорсчет");
	Результат.Вставить("КОДБЮДЖЕТНОЙКЛАССИФИКАЦИИ",  "ПоказательКБК");
