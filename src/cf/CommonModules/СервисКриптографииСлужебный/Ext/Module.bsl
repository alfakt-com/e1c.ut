...
Процедура ПолучитьСеансовыйКлюч(Параметры, АдресРезультата) Экспорт
	...
	Попытка
		Результат = ПолучитьСеансовыеКлючи(Параметры.ИдентификаторСертификата, Параметры.ВременныйПароль);
		ПоместитьВоВременноеХранилище(Результат, АдресРезультата);
	Исключение
...
Функция ПолучитьСеансовыеКлючи(ИдентификаторСертификата, ВременныйПароль)
	...
	Попытка
		...
		МаркерыБезопасности = ВыполнитьМетодКриптосервиса("crypto/all_security_tokens", ПараметрыМетода);
