...
//	Договор - СправочникСсылка.ДоговорыКонтрагентов - Договор, выбранный в документе
...
//
Функция НалогообложениеНДС(Знач Организация,
	                       Знач Договор,
	                       Знач НаправлениеДеятельности = Неопределено,
		...
		ИначеЕсли ТекЭлемент = "СоответствиеТоваровУсловиямЗакупок" Тогда
			...
			Пока Выборка.Следующий() Цикл
				Если ЕстьОтклоненияОтЦеновыхУсловий Тогда
					Прервать;
				КонецЕсли;
				Если Выборка.ЕстьОшибкиМаксимальноДопустимаяЦена // Цена не должна превышать максимально допустимую
					ИЛИ Выборка.ЕстьОшибкиЗаполненияСтавкаНДС Тогда // Ставка НДС не должна отличаться от ставки, в соглашении
...
//   * РазрешенВводНового                - Булево - Истина - Доступен ввод заявления о ввозе товаров по команде "Создать
//                                                  заявление о ввозе товаров из страны-члена ЕАЭС";
//
...
Процедура СформироватьЗапросТоварыДокументаЗакупки(ТекстЗапроса)
	...
	ТекстЗапроса = ТекстЗапроса + "
		...
		|			И ВременнаяТаблицаТовары.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК))
		|		ТОГДА
		|			ИСТИНА
		|		КОГДА
...
Функция ПредставлениеЗаявленияОВвозеТоваров(Номер, Дата, Проведен, ДатаПодтвержденияОплаты)
	...
	Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Заявление о ввозе товаров № %1 от %2 г.%3%4'"),
				СокрЛП(Номер),
				Формат(Дата, "ДЛФ=D"),
				?(НЕ Проведен, " " + НСтр("ru='(не проведен)'"), ""),
				СтрокаЕстьОплата);
