...
Процедура ОчиститьНеиспользуемыеРеквизитыОбъекта(Объект, СвойстваЭлементов) Экспорт
...
Процедура НастройкиЭлементовБанков(Настройки) Экспорт
	...
	Элемент.Свойства.Вставить("Маска", НСтр("ru = 'UUUUUUUU'"));
	
	Элемент = Настройки.Добавить();
	Элемент.Поля.Добавить("БИКБанка");
	Финансы.НовыйОтбор(Элемент.Условие, "ИностранныйБанк", Истина);
	Элемент.Свойства.Вставить("Заголовок", НСтр("ru = 'Национальный код банка'"));
	Элемент.Свойства.Вставить("Маска", НСтр("ru = 'UUUUUUUUU'"));
...
//
Процедура УстановитьПараметрыВыбораБанковскогоСчетаКонтрагента(Объект, Элемент, ИсключитьВыплатаЗарплатыНаЛицевыеСчета = Ложь) Экспорт
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику
		...
		// ЗУПК
		ИЛИ (Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета И НЕ ИсключитьВыплатаЗарплатыНаЛицевыеСчета)
		// ЗУПК
...
// Процедура устанавливает параметры выбора для типа налогов в связи с разделением отдельно на взносы при оплате
//
// Параметры:
//	Объект - ДанныеФормыСтруктура - Текущий объект
//	ЭлементСтатья - ПолеФормы - Поле для ввода статьи расходов.
//
Процедура УстановитьПараметрыВыбораТипаНалога(Объект, Элемент) Экспорт
	...
	Если (ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.СписаниеБезналичныхДенежныхСредств")
		...
		МассивВзносов.Добавить(Перечисления.ТипыНалогов.СО);
		МассивВзносов.Добавить(Перечисления.ТипыНалогов.ОПВ);
		МассивВзносов.Добавить(Перечисления.ТипыНалогов.ОППВ);
		МассивВзносов.Добавить(Перечисления.ТипыНалогов.ООСМС);
		МассивВзносов.Добавить(Перечисления.ТипыНалогов.ВОСМС);
		
		Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВзносов Тогда
			
			МассивПараметровВыбора.Добавить(Новый ПараметрВыбора("Отбор.Ссылка", МассивВзносов));
			
		ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВБюджет Тогда
			
			МассивТиповНалогов = Новый Массив;
			Для ИндексТипаНалога = 0 По Перечисления.ТипыНалогов.Количество() - 1 Цикл
				
				ТипНалога = Перечисления.ТипыНалогов.Получить(ИндексТипаНалога);
				Если МассивВзносов.Найти(ТипНалога) = Неопределено Тогда
					МассивТиповНалогов.Добавить(ТипНалога);
				КонецЕсли;
				
			КонецЦикла;
			
			МассивПараметровВыбора.Добавить(Новый ПараметрВыбора("Отбор.Ссылка", МассивТиповНалогов));
...
Функция ПредставлениеЗаписиКнигиПокупокВАвансовомОтчете(Основание) Экспорт
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		СтрокаГиперссылки = Новый ФорматированнаяСтрока(
			НСтр("ru='Оформить запись по НДС к возмещению'"), ,
	...
	Иначе
		...
		Если Выборка.Количество() > 1 Тогда
			...
			ТекстЗаписьКнигиПокупок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Записи по НДС к возмещению (%1)'"), Строка(Выборка.Количество()));
		...
		Иначе
			...
			СтрокаГиперссылки = Новый ФорматированнаяСтрока(
				НСтр("ru='Запись по НДС к возмещению'") + ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(Выборка), ,
				ЦветаСтиля.ЦветГиперссылки, ,
...
Процедура ЗаполнитьДокументПоЗаявкамНаРасходованиеДенежныхСредств(ДанныеЗаполнения, ТабличнаяЧасть, ФормаОплаты) Экспорт
	...
	|	ВЫБОР КОГДА Заявка.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеречислениеВзносов) ТОГДА
	|		ИСТИНА
	|	ИНАЧЕ
	|		ЛОЖЬ
	|	КОНЕЦ КАК ЭтоПеречислениеВзносов,
	...
	|	, Заявка.КодНазначенияПлатежа КАК КодНазначенияПлатежа 
	|
	|ИЗ
	...
	Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику
		...
		Если Не ЗначениеЗаполнено(ДокументФизическогоЛица) Тогда
			ДокументФизическогоЛица = ФизическиеЛицаУТ.ПолучитьДокументФизическогоЛицаПоУмолчанию(
				...
				Справочники.ВидыДокументовФизическихЛиц.УдостоверениеЛичностиРК).Представление;
		КонецЕсли;
		УстановитьПривилегированныйРежим(Ложь);
	...
	|			СУММА(РасшифровкаПлатежа.СуммаВзаиморасчетов) * КурсыВалют.КоэффициентПересчета *
	|			ВЫБОР
	|				КОГДА РасшифровкаПлатежа.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС12) ТОГДА
	|					0.12
...
Функция ТекстСуммаНДСПлатежа(ВалютаДокумента, РасшифровкаПлатежа, ДобавлятьТекстВТомЧисле) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаНДС Цикл
		Если СтрокаТаблицы.СтавкаНДС = Справочники.СтавкиНДС.БезНДС
			Или СтрокаТаблицы.СтавкаНДС = Справочники.СтавкиНДС.БезНДСНеРК
...
Функция СписокКППОрганизации(Организация) Экспорт
	...
	Запрос.Текст = "
	|ВЫБРАТЬ
	|	ЕСТЬNULL(Регистрации.РегистрацияВНалоговомОргане.ИНН, """")  КАК КПП,
	...
	|	
	|ВЫБРАТЬ
	|	ЕСТЬNULL(Организации.РегистрацияВНалоговомОргане.ИНН, """"),
	...
	|//////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Организации.ИНН КАК КПП,
	|	Организации.Наименование КАК Подразделение,
...
Процедура ОчиститьНеиспользуемыеРеквизиты(ДокументОбъект, Знач МассивВсехРеквизитов, Знач МассивРеквизитовОперации) Экспорт
	...
	// Для корректного заполнения печатной формы ПФ_MXL_Т3 в документе "Приобретение товаров и услуг" необходимо поле Контрагент, которое удаляется из массива МассивРеквизитовОперации 
	// при выборе ХО "Закупка через подотчетное лицо". Возвращаем его в массив.
	Если ТипЗнч(ДокументОбъект.Ссылка) = Тип("ДокументСсылка.ПриобретениеТоваровУслуг") Тогда 
		Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаЧерезПодотчетноеЛицо И Не ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") Тогда
...
Процедура ПроверитьЗаполнениеНалоговыхРеквизитов(Объект, Отказ, НепроверяемыеРеквизиты, ФлагОбменСБанками = Ложь, ОшибкиЗаполнения = Неопределено) Экспорт
	...
	ПроверитьЗаполнениеНалоговыхРеквизитовРК(Объект, Отказ, НепроверяемыеРеквизиты, ФлагОбменСБанками, ОшибкиЗаполнения);
	...
	ПараметрыПроверкиЗаполнения = Новый Структура("ПрименениеПриказа107н, ПрименениеПриказа126н",
		...
		ДатаНачалаПримененияПриказа126н <> '00010101' И Дата >= ДатаНачалаПримененияПриказа126н);
	...
	Если ПеречислениеВБюджет
	 И МассивОпераций.Найти(ХозяйственнаяОперация) <> Неопределено Тогда
		...
		Если ПлатежиВБюджетКлиентСервер.СтатусыПлательщика(ПараметрыПроверкиЗаполнения.ПрименениеПриказа107н).НайтиПоЗначению(СтатусСоставителя) = Неопределено Тогда
...
Процедура ПроверитьИННиКППНаСоответствие148н(Объект, Отказ) Экспорт
	...
	Если Объект.Дата < ПлатежиВБюджетКлиентСервер.НачалоДействияУказания3844У() 
		ИЛИ НЕ ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") Тогда
		Возврат;
	КонецЕсли;
...
//*Айнур
Процедура ОтразитьДетализацияПлатежа(ДополнительныеСвойства, Движения, Отказ) Экспорт

	ТаблицаДетализацияПлатежа = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаДетализацияПлатежа;
	
	Если Отказ ИЛИ ТаблицаДетализацияПлатежа.Количество() = 0 Тогда
	
		Возврат;
		
	КонецЕсли;
	
	ДвиженияДетализацияПлатежа = Движения.ДетализацияПлатежа;
	ДвиженияДетализацияПлатежа.Записывать = Истина;
	ДвиженияДетализацияПлатежа.Загрузить(ТаблицаДетализацияПлатежа);
...
Функция НаименованиеОрганизации(Знач Наименование) Экспорт
	...
	ИначеЕсли УбратьИзНаименованияОрганизационнуюФорму(Наименование, НСтр("ru='Индивидуальный предприниматель'")) Тогда
		ОрганизационноПравоваяФорма				= НСтр("ru='Индивидуальный предприниматель'");
		ОрганизационноПравоваяФормаСокращенно	= НСтр("ru='ИП'");
	ИначеЕсли УбратьИзНаименованияОрганизационнуюФорму(Наименование, НСтр("ru='ТОО'")) Тогда
...
Процедура УстановитьПараметрыВыбораКонтрагента(Объект, Элемент) Экспорт
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеТаможне
		...
		Или Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеречислениеВзносов	Тогда
...
Процедура УстановитьПараметрыВыбораСтавкиНДС(Элемент) Экспорт
	...
	Если Не Константы.ИспользоватьПродажиНаЭкспорт.Получить() Тогда
		ИсключаемыеЗначения.Добавить(Справочники.СтавкиНДС.НДС0);
	КонецЕсли;
...
Функция РасшифровкаПлатежаНДС(ДатаДокумента, ВалютаДокумента, ОбъектыРасчетов, ОплатаОтКлиента = Ложь) Экспорт
	...
	|		ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18)
	|	КОГДА ТаблицаСуммНДС.СтавкаНДС В (&СтавкаНДС12) ТОГДА
	|		ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС12)
	...
	СтавкаНДС12 = Новый Массив;
	СтавкаНДС12.Добавить(Справочники.СтавкиНДС.НДС12);
	...
	Запрос.УстановитьПараметр("СтавкаНДС12", СтавкаНДС12);
...
Процедура ЗаполнитьНДСВРасшифровке(РасшифровкаПлатежа, ТаблицаНДС) Экспорт
	...
	Для каждого Платеж Из Платежи Цикл
		...
		Если ТипЗнч(Платеж.Ссылка) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
			...
			Если Платеж.СтавкаНДС = Справочники.СтавкиНДС.НДС12 Тогда
				Платеж.СуммаНДС = Платеж.Сумма * 12 / 112;
...
Функция НазначениеПлатежа(Объект)
	...
	Если РеквизитыВалюты.Код = "398" Тогда
		ВалютаДляПечати = НСтр("ru = 'тенге'");
	Иначе
...
Функция РеквизитыПлательщика(Объект) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		СписокРеквизитов.Вставить("КПППлательщика", "РегистрацияВНалоговомОргане");
...
Функция НадписьРеквизитыПлательщика(ИНН, КПП, ТекстКорреспондента, ЮрФизЛицо, ХозяйственнаяОперация) Экспорт
	...
	ШаблонНадписи = НСтр("ru = 'БИН %1%2, %3'");
	...
	СтрокаКПП = ", " + НСтр("ru = ''"); // Регистрация в налоговом органе
...
Функция ПараметрыОперацииФискализацииЧека(ДокументСсылка, СуммаПредоплатыКорректировка = Неопределено, ВерсияФФД = Неопределено) Экспорт
	...
	|			ТОГДА ДанныеДокумента.КассаОтправитель.Владелец.ИНН
	...
	|			ТОГДА ДанныеДокумента.КассаПолучатель.Владелец.ИНН
	|		ИНАЧЕ ДанныеДокумента.Контрагент.ИНН
	|	КОНЕЦ КАК ПолучательИИН,
	|	
	...
	|	ДанныеДокумента.СуммаДокумента КАК СуммаДокумента,
	|	ДанныеДокумента.Валюта         КАК Валюта,
	|	ДанныеДокумента.Контрагент.ИНН КАК ПолучательИИН,
	|	
	...
	ПараметрыФискализацииЧека.СерияСвидетельстваПоНДС = СведенияОЮрФизЛице.СерияСвидетельстваПоНДС;
	ПараметрыФискализацииЧека.НомерСвидетельстваПоНДС = СведенияОЮрФизЛице.НомерСвидетельстваПоНДС;
	ПараметрыФискализацииЧека.ПолучательИИН           = Выборка.ПолучательИИН;
...
Процедура ЗаполнитьПозицииИТаблицуОплатЧека(ПараметрыФискализацииЧека, ДанныеДокумента, СуммаПредоплатыКорректировка)
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС) КОНЕЦ КАК СтавкаНДС,
	|	ВЫБОР
...
Процедура ЗаполнитьПозицииИТаблицуОплатЧека10(ПараметрыФискализацииЧека, ДанныеДокумента, СуммаПредоплатыКорректировка)
	...
	|
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС12) КАК СтавкаНДС,
	...
	ИтогоСуммаЧека = 0;
	//*Айнур 14032022
	ДокиОснования = РезультатЗапроса[1].Выгрузить().ВыгрузитьКолонку("Основание");
	
	ЗапросНоменклатуры = Новый Запрос;
	ЗапросНоменклатуры.Текст = ТекстЗапросаОплаченныеПозицииНоменклатуры10();
	ЗапросНоменклатуры.УстановитьПараметр("МассивДокументов", ДокиОснования);
	ЗапросНоменклатуры.УстановитьПараметр("ДатаДокумента", ДанныеДокумента.Дата);
	ЗапросНоменклатуры.УстановитьПараметр("ВалютаДокумента", ДанныеДокумента.Валюта);
	ЗапросНоменклатуры.УстановитьПараметр("ОплатаОтКлиента", ДанныеДокумента.ТипРасчета = Перечисления.ТипыРасчетаДенежнымиСредствами.ПриходДенежныхСредств);
	ЗапросНоменклатуры.Текст = СтрЗаменить(ЗапросНоменклатуры.Текст,
		"&ТекстЗапросаКоэффициентУпаковки1",
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаКоэффициентаУпаковки(
		"ТаблицаТовары.Упаковка",
		"ТаблицаТовары.Номенклатура"));
	СписокНоменклатур = ЗапросНоменклатуры.Выполнить().Выгрузить().Скопировать(,"НоменклатураНаименование");
	СписокНоменклатур.Свернуть("НоменклатураНаименование");
	
	сзНоменклатуры = Новый СписокЗначений;
	сзНоменклатуры.ЗагрузитьЗначения(СписокНоменклатур.ВыгрузитьКолонку("НоменклатураНаименование"));
	//*Айнур 14032022
	...
	Запрос.УстановитьПараметр("МассивДокументов", ДокиОснования);
	Запрос.УстановитьПараметр("ДатаДокумента", ДанныеДокумента.Дата);
	Запрос.УстановитьПараметр("ВалютаДокумента", ДанныеДокумента.Валюта);
	Запрос.УстановитьПараметр("ОплатаОтКлиента", ДанныеДокумента.ТипРасчета = Перечисления.ТипыРасчетаДенежнымиСредствами.ПриходДенежныхСредств);
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Если ВыборкаСтрокБезТоваров.Количество() > 0 Тогда Продолжить; КонецЕсли;  //Если вывели строку с информацией от кого оплата. Тогда детализация по товарам не нужно.
		...
		СтрокаПозицииЧека.КодЕдиницыИзмерения    = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Выборка.КодЕдиницыИзмерения);
		СтрокаПозицииЧека.НаименованиеЕдиницыИзмерения = Выборка.НаименованиеЕдиницыИзмерения;
...
Функция ТекстЗапросаОплаченныеПозицииНоменклатуры10()
	...
	|	ТаблицаДокумента.Ссылка.Валюта          КАК Валюта,
	|	ТаблицаДокумента.Ссылка.ЦенаВключаетНДС КАК ЦенаВключаетНДС
	|	,&ТекстЕдиницыТолькоНоменклатура
	|
	|ПОМЕСТИТЬ ТаблицаНоменклатуры
	|ИЗ
	|	Документ.АктВыполненныхРабот.Услуги КАК ТаблицаДокумента
	...
	|	ТаблицаДокумента.Ссылка.Валюта          КАК Валюта,
	|	ТаблицаДокумента.Ссылка.ЦенаВключаетНДС КАК ЦенаВключаетНДС
	|	,&ТекстЕдиницыНоменклатураУпаковка
	|ИЗ
	|	Документ.РеализацияТоваровУслуг.Товары КАК ТаблицаДокумента
	|	
	|		ЛЕВОЕ СОЕДИНЕНИЕ СчетаФактуры КАК СчетаФактуры
	...
	|	,&ТекстЕдиницыНоменклатураУпаковка
	|ИЗ
	|	Документ.ПредзаказКлиента.Товары КАК ТаблицаДокумента
	|
	|ГДЕ
	|	ТаблицаДокумента.Ссылка В (&МассивДокументов)
	|	И НЕ ТаблицаДокумента.Отменено
	...
	ТекстЕдиницыТолькоНоменклатура = "ТаблицаДокумента.Номенклатура.ЕдиницаИзмерения.Код КАК КодЕдиницыИзмерения,
	|	ПРЕДСТАВЛЕНИЕ(ТаблицаДокумента.Номенклатура.ЕдиницаИзмерения) КАК НаименованиеЕдиницыИзмерения
	|";
	ТекстЕдиницыНоменклатураУпаковка = "ВЫБОР
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|			ТОГДА ПРЕДСТАВЛЕНИЕ(ТаблицаДокумента.Номенклатура.ЕдиницаИзмерения)
	|		ИНАЧЕ ПРЕДСТАВЛЕНИЕ(ТаблицаДокумента.Упаковка.ЕдиницаИзмерения)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстЕдиницыТолькоНоменклатура", ТекстЕдиницыТолькоНоменклатура);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТекстЕдиницыНоменклатураУпаковка", ТекстЕдиницыНоменклатураУпаковка);
...
Процедура ПроверитьЗаполнениеНалоговыхРеквизитовРК(Объект, Отказ, НепроверяемыеРеквизиты, ФлагОбменСБанками = Ложь, ОшибкиЗаполнения = Неопределено) Экспорт
