...
Функция РегионАдресаКонтактнойИнформации(Знач XMLСтрока) Экспорт
	...
	Если Адрес = Неопределено Или Адрес.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "Адрес") Тогда
		ВызватьИсключение НСтр("ru = 'Невозможно определить субъекта, ожидается адрес.'");
	КонецЕсли;
...
//        * Идентификаторы      - Структура - Идентификаторы адресных объектоа, если установлен параметр КодыКЛАДР.
...
//         * ИдентификаторАдресногоОбъекта  - Идентификатор адресного объекта наименьшего уровня в иерархи адреса(без учета домов), если установлен параметр КодыКЛАДР.
//                                            (Например, улицы для Адреса: Московская обл, Коломенский р-н, Барановка д, Озерная ул, дом № 7).
