...
Функция ТекстБазисПриходыЗаявлениеОВвозеСебестоимость() // использует АналитикиРасходовВсе

	Возврат "
		|ВЫБРАТЬ
		|	""ТекстБазисПриходыЗаявлениеОВвозеСебестоимость"" КАК ЗапросИсточник,
		|	90 КАК Приоритет,
		|	""Потребление"" КАК ТипЗаписи,
		|	ЛОЖЬ КАК РасчетЗавершен,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения,
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Склады КАК Склады
		|		ПО (АналитикиТоваров.Склад = Склады.Ссылка)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыКОформлениюЗаявленийОВвозе КАК ЗаявленийОВвозе
		|		ПО (ДД.Регистратор = ЗаявленийОВвозе.ДокументПоступления)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыОрганизаций КАК Товары
		|		ПО (Товары.Регистратор = ЗаявленийОВвозе.Регистратор)
		...
		|	И ДД.Организация В(&МассивОрганизаций)
		|	И ДД.Активность
		|	И НЕ ДД.РазделУчета В (ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссию), ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.МатериалыПринятыеВПереработку))
		|	И (ДД.Организация, АналитикиТоваров.Номенклатура, АналитикиТоваров.Склад, ЗаявленийОВвозе.ДокументПоступления) В
		|			(ВЫБРАТЬ РАЗЛИЧНЫЕ
		|				КлючиРасходов.Организация,
		|				АналитикиТоваров.Номенклатура,
		|				АналитикиТоваров.Склад,
		...
		|				АналитикиРасходовВсе КАК КлючиРасходов
		|			ГДЕ
		|				(КлючиРасходов.АналитикаРасходов В (АналитикиТоваров.Номенклатура, ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка), АналитикиТоваров.Склад, ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка), ДД.Регистратор)
		|					ИЛИ ТИПЗНАЧЕНИЯ(ДД.Регистратор) = ТИПЗНАЧЕНИЯ(КлючиРасходов.АналитикаРасходов)
		|						И КлючиРасходов.АналитикаРасходов В (ЗНАЧЕНИЕ(Документ.ПриобретениеТоваровУслуг.ПустаяСсылка), ЗНАЧЕНИЕ(Документ.СборкаТоваров.ПустаяСсылка), ЗНАЧЕНИЕ(Документ.ПередачаТоваровМеждуОрганизациями.ПустаяСсылка))))
		|
		|СГРУППИРОВАТЬ ПО
		|	ДД.Период,
		|	ДД.Регистратор,
...
Функция ДанныеДляПартийПрочихСебестоимость(НачалоПериода, ОкончаниеПериода, МассивОрганизаций)
	...
	ИсходныйТекстЗапроса =
		...
		"ОБЪЕДИНИТЬ ВСЕ" + ТекстБазисПриходыЗаявлениеОВвозеСебестоимость() + // использует АналитикиРасходовВсе
...
Функция ТекстПоступленияПартийПрочихРасходовИзПартий()
	Возврат "
        ...
        //УПК по внеоборотным активам не ведется учет по налогообложению НДС
        |           И ЛОЖЬ
