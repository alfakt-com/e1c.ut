...
Функция ПечатьЧекаПоШаблону(ОбщийМодульОборудования, ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры) Экспорт
	...
	Иначе
		...
		ТипЧека =  ?(ТипРасчета = 1, Ложь, Истина);
		// Открываем чек
		Параметры.Вставить("ПолучательИИН", ОбщиеПараметры.ПолучательИИН);
	...
	Если Результат Тогда
		...
		Для ИндексМассива = 0 По ПозицииЧека.Количество() - 1 Цикл
			...
			Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
				...
				////////
				//Если ПозицияЧека.ДанныеКодаТоварнойНоменклатуры.Свойство("ПредставлениеШтрихКода") Тогда 
				//	КодМаркировки = ШтрихкодированиеИСКлиентСервер.Base64ВШтрихкод(ПозицияЧека.ДанныеКодаТоварнойНоменклатуры.ПредставлениеШтрихкода);
				//Иначе 
				//	КодМаркировки = "";
				//КонецЕсли;
				...
				Параметры.Вставить("ОсвобожденныйОборотНДС", ПозицияЧека.ОсвобожденныйОборотНДС);
