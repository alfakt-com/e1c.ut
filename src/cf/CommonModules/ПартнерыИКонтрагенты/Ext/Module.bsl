...
//
Функция ИННКППУжеИспользуетсяВИнформационнойБазе(ИНН,КПП,ИсключаяСсылку = Неопределено, ИсключаяСсылкуГоловного = Неопределено) Экспорт
	...
	|ГДЕ
	|	Контрагенты.ИНН = &ИНН
	// Отсечем контрагентов обособленные подразделения головной организации с таким же ИИН.
	|	И НЕ (Контрагенты.ОбособленноеПодразделение
	|		И Контрагенты.ГоловнойКонтрагент = &СсылкаГоловного
	|		И &СсылкаЗаполнена)
	|	И НЕ Контрагенты.Ссылка В (&Ссылка, &СсылкаГоловного)";
	
	Запрос.УстановитьПараметр("ИНН",ИНН);
	Запрос.УстановитьПараметр("КПП",КПП);
	Запрос.УстановитьПараметр("Ссылка",ИсключаяСсылку);
	
	// При обращении к ИсключаяСсылку.ГоловнойКонтрагент, в случае изменения головы при перезаписи объекта
	// получаем версию головного контрагента до изменения, поэтому передаем из формы нового головного контрагента 
	Запрос.УстановитьПараметр("СсылкаГоловного",ИсключаяСсылкуГоловного);
	Запрос.УстановитьПараметр("СсылкаЗаполнена",ЗначениеЗаполнено(ИсключаяСсылкуГоловного));
...
Процедура ЗаполнитьДанныеКонтрагентаПартнера(Приемник, Партнер, СИсторическимиДанными = Истина) Экспорт
	...
	|" + ?(Не ПолучитьФункциональнуюОпцию("ОтключенныйФункционал"), ",Контрагенты.ГосударственноеУчреждение", "") + " 
	|ИЗ
	|	Справочник.Контрагенты КАК Контрагенты
	|ГДЕ
	|	Контрагенты.Партнер = &Партнер";
...
Процедура ПартнерыФормаВыбораСпискаПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Не СтрНайти(Форма.ИмяФормы, "Контрагенты") > 0 Тогда
		//Начало: МК Баубериков А - 26.11.2020
		ЗапросСИнформациейПоКонтрагенту = Истина;
		//Конец: МК Баубериков А - 26.11.2020
...
Процедура ПроверитьКорректностьЗаполненияКонтрагента(Объект, Контрагент, Отказ) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.ИНН) Тогда 
		...
		ВыборкаКонтрагент = ИННКППУжеИспользуетсяВИнформационнойБазе(Объект.ИНН, Объект.КПП, Контрагент, ?(Объект.ОбособленноеПодразделение, Объект.ГоловнойКонтрагент, Неопределено));
		
		Если ВыборкаКонтрагент <> Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			                                СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			                                  НСтр("ru='Данные БИН / ИИН уже указаны для другого контрагента , ответственный - %1.'"),
	...
	//++Аян_К++ 080921 старт
	Если Не ЗначениеЗаполнено(Объект.КонтактнаяИнформацияПоле5b767338x64f3x11e8x8117x0cc47a7695e11) Тогда
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
		НСтр("ru = 'Не указан телефон'"),
		КлючДанных,
