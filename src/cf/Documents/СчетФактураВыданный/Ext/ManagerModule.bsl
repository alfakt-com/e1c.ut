...
Функция ДобавитьКомандуСоздатьНаОснованииКорректировочныйСчетФактура(КомандыСозданияНаОсновании) Экспорт
	Если ПравоДоступа("Добавление", Метаданные.Документы.СчетФактураВыданный) Тогда
		...
		КомандаСоздатьНаОсновании.Представление = НСтр("ru = 'Дополнительный/исправительный счет-фактура'");
		КомандаСоздатьНаОсновании.РежимЗаписи = "Проводить";
		КомандаСоздатьНаОсновании.ФункциональныеОпции = "ОтключенныйФункционал";
...
Функция ИсправляемыйСчетФактураПоОснованию(Ссылка, Основание, Организация = Неопределено) Экспорт
	...
	|
	|ВЫБРАТЬ
	|	ДанныеВозврата.ДокументРеализации КАК Ссылка
	|ИЗ
	|	Документ.ВозвратТоваровОтКлиента.Товары КАК ДанныеВозврата
	|ГДЕ
	|	ДанныеВозврата.Ссылка = &ДокументОснование
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ДанныеВозврата.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ВозвратТоваровОтКлиента.Товары КАК ДанныеОснования
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.ВозвратТоваровОтКлиента.Товары КАК ДанныеВозврата
	|	ПО
	|		ДанныеВозврата.ДокументРеализации = ДанныеОснования.ДокументРеализации
	|
	|ГДЕ
	|	ДанныеОснования.Ссылка = &ДокументОснование
	|
	...
	|
	|ВЫБРАТЬ
	|	ДанныеВозврата.ДокументПоступления КАК Ссылка
	|ИЗ
	|	Документ.ВозвратТоваровМеждуОрганизациями КАК ДанныеВозврата
	|ГДЕ
	|	ДанныеВозврата.Ссылка = &ДокументОснование
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	ДанныеВозврата.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.ВозвратТоваровМеждуОрганизациями КАК ДанныеОснования
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.ВозвратТоваровМеждуОрганизациями КАК ДанныеВозврата
	|	ПО
	|		ДанныеВозврата.ДокументПоступления = ДанныеОснования.ДокументПоступления
	|
	|ГДЕ
	|	ДанныеОснования.Ссылка = &ДокументОснование
	|;
	...
	|	СчетФактураВыданный.БанковскийСчетОрганизации     КАК БанковскийСчетОрганизации,
	|	СчетФактураВыданный.БанковскийСчетКонтрагента     КАК БанковскийСчетКонтрагента,
...
Функция КорректировочныеДокументыПоОснованию(Основание) Экспорт
	...
	|	ВозвратТоваровОтКлиента.Ссылка.Дата КАК Дата
	|ИЗ
	|	ВТ_СчетаФактурыИсходные КАК СчетаФактурыИсходные
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ДокументыОснования КАК ОснованияСчетаФактуры
	|			ПО СчетаФактурыИсходные.Ссылка = ОснованияСчетаФактуры.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровОтКлиента.Товары КАК ВозвратТоваровОтКлиента
	|			ПО ОснованияСчетаФактуры.ДокументОснование = ВозвратТоваровОтКлиента.ДокументРеализации
	|			И ВозвратТоваровОтКлиента.Ссылка.ХозяйственнаяОперация В (
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТребуетсяОформлениеСчетаФактуры КАК ТребуетсяСчетФактура
	|			ПО (ВозвратТоваровОтКлиента.Ссылка = ТребуетсяСчетФактура.Основание)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВозвратТоваровМеждуОрганизациями.Ссылка КАК Ссылка,
	|	ВозвратТоваровМеждуОрганизациями.Дата КАК Дата
	|ИЗ
	|	ВТ_СчетаФактурыИсходные КАК СчетаФактурыИсходные
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.ДокументыОснования КАК ОснованияСчетаФактуры
	|			ПО СчетаФактурыИсходные.Ссылка = ОснованияСчетаФактуры.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровМеждуОрганизациями КАК ВозвратТоваровМеждуОрганизациями
	|			ПО ОснованияСчетаФактуры.ДокументОснование = ВозвратТоваровМеждуОрганизациями.ДокументПоступления
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТребуетсяОформлениеСчетаФактуры КАК ТребуетсяСчетФактура
	|			ПО (ВозвратТоваровМеждуОрганизациями.Ссылка = ТребуетсяСчетФактура.Основание)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата УБЫВ");
...
Функция ПараметрыЗаполненияПоСчетуФактуре(СчетФактура) Экспорт
	...
	|
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ВозвратТоваровОтКлиента.ДокументРеализации, ОснованияСчетаФактуры.ДокументОснование) КАК ДокументОснование
	|ИЗ
	|	Документ.СчетФактураВыданный.ДокументыОснования КАК ОснованияСчетаФактуры
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровОтКлиента.Товары КАК ВозвратТоваровОтКлиента
	|		ПО ОснованияСчетаФактуры.ДокументОснование = ВозвратТоваровОтКлиента.Ссылка
	|ГДЕ
	|	ОснованияСчетаФактуры.Ссылка = &Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ВозвратТоваровМеждуОрганизациями.ДокументПоступления, ОснованияСчетаФактуры.ДокументОснование) КАК ДокументОснование
	|ИЗ
	|	Документ.СчетФактураВыданный.ДокументыОснования КАК ОснованияСчетаФактуры
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровМеждуОрганизациями КАК ВозвратТоваровМеждуОрганизациями
	|		ПО ОснованияСчетаФактуры.ДокументОснование = ВозвратТоваровМеждуОрганизациями.Ссылка
	|ГДЕ
	|	ОснованияСчетаФактуры.Ссылка = &Ссылка
	|;
	|
	...
	|	ВТ_ОснованияИсходныхСФ КАК ВТ_ОснованияИсходныхСФ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровОтКлиента.Товары КАК ВозвратТоваровОтКлиента
	|		ПО ВТ_ОснованияИсходныхСФ.ДокументОснование = ВозвратТоваровОтКлиента.ДокументРеализации
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТребуетсяОформлениеСчетаФактуры КАК ТребуетсяСчетФактура
	|		ПО (ВозвратТоваровОтКлиента.Ссылка = ТребуетсяСчетФактура.Основание)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВозвратТоваровМеждуОрганизациями.Ссылка                КАК Ссылка,
	|	ВозвратТоваровМеждуОрганизациями.Ссылка.Дата                  КАК Дата
	|ИЗ
	|	ВТ_ОснованияИсходныхСФ КАК ВТ_ОснованияИсходныхСФ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровМеждуОрганизациями КАК ВозвратТоваровМеждуОрганизациями
	|		ПО ВТ_ОснованияИсходныхСФ.ДокументОснование = ВозвратТоваровМеждуОрганизациями.ДокументПоступления
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТребуетсяОформлениеСчетаФактуры КАК ТребуетсяСчетФактура
	|		ПО (ВозвратТоваровМеждуОрганизациями.Ссылка = ТребуетсяСчетФактура.Основание)
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция ТребуетсяСозданиеКорректировочного(СчетФактура, Исходный = Истина) Экспорт
	...
	Если Исходный Тогда
		...
		|	Документ.СчетФактураВыданный.ДокументыОснования КАК ОснованияСчетаФактуры
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровМеждуОрганизациями КАК ВозвратТоваровМеждуОрганизациями
		|		ПО ОснованияСчетаФактуры.ДокументОснование = ВозвратТоваровМеждуОрганизациями.Основание
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТребуетсяОформлениеСчетаФактуры КАК ТребуетсяСчетФактура
		|		ПО (ВозвратТоваровМеждуОрганизациями.Ссылка = ТребуетсяСчетФактура.Основание)
		|ГДЕ
		|	ОснованияСчетаФактуры.Ссылка = &Ссылка
	...
	Если ДополнительныеСвойства.ТаблицыДляДвижений.Свойство("ТаблицаЖурналУчетаСчетовФактур") Тогда
		...
		Для Каждого СтрокаТаблицы Из ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаЖурналУчетаСчетовФактур Цикл
			Если ЗначениеЗаполнено(СтрокаТаблицы.НомерКорректировочногоСчетаФактуры) Тогда
				СтрокаТаблицы.НомерСчетаФактуры = СокрЛП(ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(СтрокаТаблицы.НомерСчетаФактуры, Истина, Истина))
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ЕСТЬNULL(ДанныеДокумента.СчетФактураОснование.Номер, """")             			  КАК НомерСчетФактурыОснования,
	...
	|	ДанныеДокумента.СтранаНазначения 												  КАК СтранаНазначения,
	...
	|	ДанныеДокумента.ДатаСовершенияОборотаПоРеализации 								  КАК ДатаСовершенияОборота,
	...
	// для УПК определяем номер документа исправления
	Номер = СокрЛП(ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Реквизиты.Номер, Истина, Истина));
	
	// для УПК определяем номер документа исправления
	НомерИсправления = СокрЛП(ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Реквизиты.Номер, Истина, Истина));
	
	// для УПК определяем номер счета-фактуры исправления
	НомерСчетФактурыОснования = СокрЛП(ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Реквизиты.НомерИсправления, Истина, Истина));
	
	Запрос.УстановитьПараметр("Период",								Реквизиты.Период);
	Запрос.УстановитьПараметр("Номер",								Номер);
	Запрос.УстановитьПараметр("Организация",						Реквизиты.Организация);
	Запрос.УстановитьПараметр("НомерСчетФактурыОснования",			НомерСчетФактурыОснования);
	...
	Запрос.УстановитьПараметр("СтранаНазначения",					Реквизиты.СтранаНазначения);
	Запрос.УстановитьПараметр("ДатаСовершенияОборота",				Реквизиты.ДатаСовершенияОборота); 
...
Процедура УстановитьПараметрЗапросаНомерНаПечать(ОсновнойЗапрос)
	...
	Если Не РезультатПроверки.Пустой() Тогда
		...
		Запрос.УстановитьПараметр("СпособДоставкиПоУмолчанию", НСтр("ru='Самовывоз'"));
...
#Область ТекстОтчетОРозничныхПродажахВидыЗапасов
	ТекстОтчетОРозничныхПродажахВидыЗапасов = "
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.Товары) КАК ВидЦенностиПродажи,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.Товары) КАК ВидЦенностиЗакупки,
	|	ТаблицаВидыЗапасов.СтавкаНДС КАК СтавкаНДС,
...
#Область ТекстОтчетОРозничныхПродажах
	ТекстОтчетОРозничныхПродажах = "
	|
	|ВЫБРАТЬ
	|	ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ПрочиеРаботыИУслуги),
	|	ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.ПрочиеРаботыИУслуги),
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СуммыДокументовВВалютеРегл.БазаНДСРегл,0) = 0
	|			ТОГДА ВЫРАЗИТЬ((ТаблицаТовары.Сумма - ТаблицаТовары.СуммаНДС) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ СуммыДокументовВВалютеРегл.БазаНДСРегл
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(СуммыДокументовВВалютеРегл.БазаНДСРегл,0) = 0
...
#Область ТекстОтчетПоКомиссииМеждуОрганизациямиВознаграждение
	...
	|	ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС),
	|	ВЫБОР
	|		КОГДА ОтчетПоКомиссии.СтавкаНДСВознаграждения = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС) 
	|			ИЛИ ОтчетПоКомиссии.СтавкаНДСВознаграждения = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК) 
...
#Область ТекстОтчетКомитентуВознаграждение
	...
	|	NULL,
	|	ВЫБОР
	|		КОГДА ОтчетКомитенту.СтавкаНДСВознаграждения = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС) 
	|			ИЛИ ОтчетКомитенту.СтавкаНДСВознаграждения = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК) 
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|	КОНЕЦ,
	|	NULL,
...
#Область ТекстЗаписьКнигиПродаж
	...
	|	NULL,
	|	ВЫБОР
	|		КОГДА ТаблицаЦенности.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС) 
	|			ИЛИ ТаблицаЦенности.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК) 
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|	КОНЕЦ,
	|	NULL,
...
#Область ТекстВозвратТоваровОтКлиента
	...
	|			ИСТИНА
	|	КОНЕЦ
	|	И ТаблицаВидыЗапасов.Ссылка.ХозяйственнаяОперация В (
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратТоваровОтКлиента),
...
#Область ТекстСторноИсправление
	...
	"ВЫБРАТЬ
	|	&ДатаСовершенияОборота КАК Период,
	...
	|,	ЖурналУчетаСчетовФактур.СтавкаНДС					                    	КАК СтавкаНДС,
	|	ЖурналУчетаСчетовФактур.СтранаНазначения			                    	КАК СтранаНазначения,
	|	ЖурналУчетаСчетовФактур.ИсходныйСчетФактура			                    	КАК ИсходныйСчетФактура,
	|	ЖурналУчетаСчетовФактур.СуммаРегл					                    	КАК СуммаРегл,
	|	ЖурналУчетаСчетовФактур.СуммаНДСРегл				                    	КАК СуммаНДСРегл
	|ИЗ
...
#Область ТекстВыставленныеСчетаФактурыНаРеализацию
	...
	|	&СтранаНазначения КАК СтранаНазначения,
	|	&СчетФактураОснование КАК ИсходныйСчетФактура,
	|	ТаблицаДанных.СуммаРегл КАК СуммаРегл,
	|	ТаблицаДанных.СуммаНДСРегл КАК СуммаНДСРегл
	|ИЗ
	|	(ВЫБРАТЬ
	|		МАКСИМУМ(ТаблицаДокументов.НалогообложениеНДС) КАК НалогообложениеНДС,
	|		ТаблицаДокументов.СтавкаНДС КАК СтавкаНДС,
	|		СУММА(ТаблицаДокументов.СуммаПоСчетуФактуре) КАК СуммаПоСчетуФактуре,
	|		СУММА(ТаблицаДокументов.СуммаНДС) КАК СуммаНДС,
	|		СУММА(ТаблицаДокументов.СуммаРегл) КАК СуммаРегл,
	|		СУММА(ТаблицаДокументов.СуммаНДСРегл) КАК СуммаНДСРегл,
	...
	|			(ТаблицаСуммДокументов.СуммаБезНДС + ТаблицаСуммДокументов.СуммаНДС) КАК СуммаРегл,
	|			ТаблицаСуммДокументов.СуммаНДС КАК СуммаНДСРегл,
...
#Область ТекстВыставленныеСчетаФактурыНаРеализациюИсправление
	...
	|			(ТаблицаКорректировки.СуммаБезНДС + ТаблицаКорректировки.СуммаНДС) КАК СуммаРегл,
	|			ТаблицаКорректировки.СуммаНДС КАК СуммаНДСРегл,
	|			0,
	|			0
	|		ИЗ
...
#Область ТекстВыставленныеСчетаФактурыНаРеализациюКорректировочный
	...
	|	ВЫБОР
	|		КОГДА &Исправление
	|			ТОГДА &НомерСчетФактурыОснования
	|		ИНАЧЕ &Номер
	|	КОНЕЦ,
	|	ВЫБОР
	|		КОГДА &Исправление
...
#Область ТекстПолученныеСчетаФактуры
	...
	|	&СтранаНазначения КАК СтранаНазначения,
	|	&СчетФактураОснование КАК ИсходныйСчетФактура,
	|	СУММА(ТаблицаСуммДокументов.СуммаБезНДС + ТаблицаСуммДокументов.СуммаНДС) КАК СуммаРегл,
	|	СУММА(ТаблицаСуммДокументов.СуммаНДС) КАК СуммаНДСРегл
	|	
	|ИЗ
	|	ВтТаблицаСуммДокументов КАК ТаблицаСуммДокументов
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДокументыОснования КАК ДанныеОснований
...
#Область ТекстПолученныеСчетаФактурыКорректировочный
	ТекстПолученныеСчетаФактурыКорректировочный = "
	|ВЫБРАТЬ
	|	&ДатаСовершенияОборота,
	|	&Организация,
	|	&Контрагент,
...
#КонецОбласти
	...
	МассивТекстов.Добавить(ТекстПолученныеСчетаФактурыКорректировочный);
...
Функция СчетФактураПредыдущееИсправление(СчетФактураОснование, НомерИсправления)
	...
	НомерИсправленияЧислом = ?(ЗначениеЗаполнено(НомерИсправления),Число(ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(НомерИсправления, Истина, Истина)),1);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		НомерИзВыборки = Число(ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Выборка.НомерИсправления, Истина, Истина));
...
Функция ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Запрос.Текст = ТекстЗапросаДанныхШапкиДляПечатиСчетаФактуры(ПараметрыПечати)
		+ ТекстЗапросаИсходныхДокументовДляПечатиСчетаФактуры(ПараметрыПечати)
...
Процедура ПоместитьВременнуюТаблицуДанныхОснований(МенеджерВременныхТаблиц, МассивОснований)
	...
	Для Каждого ТипОснования Из ТипыОснований Цикл
		...
		РеквизитыОтветственныеЛица = Неопределено;
		...
		ОтветственныеЛицаСервер.СформироватьВременнуюТаблицуОтветственныхЛицДокументов(
			ОбъектыТипа, МенеджерВременныхТаблиц, , РеквизитыОтветственныеЛица,
			ИмяВременнойТаблицы);
		...
		|	Таблица.ГлавныйБухгалтерДолжность    КАК ГлавныйБухгалтерДолжность
		|//ОператорПОМЕСТИТЬ
		|ИЗ
...
Процедура СформироватьВременнуюТаблицуТоваровДляПечати(МенеджерВременныхТаблиц, МассивОснований, ПараметрыПечати)
	...
	Для Каждого ТипОснования Из ТипыОснований Цикл
		...
		ПараметрыЗаполнения.Вставить("ПечатьСчетаФактуры", Истина);
		...
		Если МенеджерВременныхТаблиц.Таблицы[ИмяОбъекта + "ТаблицаТоваров"].Колонки.Найти("БазаНДСРегл") = Неопределено Тогда
			ТекстОбщейТаблицыТовары = СтрЗаменить(ТекстОбщейТаблицыТовары, "ТаблицаДокумента.БазаНДСРегл", "ТаблицаДокумента.СуммаБезНДС");
		КонецЕсли;
		...
		Если ИмяОбъекта = "АктВыполненныхРабот" Или ИмяОбъекта = "РеализацияТоваровУслуг" Тогда
			...
			|	ВЫБОР
			|		КОГДА &ПредставлениеНаборов = НЕОПРЕДЕЛЕНО
			|		ТОГДА ТаблицаДокумента.ВариантПредставленияНабораВПечатныхФормах
			|		ИНАЧЕ &ПредставлениеНаборов
		...
		Если ИмяОбъекта = "КорректировкаРеализации" Тогда
			...
			|	ТаблицаДокумента.БазаНДСРеглДо                               КАК БазаНДСРеглДо,
			...
			|	ТаблицаДокумента.РазницаБазаНДСУвеличение                    КАК РазницаБазаНДСУвеличение,
			|	ТаблицаДокумента.РазницаБазаНДСУменьшение                    КАК РазницаБазаНДСУменьшение,
		...
		Иначе
			...
			|	0                                                            КАК РазницаБазаНДСУвеличение,
			|	0                                                            КАК РазницаБазаНДСУменьшение,
		...
		Если ИмяОбъекта = "ЗаписьКнигиПродаж"
			Или ИмяОбъекта = "РеализацияТоваровУслуг" 
			Или ИмяОбъекта = "КорректировкаРеализации" Тогда
			ТекстЗамены = "	ВЫБОР КОГДА ТаблицаДокумента.НомерГТД.КодТНВЭД <> """"
			|		ТОГДА ТаблицаДокумента.НомерГТД.КодТНВЭД
			|	КОГДА ТаблицаДокумента.КодТНВЭД <> ЗНАЧЕНИЕ(Справочник.КлассификаторТНВЭД.ПустаяСсылка)
			|		ТОГДА ТаблицаДокумента.КодТНВЭД
			|	ИНАЧЕ ЕСТЬNULL(ТаблицаДокумента.Номенклатура.КодТНВЭД, ЗНАЧЕНИЕ(Справочник.КлассификаторТНВЭД.ПустаяСсылка))
			|	КОНЕЦ КАК КодТНВЭД,";
		ИначеЕсли ИмяОбъекта = "РеализацияУслугПрочихАктивов" Тогда
		...
		Иначе	
			ТекстЗамены = "	ТаблицаДокумента.Номенклатура.КодТНВЭД                          КАК КодТНВЭД,";
		КонецЕсли;
	...
	|	СУММА(ТаблицаТоваров.БазаНДСРегл)    КАК БазаНДСРегл,
	...
	|	ВЫБОР
	|		КОГДА &ПредставлениеНаборов = НЕОПРЕДЕЛЕНО
	|		ТОГДА Товары.ВариантПредставленияНабораВПечатныхФормах
	|		ИНАЧЕ &ПредставлениеНаборов
	|	КОНЕЦ КАК ВариантПредставленияНабораВПечатныхФормах,
	...
	|	ВЫБОР
	|		КОГДА &ПредставлениеНаборов = НЕОПРЕДЕЛЕНО
	|		ТОГДА ВариантыКомплектацииНоменклатурыТовары.Ссылка.ВариантПредставленияНабораВПечатныхФормах
	|		ИНАЧЕ &ПредставлениеНаборов
	|	КОНЕЦ КАК ВариантПредставленияНабораВПечатныхФормах,
	...
	|	ДанныеНоменклатуры.ИдентификаторТовараЭСФ                КАК ИдентификаторТовара,
	...
	|	ТаблицаТоваров.БазаНДСРегл                               КАК БазаНДСРегл,
	|	ТаблицаТоваров.БазаНДСРеглДо                             КАК БазаНДСРеглДо,
	...
	|	ТаблицаТоваров.РазницаБазаНДСУвеличение                  КАК РазницаБазаНДСУвеличение,
	|	ТаблицаТоваров.РазницаБазаНДСУменьшение                  КАК РазницаБазаНДСУменьшение,
	...
	Если ЕстьНаборы Тогда
		...
		|	0                                           КАК СуммаБезНДСДо,
		|	ВременнаяТаблицаНаборы.БазаНДСРегл          КАК БазаНДСРегл,
		|	0                                           КАК БазаНДСРеглДо,
	...
	МассивОперацийПередачаНаКомиссию.Добавить(Перечисления.ХозяйственныеОперации.ПередачаНаКомиссиюВДругуюОрганизацию);
	Запрос.УстановитьПараметр("ХозяйственныеОперацииПередачаНаКомиссию", МассивОперацийПередачаНаКомиссию);
	Запрос.УстановитьПараметр("ПредставлениеНаборов", ?(ПараметрыПечати.Свойство("ПредставлениеНаборов"), ПараметрыПечати.ПредставлениеНаборов, Неопределено));
...
Функция ТекстЗапросаДанныхШапкиДляПечатиСчетаФактуры(ПараметрыПечати)
	...
	|
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(СчетФактураВыданный.Контрагент КАК Справочник.Контрагенты).ОбособленноеПодразделение
	|			ТОГДА СчетФактураВыданный.Контрагент.ГоловнойКонтрагент
	|		ИНАЧЕ
	|			СчетФактураВыданный.Контрагент 
	|	КОНЕЦ КАК Контрагент,
	|
	|	СчетФактураВыданный.Контрагент.ГосударственноеУчреждение КАК ГосударственноеУчреждение,
	|	СчетФактураВыданный.ГосучреждениеКодТоваров,
	|	СчетФактураВыданный.ГосучреждениеНазначениеПлатежа,
	|	
	...
	|	ДанныеОснований.УсловияПоставки                       КАК УсловияПоставки,
	|	ДанныеОснований.СпособОтправленияПоКлассификатору     КАК СпособОтправленияПоКлассификатору,
	|	ВЫБОР
	|		КОГДА ДанныеОснований.СпособДоставки = НЕОПРЕДЕЛЕНО ТОГДА
	|			&СпособДоставкиПоУмолчанию
	|		ИНАЧЕ
	|			ДанныеОснований.СпособДоставки
	|	КОНЕЦ                                                 КАК СпособДоставки,
	|	ДанныеОснований.АдресДоставки                         КАК АдресДоставки,
	|	ДанныеОснований.Договор                               КАК Договор,
	|	ДанныеОснований.ФормаОплаты                           КАК ФормаОплаты,
	...
	Если ПечататьСчетаФактурыПолученные Тогда
		...
		|	КОНЕЦ КАК Контрагент,
		|
		|	ЛОЖЬ КАК ГосударственноеУчреждение,
		|	"""" КАК ГосучреждениеКодТоваров,
		|	"""" КАК ГосучреждениеНазначениеПлатежа,
		|	
	...
	|	КОНЕЦ                                                 КАК ДатаИсходногоДокумента,
	|	ВЫБОР КОГДА ДанныеДокумента.Исправление ТОГДА
	|		ЕСТЬNULL(ДанныеДокумента.СчетФактураОснование.Номер, """")
	|	ИНАЧЕ 
	...
	|	ДанныеДокумента.ДатаСовершенияОборотаПоРеализации     КАК ДатаОборотаПоРеализации,
	|	ДанныеДокумента.Исправление                           КАК Исправление,
	...
	|	ДанныеОснований.ГосударственноеУчреждение             КАК ГосударственноеУчреждение,
	|	ДанныеОснований.ГосучреждениеКодТоваров               КАК ГосучреждениеКодТоваров,
	|	ДанныеОснований.ГосучреждениеНазначениеПлатежа        КАК ГосучреждениеНазначениеПлатежа,
	...
	|	ТаблицаОтветственныеЛица.ГлавныйБухгалтерДолжность    КАК ДолжностьГлавногоБухгалтера,
	...
	|	ДанныеОснований.Кладовщик                             КАК Кладовщик,
	|	ДанныеОснований.ДолжностьКладовщика                   КАК ДолжностьКладовщика,
	|	ДанныеОснований.СпособДоставки                        КАК СпособДоставки,
	|	ДанныеОснований.АдресДоставки                         КАК АдресДоставки,
	|	ДанныеОснований.Договор                               КАК Договор,
	|	ДанныеОснований.ФормаОплаты                           КАК ФормаОплаты,
	|	ДанныеОснований.УсловияПоставки                       КАК УсловияПоставки,
	...
	Если ПечататьСчетаФактурыПолученные Тогда
		...
		|	ВЫБОР КОГДА ДанныеДокумента.Корректировочный ТОГДА
		|		ЕСТЬNULL(ДанныеДокумента.СчетФактураОснование.Дата, ДАТАВРЕМЯ(1,1,1))
		|	ИНАЧЕ
		|		НЕОПРЕДЕЛЕНО
		|	КОНЕЦ                                                 КАК ДатаИсходногоДокумента,
		...
		|		НЕОПРЕДЕЛЕНО
		|	КОНЕЦ                                                 КАК ДатаИсправления,
		|	ДанныеДокумента.Дата                                  КАК ДатаОборотаПоРеализации,
		|	ДанныеДокумента.Исправление                           КАК Исправление,
		|	ДанныеДокумента.Корректировочный                      КАК КорректировочныйСчетФактура,
		...
		|	ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)          КАК СтранаНазначения,
		|	ДанныеОснований.Контрагент                            КАК Контрагент,
		|	ДанныеОснований.ГосударственноеУчреждение             КАК ГосударственноеУчреждение,
		|	ДанныеОснований.ГосучреждениеКодТоваров               КАК ГосучреждениеКодТоваров,
		|	ДанныеОснований.ГосучреждениеНазначениеПлатежа        КАК ГосучреждениеНазначениеПлатежа,
		...
		|	""""                                                  КАК ДолжностьГлавногоБухгалтера,
		|	ДанныеОснований.Валюта                                КАК Валюта,
		|	ДанныеОснований.ВалютаНаименованиеПолное              КАК ВалютаНаименованиеПолное,
		|	ДанныеОснований.ВалютаКод                             КАК ВалютаКод,
		|	ДанныеОснований.Основание                             КАК Основание,
	...
	Если ПараметрыПечати.Свойство("ДанныеДоговораГЗ") И ПараметрыПечати.ДанныеДоговораГЗ Тогда
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//РеквизитыДоговораГЗ",
		"	ДанныеОснований.НомерДокументаГЗ    КАК НомерДокументаГЗ,
		|	ДанныеОснований.ДатаДокументаГЗ   	КАК ДатаДокументаГЗ,
		|	ДанныеОснований.ДатаПодписанияГЗ    КАК ДатаПодписанияГЗ,");
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//ДанныеДоговораГЗВыданного",
		"	ДанныеОснований.НомерДокументаГЗ КАК НомерДокументаГЗ,
		|	ДанныеОснований.ДатаДокументаГЗ  КАК ДатаДокументаГЗ,
		|	ДанныеОснований.ДатаПодписанияГЗ КАК ДатаПодписанияГЗ,");
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//ДанныеДоговораГЗПолученного",
		"	НЕОПРЕДЕЛЕНО КАК НомерДокументаГЗ,
		|	НЕОПРЕДЕЛЕНО КАК ДатаДокументаГЗ,
		|	НЕОПРЕДЕЛЕНО КАК ДатаПодписанияГЗ,");
	КонецЕсли;
...
Функция ТекстЗапросаДанныхТабличнойЧастиДляПечатиСчетаФактуры(ПараметрыПечати)
	...
	|	ТаблицаДокумента.ЭтоТовар                                	 КАК ЭтоТовар,
	|	ТаблицаДокумента.ИдентификаторТовара          				 КАК ИдентификаторТовара,
	|	ТаблицаДокумента.Номенклатура                                КАК Номенклатура,
	...
	|	ВЫБОР 
	|		КОГДА НЕ ДанныеЕдиницыИзмерения.ТипИзмеряемойВеличины ЕСТЬ NULL ТОГДА 
	|			ВЫБОР 
	|				КОГДА НЕ ДанныеЕдиницыИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.Упаковка) 
	|					ТОГДА ДанныеЕдиницыИзмерения.КодЭСФ
	|				ИНАЧЕ ДанныеЕдиницыИзмерения.ЕдиницаИзмерения.КодЭСФ
	|			КОНЕЦ	
	|		ИНАЧЕ ""796""
	|	КОНЕЦ	 													 КАК ЕдиницаИзмеренияКодЭСФ,
	...
	|	ТаблицаДокумента.НомерГТД									 КАК НомерГТД,
	|	ТаблицаДокумента.НомерГТД.Код                                КАК НомерГТДКод,
	...
	|	СУММА(ТаблицаДокумента.БазаНДСРеглДо)                        КАК БазаНДСРеглДо,
	...
	|	СУММА(ТаблицаДокумента.РазницаБазаНДСУвеличение)             КАК РазницаБазаНДСУвеличение,
	|	СУММА(ТаблицаДокумента.РазницаБазаНДСУменьшение)             КАК РазницаБазаНДСУменьшение,
	...
	|	И НЕ (СчетаФактурыОснования.Ссылка.Корректировочный
	|		И ТИПЗНАЧЕНИЯ(ТаблицаДокумента.Ссылка) = ТИП(Документ.КорректировкаРеализации))
	|
	|СГРУППИРОВАТЬ ПО
	|	СчетаФактурыОснования.Ссылка,
	|	ТаблицаДокумента.ЭтоТовар,
	|	ТаблицаДокумента.ИдентификаторТовара,
	...
	|	ДанныеСтраныПросхождения.Ссылка                              КАК СтранаПроисхождения,
	|	ДанныеСтраныПросхождения.Код                                 КАК СтранаПроисхожденияКод,
	|	СУММА(ТаблицаДокумента.Количество - ТаблицаДокумента.КоличествоДо) КАК Количество,
	|	ТаблицаДокумента.Цена                                        КАК Цена,
	|	СУММА(
	|		ВЫБОР КОГДА ТаблицаДокумента.РазницаБезНДСУвеличение = 0
	|			ТОГДА -ТаблицаДокумента.РазницаБезНДСУменьшение
	|		ИНАЧЕ ТаблицаДокумента.РазницаБезНДСУвеличение
	|		КОНЕЦ)                                                   КАК СуммаБезНДС,
	|	СУММА(
	|		ВЫБОР КОГДА ТаблицаДокумента.РазницаБазаНДСУвеличение = 0
	|			ТОГДА -ТаблицаДокумента.РазницаБазаНДСУменьшение
	|		ИНАЧЕ ТаблицаДокумента.РазницаБазаНДСУвеличение
	|		КОНЕЦ)                                                   КАК БазаНДСРегл,
	|	СУММА(
	|		ВЫБОР КОГДА ТаблицаДокумента.РазницаНДСУвеличение = 0
	|			ТОГДА -ТаблицаДокумента.РазницаНДСУменьшение
	|		ИНАЧЕ ТаблицаДокумента.РазницаНДСУвеличение
	|		КОНЕЦ)                                                   КАК СуммаНДС,
	|	СУММА(
	|		ВЫБОР КОГДА ТаблицаДокумента.РазницаБезНДСУвеличение = 0
	|			ТОГДА -(ТаблицаДокумента.РазницаБезНДСУменьшение + ТаблицаДокумента.РазницаНДСУменьшение)
	|		ИНАЧЕ ТаблицаДокумента.РазницаБезНДСУвеличение + ТаблицаДокумента.РазницаНДСУвеличение
	|		КОНЕЦ)                                                   КАК СуммаСНДС,
	|	ТаблицаДокумента.СтавкаНДС                                   КАК СтавкаНДС,
	|
	|	СУММА(ТаблицаДокумента.КоличествоДо)                         КАК КоличествоДо,
	...
	|	СУММА(ТаблицаДокумента.РазницаБезНДСУвеличение)              КАК РазницаБезНДСУвеличение,
	|	СУММА(ТаблицаДокумента.РазницаБезНДСУменьшение)              КАК РазницаБезНДСУменьшение,
	|	СУММА(ТаблицаДокумента.РазницаБазаНДСУвеличение)             КАК РазницаБазаНДССУвеличение,
	|	СУММА(ТаблицаДокумента.РазницаБазаНДСУменьшение)             КАК РазницаБазаНДСУменьшение,
	|	СУММА(ТаблицаДокумента.СуммаНДСДо)                           КАК СуммаНДСДо,
	|	СУММА(ТаблицаДокумента.РазницаНДСУвеличение)                 КАК РазницаНДСУвеличение,
	|	СУММА(ТаблицаДокумента.РазницаНДСУменьшение)                 КАК РазницаНДСУменьшение,
	...
	|ГДЕ
	|	НЕ ТаблицаДокумента.ЭтоВозвратнаяТара
	|	И (СчетаФактурыОснования.Ссылка.Корректировочный
	|		И ТИПЗНАЧЕНИЯ(ТаблицаДокумента.Ссылка) = ТИП(Документ.КорректировкаРеализации))
	|
	|СГРУППИРОВАТЬ ПО
	|	СчетаФактурыОснования.Ссылка,
...
Функция ТекстЗапросаИсходныхДокументовДляПечатиСчетаФактуры(ПараметрыПечати)
...
Функция ПараметрыОбновленияРеквизитовСчетаФактуры(Основание, РеквизитыСчетаФактуры)
	...
	Если ТипЗнч(Основание) = Тип("ДокументОбъект.ПередачаТоваровМеждуОрганизациями")
			И Не Основание.РасчетыЧерезОтдельногоКонтрагента Тогда
		
		ПроверяемыеРеквизиты.Контрагент = Основание.ОрганизацияПолучатель;
		
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументОбъект.ВозвратТоваровМеждуОрганизациями") Тогда
		
		ПроверяемыеРеквизиты.Организация = Основание.ОрганизацияПолучатель;
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	// в УПК признак СводныйКорректировочный не используется (всегда в ЛОЖЬ)
	|	И ЛОЖЬ 
	|СГРУППИРОВАТЬ ПО
