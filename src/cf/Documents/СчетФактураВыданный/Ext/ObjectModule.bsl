...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НетИсходногоСчетФактуры Тогда
		Если КоличествоОснований > 1 Тогда
			ТекстСообщения = НСтр("ru = 'Перед добавлением дополнительного/исправительного счета-фактуры создайте исходный счет-фактуру для всех документов списка ""Документы-основания"".'");
		Иначе
			ТекстСообщения = НСтр("ru = 'Перед добавлением дополнительного/исправительного счета-фактуры создайте исходный счет-фактуру для документа-основания.'");
		КонецЕсли;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ДокументыОснованияПредставление", , Отказ);
	КонецЕсли;
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	Если ТипЗнч(ДанныеЗаполнения) <> Тип("Структура") Или Не ДанныеЗаполнения.Свойство("БанковскийСчетОрганизации") Тогда
	...
	Если ТипЗнч(ДанныеЗаполнения) <> Тип("Структура") Или Не ДанныеЗаполнения.Свойство("БанковскийСчетКонтрагента") Тогда
		...
		Иначе
			СтруктураПараметров = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизацииПоУмолчанию();
			СтруктураПараметров.Организация    = Контрагент;
			СтруктураПараметров.БанковскийСчет = БанковскийСчетКонтрагента;
			СтруктураПараметров.Валюта         = Валюта;
...
Процедура ЗаполнитьПараметрыСчетаФактурыПоОснованию(ВыбранныйСчетФактура = Неопределено) Экспорт
	...
	Если Не ПараметрыСчетаФактуры.Организация = Неопределено И Не ПараметрыСчетаФактуры.Организация = Организация Тогда
		...
		БанковскийСчетОрганизации = ПараметрыСчетаФактуры.БанковскийСчетОрганизации;
	КонецЕсли;
	...
	Если Не ПараметрыСчетаФактуры.Контрагент = Неопределено И Не ПараметрыСчетаФактуры.Контрагент = Контрагент Тогда
		...
		БанковскийСчетКонтрагента = ПараметрыСчетаФактуры.БанковскийСчетКонтрагента;
		
		СтранаНазначения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "СтранаРегистрации");
		Если НЕ ЗначениеЗаполнено(СтранаНазначения) Тогда
			СтранаНазначения = Справочники.СтраныМира.Казахстан;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПоДокументуОснованию(ДанныеЗаполнения)
	...
	ОснованиеСодержитДанныеГЗ = ТипЗнч(ДанныеЗаполнения.ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
						ИЛИ ТипЗнч(ДанныеЗаполнения.ДокументОснование) = Тип("ДокументСсылка.АктВыполненныхРабот");
						
	Если ОснованиеСодержитДанныеГЗ Тогда
		
		СтруктураДат = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДанныеЗаполнения.ДокументОснование, "Дата, ДатаПодписанияГЗ");
		...
		Если ТипЗнч(ДанныеЗаполнения.ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			...
			|ГДЕ
			|	РеализацияТоваровУслугТовары.Номенклатура.ВидНоменклатуры.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
			|	И РеализацияТоваровУслугТовары.Ссылка = &Ссылка";
			Запрос.УстановитьПараметр("Ссылка", ДанныеЗаполнения.ДокументОснование);
			Результат = Запрос.Выполнить();
			ЕстьУслуги = Не Результат.Пустой();
		Иначе
			ЕстьУслуги = Истина;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СтруктураДат.ДатаПодписанияГЗ) И ЕстьУслуги Тогда
			ДанныеЗаполнения.Вставить("ДатаСовершенияОборотаПоРеализации", СтруктураДат.ДатаПодписанияГЗ);
		Иначе 
			ДанныеЗаполнения.Вставить("ДатаСовершенияОборотаПоРеализации", СтруктураДат.Дата);
		КонецЕсли;
		
	Иначе
		ДанныеЗаполнения.Вставить("ДатаСовершенияОборотаПоРеализации", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения.ДокументОснование, "Дата"));
	КонецЕсли;
	...
	Если Не ПараметрыСчетаФактуры.Подразделение = Неопределено Тогда
		...
		ДанныеЗаполнения.Вставить("БанковскийСчетОрганизации", ПараметрыСчетаФактуры.БанковскийСчетОрганизации);
	КонецЕсли;
	...
	Если Не ПараметрыСчетаФактуры.Контрагент = Неопределено Тогда
		...
		ДанныеЗаполнения.Вставить("БанковскийСчетКонтрагента", ПараметрыСчетаФактуры.БанковскийСчетКонтрагента);
		
		СтранаНазначения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыСчетаФактуры.Контрагент, "СтранаРегистрации");
		Если НЕ ЗначениеЗаполнено(СтранаНазначения) Тогда
			СтранаНазначения = Справочники.СтраныМира.Казахстан;
		КонецЕсли;
		ДанныеЗаполнения.Вставить("СтранаНазначения", СтранаНазначения);
	КонецЕсли;
	...
	Если НЕ ПараметрыСчетаФактуры.СчетФактураОснование = Неопределено И ДанныеЗаполнения.Исправление Тогда
		...
		ДанныеЗаполнения.Вставить("ДатаСовершенияОборотаПоРеализации", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыСчетаФактуры.СчетФактураОснование, "ДатаСовершенияОборотаПоРеализации"));
	КонецЕсли;
	...
	Если ПараметрыСчетаФактуры.Корректировочный = Истина И ПараметрыСчетаФактуры.РеквизитыОснований.Количество() > 0
		И ЗначениеЗаполнено(ПараметрыСчетаФактуры.РеквизитыОснований[0].ИсходныйДокумент) Тогда
		// п. 1 ст. 265 НК РК дата оборота, по дате наступления случаев
		ДанныеЗаполнения.Вставить("ДатаСовершенияОборотаПоРеализации", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДанныеЗаполнения.ДокументОснование, "Дата"));
	КонецЕсли;
...
Функция ПолучитьПараметрыСчетаФактурыПоОснованиям()
	...
	Результат = Новый Структура("Организация, Контрагент, КППКонтрагента, ИННКонтрагента, Валюта, БанковскийСчетОрганизации, БанковскийСчетКонтрагента, СчетФактураОснование, РеквизитыОснований,
	...
	Колонки.Добавить("НомерИсправленияИсходногоДокумента", ОбщегоНазначения.ОписаниеТипаСтрока(12));
	...
	Запрос.Параметры.Вставить("ДокументОснование_КорректировкаРеализации", Новый Массив);
	Запрос.Параметры.Вставить("ДокументОснование_ВозвратТоваровОтКлиента", Новый Массив);
	Запрос.Параметры.Вставить("ДокументОснование_ВозвратТоваровМеждуОрганизациями", Новый Массив);
	...
	Для Каждого ТипОснования Из ТипыОснований Цикл
		...
		ЕстьБанковскийСчетОрганизации = ОбщегоНазначения.ЕстьРеквизитОбъекта("БанковскийСчетОрганизации", МетаданныеДокумента);
		ЕстьБанковскийСчетКонтрагента = ОбщегоНазначения.ЕстьРеквизитОбъекта("БанковскийСчетКонтрагента", МетаданныеДокумента);
		Если ИмяОбъекта = "ПередачаТоваровМеждуОрганизациями" Тогда
		 
			ТекстЗапросаОснований = ТекстЗапросаОснований + 
			"ВЫБРАТЬ
			|	Таблица.Организация КАК Организация,
			|	Таблица.БанковскийСчетОрганизации КАК БанковскийСчетОрганизации,
			...
			|	ВЫБОР КОГДА Таблица.РасчетыЧерезОтдельногоКонтрагента ТОГДА
			|		Таблица.БанковскийСчетКонтрагента
			|	ИНАЧЕ
			|		Таблица.БанковскийСчетОрганизацииПолучателя
		...
		ИначеЕсли ИмяОбъекта = "ВозвратТоваровМеждуОрганизациями" Тогда
			
			ТекстЗапросаОснований = ТекстЗапросаОснований + 
			"ВЫБРАТЬ
			|	Таблица.ОрганизацияПолучатель КАК Организация,
			|	Таблица.БанковскийСчетОрганизацииПолучателя КАК БанковскийСчетОрганизации,
			|	ВЫБОР КОГДА Таблица.РасчетыЧерезОтдельногоКонтрагента ТОГДА
			|		Таблица.Контрагент
			|	ИНАЧЕ
			|		Таблица.Организация
			...
			|		Таблица.БанковскийСчетКонтрагента
			|	ИНАЧЕ
			|		Таблица.БанковскийСчетОрганизации
			|	КОНЕЦ               КАК БанковскийСчетКонтрагента,
		...
		ИначеЕсли ИмяОбъекта = "ОтчетОРозничныхПродажах" Тогда
			
			ТекстЗапросаОснований = ТекстЗапросаОснований + 
			"ВЫБРАТЬ
			|	Таблица.КассаККМ.Владелец КАК Организация,
			|	НЕОПРЕДЕЛЕНО        КАК БанковскийСчетОрганизации,
			|	ЗНАЧЕНИЕ(Справочник.Контрагенты.РозничныйПокупатель) КАК Контрагент,
			|	""""                КАК КППКонтрагента,
			|	""""                КАК ИННКонтрагента,
			|	ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка) КАК БанковскийСчетКонтрагента,
			|	НЕОПРЕДЕЛЕНО КАК ИдентификаторГосКонтракта,
			|	Таблица.Валюта      КАК Валюта
			|ИЗ
			|	Документ." + ИмяОбъекта + " КАК Таблица
		...
		ИначеЕсли Не ОбщегоНазначения.ЕстьРеквизитОбъекта("Грузополучатель", МетаданныеДокумента) Тогда
			...
			"ВЫБРАТЬ
			|	Таблица.Организация    КАК Организация,
			|	" + ?(ИмяОбъекта = "ВозвратТоваровОтКлиента", "ЕСТЬNULL(Таблица.ДокументРеализации.БанковскийСчетОрганизации, ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаКонтрагентов.ПустаяСсылка))", 
					?(ЕстьБанковскийСчетОрганизации, "Таблица.БанковскийСчетОрганизации", "ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаКонтрагентов.ПустаяСсылка)")) + " КАК БанковскийСчетОрганизации,
			|	Таблица.Контрагент     КАК Контрагент,
			|	Таблица.Контрагент.КПП КАК КППКонтрагента,
			|	Таблица.Контрагент.ИНН КАК ИННКонтрагента,
			|	Таблица.Подразделение  КАК Подразделение,
			|	" + ?(ЕстьБанковскийСчетКонтрагента, "Таблица.БанковскийСчетКонтрагента", "ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка)") + " КАК БанковскийСчетКонтрагента,
			|	%ИдентификаторГосКонтракта% КАК ИдентификаторГосКонтракта,
			|	Таблица.Валюта        КАК Валюта
			|ИЗ
			|	Документ." + ИмяОбъекта + " КАК Таблица
		...
		Если ИмяОбъекта = "КорректировкаРеализации" 
			...
			|	ТекущийДокумент.Ссылка.ХозяйственнаяОперация КАК ХозяйственнаяОперация
			|ИЗ
			|	Документ.ВозвратТоваровОтКлиента.Товары КАК ТекущийДокумент
			|ГДЕ
			|	ТекущийДокумент.Ссылка В(&ДокументОснование_ВозвратТоваровОтКлиента)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			...
			|ИЗ
			|	Документ.ВозвратТоваровМеждуОрганизациями КАК ТекущийДокумент
			|ГДЕ
			|	ТекущийДокумент.Ссылка В(&ДокументОснование_ВозвратТоваровМеждуОрганизациями)
			|;
			|
			|/////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|		ПО ОснованияИсходных.ДокументОснование = ВозвратТоваровМеждуОрганизациями.ДокументПоступления
			...
			|	ИНАЧЕ
			|		ЛОЖЬ
			|	КОНЕЦ КАК ИсправлениеИсходногоДокумента,
			|	
			|	ВЫБОР КОГДА ОснованияИсходных.ХозяйственнаяОперация В(
			|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон),
			...
			|	ВЫБОР КОГДА ОснованияИсходных.ХозяйственнаяОперация В(
			|			ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ИсправлениеОшибок),
			|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияПерепоставленногоТовара)) ТОГДА
			|		ИСТИНА
			|	ИНАЧЕ
			|		ЛОЖЬ
			...
			|
			|УПОРЯДОЧИТЬ ПО
			|	ИсходныеДокументы.Ссылка.МоментВремени УБЫВ,
			...
			Иначе
				...
				Пока ВыборкаПоИсходнымСчетамФактуры.Следующий() Цикл
					...
					Пока ВыборкаПоДокументам.Следующий() Цикл
						...
						Если РеквизитыОснований.Найти(ВыборкаПоДокументам.ДокументОснование) = Неопределено 
							И (ВыборкаПоДокументам.ИсходныйСчетФактура = ВыборкаПоДокументам.ИсходныйДокумент ИЛИ ВыборкаПоДокументам.ИсправлениеИсходногоДокумента) Тогда
							СтрокаСРеквизитами = РеквизитыОснований.Добавить();
							ЗаполнитьЗначенияСвойств(СтрокаСРеквизитами, ВыборкаПоДокументам);
							Если ЗначениеЗаполнено(ВыборкаПоДокументам.ИсходныйДокумент) Тогда
								СтрокаСРеквизитами.ИсходныйДокумент = ВыборкаПоДокументам.ИсходныйДокумент;
								ДанныеИсходныйСчетФактура = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВыборкаПоДокументам.ИсходныйДокумент, "Номер, Дата");
							Иначе
								СтрокаСРеквизитами.ИсходныйДокумент = ВыборкаПоИсходнымСчетамФактуры.ИсходныйСчетФактура;
								ДанныеИсходныйСчетФактура = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВыборкаПоИсходнымСчетамФактуры.ИсходныйСчетФактура, "Номер, Дата");
							КонецЕсли;
							СтрокаСРеквизитами.НомерИсправленияИсходногоДокумента = СокрЛП(ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(СтрокаСРеквизитами.НомерИсправленияИсходногоДокумента, Истина, Истина));;
							СтрокаСРеквизитами.НомерИсходногоДокумента = ДанныеИсходныйСчетФактура.Номер;
							СтрокаСРеквизитами.ДатаИсходногоДокумента = ДанныеИсходныйСчетФактура.Дата;
						КонецЕсли;
				...
				// документ-основание для СФ дополнительного является основной, при корректировке заполнения СФ (см. выше Если) данные в разных строках могут быть идентичны
				РеквизитыОснований.Свернуть("ДокументОснование, ИсходныйДокумент, НомерИсходногоДокумента, ДатаИсходногоДокумента, НомерИсправленияИсходногоДокумента, ДатаИсправленияИсходногоДокумента",);
	...
	Если ЭтотОбъект.Корректировочный Тогда
		Для каждого СтрокаОснование Из ЭтотОбъект.ДокументыОснования Цикл
			...
			Если КоличествоКорректировокРеализаций > 1 Тогда //Если корректировок несколько, то СФ сводный.
				// в УПК не используется СводныйКорректировочный 
