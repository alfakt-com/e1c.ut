...
Функция ТекстЗапросаДанныхОснованийКОформлениюСчетФактур(Разрешенные=Ложь)
	...
	Для Каждого ИмяТипа Из МассивТипов Цикл
		...
		ИначеЕсли ИмяТипа = "ОтчетОРозничныхПродажах" Тогда
			ТекстЗапросаДляТипа = СтрЗаменить(ТекстЗапросаДляТипа, "ДанныеОснования.Партнер",  "ЗНАЧЕНИЕ(Справочник.Контрагенты.РозничныйПокупатель)");
			ТекстЗапросаДляТипа = СтрЗаменить(ТекстЗапросаДляТипа, "ДанныеОснования.Договор",  "ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)");
			ТекстЗапросаДляТипа = СтрЗаменить(ТекстЗапросаДляТипа, "ДанныеОснования.Менеджер", "ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)");
			ТекстЗапросаДляТипа = СтрЗаменить(ТекстЗапросаДляТипа, "ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка).* КАК Договор,",  "");
			ТекстЗапросаДляТипа = СтрЗаменить(ТекстЗапросаДляТипа, "	ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка).* КАК Менеджер,", "");
		КонецЕсли;
...
Функция ТипыДокументовОснованийКОформлению()
	...
	МассивТипов.Добавить("ОтчетОРозничныхПродажах");
