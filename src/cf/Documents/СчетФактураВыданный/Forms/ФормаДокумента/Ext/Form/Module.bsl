...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// ЭСФ
	Если Параметры.Свойство("АктуальныйЭСФ") Тогда
		АктуальныйЭСФ = Параметры.АктуальныйЭСФ;
	КонецЕсли;
	// КонецЭСФ
КонецПроцедуры
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	// Подсистема "ЭлектронныеСчетаФактуры"
	Если ИмяСобытия = "ОбновитьСостояниеЭД" 
		ИЛИ ИмяСобытия = ЭСФКлиентСервер.ИмяСобытияЗаписьЭСФ() Тогда
		УстановитьТекстСостоянияЭДНаСервере();
	КонецЕсли;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ПриЧтенииСозданииНаСервере(Истина);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не ЗначениеЗаполнено(ЭтотОбъект.СписокОснований) Тогда
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	КонецЕсли;
	// ЭСФ
	ПараметрОповещения.Вставить("АктуальныйЭСФ", АктуальныйЭСФ);
	// Конец ЭСФ
...
Процедура ДекорацияЭДОНажатие(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ЭСФКлиентУТ.СостояниеЭДНажатие(ЭтаФорма, Новый Структура, Элементы.ДекорацияСостояниеЭДО.Заголовок);
...
&НаСервере
Процедура ЗаполнитьЗависимыеОтДокументовОснованийРеквизитыФормы(ПриЧтенииНаСервере = Ложь)
	...
	Если ЗначениеЗаполнено(Объект.Контрагент) И ТипЗнч(Объект.Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		Если НЕ ПриЧтенииНаСервере Тогда
			...
			Объект.СтранаНазначения = СтранаРегистрации;
		КонецЕсли;
		
	ИначеЕсли ЗначениеЗаполнено(Объект.Контрагент) И ТипЗнч(Объект.Контрагент) = Тип("СправочникСсылка.Организации") Тогда
		
		Если НЕ ПриЧтенииНаСервере Тогда
			СтруктураПараметров = ДенежныеСредстваСервер.ПараметрыЗаполненияБанковскогоСчетаОрганизацииПоУмолчанию();
			СтруктураПараметров.Организация    = Объект.Контрагент;
			СтруктураПараметров.БанковскийСчет = Объект.БанковскийСчетКонтрагента;
			СтруктураПараметров.Валюта         = Объект.Валюта;
			Объект.БанковскийСчетКонтрагента = ЗначениеНастроекПовтИсп.ПолучитьБанковскийСчетОрганизацииПоУмолчанию(СтруктураПараметров);
		
			Объект.СтранаНазначения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Контрагент, "СтранаРегистрации");
		КонецЕсли;
...
Процедура ОрганизацияПриИзмененииНаСервере()
	...
	ДенежныеСредстваСервер.ПроверитьЗаполнитьБанковскийСчетОрганизацииПоВладельцу(Объект.Организация, Объект.БанковскийСчетОрганизации);
...
Процедура УправлениеЭлементамиФормы()
	...
	Элементы.НомерПриИсправлении.Видимость = ВидимостьЭлемента;
	Элементы.ДатаПриИсправлении.Видимость             = ВидимостьЭлемента;
	
	// если СФ корректировочный, не даем возможность редактировать дату оборота
	Элементы.ДатаСовершенияОборотаПоРеализации.ТолькоПросмотр = Объект.Корректировочный;
...
&НаСервере
Процедура ПриЧтенииСозданииНаСервере(ПриЧтенииНаСервере = Ложь)
	
	ИспользоватьПартнеровКакКонтрагентов = ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов");
	ИспользоватьНесколькоОрганизаций     = ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций");
	
	ИспользоватьОбменЭСФЧерезAPI     = ПолучитьФункциональнуюОпцию("ИспользоватьОбменЭСФЧерезAPI");
	ИспользоватьОбменЭСФЧерезXML     = ПолучитьФункциональнуюОпцию("ИспользоватьОбменЭСФЧерезXML");
	
	ЕстьПравоНаРедактирование = ПравоДоступа("Изменение", Метаданные.Документы.СчетФактураВыданный);
	
	ЗаполнитьЗависимыеОтДокументовОснованийРеквизитыФормы(ПриЧтенииНаСервере);
	...
	Элементы.БанковскийСчетОрганизации.Видимость  = ИспользоватьНесколькоОрганизаций;
	Элементы.БанковскийСчетОрганизации1.Видимость = Не ИспользоватьНесколькоОрганизаций ИЛИ НЕ ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоРасчетныхСчетов");
	
	УстановитьВидимостьКомандыСоздатьКорректировочный();
	
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Если ИспользоватьОбменЭСФЧерезAPI ИЛИ ИспользоватьОбменЭСФЧерезXML Тогда
			Объект.ВыставленВЭлектронномВиде = Истина;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьДоступностьИзмененийПоФлагуВыставлен()
	...
	МассивИменЭлементов.Добавить("ДатаСовершенияОборотаПоРеализации");
	МассивИменЭлементов.Добавить("Организация");
	МассивИменЭлементов.Добавить("Контрагент");
	МассивИменЭлементов.Добавить("БанковскийСчетОрганизации");
	МассивИменЭлементов.Добавить("БанковскийСчетОрганизации1");
	МассивИменЭлементов.Добавить("БанковскийСчетКонтрагента");
	...
	МассивИменЭлементов.Добавить("СтранаНазначения");
	МассивИменЭлементов.Добавить("ГосучреждениеКодТоваров");
	МассивИменЭлементов.Добавить("ГосучреждениеНазначениеПлатежа");
	...
	// если СФ корректировочный, не даем возможность редактировать дату оборота
	Элементы.ДатаСовершенияОборотаПоРеализации.ТолькоПросмотр = Объект.Корректировочный ИЛИ (Выставлен И НЕ ЭтоНовый);
...
Процедура НастроитьВидимостьВалютыКодаОперации()
	...
	ОтключенныйФункционал = ПолучитьФункциональнуюОпцию("ОтключенныйФункционал");
	КодыВидовОперацийНаУвеличениеУменьшение = Объект.Корректировочный И ВерсияКодовВидовОпераций >= 3;
	Элементы.ГруппаКодВидаОперации.Видимость = ОтключенныйФункционал И НЕ КодыВидовОперацийНаУвеличениеУменьшение;
	Элементы.ГруппаКодВидаОперацииНаУвеличение.Видимость = ОтключенныйФункционал И КодыВидовОперацийНаУвеличениеУменьшение;
	Элементы.ГруппаКодВидаОперацииНаУменьшение.Видимость = ОтключенныйФункционал И КодыВидовОперацийНаУвеличениеУменьшение;
...
Процедура УстановитьТекстСостоянияЭДНаСервере()
	...
	ТекстСостоянияЭД = ЭСФСерверУТ.ПолучитьТекстСостоянияЭД(Объект.Ссылка, ЭтаФорма);
	
	// Проверим проведен ли документ в учете, ЭСФ можно создавать только для валидных СФ
	Если НЕ Объект.Проведен И ТекстСостоянияЭД = НСтр("ru = ''") Тогда
		//ТекстСостоянияЭД = "";	
		ТекстСостоянияЭД = НСтр("ru = 'Не сформирован'");
		Элементы.ДекорацияСостояниеЭДО.Заголовок = ТекстСостоянияЭД;
	Иначе
		//Элементы.ДекорацияСостояниеЭДО.Заголовок = ЭСФСерверУТ.ПолучитьТекстСостоянияЭД(Объект.Ссылка, ЭтаФорма);
		Элементы.ДекорацияСостояниеЭДО.Заголовок = ТекстСостоянияЭД;
	КонецЕсли;
	
	ДоступнаНастройкаЭД = ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") 
		ИЛИ НЕ(ПолучитьФункциональнуюОпцию("ИспользоватьОбменЭСФЧерезAPI")
			ИЛИ ПолучитьФункциональнуюОпцию("ИспользоватьОбменЭСФЧерезXML"));
