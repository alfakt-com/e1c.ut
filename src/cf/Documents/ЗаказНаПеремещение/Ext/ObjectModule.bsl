...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//++ Кужанбаева Айнур 21.01.2022,закоментировала текст ТишбаеваОМ, и внесла условие Халыковой А.
	////++ ТишбаевОМ Временно по задаче Фальк Андре
	...
	//				 |Действуйте согласно приказа №40 от 03.07.2021.");
	//		Отказ = Истина;
	//		Прервать;
	//	КонецЕсли;		
	//КонецЦикла;
	////++
	//++Халыкова А. 12.01.2022
	
	//ТТ
	Если Найти(ЭтотОбъект.Комментарий,"#Изменен по перемещению#") > 0 Тогда
		Возврат;
	КонецЕсли;
	
	//++Аян_К++ 231221
	Если Альфа_КТ.ПроверитьДоступ("ЗаказНаПеремещение", ПараметрыСеанса.ТекущийПользователь, ТекущаяДата(), ТекущаяДата()) Тогда 
		
		Альфа_КТ.ПроверитьОснованиеПлатежа(Отказ, ЭтотОбъект);
		
	КонецЕсли;
	//Кужанбаева А. финиш  
	
	//ALFA-KT 22.02.2022 begin Oleg_D
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда 
		Если Не ЗначениеЗаполнено(ссылка.ДокументОснование) Тогда 
			Отказ =Истина;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Необходимо создать документ на основании вышестоящего документа",,,,Отказ);
		КонецЕсли;	
	КонецЕсли;
	//ALFA-KT 22.02.2022 end Oleg_D
КонецПроцедуры
...
Процедура ЗаполнитьПоЗаказуКлиента(ДанныеЗаполнения)
	...
	СпособДоставки          = Перечисления.СпособыДоставки.ДоКлиента; //++Аян_К++ 042022
	// Заполнение табличной части.
