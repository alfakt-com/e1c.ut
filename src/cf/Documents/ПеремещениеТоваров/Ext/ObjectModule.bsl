...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ Альфа_КТ.ПроверитьДоступ("ЗаписьПеремещенияСтенды", ПользователиКлиентСервер.ТекущийПользователь(), ТекущаяДата(), ТекущаяДата()) Тогда //++Аян_К++ 030622 
		ПроверитьСкладыОтправления(Отказ, РежимПроведения);
	КонецЕсли;
...
Процедура ПроверитьСкладыОтправления(Отказ, РежимЗаписи)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	РазрешенияСкладов.Ответственный КАК Ответственный
	               |ИЗ
	               |	РегистрСведений.СоответствиеПеремещенияМеждуСкладами КАК РазрешенияСкладов
	               |ГДЕ
	               |	РазрешенияСкладов.СкладОтправитель = &СкладОтправитель
	               |	И РазрешенияСкладов.СкладПолучатель = &СкладПолучатель";
	...
	Если Результат[0].Ответственный <> ПараметрыСеанса.ТекущийПользователь Тогда 
		Сообщить("Перемещения между " + СкладОтправитель + " и " + СкладПолучатель + " запрещено");
		Отказ = Истина;
	КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//Проверить заполнение реквизита "Дилеры" если используется склад "образцы"		
	...
	Если ЗначениеЗаполнено(СкладОбразцев) Тогда
		Если СкладОтправитель = СкладОбразцев ИЛИ  СкладПолучатель = СкладОбразцев Тогда
			Если Не ЗначениеЗаполнено(Дилеры) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не заполнено поле Дилеры!'"),
				ЭтотОбъект,
				"Дилеры",
				,
				Отказ);
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// МК Жакупов К 07.12.2020
	РегистрыНакопления.ОбразцыТоваровУДилера.ЗаписатьДанныеДокумента(Ссылка, ДополнительныеСвойства, Отказ);
	...
	//begin ALFA-KT 22.02.2022 Oleg_D
	АльфаКТКонтрольОплатыСтенда(Ссылка, Отказ);
	//end ALFA-KT 22.02.2022 Oleg_D

КонецПроцедуры
//begin ALFA-KT 22.02.2022 Oleg_D
//
Процедура АльфаКТКонтрольОплатыСтенда(Ссылка,Отказ)
	
	Если  ЗначениеЗаполнено(ЗаказНаПеремещение ) Тогда
		Возврат ;
	КонецЕсли;
		Если Ссылка.СкладПолучатель.ВидСклада = Перечисления.ВидыСкладов.Стенд Тогда
		...
		|	СУММА(ПеремещениеТоваровТовары.КоличествоУпаковок) КАК КоличествоУпаковок1
		|ПОМЕСТИТЬ ВТ
		|ИЗ
		...
		|	СУММА(ВТ.КоличествоУпаковок1) КАК КоличествоУпаковок1,
		|	Максимум(ЕстьNull(Номенклатура.ПлощадьЕдиницы,0)) КАК ПлощадьЕдиницы
		|ИЗ
		|	ВТ КАК ВТ
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК Номенклатура
		|		ПО ВТ.Номенклатура1 = Номенклатура.Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ВТ.Номенклатура1";
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Если ВыборкаДетальныеЗаписи.ПлощадьЕдиницы < ВыборкаДетальныеЗаписи.КоличествоУпаковок  Тогда
				Текст = "Превышен лимит на отгрузку стендового образца " 
				+ Строка(ВыборкаДетальныеЗаписи.Номенклатура1) +" на "
				+(ВыборкаДетальныеЗаписи.КоличествоУпаковок-ВыборкаДетальныеЗаписи.ПлощадьЕдиницы);
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	//+ТТ
	Если ОбъектКопирования.ИспользоватьТранзитныйСклад Тогда
		СкладОтправитель = ОбъектКопирования.СкладПолучатель;
		СкладПолучатель = ОбъектКопирования.СкладКонечный;
		ИспользоватьТранзитныйСклад = Ложь;
	Иначе
...
Процедура ПриЗаписи(Отказ)
	...
	//+ТТ Запишем привязки к заказам на перемещения
	ТЗЗаказы = Товары.Выгрузить();
	ТЗЗаказы.Свернуть("ЗаказНаПеремещение");
	СвойствоПерем = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя","НомерПеремещения");
	Для каждого текЗаказ из ТЗЗаказы Цикл
		Если ЗначениеЗаполнено(текЗаказ.ЗаказНаПеремещение) Тогда
			Если НЕ СвойствоПерем.Пустая() Тогда
				МенеджерЗаписи = РегистрыСведений.ДополнительныеСведения.СоздатьМенеджерЗаписи();
				МенеджерЗаписи.Объект = текЗаказ.ЗаказНаПеремещение;
				МенеджерЗаписи.Свойство = СвойствоПерем;
				МенеджерЗаписи.Значение = Номер;
	...
	Если ЗначениеЗаполнено(ЗаказНаПеремещение) Тогда
		МенеджерЗаписи = РегистрыСведений.ДополнительныеСведения.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.Объект = ЗаказНаПеремещение;
		МенеджерЗаписи.Свойство = СвойствоПерем;
		МенеджерЗаписи.Значение = Номер;
		МенеджерЗаписи.Записать(Истина);
	КонецЕсли;
