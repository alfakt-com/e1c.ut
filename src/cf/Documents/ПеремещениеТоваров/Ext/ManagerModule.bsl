...
Функция ДанныеДляПроверкиВводаНаОсновании(Распоряжения, ХозяйственнаяОперация = Неопределено) Экспорт
	...
	ДанныеДляВвода.МожноСгруппировать = ЗначениеЗаполнено(ДанныеДляВвода.РаспоряженияКОформлению);
...
Функция РезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, КонтекстВызова) Экспорт
	...
	//Ахметов Ержан 24112022  +++
	ПараметрыЗапросов = Новый Структура("ОтобратьПоЗаказу, МассивЗаказов,
	                                    |Организация, СкладОтправитель, СкладПолучатель, Подразделение,
...
Процедура ОбновитьЗависимыеРеквизитыТабличнойЧасти(Товары, ПараметрыЗаполнения) Экспорт
	...
	//Товары.Свернуть("Номенклатура,  Упаковка, ЗаказНаПеремещение, Сделка, Назначение, НазначениеОтправителя, Серия, АналитикаУчетаНоменклатуры", "КоличествоУпаковок, Количество"); //++Аян_К++ 311121
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, ДополнительныеСвойства, Регистры = Неопределено) Экспорт
	...
	ТекстЗапросаТаблицаДвиженияПоСкладуОбразцов(Запрос, ТекстыЗапроса, Регистры);
...
Функция АдаптированныйТекстЗапросаДвиженийПоРегистру(ИмяРегистра) Экспорт
	...
	ИначеЕсли ИмяРегистра = "ОбразцыТоваровУДилера" Тогда
		
		ТекстЗапроса = ТекстЗапросаТаблицаДвиженияПоСкладуОбразцов(Запрос, ТекстыЗапроса, ИмяРегистра);
		СинонимТаблицыДокумента = "ТаблицаОбразцыТоваровУДилера";
...
Функция ТекстЗапросаТаблицаДвиженияПоСкладуОбразцов(Запрос, ТекстыЗапроса, Регистры)
	
	ИмяРегистра = "ОбразцыТоваровУДилера";
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               |	ПеремещениеТоваровТовары.Ссылка.Подразделение КАК Подразделение,
	               |	ПеремещениеТоваровТовары.Ссылка.Дилеры КАК Дилеры,
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'Накладная на внутреннее перемещение (З-5)'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ТОРГ13") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			"ТОРГ13",
			НСтр("ru = 'Накладная на внутреннее перемещение (З-5)'"),
...
Функция КомплектПечатныхФорм() Экспорт
	...
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ТОРГ13", НСтр("ru = 'Накладная на внутреннее перемещение (З-5)'"),                2);
...
//++27.09.2022 Саканаев Д. Обновил процедуру, тепер будут отображатся артикул и код товара 
...
Функция СформироватьПечатнуюФормуНакладнойНаПеремещениеТоваров(МассивОбъектов, ОбъектыПечати)
	...
	//ОбластьКодовШапка.Параметры.ИмяКолонкиКодов = "Код товара";
	
	ОбластьКодовСтрока = Макет.ПолучитьОбласть("Строка|КолонкаКодов");
	ОбластьКодовПодвал = Макет.ПолучитьОбласть("Подвал|КолонкаКодов");
	
	ОбластьАртикулШапка  = Макет.ПолучитьОбласть("ШапкаТаблицы|КолонкаАртикула");
	///ОбластьКодовШапка.Параметры.ИмяКолонкиКодов = "Артикул";
	ОбластьАртикулСтрока = Макет.ПолучитьОбласть("Строка|КолонкаАртикула");
	ОбластьАртикулПодвал = Макет.ПолучитьОбласть("Подвал|КолонкаАртикула");
	...
	Для Каждого ИспользованиеСерий Из ДеревоОбъектов.Строки Цикл
		...
		Для Каждого Шапка Из ИспользованиеСерий.Строки Цикл
			...
			ТабличныйДокумент.Присоединить(ОбластьАртикулШапка);
			...
			Пока ВыборкаПоТоварам.НайтиСледующий(Новый Структура("Ссылка",Шапка.Ссылка)) Цикл
				...
				ОбластьКодовСтрока.Параметры.КодТовара = ВыборкаПоТоварам.Код;
				ОбластьАртикулСтрока.Параметры.артикул = ВыборкаПоТоварам.Артикул;
				
				ТабличныйДокумент.Присоединить(ОбластьКодовСтрока);
				ТабличныйДокумент.Присоединить(ОбластьАртикулСтрока);
			...
			//++Аян_К++ 200921 старт
			ОбластьПодписей.Параметры.ФИО 	= ПараметрыСеанса.ТекущийПользователь;
			ОбластьПодписей.Параметры.Дата 	= ТекущаяДата();
			//++Аян_К++ 200921 финиш
...
//  Табличный документ - печатная форма накладной.
//
Функция СформироватьПечатнуюФормуНакладнойНаПеремещениеТоваровOLD_27092022(МассивОбъектов, ОбъектыПечати)
	
	УстановитьПривилегированныйРежим(Истина);
	
	Колонка = ФормированиеПечатныхФорм.ИмяДополнительнойКолонкиПоИнвентаризациям();
	ВыводитьКоды = ЗначениеЗаполнено(Колонка);
...
//++27.09.2022 Саканаев Д. Обновил процедуру, тепер будут отображатся артикул и код товара 
// Формирует табличный документ по унифицированной форме З-5
// 
// Возвращаемое значение
//  Табличный документ - унифицированная форма З-5
//
Функция СформироватьПечатнуюФормуТОРГ13(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПеремещениеТоваров_З5_";
	...
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(Перемещение.СкладОтправитель)           КАК Отправитель,
	|	ПРЕДСТАВЛЕНИЕССЫЛКИ(Перемещение.СкладПолучатель)            КАК Получатель,
	...
	|	Перемещение.Ответственный                                   КАК Ответственный,
	|	Перемещение.Ответственный.ФизическоеЛицо                    КАК ФизическоеЛицо,
	|	ТаблицаОтветственныеЛица.РуководительНаименование           КАК ФИОРазрешил,
	|	ТаблицаОтветственныеЛица.РуководительДолжность              КАК ДолжностьРазрешил
	|ИЗ
	|	Документ.ПеремещениеТоваров КАК Перемещение
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаОтветственныеЛица КАК ТаблицаОтветственныеЛица
	|		ПО Перемещение.Ссылка = ТаблицаОтветственныеЛица.Ссылка
	|ГДЕ
	...
	|	ПеремещениеТоваров.Номенклатура.Артикул КАК Артикул,
	|	ПеремещениеТоваров.Номенклатура.Код КАК ТоварКод,
	|	ПеремещениеТоваров.Номенклатура.ВесЧислитель КАК ВесЧислитель,
	|	ПеремещениеТоваров.Номенклатура.ВесЕдиницаИзмерения КАК ВесЕдиницаИзмерения,
	...
	|	(ВЫРАЗИТЬ(ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена, 0) / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки2, 1) * ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) * ЕСТЬNULL(КурсыВалют.Курс, 1) / ЕСТЬNULL(КурсыВалют.Кратность, 1) КАК ЧИСЛО(15, 2))) * ПеремещениеТоваров.КоличествоУпаковок КАК Сумма,
	|	ВЫРАЗИТЬ(ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена, 0) / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки2, 1) * ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки1, 1) * ЕСТЬNULL(КурсыВалют.Курс, 1) / ЕСТЬNULL(КурсыВалют.Кратность, 1) КАК ЧИСЛО(15, 2)) КАК Цена,
	|	ВЫБОР
	|		КОГДА ПеремещениеТоваров.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	...
	|				Валюта В
	|					(ВЫБРАТЬ
	|						Документы.ВидЦены.ВалютаЦены
	|					ИЗ
	|						Документ.ПеремещениеТоваров КАК Документы
	|					ГДЕ
	|						Документы.Ссылка = &ПеремещениеТоваров)) КАК КурсыВалют
	|		ПО ПеремещениеТоваров.Ссылка.ВидЦены.ВалютаЦены = КурсыВалют.Валюта
	|ГДЕ
	...
	Пока Шапка.Следующий() Цикл
		...
		ОбластьМакетаШапка.Параметры.ОрганизацияБИН           = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации,"ИНН", Ложь);
		...
		Если ЗначениеЗаполнено(Шапка.Ответственный) Тогда
			ОбластьМакетаШапка.Параметры.МОЛОтправитель = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(Шапка.ФизическоеЛицо);
		КонецЕсли;
		...
		ОбластьЗаголовокТаблицы.Параметры.Валюта = ВалютаРеглУчета;
		...
		ОбластьМакетаВсего      = Макет.ПолучитьОбласть("ИтогоТаблицы");
		...
		Пока ВыборкаСтрокТовары.Следующий() Цикл
			...
			ОбластьМакетаСтрока.Параметры.НомерПП           = Ном;
			...
			//++04.10.2022 Саканаев Д.
			Попытка
				ВесСтроки = Окр(ВыборкаСтрокТовары.ВесЧислитель * ВыборкаСтрокТовары.Количество,3);
				ОбластьМакетаСтрока.Параметры.Вес = Строка(ВесСтроки) + " " + Строка(ВыборкаСтрокТовары.ВесЕдиницаИзмерения);
			Исключение
			КонецПопытки;
			//--04.10.2022 Саканаев Д.
		...
		ОбластьМакетаВсего.Параметры.ИтогоВес            = ИтогоВес;
		ТабличныйДокумент.Вывести(ОбластьМакетаВсего);

		// Вывод подвала документа.
		ОбластьМакетаПодвал.Параметры.Заполнить(Шапка);
		ОбластьМакетаПодвал.Параметры.ДолжностьОтпустил  = Шапка.ДолжностьКладовщикаОтправителя;
		ОбластьМакетаПодвал.Параметры.ФИООтпустил        = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(Шапка.КладовщикОтправитель);
		ОбластьМакетаПодвал.Параметры.ДолжностьПолучил   = Шапка.ДолжностьКладовщикаПолучателя;
		ОбластьМакетаПодвал.Параметры.ФИОПолучил         = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(Шапка.КладовщикПолучатель);
		ОбластьМакетаПодвал.Параметры.КоличествоПрописью = КоличествоПрописью(ИтогоКоличество);
		ОбластьМакетаПодвал.Параметры.Валюта             = ВалютаРеглУчета;
		ОбластьМакетаПодвал.Параметры.СуммаПрописью      = РаботаСКурсамиВалют.СформироватьСуммуПрописью(ИтогоСумма, ВалютаРеглУчета);
		
		//++Аян_К++ 200921 старт
		ОбластьМакетаПодвал.Параметры.ФИО 	= ПараметрыСеанса.ТекущийПользователь;
		ОбластьМакетаПодвал.Параметры.Дата 	= ТекущаяДата();
		//++Аян_К++ 200921 финиш
...
//  Табличный документ - унифицированная форма З-5
//
Функция СформироватьПечатнуюФормуТОРГ13_Old_27092022(МассивОбъектов, ОбъектыПечати)
	
	УстановитьПривилегированныйРежим(Истина);
	
	ТоварКод = ФормированиеПечатныхФорм.ИмяДополнительнойКолонкиПоИнвентаризациям();
	Если Не ЗначениеЗаполнено(ТоварКод) Тогда
		ТоварКод = "Код";
	КонецЕсли;
	...
	|					Документ.ПеремещениеТоваров КАК Документы
	|				ГДЕ
	|					Документы.Ссылка = &ПеремещениеТоваров)
	|		) КАК КурсыВалют
	|		ПО ПеремещениеТоваров.Ссылка.ВидЦены.ВалютаЦены = КурсыВалют.Валюта
	|ГДЕ
	|	ПеремещениеТоваров.Ссылка = &ПеремещениеТоваров
...
Процедура ПараметрыЗапросаДляЗаполненияПоЗаказам(ПараметрыЗапросов, ИменаЗапросов, ИменаПодзапросов, ДанныеОтбора)
	...
	//Ахметов Ержан 2411202 ++ был сделано отбор только по подразделениям по задаче 5984
	//ПараметрыЗапросов.Вставить("СкладОтправитель", ДанныеОтбора.РеквизитыШапки.СкладОтправитель);
	//ПараметрыЗапросов.Вставить("СкладПолучатель",  ДанныеОтбора.РеквизитыШапки.СкладПолучатель);
	ПараметрыЗапросов.Вставить("Подразделение",    ДанныеОтбора.РеквизитыШапки.Подразделение);//АЕ
	//Ахметов Ержан 2411202 --
...
Функция ТекстЗапросаВременнойТаблицыРаспоряжений()
	...
	ТекстЗапроса =
		...
		//Ахметов Ержан 24112022 задача 5984 ++
		//|			И ЗаказНаПеремещение.СкладОтправитель = &СкладОтправитель    
		//|			И ЗаказНаПеремещение.СкладПолучатель  = &СкладПолучатель    
...
Функция КоличествоПрописью(Количество)
	...
	Возврат ЧислоПрописью(Количество,"Л=ru_RU" ,ПараметрыПрописи);

КонецФункции

#КонецОбласти

//ТТ 6076 При изменении в перемещении, редактировать заказы на перемещения
Процедура ОбновитьЗаказНаПеремещение(Перемещение) Экспорт
	УстановитьПривилегированныйРежим(Истина);
	
	//Получаем список заказов
	ТЗЗаказы = Перемещение.Товары.Выгрузить();
	ТЗЗаказы.Свернуть("ЗаказНаПеремещение");
	Если ЗначениеЗаполнено(Перемещение.ЗаказНаПеремещение) Тогда
		НовЗаказ = ТЗЗаказы.Добавить();
		НовЗаказ.ЗаказНаПеремещение = Перемещение.ЗаказНаПеремещение;
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ДополнительныеСведения.Объект КАК ЗаказНаПеремещение
	               |ИЗ
	               |	РегистрСведений.ДополнительныеСведения КАК ДополнительныеСведения
	               |ГДЕ
	               |	ДополнительныеСведения.Значение = &Значение
	               |	И ДополнительныеСведения.Свойство.Имя = ""НомерПеремещения""
	               |	И ДополнительныеСведения.Объект.Дата >= &ОтДаты";
	Запрос.УстановитьПараметр("Значение",	Перемещение.Номер);
	Запрос.УстановитьПараметр("ОтДаты",		ДобавитьМесяц(Перемещение.Дата,-3));
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		НовЗаказ = ТЗЗаказы.Добавить();
		НовЗаказ.ЗаказНаПеремещение = Выборка.ЗаказНаПеремещение;
	КонецЦикла;
	ТЗЗаказы.Свернуть("ЗаказНаПеремещение");
	
	Для каждого ТекЗаказ из ТЗЗаказы Цикл
		Если НЕ ЗначениеЗаполнено(ТекЗаказ.ЗаказНаПеремещение) Тогда //если заказа нет, тогда ничего менять не нужно
			Продолжить;
		КонецЕсли;
		ОбЗаказ = ТекЗаказ.ЗаказНаПеремещение.ПолучитьОбъект();
		ОбИзменен = Ложь;  
		МасУдалить = Новый Массив;
		НашелСтроки = Перемещение.Товары.НайтиСтроки(Новый Структура("ЗаказНаПеремещение",ТекЗаказ.ЗаказНаПеремещение));
		Если НашелСтроки.Количество() > 0 Тогда		
			//Находим строки в заказе и сравниваем. Если не находим, значит надо удалить
			Для каждого нСтрока из НашелСтроки Цикл
				НашелСтрокиЗаказа = ОбЗаказ.Товары.НайтиСтроки(Новый Структура("Номенклатура,ВариантОбеспечения",нСтрока.Номенклатура,Перечисления.ВариантыОбеспечения.Отгрузить));
				Если НашелСтрокиЗаказа.Количество() > 0 Тогда
					ПерваяСтрока = Истина;
					Для каждого нСтрокаЗаказа из НашелСтрокиЗаказа Цикл
						Если ПерваяСтрока Тогда
							Если нСтрокаЗаказа.Количество <> нСтрока.Количество Тогда
								нСтрокаЗаказа.Количество 			= нСтрока.Количество;
								нСтрокаЗаказа.КоличествоУпаковок 	= нСтрока.КоличествоУпаковок;
								нСтрокаЗаказа.Упаковка 				= нСтрока.Упаковка;
								ОбИзменен = Истина;
							КонецЕсли;
						Иначе
							МасУдалить.Добавить(нСтрокаЗаказа);
						КонецЕсли;
						ПерваяСтрока = Ложь;	
					КонецЦикла;
				КонецЕсли;
		...
		//Так же надо удалить то что есть в заказе, но нет в перемещении
		Для каждого ТекСтрока из ОбЗаказ.Товары Цикл
			НашелСтрокиПерем = Перемещение.Товары.НайтиСтроки(Новый Структура("Номенклатура,ЗаказНаПеремещение",ТекСтрока.Номенклатура,ТекЗаказ.ЗаказНаПеремещение));
			Если НашелСтрокиПерем.Количество() = 0 Тогда
				МасУдалить.Добавить(ТекСтрока);
			КонецЕсли;
		КонецЦикла;
		
		
		//Удаление строк
		Для каждого ТекМас из МасУдалить Цикл
			ТекМас.Отменено = Истина;
			ОбИзменен = Истина;
		КонецЦикла;
		
		//Запись. Только если есть что менять
		Если ОбИзменен Тогда
			ОбЗаказ.Комментарий = ОбЗаказ.Комментарий + " #Изменен по перемещению#";
			ОтмененПолностью = (ОбЗаказ.Товары.НайтиСтроки(Новый Структура("Отменено",Ложь)).Количество() = 0);
			Попытка
				Если ОбЗаказ.Ссылка.Проведен и НЕ ОтмененПолностью Тогда
					ОбЗаказ.Записать(РежимЗаписиДокумента.Проведение);
				ИначеЕсли ОбЗаказ.Ссылка.Проведен и ОтмененПолностью Тогда
				...
				КонецЕсли;
			Исключение
				Сообщить("Не удалось обновить заказ на перемещения. Откорректируйте вручную.");
			КонецПопытки;
		КонецЕсли;
	КонецЦикла;
