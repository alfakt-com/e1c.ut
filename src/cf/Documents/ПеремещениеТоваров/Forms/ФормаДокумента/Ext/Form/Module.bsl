...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//МК Жакупов К. Доступ к дилерам 07.12.2020
	Элементы.Дилер.Доступность = РольДоступность("Доступ к складу образцов (Роль)");
	...
	//	Если РольДоступна("мкРазрешитьРедактированиеВыгруженныхВWMSДокументов") тогда
	//		ЭтаФорма.ТолькоПросмотр = Ложь;
	//	Иначе
	//		Если ВыгруженВВМС(Объект.Ссылка) тогда
	//			ЭтаФорма.ТолькоПросмотр=Истина;
	//			Элементы.ФормаГруппаКомандыФормы.Доступность=ложь;
	//		КонецЕсли;
	//		
	//	КонецЕсли;
	//	
	...
	//ТТ
	Элементы.ВидЦены.СписокВыбора.ЗагрузитьЗначения(Справочники.ВидыЦен.ДоступныеВидыЦен(Новый Структура("ИспользоватьПриПродаже",Истина)));

	Объект.СпособДоставки          = Перечисления.СпособыДоставки.ДоКлиента; //++Аян_К++ 042022  №6072
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Обработка.ЗагрузкаДанныхИзВнешнихФайлов.Форма.Форма" Тогда  //*Айнур 11.05.2022
		
		ПолучитьЗагруженныеТоварыИзХранилища(ВыбранноеЗначение.АдресТоваровВХранилище);   //*Айнур 11.05.2022
...
Процедура ПриОткрытии(Отказ)
	...
	// sw begin
	// Швецов Тимур, 03.05.2019
	//swУстановитьВидимостьЭлементов();
	// sw end
	
	//+ТТ
	ИспользоватьТранзитныйСкладПриИзменении(Истина);
	//-ТТ
	
	//++Аян_К++ 260722 старт
	Если Объект.Проведен Тогда
		СтатусДоступа 							= ПолучитьДоступЗаписьСклад();
		Элементы.СкладОтправитель.Доступность 	= СтатусДоступа;
		Элементы.СкладПолучатель.Доступность  	= СтатусДоступа;
	КонецЕсли;
	//++Аян_К++ 260722 финиш
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//ТТ Если документ изменен, тогда надо изменить и заказы на перемещения
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		Документы.ПеремещениеТоваров.ОбновитьЗаказНаПеремещение(Объект.Ссылка);
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//Доступ к складу образцов
	Если НЕ Отказ Тогда 
		Отказ = ДоступностьЗаписиСклада("Доступ к складу образцов (Роль)",Отказ);
		Если Отказ Тогда
			Сообщить("Движение по складу ""образцов"" запрещено, не хватает права доступа!");
			Возврат;
		КонецЕсли;	
	КонецЕсли;
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение 
		...
		Возврат;
		Элементы.СкладОтправитель.Доступность 	= Ложь;//++Аян_К++ 260722 старт
		Элементы.СкладПолучатель.Доступность 	= Ложь;//++Аян_К++ 260722 финиш
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ИмяДобавляемогоРеквизита = "ВидыЗапасовБылиИзменены";
	РеквизитыФормы = ЭтаФорма.ПолучитьРеквизиты();
	Для Каждого РеквизитФормы Из РеквизитыФормы Цикл
		Если РеквизитФормы.Имя = ИмяДобавляемогоРеквизита
			И ЭтаФорма[ИмяДобавляемогоРеквизита] Тогда
			ТекущийОбъект.ДополнительныеСвойства.Вставить("ПерезаполнитьВидыЗапасов", Истина);
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Функция ДоступностьЗаписиСклада(РольПоль,Отказ)
	
	СкладОбразцев = Справочники.Склады.НайтиПоРеквизиту("ВидСклада",Перечисления.ВидыСкладов.Образцы);
	Если ЗначениеЗаполнено(СкладОбразцев) Тогда
		Если СкладОтправитель = СкладОбразцев ИЛИ  СкладПолучатель = СкладОбразцев Тогда
			Отказ = НЕ РольДоступность(РольПоль);
		КонецЕсли;
	КонецЕсли;
...
Функция ПроверитьДатуЗапрета()
	Отказ = Ложь;
	ДопСвойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя","ЖелаемаяДатаОтгрузки_Доп");
	...
	Если МассивСтрок.Количество() > 0 Тогда
		ЖелаемаяДатаОтгрузки = ЭтотОбъект[МассивСтрок[0].ИмяРеквизитаЗначение];
		Если НЕ ЦС_ОбщийМодуль.ПроверитьДатуЗапрета(Объект.СкладОтправитель,ЖелаемаяДатаОтгрузки) Тогда
			Сообщить("Выбранная желаемая дата запрещена для отгрузки. Выберите другую дату!");
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
	Возврат Отказ;
...
Процедура ПолучитьВидыЗапасовИзХранилища(АдресВидовЗапасовВХранилище)
	...
	ЭтаФорма[ИмяДобавляемогоРеквизита] = Объект.ВидыЗапасовУказаныВручную;
...
Процедура ПеремещениеПоЗаказуПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Элементы.СтраницаДоставка.Видимость = Истина;//++Аян_К++ 131022 №6072
	Объект.СпособДоставки 				= ПредопределенноеЗначение("Перечисление.СпособыДоставки.ДоКлиента"); //++Аян_К++ 131022 №6072
...
&НаКлиенте
Процедура ИспользоватьТранзитныйСкладПриИзменении(Элемент)
	Элементы.СкладКонечный.Видимость = Объект.ИспользоватьТранзитныйСклад;
КонецПроцедуры

&НаСервере
Процедура ПолучитьЗагруженныеТоварыИзХранилища(АдресТоваровВХранилище) //*Айнур 11.05.2022
...
КонецПроцедуры //*Айнур 11.05.2022

&НаКлиенте
Процедура ЗагрузитьИзВнешнегоФайла(Команда) //*Айнур 11.05.2022
	ПараметрыФормы = Новый Структура();
...
//	СтандартнаяОбработка = Ложь;
//	
//	ДанныеВыбора = ПолучитьСкладыОтправительНаСервере();
//	
//КонецПроцедуры

//&НаСервере
//Функция ПолучитьСкладыОтправительНаСервере()
//	Возврат Справочники.Склады.ПолучитьДанныеВыбора(Новый Структура("СкладОтправитель", Истина));
//КонецФункции
#КонецОбласти
...
//// Швецов Тимур, 03.05.2019
//&НаКлиенте
//Процедура swПеремещениеЧерезЭкспедитораПриИзменении(Элемент)
//	swУстановитьВидимостьЭлементов();
...
//// Швецов Тимур, 03.05.2019
//&НаСервере
//Процедура swУстановитьВидимостьЭлементов()
//	//Если Объект.swПеремещениеЧерезЭкспедитора Тогда
//	//	Элементы.swВодительЭкспедитор.Видимость = Истина;
//	//Иначе
//	//	Элементы.swВодительЭкспедитор.Видимость = Ложь;
//	//КонецЕсли;
//	//Если Объект.СкладПолучатель.swОсобыйУчетТоваровПоКонтрагентам Тогда
//	//	Элементы.swКонтрагентДляОсобогоУчетаТоваров.Видимость = Истина;
//	//Иначе
//	//	Элементы.swКонтрагентДляОсобогоУчетаТоваров.Видимость = Ложь;
//	//КонецЕсли;
//КонецПроцедуры // sw end
