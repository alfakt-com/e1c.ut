...
&НаКлиенте
Процедура СтатьяРасходовНДСНеРазрешенногоКЗачетуПриИзменении(Элемент)
...
Процедура УстановитьУсловноеОформление()
	...
	// НДСНеРазрешенногоКЗачету
	
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных("АналитикаРасходовНДСНеРазрешенногоКЗачету");
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("АналитикаРасходовНеНДСОбязательна");
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных("СтатьяРасходовНДСНеРазрешенногоКЗачету");
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СуммаНДСНеРазрешенногоКЗачету");
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Если НЕ Объект.БазаРаспределенияУстанавливаетсяВручную Тогда
		ОбновитьДанныеПоВыручкеСервер();
	Иначе
		ЗаполнитьЗначенияСвойств(ЭтаФорма, Объект, "СуммаНДСРазрешенногоКЗачету, СуммаНДСНеРазрешенногоКЗачету, СуммаНДСНачисленного, СуммаНДСМетодомЗачета, СуммаНДСПереносимыйЗачет");	
	КонецЕсли;
...
Процедура ЗаполнитьБазуРаспределенияПоВыручкеСервер()
	...
	ТаблицыБазы = Документы.РаспределениеНДС.ПолучитьБазуРаспределения(Объект.Дата, Объект.НачалоПериода, Объект.Организация);
	...
	БазаРаспределения.Вставить("ВыручкаНДС0",  0);
	БазаРаспределения.Вставить("СуммаНДСНачисленного",  ТаблицыБазы.ТаблицаВыручка.Итог("СуммаНДСНачисленного"));
	Объект.НДСОбособленныхПодразделений.Загрузить(ТаблицыБазы.ТаблицаНДСОбособленныхПодразделений);
	
	ЗаполнитьЗначенияСвойств(ЭтаФорма, БазаРаспределения, "СуммаНДСНачисленного");
	ЗаполнитьЗначенияСвойств(Объект, БазаРаспределения);
	
	ТаблицыСуммыЗачета = Документы.РаспределениеНДС.ПолучитьСуммыЗачета(Объект.Дата, Объект.Организация, ТаблицыБазы.ТаблицаВыручка.Итог("ВыручкаНДС"), ТаблицыБазы.ТаблицаВыручка.Итог("ВыручкаНеНДС"));
	СуммыЗачета = Новый Структура;
	СуммыЗачета.Вставить("СуммаНДСРазрешенногоКЗачету",   ТаблицыСуммыЗачета.ТаблицаСуммЗачета.Итог("СуммаНДСРазрешенногоКЗачету"));
	СуммыЗачета.Вставить("СуммаНДСНеРазрешенногоКЗачету", ТаблицыСуммыЗачета.ТаблицаСуммЗачета.Итог("СуммаНДСНеРазрешенногоКЗачету"));
	СуммыЗачета.Вставить("СуммаНДСМетодомЗачета", ТаблицыСуммыЗачета.ТаблицаСуммЗачета.Итог("СуммаНДСМетодомЗачета"));
	СуммыЗачета.Вставить("СуммаНДСПереносимыйЗачет", ТаблицыСуммыЗачета.ТаблицаСуммЗачета.Итог("СуммаНДСПереносимыйЗачет"));
	
	// заполняем синхронно на форме и в объекте
	ЗаполнитьЗначенияСвойств(Объект, СуммыЗачета);
	ЗаполнитьЗначенияСвойств(ЭтаФорма, СуммыЗачета);
...
Процедура ОбновитьДанныеПоВыручкеСервер()
	...
	БазаРаспределения.Вставить("ВыручкаНеНДС", ТаблицыБазы.ТаблицаВыручка.Итог("ВыручкаНеНДС"));
	БазаРаспределения.Вставить("ВыручкаЕНВД",  ТаблицыБазы.ТаблицаВыручка.Итог("ВыручкаЕНВД"));
	БазаРаспределения.Вставить("ВыручкаНДС0", 0); //ТаблицыБазы.ТаблицаДокументыЭкспорт.Итог("Сумма"));
	БазаРаспределения.Вставить("СуммаНДСНачисленного",  ТаблицыБазы.ТаблицаВыручка.Итог("СуммаНДСНачисленного"));
	Объект.НДСОбособленныхПодразделений.Загрузить(ТаблицыБазы.ТаблицаНДСОбособленныхПодразделений);
