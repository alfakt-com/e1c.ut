...
&НаСервере
Процедура ЗаполнитьНеактивныеСертификатыНаСервере()
	
	//++Айнура 14.01.22 старт 
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	вт.Ссылка КАК ПодарочныйСертификат,
	...
	|	10000 КАК Номинал,
	|	10000 КАК Сумма,
	|	вт.Валюта КАК ВалютаНоминала
	|ИЗ
	|	вт КАК вт
	|ГДЕ
	|	вт.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПодарочныхСертификатов.НеАктивирован)";
	Результат = Запрос.Выполнить().Выгрузить();
	Объект.ПодарочныеСертификаты.Очистить();
	Объект.ПодарочныеСертификаты.Загрузить(Результат);
	//++Айнура 14.01.22 финиш
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьНеактивныеСертификаты(Команда)
	ЗаполнитьНеактивныеСертификатыНаСервере();
КонецПроцедуры
