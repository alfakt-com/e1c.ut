...
Процедура ОткрытьФормуПодбораРаспоряжений()
	...
	РаспоряженияЗадания = Новый Массив;
	Для Каждого Строка Из ЭтаФорма.Объект.Распоряжения Цикл
		Если Не ЗначениеЗаполнено(Строка.Распоряжение) Тогда
			Продолжить;
		КонецЕсли;
		РаспоряженияЗадания.Добавить(Строка.Распоряжение);
	КонецЦикла;
	
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("Склад",Склад);
	ПараметрыФормы.Вставить("Город",Объект.кт_Города); //++Аян_К++ Заявка №5996 финиш 010922
	...
	ПараметрыФормы.Вставить("РаспоряженияЗадания", РаспоряженияЗадания);
...
Процедура ДобавитьРаспоряжение(Команда)
	...
	Если ПроверитьБлокировкуЗадания() Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.кт_Города) Тогда
			ПоказатьПредупреждение(,НСтр("ru = 'Перед подбором распоряжений необходимо указать город.'"));
			Возврат;
		КонецЕсли;
