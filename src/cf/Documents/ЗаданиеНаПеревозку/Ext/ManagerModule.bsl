				...
				Пока ВыборкаПоКодамСтрок.Следующий() Цикл
					...
					Пока ВыборкаПоПолучателям.Следующий() Цикл
						...
						Пока Выборка.Следующий() Цикл
							...
							//Начало: МК Баубериков А - 09.10.2020
							Если ТипЗнч(Выборка.Распоряжение) = Тип("ДокументСсылка.ЗаказКлиента") тогда
								ЗаполняемаяОбласть.Параметры.Получатель = Строка(ЗаполняемаяОбласть.Параметры.Получатель) +"; "+ Строка(Выборка.Распоряжение.НомерТелефонаКлиентаДляДоставки)
								+", "+ Строка(Выборка.Распоряжение.НомерТелефонаКлиента);
							КонецЕсли;
							//Конец: МК Баубериков А - 09.10.2020
