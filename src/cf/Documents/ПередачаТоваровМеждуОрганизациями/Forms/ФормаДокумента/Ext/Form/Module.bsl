...
Процедура АктуализироватьПередачуПодДеятельность(Заполнить = Истина)
	...
	Если Заполнить Тогда
		ТекущийДоговор = ?(Объект.РасчетыЧерезОтдельногоКонтрагента, Объект.ДоговорПокупки, Объект.Договор);
		Объект.ПередачаПодДеятельность = ЗначениеНастроекПовтИсп.ЗакупкаПодДеятельность(
...
Процедура ЗаполнитьНалогообложениеНДС()
	...
	Если Объект.РасчетыЧерезОтдельногоКонтрагента Тогда
		Объект.НалогообложениеНДС = ЗначениеНастроекПовтИсп.НалогообложениеНДС(Объект.Организация, , Объект.ДоговорПродажи, Объект.НаправлениеДеятельности, Объект.Дата);
	Иначе
