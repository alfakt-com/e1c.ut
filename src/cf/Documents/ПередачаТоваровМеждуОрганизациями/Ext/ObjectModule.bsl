...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	ПараметрыЗаполнененияРезервовПоОтправителю = ПараметрыЗаполненияВидовЗапасов("Организация", Ложь);
	...
	ПараметрыЗаполнененияРезервовПоПолучателю = ПараметрыЗаполненияВидовЗапасов("ОрганизацияПолучатель", Ложь);
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	Если Не ЗначениеЗаполнено(НалогообложениеНДС) Тогда
		Если РасчетыЧерезОтдельногоКонтрагента Тогда
			НалогообложениеНДС = ЗначениеНастроекПовтИсп.НалогообложениеНДС(Организация, , ДоговорПродажи, НаправлениеДеятельности, Дата);
		Иначе
			НалогообложениеНДС = ЗначениеНастроекПовтИсп.НалогообложениеНДС(Организация, , Договор, НаправлениеДеятельности, Дата);
		КонецЕсли;
	КонецЕсли;
...
Функция ПараметрыЗаполненияВидовЗапасов(ИмяПоляОрганизация, ЗаполнениеВидовЗапасов)
	...
	Если ЗаполнениеВидовЗапасов Тогда
		...
		УчетНДСУТ.ПараметрыЗаполненияВидовЗапасовПоНалогообложению(ПараметрыЗаполнения.ОтборыВидовЗапасов, ЭтотОбъект[ИмяПоляОрганизация], Дата, НалогообложениеНДС);
	КонецЕсли;
...
Процедура ЗаполнитьПередачуПодДеятельность()
	...
	ТекущийДоговор = ?(РасчетыЧерезОтдельногоКонтрагента, ДоговорПокупки, Договор);
	ПередачаПодДеятельность = ЗначениеНастроекПовтИсп.ЗакупкаПодДеятельность(
								ОрганизацияПолучатель, 
								Склад, 
								ТекущийДоговор, 
