...
Функция ТекстЗапросаДляРасчетаНеобходимостиСчетаФактуры() Экспорт
	...
	|			И ТоварыСНДС.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|			И ТоварыСНДС.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК)
	|ГДЕ
...
Функция ТекстЗапросаВтТаблицаАналитикУчетаПартий(Запрос, ТекстыЗапроса)
	...
	|						ИЛИ ТаблицаДокумента.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК))
...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ12(ПараметрыПечати, МассивОбъектов) Экспорт 
	...
	|	ДанныеДокумента.Менеджер.ФизическоеЛицо КАК Ответственный,
...
Функция ТекстЗапросаДанныхОснованияДляПечатнойФормыСчетФактура() Экспорт
	...
	|,	НЕОПРЕДЕЛЕНО                                              КАК УсловияПоставки,
	|	НЕОПРЕДЕЛЕНО                                              КАК СпособОтправленияПоКлассификатору,
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если НЕ ПраваПользователяПовтИсп.ЭтоПартнер() Тогда
		...
		КомандаПечати.Представление = НСтр("ru = 'Приходный ордер запасов (З-1)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	КонецЕсли;
...
// Функция получает данные для формирования печатной формы  З-1
//
Функция ПолучитьДанныеДляПечатнойФормыМ4(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ПередачаТоваровМеждуОрганизациями.Дата                 КАК ДатаСоставления,
	|	ПередачаТоваровМеждуОрганизациями.НомерВходящегоДокумента КАК НомерСопроводительногоДокумента,
	...
	|	ПередачаТоваровМеждуОрганизациями.ОрганизацияПолучатель.Префикс КАК Префикс,
	|	ПередачаТоваровМеждуОрганизациями.БанковскийСчетОрганизации КАК БанковскийСчетОрганизации,
	|	ВЫБОР
	|		КОГДА
	|			ПередачаТоваровМеждуОрганизациями.Грузоотправитель = ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)
	|		ТОГДА
	...
	|	ПередачаТоваровМеждуОрганизациями.Подразделение.Наименование КАК ПредставлениеПодразделения,
	...
	|		ТаблицаДанныхДокументов КАК ДанныеДокументов
	|	ПО
	|		ПередачаТоваровМеждуОрганизациями.Ссылка = ДанныеДокументов.Ссылка
	|	
	|ГДЕ
	|	НЕ ПередачаТоваровМеждуОрганизациями.РасчетыЧерезОтдельногоКонтрагента
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|			ПередачаТоваровМеждуОрганизациями.Грузоотправитель
	|	КОНЕЦ,
	|	ПередачаТоваровМеждуОрганизациями.Подразделение,
	|	ПередачаТоваровМеждуОрганизациями.Подразделение.Наименование,
	...
	|		ПередачаТоваровМеждуОрганизациями.Ссылка = ДанныеДокументов.Ссылка
	|ГДЕ
	|	ПередачаТоваровМеждуОрганизациями.РасчетыЧерезОтдельногоКонтрагента
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ПередачаТоваровМеждуОрганизациями.Контрагент           КАК Поставщик,
	|	ПередачаТоваровМеждуОрганизациями.ОрганизацияПолучатель.Префикс КАК Префикс,
	|	ПередачаТоваровМеждуОрганизациями.БанковскийСчетОрганизации КАК БанковскийСчетОрганизации,
	|	ВЫБОР
	|		КОГДА
	...
	|	ТаблицаТоваров.Ссылка.Склад.Наименование               КАК СкладНаименование,
	|	ТаблицаТоваров.Номенклатура                            КАК Номенклатура,
...
Функция ПолучитьДанныеДляПечатнойФормыАктВыполненныхРаботР1(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|		ПО ДанныеДокумента.Ссылка = ТаблицаОтветственныеЛица.Ссылка
	|ГДЕ
	|	ДанныеДокумента.Ссылка В(&МассивДокументов) И Не ДанныеДокумента.РасчетыЧерезОтдельногоКонтрагента
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|ВЫБРАТЬ
	|	ДанныеДокумента.Ссылка КАК Ссылка,
	...
	|		ПО ДанныеДокумента.Ссылка = ТаблицаОтветственныеЛица.Ссылка
	|ГДЕ
	|	ДанныеДокумента.Ссылка В(&МассивДокументов) И ДанныеДокумента.РасчетыЧерезОтдельногоКонтрагента
	|	И ДанныеДокумента.Организация <> ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация)
	|	
	|ОБЪЕДИНИТЬ ВСЕ
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.Упаковка = ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)
	|			ТОГДА ДанныеДокумента.Номенклатура.ЕдиницаИзмерения
	|		ИНАЧЕ ДанныеДокумента.Упаковка.ЕдиницаИзмерения
	|	КОНЕЦ КАК ЕдиницаИзмерения,
	...
	|ИЗ
	|	ПередачаТоваровМеждуОрганизациямиТаблицаТоваров КАК ДанныеДокумента
	|ГДЕ
