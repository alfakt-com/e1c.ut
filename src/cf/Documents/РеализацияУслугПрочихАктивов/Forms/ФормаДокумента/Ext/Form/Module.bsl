...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьДоступностьИзмененийПоСпособуВыпискиАкта();
	УстановитьВидимостьЭлементовЭАВР();
...
Процедура ТекстСчетФактураОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
	...
	Если (Объект.СпособВыпискиАктовВыполненныхРабот = ПредопределенноеЗначение("Перечисление.СпособыВыпискиАктовВыполненныхРабот.НаПорталеИСЭСФ") 
		ИЛИ Объект.СпособВыпискиАктовВыполненныхРабот = ПредопределенноеЗначение("Перечисление.СпособыВыпискиАктовВыполненныхРабот.НаПорталеГосЗакупа"))
		И НавигационнаяСсылка = "ВвестиНовыйСчетФактуру" И Объект.ДатаПодписанияГЗ = Дата(1,1,1) Тогда
		
		СтандартнаяОбработка = Ложь;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Дата подписания документа не установлена, ввод счета-фактуры не возможен.'"), Объект.Ссылка, "Объект.ДатаПодписанияГЗ");		
	Иначе
...
Процедура ТекстОстатокДопустимогоКредитаНажатие(Элемент, СтандартнаяОбработка)
	...
	ИначеЕсли Объект.Доходы.НайтиСтроки(Новый Структура("СуммаВзаиморасчетов", 0)).Количество() = 0 Тогда
...
Процедура ОрганизацияПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		...
		Объект.НалогообложениеНДС = ЗначениеНастроекПовтИсп.НалогообложениеНДС(Объект.Организация, Неопределено, Объект.Договор, Объект.НаправлениеДеятельности, Объект.Дата);
		НалогообложениеНДСПриИзмененииСервер();
	КонецЕсли;
	ЗаполнитьДанныеФормы();
	НастроитьЭлементыФормы();
...
Процедура ДоговорПриИзмененииСервер()
	...
	Если ТипДоговора = Перечисления.ТипыДоговоров.СПокупателем
		И ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "СпособВыпискиАктовВыполненныхРабот"))
		И Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту Тогда
		Объект.СпособВыпискиАктовВыполненныхРабот = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "СпособВыпискиАктовВыполненныхРабот");
		УстановитьДоступностьИзмененийПоСпособуВыпискиАкта();
	КонецЕсли;
...
&НаСервере
Процедура УстановитьДоступностьИзмененийПоСпособуВыпискиАкта()
	
	Элементы.ДатаПодписания.Доступность = НЕ (Объект.СпособВыпискиАктовВыполненныхРабот = Перечисления.СпособыВыпискиАктовВыполненныхРабот.ВБумажномВиде);
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьЭлементовЭАВР()
	
	ВидимостьЭлементов = Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту;
	
	Элементы.ГруппаДатаПодписания.Видимость = ВидимостьЭлементов;
	Элементы.ГруппаСведенияПорталаГосЗакупок.Видимость = ВидимостьЭлементов;
...
Процедура УстановитьВидимостьОпераций(ИзменитьОперациюНаДоступную)
	...
	Если Элементы.Найти("ХозяйственнаяОперация") <> Неопределено Тогда
		...
		Если ВозможнаРеализацияБезПереходаПраваСобственности И ПолучитьФункциональнуюОпцию("ОтключенныйФункционал") Тогда
