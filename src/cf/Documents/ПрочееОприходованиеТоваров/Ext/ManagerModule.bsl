...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса)
	...
	|	ТаблицаТовары.СуммаУпр                    КАК СуммаБезНДСУпр,
	|	ВЫБОР
	|		КОГДА ТаблицаТовары.СтатьяРасходов.ПринятиеКналоговомуУчету
	|			ТОГДА ТаблицаТовары.СуммаРегл
	|		ИНАЧЕ 0
...
Функция ПолучитьДанныеДляПечатнойФормыМ4(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|ИЗ
	|	Документ.ПрочееОприходованиеТоваров КАК ДанныеДокументов
	|
	|ГДЕ
	...
	|	ПрочееОприходованиеТоваров.Номер                       КАК Номер,
	|	ПрочееОприходованиеТоваров.Дата                        КАК Дата,
	|	ПрочееОприходованиеТоваров.Дата                        КАК ДатаСоставления,
	|	ПрочееОприходованиеТоваров.Организация                 КАК Организация,
	|	ПрочееОприходованиеТоваров.Организация.Префикс         КАК Префикс,
	|	ПрочееОприходованиеТоваров.Подразделение               КАК Подразделение,
	|	ПрочееОприходованиеТоваров.Подразделение.Наименование  КАК ПредставлениеПодразделения,
	|	""""                                                   КАК МОЛ,
	...
	|		ПО ПрочееОприходованиеТоваров.Ссылка = ДанныеДокументов.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка
	|;
...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц) Экспорт
	...
	|	ПрочееОприходованиеТоваровТовары.Ссылка КАК Ссылка,
	|	ПрочееОприходованиеТоваровТовары.НомерСтроки КАК НомерСтроки,
	...
	|	0 КАК СтавкаНДС,
	|	0 КАК СуммаНДС,
	|	ПрочееОприходованиеТоваровТовары.Упаковка КАК Упаковка,
	|	&ТекстЗапросаНаименованиеЕдиницыИзмерения КАК ЕдиницаИзмерения,
	|	ПрочееОприходованиеТоваровТовары.Ссылка.Склад КАК Склад
	|ПОМЕСТИТЬ ТаблицаТоваров
	|ИЗ
	|	Документ.ПрочееОприходованиеТоваров.Товары КАК ПрочееОприходованиеТоваровТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаДанныхДокументов КАК ДанныеДокументов
	|		ПО ПрочееОприходованиеТоваровТовары.Ссылка = ДанныеДокументов.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрСведений.СуммыДокументовВВалютеРегл КАК СуммыДокументовВВалютеРегл
	|	ПО
	|		ПрочееОприходованиеТоваровТовары.Ссылка = СуммыДокументовВВалютеРегл.Регистратор
	|		И ПрочееОприходованиеТоваровТовары.ИдентификаторСтроки = СуммыДокументовВВалютеРегл.ИдентификаторСтроки
	|
	|ГДЕ
	|	СуммыДокументовВВалютеРегл.Активность
	|";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаНаименованиеЕдиницыИзмерения",
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаЗначениеРеквизитаЕдиницыИзмерения(
			"Ссылка", "ПрочееОприходованиеТоваровТовары.Упаковка", "ПрочееОприходованиеТоваровТовары.Номенклатура"));
