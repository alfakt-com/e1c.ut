...
Функция ТекстЗапросаДляРасчетаНеобходимостиСчетаФактуры() Экспорт
	...
	|						ТОГДА ИСТИНА
	|					КОГДА НЕ УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС И ДанныеДокумента.СтавкаНДСВознаграждения <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|						И ДанныеДокумента.СтавкаНДСВознаграждения <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК)
	|						ТОГДА ИСТИНА
	|					ИНАЧЕ ЛОЖЬ
	|				КОНЕЦ
	|		ИНАЧЕ ЛОЖЬ
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|			ИЛИ ДанныеДокумента.СтавкаНДСВознаграждения = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|		КОГДА ДанныеДокумента.СтавкаНДСВознаграждения = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС0)
...
Функция ТекстЗапросаТаблицаУслугиКОформлениюОтчетовПринципалу(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИНАЧЕ КОформлению.СуммаПродажи - КОформлению.СуммаПродажи / (1 + КОформлению.СтавкаНДС.Ставка / 100)
	|	КОНЕЦ   								  КАК СуммаНДС,
...
Процедура ПересчитатьТаблицуТоваров(Запрос)
	...
	|	ТаблицаДокумента.НомерСтроки                              КАК НомерСтроки,
	|	ТаблицаДокумента.ИдентификаторСтроки                      КАК ИдентификаторСтроки,
	|	ТаблицаДокумента.СуммаПродажи - ТаблицаДокумента.СуммаНДС КАК СуммаБезНДС,
	|	ТаблицаДокумента.СтавкаНДС                                КАК СтавкаНДС,
	|	ТаблицаДокумента.СуммаНДС                                 КАК СуммаНДС,
...
Функция ПолучитьДанныеДляПечатнойФормыАктВыполненныхРаботР1(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ДанныеДокумента.СуммаБезНДС КАК Цена,
	|	1 КАК Количество,
	...
	|	1 КАК НомерСтроки
	|ИЗ
	|	ОтчетКомитентуТаблицаТоваров КАК ДанныеДокумента
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка,
	|	НомерСтроки
