...
Процедура ФискальнаяОперацияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если "ПробитьЧек" = НавигационнаяСсылкаФорматированнойСтроки Тогда
		...
		//ЦС Владимир
		Если Объект.РасшифровкаПлатежа[0].ОснованиеПлатежа <> Неопределено тогда 
			ПараметрыОперации.Вставить("СсылкаНаДокТовары", Объект.РасшифровкаПлатежа[0].ОснованиеПлатежа);
		КонецЕсли;
		//ЦС Владимир
...
Процедура ЗаполнитьОрганизациюПлательщик()
	...
	Тогда
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитов.Организация) Тогда
			...
			Объект.ПринятоОт = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОрганизацияПлательщик, "НаименованиеПолное");
		КонецЕсли;
	КонецЕсли;
