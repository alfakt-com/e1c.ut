...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, ДополнительныеСвойства, Регистры = Неопределено) Экспорт
	...
	ТекстЗапросаДетализацияПлатежа(Запрос, ТекстыЗапроса, Регистры);
...
Функция ТекстЗапросаТаблицаСуммыДокументовВВалютеРегл(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ДанныеДокумента.СуммаДокумента КАК СуммаБезНДС,
	|	0 КАК СуммаНДС,
	|	ВЫБОР КОГДА &Валюта = &ВалютаРегламентированногоУчета
	|		ТОГДА ДанныеДокумента.СуммаДокумента
	|		ИНАЧЕ ДанныеДокумента.СуммаДокумента * &КоэффициентПересчетаВВалютуРегл
	|	КОНЕЦ КАК СуммаБезНДСРегл,
	|	0 КАК СуммаНДСРегл,
	|	НЕОПРЕДЕЛЕНО КАК ТипРасчетов,
	|	ВЫБОР КОГДА &Валюта = &ВалютаУправленческогоУчета
	|		ТОГДА ДанныеДокумента.СуммаДокумента
	|		ИНАЧЕ ДанныеДокумента.СуммаДокумента * &КоэффициентПересчетаВВалютуУпр
...
Функция ТекстЗапросаДетализацияПлатежа(Запрос, ТекстыЗапроса, Регистры)
	
	ИмяРегистра = "ДетализацияПлатежа";
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               |	0 КАК НомерСтроки,
	               |	&Период КАК Период,
	               |	ЗНАЧЕНИЕ(Справочник.ВидыОплат.Наличные) КАК ВидОплаты,
	               |	ЗНАЧЕНИЕ(Справочник.КлассификаторБанков.ПустаяСсылка) КАК Банк,
	               |	ЗНАЧЕНИЕ(Справочник.ТипыОплат.ПустаяСсылка) КАК ТипОплаты,
	               |	ПриходныйКассовыйОрдер.СуммаДокумента КАК СуммаОперации,
	               |	0 КАК КомиссияПоОсновномуПроценту,
	               |	0 КАК КомиссияПоДопПроценту,
	               |	ЛОЖЬ КАК ОнлайнПокупка,
	               |	ПриходныйКассовыйОрдер.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	               |	ПриходныйКассовыйОрдерРасшифровкаПлатежа.ОснованиеПлатежа КАК ДокументОснование,
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	// sw begin
	// Швецов Тимур, 16.11.2018
	Если ПравоДоступа("Изменение", Метаданные.Документы.ПриходныйКассовыйОрдер) Тогда
		// Приходный кассовый ордер
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.Идентификатор = "КО1_ЗаПодписьюУполномоченного";
		КомандаПечати.Представление = НСтр("ru = 'Приходный кассовый ордер за подписью уполномоченного лица'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	КонецЕсли;
	// sw end
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	// sw begin
	// Швецов Тимур, 16.11.2018
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КО1_ЗаПодписьюУполномоченного") Тогда
		
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			КоллекцияПечатныхФорм,
			"КО1_ЗаПодписьюУполномоченного",
			НСтр("ru='Приходный кассовый ордер за подписью уполномоченного лица'"),
			СформироватьПечатнуюФормуКО1ЗаПодписьюУполномоченного(МассивОбъектов, ОбъектыПечати, Неопределено, ПараметрыПечати));
...
Функция СформироватьПечатнуюФормуКО1(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	|	ДанныеДокумента.Организация.ИНН КАК ОрганизацияБИН,
	|	ВЫБОР 
	...
	Пока Выборка.Следующий() Цикл
		...
		СуммаПрописью = РаботаСКурсамиВалют.СформироватьСуммуПрописью(
				...
				Выборка.Валюта, 
				Ложь);
		Если Не ПустаяСтрока(Выборка.ВТомЧислеНДС) Тогда
			ОбластьМакета.Параметры.СуммаПрописью = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1. В т.ч. %2'"), СуммаПрописью, Выборка.ВТомЧислеНДС);
		Иначе
		...
		ОбластьМакета.Параметры.НомерДокумента = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(
			...
			Ложь,
			Истина);
		ОбластьМакета.Параметры.ДатаДокумента = Формат(Выборка.ДатаДокумента, "ДФ=dd.MM.yyyy")+" года";
		ОбластьМакета.Параметры.СубСчет = КорреспондирующийСчет(Выборка, КорСчета, ИспользуетсяРеглУчет);
		
		ОбластьМакета.Параметры.ФИОГлавногоБухгалтера = ?(НЕ ЗначениеЗаполнено(Выборка.ГлавныйБухгалтер), НСтр("ru = 'Не предусмотрен'"), Выборка.ГлавныйБухгалтер);
...
// sw begin
// Швецов Тимур, 16.11.2018
Функция СформироватьПечатнуюФормуКО1ЗаПодписьюУполномоченного(МассивОбъектов, ОбъектыПечати, КомплектыПечати, ПараметрыПечати)
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_КО1ЗаПодписьюУполномоченногоЛица";
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПриходныйКассовыйОрдер.ПФ_MXL_КО1ЗаПодписьюУполномоченногоЛица");
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеУполномоченногоЛица = swОбщегоНазначения.ПолучитьДанныеУполномоченногоЛицаДляПодписиПКО();
		Если ДанныеУполномоченногоЛица.ЕстьУполномоченноеЛицо Тогда
			ОбластьМакета.Параметры.ЗаголовокУполномоченноеЛицо = ДанныеУполномоченногоЛица.ЗаголовокУполномоченноеЛицо;
			ОбластьМакета.Параметры.ФИОУполномоченногоЛица = ДанныеУполномоченногоЛица.ФИОУполномоченногоЛица;
		Иначе
			ОбластьМакета.Параметры.ЗаголовокУполномоченноеЛицо = "Главный бухгалтер";
			ОбластьМакета.Параметры.ФИОУполномоченногоЛица = ?(НЕ ЗначениеЗаполнено(Выборка.ГлавныйБухгалтер), НСтр("ru = 'Не предусмотрен'"), Выборка.ГлавныйБухгалтер);
		КонецЕсли;
		ОбластьМакета.Параметры.ФИОКассира = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(Выборка.Кассир, Выборка.ДатаДокумента);
