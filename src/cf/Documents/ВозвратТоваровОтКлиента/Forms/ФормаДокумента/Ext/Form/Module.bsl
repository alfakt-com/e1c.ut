...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Начало: МК Баубериков А - 16.11.2020
	 Если Объект.Ссылка.Пустая() тогда
		 Если ТипЗнч(Объект.ДокументРеализации) = Тип("ДокументСсылка.РеализацияТоваровУслуг") тогда
			 Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(Объект.ДокументРеализации, "Менеджер") Тогда
				 Объект.Менеджер = Объект.ДокументРеализации.Менеджер;
				 //Объект.Подразделение = Объект.ДокументРеализации.Менеджер.Подразделение;
			 КонецЕсли;
		 КонецЕсли;
	 КонецЕсли;
	 //Окончание: МК Баубериков А - 16.11.2020
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Документ.ВозвратТоваровОтКлиента.Форма.ФормаВыбораДокументаРеализации" Тогда
		
		Объект.ДокументРеализации = ВыбранноеЗначение;
		Элементы.СтраницыРеализации.ТекущаяСтраница = Элементы.СтраницаРеализация;
		ЗаполнитьВозвращаемыеТоварыПоЧеку();
		
		Для Каждого СтрокаТовары Из Объект.Товары Цикл
			СтрокаТовары.ДокументРеализации = Объект.ДокументРеализации;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.Дата >= '20190101' Тогда
		swВыровнятьЦеныВСоответствииСДокументамиОснованиями(Отказ);
	КонецЕсли;
	// sw end
...
Процедура ПриОткрытии(Отказ)
	...
	//Ербол - старт
	Если ПолучитьИзАльфаКТ(Объект.Ссылка) Тогда //Альфа_КТ.ПолучитьСтатусОтгрузкиВМС(Ссылка) Тогда
	...
	//СНТ
	ОпределитьДоступностьГруппыСНТ();
	// Конец СНТ
	
	//МК Жакупов К. Доступ к изменению "Менеджера" и "Подразделение"
	ДоступностьРоли = РольДоступность("Доступ к изменению реал и возврат (Роль)");
...
Функция ПолучитьИзАльфаКТ(Ссылк)
	Возврат Альфа_КТ.ПолучитьСтатусОтгрузкиВМС(Ссылк);
КонецФункции
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если (ИмяСобытия = "Запись_СчетФактураВыданный"
		И ((Параметр.Свойство("ФормаВладелец") И Параметр.ФормаВладелец = УникальныйИдентификатор)
			Или Параметр.СписокОснований.НайтиПоЗначению(Объект.Ссылка) <> Неопределено))
		ИЛИ ИмяСобытия = "Запись_СчетФактура" Тогда   //При создании СФ из СНТ
	...
	// СНТ
	Если ИмяСобытия = "Запись_СНТ" Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ПроверитьИсключениеДляВозвратаПодарка() Тогда //++Аян_К++ 160622 
		БылПодарок = ПроверитьВыданныеПодарки();
		
		Если БылПодарок <> ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка") Тогда
			Сообщить("Для осуществления возврата верните подарок " + БылПодарок);
			Отказ = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииЧекаККМСервер(ПерезаполнитьТовары)
	...
	ДокументРеализации = ПолучитьОтчетОРозничныхПродажахПоЧекуККМ(Объект.ЧекККМ, , Объект.Ссылка);
	...
	Если ИспользоватьСоглашенияСКлиентами И ЗначениеЗаполнено(Объект.Соглашение) Тогда
		...
		Если Объект.Контрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо тогда
			СтруктураДействий.Вставить("swОкруглятьСуммуДоЦелого"); ///Сергей 30.09.2020 +++ Округление суммы
		КонецЕсли;	
...
Процедура ТоварыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	///Сергей 30.09.2020 +++ Расчет суммы списания бонусов
	Если ЗначениеЗаполнено(Объект.bon_БонусныйСчет) Тогда
		Если Объект.bon_БонусПартНачис.Количество() > 0 Тогда
			Для Каждого СтрТов Из Объект.Товары Цикл 
				Для Каждого СтрБонВыд Из Объект.bon_БонусПартНачис Цикл 
					Если СтрТов.Номенклатура = 	СтрБонВыд.Номенклатура и 
						СтрТов.КлючСтроки = СтрБонВыд.КлючПримВыдано Тогда
						КолПрод = Неопределено;
						Если СтрБонВыд.КоличествоПрод <> 0 Тогда
							КолПрод = СтрБонВыд.КоличествоПрод;
							СтрБонВыд.БуферРасчет = Окр((СтрБонВыд.Буфер / КолПрод)*СтрТов.Количество,0);
							СтрБонВыд.ОстатокРасчет = Окр((СтрБонВыд.Остаток / КолПрод)*СтрТов.Количество,0);
						КонецЕсли;
					КонецЕсли;					
				КонецЦикла;				
			КонецЦикла;
...
Процедура ТоварыНоменклатураОприходованиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ПолучитьЗначениеРеквизита(Объект.Контрагент, "ЮрФизЛицо") = ПредопределенноеЗначение("Перечисление.ЮрФизЛицо.ФизЛицо") тогда
		СтруктураДействий.Вставить("swОкруглятьСуммуДоЦелого"); ///Сергей 30.09.2020 +++ Округление суммы
	КонецЕсли;
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	// СНТ
	СНТСерверПереопределяемый.ОбновитьРеквизитыСНТ(ЭтаФорма,,,"ГруппаСчетФактура");
	// Конец СНТ
...
Процедура ОбновитьИнформациюПоРеализациям()
	...
	Если НЕ ЗначениеЗаполнено(Объект.ЧекККМ) Тогда
		...
		ДокументРеализацииИнформация = Новый ФорматированнаяСтрока(
				НСтр("ru = 'Открыть'"), ,
				ЦветаСтиля.ЦветГиперссылки, ,"ОткрытьДокумент");
...
&НаСервереБезКонтекста
Функция ПолучитьОтчетОРозничныхПродажахПоЧекуККМ(ЧекККМ, ОдинДокумент = Истина, ДокументВозврата = Неопределено)
	
	Возврат РозничныеПродажи.ПолучитьОтчетОРозничныхПродажахПоЧекуККМ(ЧекККМ, ОдинДокумент, ДокументВозврата);
...
&НаКлиенте
Процедура ДокументРеализацииНажатие(Элемент, СтандартнаяОбработка)
	
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратОтРозничногоПокупателя") Тогда 
		СписокДоступныхДокументов = ПолучитьСписокОтчетовОРозничныхПродажахПоЧекуККМ(Объект.ЧекККМ);
		
		Если СписокДоступныхДокументов.Количество() > 1 Тогда 
			
			СтандартнаяОбработка = Ложь;
			
			ОткрытьФорму(
			"Документ.ВозвратТоваровОтКлиента.Форма.ФормаВыбораДокументаРеализации",
			Новый Структура("СписокДокументов, ТекущийОтчет",
			ПолучитьСписокОтчетовОРозничныхПродажахПоЧекуККМ(Объект.ЧекККМ), Объект.ДокументРеализации
			),
			ЭтаФорма,,,, Неопределено, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаСервере
Функция ПолучитьСписокОтчетовОРозничныхПродажахПоЧекуККМ(ЧекККМ)
	
	ВыборкаДоступныхДокументов = РозничныеПродажи.ПолучитьОтчетОРозничныхПродажахПоЧекуККМ(ЧекККМ, Ложь);
	
	СписокДоступныхДокументов = Новый СписокЗначений;
	
	Если ЗначениеЗаполнено(ВыборкаДоступныхДокументов) Тогда
		Пока ВыборкаДоступныхДокументов.Следующий() Цикл
			СписокДоступныхДокументов.Добавить(ВыборкаДоступныхДокументов.Ссылка);
		КонецЦикла;
	КонецЕсли;
	
	Возврат СписокДоступныхДокументов;
	
КонецФункции

&НаКлиенте
Процедура ДокументРеализацииИнформацияОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ПоказатьЗначение(,Объект.ДокументРеализации);
...
&НаКлиенте
Процедура ОпределитьДоступностьГруппыСНТ()
	
	Если ЭтаФорма.Элементы.Найти("ГруппаСНТ") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Если Объект.Контрагент = ПредопределенноеЗначение("Справочник.Контрагенты.РозничныйПокупатель") 
		ИЛИ Объект.Партнер = ПредопределенноеЗначение("Справочник.Партнеры.РозничныйПокупатель") Тогда
		
		ЭтаФорма.Элементы.ГруппаСНТ.Видимость = Истина;
	Иначе
		ЭтаФорма.Элементы.ГруппаСНТ.Видимость = Ложь;
	КонецЕсли;
...
// Швецов Тимур, 23.01.2019
&НаСервере
Процедура swВыровнятьЦеныВСоответствииСДокументамиОснованиями(Отказ)
	Запрос = Новый Запрос;
	Текст = "ВЫБРАТЬ
	        |	Данные.Ссылка КАК Ссылка,
	        |	Данные.Ссылка.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	        ...
	        |ГДЕ
	        |	Данные.Ссылка В(&СсылкиРеализаций)
	        |
	        |СГРУППИРОВАТЬ ПО
	        |	Данные.Ссылка,
	        |	Данные.Номенклатура";
	Запрос.Текст = Текст;
	Запрос.УстановитьПараметр("СсылкиРеализаций", Объект.Товары.Выгрузить().ВыгрузитьКолонку("ДокументРеализации"));
	ТаблицаДанныхРеализаций = Запрос.Выполнить().Выгрузить();
	Для Каждого СтрокаТовара Из Объект.Товары Цикл
		СтруктураПоиска = Новый Структура;
		СтруктураПоиска.Вставить("Ссылка", СтрокаТовара.ДокументРеализации);
		СтруктураПоиска.Вставить("Номенклатура", СтрокаТовара.Номенклатура);
		СтрокиИзРеалиазации = ТаблицаДанныхРеализаций.НайтиСтроки(СтруктураПоиска);
		Если СтрокиИзРеалиазации.Количество() = 0 Тогда
			ТекстОшибки = НСтр("ru='Номенклатура " + СтрокаТовара.Номенклатура + " из строки " + СтрокаТовара.НомерСтроки + " не найдена в документе " + СтрокаТовара.ДокументРеализации + "'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.Товары", , Отказ);
			Продолжить;
		КонецЕсли;
		СтрокаИзРеализации = СтрокиИзРеалиазации[0];
		Если Не СтрокаИзРеализации.ЦенаВключаетНДС = Объект.ЦенаВключаетНДС Тогда
			ТекстОшибки = НСтр("ru='Признак ""Цена включает НДС"" не совпадает с этим признаком в документе " + СтрокаТовара.ДокументРеализации + "'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.Товары", , Отказ);
			Продолжить;
		КонецЕсли;
		//Начало: МК Баубериков А - 06.11.2020
		РасчетнаяЦенаВРеализации = ?(СтрокаИзРеализации.Количество = 0, 0, Окр(СтрокаИзРеализации.Сумма / СтрокаИзРеализации.Количество, 2, РежимОкругления.Окр15как10));
		//РасчетнаяЦенаВРеализации = ?(СтрокаИзРеализации.Количество = 0, 0, Окр(СтрокаИзРеализации.Сумма / СтрокаИзРеализации.Количество, 2, РежимОкругления.Окр15как20));
		//Конец: МК Баубериков А - 06.11.2020
		РасчетнаяЦенаВВозврате = ?(СтрокаТовара.Количество = 0, 0, Окр(СтрокаТовара.Сумма / СтрокаТовара.Количество, 2, РежимОкругления.Окр15как10));
		Если РасчетнаяЦенаВВозврате > РасчетнаяЦенаВРеализации Тогда
			СтрокаТовара.Цена =  РасчетнаяЦенаВРеализации;
			СтрокаТовара.Сумма = СтрокаТовара.Количество * РасчетнаяЦенаВРеализации;
			СтруктураПересчетаСуммы = ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруПересчетаСуммыНДСВСтрокеТЧ(Объект);
			СтруктураДействий = Новый Структура;
			//СтруктураДействий.Вставить("ПересчитатьЦенуСкидкуПоСуммеВПродажах");
			СтруктураДействий.Вставить("ПересчитатьСуммуНДС", СтруктураПересчетаСуммы);
			...
			Сообщить("В строке " + СтрокаТовара.НомерСтроки + " цена была подогнана под цену в реализации");
			//ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.Товары", , Отказ);
			РассчитатьИтоговыеПоказателиВозврата(ЭтаФорма);
			ФинансыКлиентСервер.ПересчитатьСуммыВСтрокеРасшифровкиПлатежа(Объект, СуммаВсего);
			РассчитатьСуммуКомпенсации(ЭтаФорма);
...
Процедура МенеджерПриИзменении(Элемент)
	...
	//Начало: Баубериков А - 28.07.2020
	УстановитьПодразделение();
	//Конец: Баубериков А - 28.07.2020
...
&НаСервере
Функция ПроверитьВыданныеПодарки()
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	bon_ВыданныеПодарки.Номенклатура КАК Номенклатура,
	|	bon_ВыданныеПодарки.ЗаказКлиента КАК ЗаказКлиента,
	|	bon_ВыданныеПодарки.КоличествоОстаток КАК Количество
	|ПОМЕСТИТЬ ВыданныеПодарки
	|ИЗ
	|	РегистрНакопления.bon_ВыданныеПодарки.Остатки КАК bon_ВыданныеПодарки
	|ГДЕ
	|	bon_ВыданныеПодарки.ЗаказКлиента = &ЗаказКлиента
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВыданныеПодарки.Номенклатура КАК Номенклатура
	|ИЗ
	|	ВыданныеПодарки КАК ВыданныеПодарки
	|		ЛЕВОЕ СОЕДИНЕНИЕ Товары КАК Товары
	|		ПО ВыданныеПодарки.Номенклатура = Товары.Номенклатура
	|ГДЕ
	|	ВыданныеПодарки.Количество - ЕСТЬNULL(Товары.КоличествоУпаковок, 0) <> 0";
	Запрос.УстановитьПараметр("ЗаказКлиента", Объект.ДокументРеализации.ЗаказКлиента);
...
Функция ПроверитьИсключениеДляВозвратаПодарка() 
	
	Возврат Альфа_КТ.ПроверитьДоступ("ВозвратПодарковБОН", ПользователиКлиентСервер.ТекущийПользователь(), ТекущаяДата(), ТекущаяДата());
