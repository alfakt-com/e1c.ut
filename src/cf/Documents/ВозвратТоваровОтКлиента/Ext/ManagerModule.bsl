...
Функция ТекстЗапросаТаблицаНДСПредъявленный(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ТаблицаВидыЗапасов.ИдентификаторСтроки = ПересчитаннаяТаблицаДокумента.ИдентификаторСтроки
	|ГДЕ
	// в УПК НДСПринятКВычету в случае возврата не формируем - рассматривается как реализация (в УП как покупка)
	|	ЛОЖЬ И
	|	&Организация <> ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация)
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВЫБОР
	               |		КОГДА ДанныеДокумента.СпособКомпенсации = ЗНАЧЕНИЕ(Перечисление.СпособыКомпенсацииВозвратовТоваров.ВернутьДенежныеСредства)
	               |			ТОГДА ИСТИНА
	               |		ИНАЧЕ ЛОЖЬ
	...
	Запрос.УстановитьПараметр("ЗаказКлиента", Реквизиты.ДокументРеализации.ЗаказКлиента);
	Запрос.УстановитьПараметр("МаркетинговаяАкция", Реквизиты.ДокументРеализации.ЗаказКлиента.МаркетинговаяАкция);
...
Функция ТекстЗапросаТаблицаВыданныеПодарки(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса =
		...
		|	&Период КАК Период,
		|	&МаркетинговаяАкция КАК МаркетинговаяАкция,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) КАК ВидДвижения
		|ИЗ
		|	(ВЫБРАТЬ
		|		bon_ВыданныеПодаркиОстатки.Номенклатура КАК Номенклатура,
		|		bon_ВыданныеПодаркиОстатки.КоличествоОстаток КАК Количество
		|	ИЗ
		|		РегистрНакопления.bon_ВыданныеПодарки.Остатки(, ) КАК bon_ВыданныеПодаркиОстатки
		|	ГДЕ
		|		bon_ВыданныеПодаркиОстатки.ЗаказКлиента = &ЗаказКлиента
		|		И bon_ВыданныеПодаркиОстатки.МаркетинговаяАкция = &МаркетинговаяАкция) КАК ВложенныйЗапрос,
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Изменение", Метаданные.Документы.ВозвратТоваровОтКлиента) Тогда
		...
		КомандаПечати.ПроверкаПроведенияПередПечатью = Ложь;  //ТТ. Убрал проверку проведения. Так как меняется процесс возврата. Сначала возврат, распечатка, приходный ордер, и затем проведение.
	КонецЕсли;
...
Функция СформироватьПечатнуюФормуВозвратОтКлиента(МассивОбъектов, ОбъектыПечати)
	...
	Запрос = Новый Запрос("
		...
		//|	И ВозвратТоваровОтКлиента.Проведен   //ТТ
		|
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка
		|;
...
Функция ТекстЗапросаДанныхОснованияДляПечатнойФормыСчетФактура() Экспорт
	...
	|	ВЫБОР
	|		КОГДА ДанныеДокументов.Соглашение = ЗНАЧЕНИЕ(Справочник.СоглашенияСКлиентами.ПустаяСсылка)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ДанныеДокументов.Соглашение.ФормаОплаты
...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	ЗнакРесурсов = 1;
	Если ПараметрыЗаполнения.Свойство("ПечатьСчетаФактуры") Тогда
		ЗнакРесурсов = -1;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ЗнакРесурсов",             ЗнакРесурсов);
	...
	|		ВЫБОР КОГДА ТаблицаДокумента.Ссылка.ЦенаВключаетНДС ТОГДА 
	...
	|		КОНЕЦ / ТаблицаДокумента.Количество
	|	ИНАЧЕ
	|		0
	|	КОНЕЦ КАК Цена,
	|	&ЗнакРесурсов * СУММА(ТаблицаДокумента.Количество)             КАК Количество,
	|	&ЗнакРесурсов * СУММА(ТаблицаДокумента.Количество)             КАК КоличествоУпаковок,
	|	
	|	&ЗнакРесурсов * СУММА(ЕСТЬNULL(
