...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// sw begin
	// Швецов Тимур, 10.01.2019
	ПроверяемыеРеквизиты.Добавить("Договор");
	ПроверяемыеРеквизиты.Добавить("Подразделение");
	Если Дата >= '20190101' Тогда
		ПроверяемыеРеквизиты.Добавить("swУсловияОплаты");
		ЮридическийАдресКонтрагента = ФормированиеПечатныхФорм.ПолучитьАдресИзКонтактнойИнформации(Контрагент, "Юридический", Дата);
		Если Не ЗначениеЗаполнено(ЮридическийАдресКонтрагента) Тогда
			ТекстОшибки = НСтр("ru='У контрагента не указан юридический адрес'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.Контрагент", , Отказ);
		КонецЕсли;
		ЕстьСтрокиБезОснования = Ложь;
		ЕстьСтрокиИзРазныхРеализаций = Ложь;
		ДатаРеализации = '00010101';
		ДокументРеализацииДляПроверки = Неопределено;
		Для Каждого СтрокаТоваров Из Товары Цикл
			Если ДокументРеализацииДляПроверки = Неопределено Тогда
				ДокументРеализацииДляПроверки = СтрокаТоваров.ДокументРеализации;
			Иначе
				Если Не ДокументРеализацииДляПроверки = СтрокаТоваров.ДокументРеализации Тогда
					ЕстьСтрокиИзРазныхРеализаций = Истина;
				КонецЕсли;
			КонецЕсли;
			Если Не ЗначениеЗаполнено(СтрокаТоваров.ДокументРеализации) Тогда
				ЕстьСтрокиБезОснования = Истина;
			Иначе
				ДатаРеализации = СтрокаТоваров.ДокументРеализации.Дата;
			КонецЕсли;
		КонецЦикла;
		Если ЕстьСтрокиБезОснования Тогда
			ТекстОшибки = НСтр("ru='Нельзя, чтобы в строке табличной части не был указан документ реализации'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.Товары", , Отказ);
		КонецЕсли;
		Если ЕстьСтрокиИзРазныхРеализаций Тогда
			ТекстОшибки = НСтр("ru='Нельзя, чтобы в строках табличной части были указаны разные документы реализации'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.Товары", , Отказ);
		КонецЕсли;
		Если ЗначениеЗаполнено(ДокументРеализацииДляПроверки) И Не Договор = ДокументРеализацииДляПроверки.Договор Тогда
			ТекстОшибки = НСтр("ru='Нельзя, чтобы договор в возврате и в документе реализации были разные'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.Договор", , Отказ);
		КонецЕсли;
		Если ЗначениеЗаполнено(ДокументРеализацииДляПроверки) И Не swУсловияОплаты = ДокументРеализацииДляПроверки.swУсловияОплаты И ДатаРеализации >= '20190101' Тогда
			ТекстОшибки = НСтр("ru='Нельзя, чтобы условия оплаты в возврате и в документе реализации были разные'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.swУсловияОплаты", , Отказ);
		КонецЕсли;
		//Если Не ЕстьСтрокиИзРазныхРеализаций И Не ЕстьСтрокиБезОснования И ДатаРеализации < '20190101' И Дата >= '20190325' Тогда
		//	ТекстОшибки = НСтр("ru='Нельзя оформлять возврат по документам реализации, выписанным до 1 января 2019 года'");
		//	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.Товары", , Отказ);
		//КонецЕсли;
	КонецЕсли;
	// sw end
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//++Аян_К++ задача №5974 020922 старт не верьте Семенову Сергею он много говорит 
	Если РежимЗаписи = РежимЗаписиДокумента.ОтменаПроведения Или ЭтотОбъект.ПометкаУдаления = Истина Тогда
		Если НЕ Альфа_КТ.ПроверитьДоступ("ВМС_ПометкаНаУдаление", ПараметрыСеанса.ТекущийПользователь, ТекущаяДата(), ТекущаяДата()) Тогда
			Если Альфа_КТ.ПолучитьСтатусОтгрузкиВМС(Ссылка) Тогда
				Сообщить("Редактировать документ запрещено после выгрузки в ВМС");
				Отказ = Истина;
				Возврат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	//++Аян_К++ задача №5974 020922 финиш
	...
	Если РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ВозвратОтКомиссионера И СуммаДокумента > 0 Тогда
			...
			ВзаиморасчетыСервер.ЗаполнитьСуммуРасшифровкиПлатежаПоСуммеДокумента(СуммаДокумента, РасшифровкаПлатежа);			
	...
	// Конец ИнтеграцияГИСМ
		...
		// Швецов Тимур, 23.01.2019
	Если Дата >= '20190101' Тогда
		swПроверитьКоличествоСтрокВДокументеПоОграничениямЭСФ(Отказ);
	КонецЕсли;
	// sw end
	
	//+ТТ
	Если Не Отказ Тогда
		ПроверитьКоличествоРазрешенныхДнейНаВозврат(Отказ);
	КонецЕсли;
	//-ТТ
...
//ТТ
Функция ПроверитьКоличествоРазрешенныхДнейНаВозврат(Отказ)
	ДопСвойство = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя","ДнейНаВозврат");
	Если ЗначениеЗаполнено(ДопСвойство) и ЗначениеЗаполнено(ЭтотОбъект.Подразделение) и ЗначениеЗаполнено(ЭтотОбъект.ДокументРеализации) Тогда
		РазрешеноДней = ЦС_ОбщийМодуль.ПолучитьЗначениеДопРеквизита(ЭтотОбъект.Подразделение,ДопСвойство);
		Если ЗначениеЗаполнено(РазрешеноДней) Тогда
			РазницаДней = Цел((ЭтотОбъект.Дата - ЭтотОбъект.ДокументРеализации.Дата)/60/60/24);
			Если РазницаДней > РазрешеноДней Тогда
				ТекстОшибки = "Возврат запрещен. Разрешенный период дней на возврат "+РазрешеноДней;	
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.ДокументРеализации", , Отказ);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецФункции
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//Ербол-начало
	Если НЕ Альфа_КТ.ПолучитьСтатусОтгрузкиВМС(Ссылка) И НЕ Альфа_КТ.ПроверитьДоступ("ПроведениеВозвратаБезВыгрузкиВВМС", ПараметрыСеанса.ТекущийПользователь, ТекущаяДата(), ТекущаяДата()) Тогда
	...
	// Если при закрытии смены и формировании отчетов о продажах товары из одного чека ККМ попали в разные отчеты (из-за ограничения строк), 
	// тогда при заполнении документа возврата есть возможность выбрать документ реализации из списка. 
	// При этом может возникнуть ситуация, когда в ТЧ документа возврата указан товар из ТЧ чека-основания, однако выбран отчет о продажах, 
	// в котором нет соответствующей номенклатуры. В этом случае выводим предупреждение о некорректном заполнении.
	ПроверитьСоответствиеНоменклатурыВТабличнойЧастиТовары();
	
	///Сергей 07.08.2020 +++
	Если Не Отказ Тогда
		Если ЗначениеЗаполнено(bon_БонусныйСчет) Тогда
			bon_БонуснаяПрограммаСервер.ДвиженияБонусовПоПартиямВозврат(ЭтотОбъект, Товары, Отказ);
		КонецЕсли;
	КонецЕсли;
	///Сергей 07.08.2020 ---
	
	//Начало: МК Баубериков А - 21.01.2021
	Если Не Отказ и НЕ ЗаданиеСуществует(ЭтотОбъект.Ссылка) Тогда
		СоздатьЗаданиеПоОформлениюВозвратаПоставщику(ЭтотОбъект);
	Конецесли;
	//Конец: МК Баубериков А - 21.01.2021
	                                                           
КонецПроцедуры

//Начало: МК Баубериков А - 21.01.2021
Функция ЗаданиеСуществует(Док)
	...
	Запрос.УстановитьПараметр("Предмет",Док.Ссылка);
...
Процедура СоздатьЗаданиеПоОформлениюВозвратаПоставщику(Док)
	
	ДопРеквизитМенеджерОтделаЗакупа = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя", "Менеджер");
	Если ДопРеквизитМенеджерОтделаЗакупа.Пустая() тогда
		Возврат;
	КонецЕсли;   
	
	ДопРеквизитСоздаватьВозвратПоставщикуПриВозвратеОтПокупателя = ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения.НайтиПоРеквизиту("Имя", "СоздаватьВозвратПоставщикуПриВозвратеОтПокупателя");
	Если ДопРеквизитСоздаватьВозвратПоставщикуПриВозвратеОтПокупателя.Пустая() тогда
		Возврат;
	КонецЕсли; 
	...
	|		ПО НоменклатураПоставщиков.Владелец = ПартнерыДополнительныеРеквизиты.Ссылка
	|			И (ПартнерыДополнительныеРеквизиты.Свойство = &СвойствоСоздаватьВозвратПоставщику)
	|			И (ПартнерыДополнительныеРеквизиты.Свойство = ИСТИНА)
	|ГДЕ
	|	НоменклатураПоставщиков.Номенклатура В(&сзНоменклатура)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НоменклатураДополнительныеРеквизиты.Значение КАК Менеджер,
	|	втПоставщики.НоменклатураПоставщика КАК НоменклатураПоставщика
	|ПОМЕСТИТЬ втПред
	|ИЗ
	|	Справочник.Номенклатура.ДополнительныеРеквизиты КАК НоменклатураДополнительныеРеквизиты
	|		ЛЕВОЕ СОЕДИНЕНИЕ втПоставщики КАК втПоставщики
	|		ПО НоменклатураДополнительныеРеквизиты.Ссылка = втПоставщики.НоменклатураПоставщика.Номенклатура
	|ГДЕ
	|	НоменклатураДополнительныеРеквизиты.Свойство = &Свойство
	|	И НоменклатураДополнительныеРеквизиты.Ссылка В(&сзНоменклатура)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втПред.Менеджер КАК Менеджер,
	|	втПред.НоменклатураПоставщика КАК НоменклатураПоставщика
	|ИЗ
	|	втПред КАК втПред
	|ГДЕ
	|	НЕ втПред.НоменклатураПоставщика ЕСТЬ NULL";
	Запрос.УстановитьПараметр("Свойство",ДопРеквизитМенеджерОтделаЗакупа);
	Запрос.УстановитьПараметр("сзНоменклатура",Док.Товары.ВыгрузитьКолонку("Номенклатура"));   
	Запрос.УстановитьПараметр("СвойствоСоздаватьВозвратПоставщику",ДопРеквизитСоздаватьВозвратПоставщикуПриВозвратеОтПокупателя); 
	тзРез = Запрос.Выполнить().Выгрузить();	
	тзРез.Свернуть("Менеджер");
	
	Для каждого стр из тзРез Цикл	
		НовЗадание = БизнесПроцессы.Задание.СоздатьБизнесПроцесс();
		НовЗадание.Автор = ПараметрыСеанса.ТекущийПользователь;
		НовЗадание.Заполнить(Док.Ссылка);
		НовЗадание.Дата = ТекущаяДата();
		НовЗадание.Наименование = "Оформлен возврат от клиента. Необходимо создать возврат поставщику.";
		НовЗадание.Исполнитель = стр.Менеджер;
		НовЗадание.НаПроверке = Ложь;
		НовЗадание.Проверяющий = Справочники.Пользователи.ПустаяСсылка();
		НовЗадание.Записать();
		НовЗадание.Старт();
	КонецЦикла;
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	//Начало: МК Баубериков А - 28.07.2020
	Менеджер = Справочники.Пользователи.ПустаяСсылка();
	Подразделение = Справочники.СтруктураПредприятия.ПустаяСсылка();
	//Окончание: МК Баубериков А - 28.07.2020
...
Процедура ПриЗаписи(Отказ)
	...
	// sw begin
	// Швецов Тимур, 23.01.2019
	Если Дата >= '20190124' Тогда
		swПроверитьПовторяющуюсяНоменклатуруПоСтрокам(Отказ);
	КонецЕсли;
	// sw end
...
Процедура ЗаполнитьДокументНаОснованииРеализацииТоваровИУслуг(Знач ДокументОснование)
	...
	|	NULL КАК КлючСтроки,
	|	NULL КАК bon_СуммаБонусовВыдано,
	|	NULL КАК bon_СуммаБонусовПринято
	...
	|	ТаблицаТовары.bon_СуммаБонусовВыдано,
	|	ТаблицаТовары.bon_СуммаБонусовПринято
	|ИЗ
	|	Документ.РеализацияТоваровУслуг.Товары КАК ТаблицаТовары
	...
	|	ТоварыДокументаПродажи.КлючСтроки КАК КлючСтроки,
	|	ТоварыДокументаПродажи.bon_СуммаБонусовВыдано КАК bon_СуммаБонусовВыдано,
	|	ТоварыДокументаПродажи.bon_СуммаБонусовПринято КАК bon_СуммаБонусовПринято
	...
	|	ТоварыДокументаПродажи.КлючСтроки,
	|	ТоварыДокументаПродажи.bon_СуммаБонусовВыдано,
	|	ТоварыДокументаПродажи.bon_СуммаБонусовПринято
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВозвратТоваровОтКлиентаТовары.bon_СуммаБонусовВыдано КАК bon_СуммаБонусовВыдано,
	|	ВозвратТоваровОтКлиентаТовары.bon_СуммаБонусовПринято КАК bon_СуммаБонусовПринято
	...
	|	ВозвратТоваровОтКлиентаТовары.КлючСтроки,
	|	ВозвратТоваровОтКлиентаТовары.bon_СуммаБонусовВыдано,
	|	ВозвратТоваровОтКлиентаТовары.bon_СуммаБонусовПринято
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Для Каждого ТекСтрока Из ТаблицаТоваров Цикл
		...
		Если ТекСтрока.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Товар
			...
			//Пересчитаем количество, в упаковки документа основания
			Если ЗначениеЗаполнено(НоваяСтрока.Упаковка) Тогда 
				КоэффициентУпаковки = Справочники.УпаковкиЕдиницыИзмерения.КоэффициентУпаковки(НоваяСтрока.Упаковка, НоваяСтрока.Номенклатура);
				Если Не НоваяСтрока.Количество = (НоваяСтрока.КоличествоУпаковок * КоэффициентУпаковки) Тогда
	...
	///Сергей 11.08.2020 +++ Бонусы
	bon_БонусныйСчет = ДокументОснование.bon_БонусныйСчет;
	//Начисленные бонусы
	Для каждого СтрокаТаблицыИсточника Из ДокументОснование.bon_БонусПартНачис Цикл
		СтрокаТаблицыПриемника = bon_БонусПартНачис.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриемника,СтрокаТаблицыИсточника);
	КонецЦикла;
	
	//Использованные бонусы
	Для каждого СтрокаТаблицыИсточника Из ДокументОснование.bon_ПартииБонусовИст Цикл
		СтрокаТаблицыПриемника = bon_ПартииБонусовИст.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриемника,СтрокаТаблицыИсточника);
	КонецЦикла;
	
	//Подарки Выданные
	Для каждого СтрокаТаблицыИсточника Из ДокументОснование.bon_ПодаркиВыданные Цикл
		СтрокаТаблицыПриемника = bon_ПодаркиВыданные.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриемника,СтрокаТаблицыИсточника);
	КонецЦикла;
	
	//Срабатывшие по счетчику
	Для каждого СтрокаТаблицыИсточника Из ДокументОснование.bon_ТаблицаВыдАкцийпоСчетчику Цикл
		СтрокаТаблицыПриемника = bon_ТаблицаВыдАкцийпоСчетчику.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриемника,СтрокаТаблицыИсточника);
	КонецЦикла;
	///Сергей 11.08.2020 --- Бонусы
	
	//Все вовзраты должны возвращаться в свои подразделения 
	ЭтотОбъект.Подразделение = ВыборкаШапка.Подразделение;
...
Процедура ПроверитьСоответствиеНоменклатурыВТабличнойЧастиТовары()
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ВозвратТоваровВидыЗапасов.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
				   |	ВозвратТоваровВидыЗапасов.ВидЗапасов КАК ВидЗапасов
	               |ПОМЕСТИТЬ ВозвратТоваровВидыЗапасов
	               |ИЗ
	               |	&ВозвратТоваровВидыЗапасов КАК ВозвратТоваровВидыЗапасов
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ВозвратТоваровВидыЗапасов.АналитикаУчетаНоменклатуры.Номенклатура КАК Номенклатура
	               |ИЗ
	               |	ВозвратТоваровВидыЗапасов КАК ВозвратТоваровВидыЗапасов
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ." + ?(ТипЗнч(ДокументРеализации) = Тип("ДокументСсылка.ОтчетОРозничныхПродажах"), "ОтчетОРозничныхПродажах", "РеализацияТоваровУслуг") + ".ВидыЗапасов КАК ДокументРеализацииВидыЗапасов
	               |		ПО ВозвратТоваровВидыЗапасов.АналитикаУчетаНоменклатуры = ДокументРеализацииВидыЗапасов.АналитикаУчетаНоменклатуры
	               |			И ВозвратТоваровВидыЗапасов.ВидЗапасов = ДокументРеализацииВидыЗапасов.ВидЗапасов
	               |ГДЕ
	               |	ДокументРеализацииВидыЗапасов.Ссылка = &Ссылка
	               |	И ДокументРеализацииВидыЗапасов.НомерСтроки ЕСТЬ NULL";
	
	Запрос.УстановитьПараметр("Ссылка", ДокументРеализации);
	Запрос.УстановитьПараметр("ВозвратТоваровВидыЗапасов", ВидыЗапасов.Выгрузить());
	Результат = Запрос.Выполнить();
	...
	Если Не Результат.Пустой() Тогда 
		...
		Пока Выборка.Следующий() Цикл 
			ПереченьТоваров = "";
			ПереченьТоваров = ПереченьТоваров + Символы.ПС + Выборка.Номенклатура + ";";
		КонецЦикла;
		ТекстСообщения = НСтр("ru='Товары отсутствуют в выбранном отчете о розничных продажах: %1'");
		ТекстСообщения = ЭСФКлиентСерверПереопределяемый.ПодставитьПараметрыВСтроку(ТекстСообщения, ПереченьТоваров);	
	КонецЕсли;
...
// sw begin
// Швецов Тимур, 23.01.2019
Процедура swПроверитьКоличествоСтрокВДокументеПоОграничениямЭСФ(Отказ)
	Если ВидыЗапасов.Количество() > 199 Тогда
		ТекстОшибки = НСтр("ru='Нельзя, чтобы количество строк в табличной части, распределенных по источникам происхождения товара, было более 199 (сейчас получается " + ВидыЗапасов.Количество() + ")'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.Товары", , Отказ);
	КонецЕсли;
...
// sw begin
// Швецов Тимур, 23.01.2019
Процедура swПроверитьПовторяющуюсяНоменклатуруПоСтрокам(Отказ)
	Запрос = Новый Запрос;
	Текст = "ВЫБРАТЬ
	        |	Данные.НомерСтроки,
	        |	ДанныеОДублях.МинимальныйНомерСтроки,
	        |	Данные.Номенклатура
	        |ИЗ
	        |	(ВЫБРАТЬ
	        ...
	        |	ГДЕ
	        |		Данные.Ссылка = &Ссылка) КАК Данные
	        ...
	        |		
	        |		СГРУППИРОВАТЬ ПО
	        |			Данные.Номенклатура) КАК ДанныеОДублях
	        |		ПО Данные.Номенклатура = ДанныеОДублях.Номенклатура
	        |			И (НЕ Данные.НомерСтроки = ДанныеОДублях.МинимальныйНомерСтроки)";
	Запрос.Текст = Текст;
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		ТекстОшибки = НСтр("ru='Номенклатура в строке " + Выборка.НомерСтроки + " в табличной части дублируется с номенклатурой в строке " + Выборка.МинимальныйНомерСтроки + "'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.Товары", , Отказ);
	КонецЦикла;
КонецПроцедуры // sw end
