...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Изменение", Метаданные.Документы.ИнвентаризационнаяОпись) Тогда
		...
		// Инвентаризационная опись (ИНВ-10)
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.МенеджерПечати = "Обработка.ПечатьИНВ3";
		КомандаПечати.Идентификатор = "ИНВ3";
		КомандаПечати.Представление = НСтр("ru = 'Инвентаризационная опись (ИНВ-10)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
...
// Функция получает данные документа. Айнура 210422
...
Функция ПолучитьДанныеДокумента(ПараметрыПечати, ДокументОснование)
	...
	//++Саканаев Д. 0102022 перенес на другую константу
	КолонкаКодов = ФормированиеПечатныхФорм.ИмяДополнительнойКолонкиПоИнвентаризациям();
	...
	//Начало: МК Баубериков А - 07.08.2020
	Запрос.УстановитьПараметр("ДокументСписания", ДокументОснование.ДокументСписания);
	Запрос.УстановитьПараметр("ДокументОприходования", ДокументОснование.ДокументОприходования);
	Запрос.УстановитьПараметр("ДокументПересортицы", ДокументОснование.ДокументПересортицы); 
	//Конец: МК Баубериков А - 07.08.2020
	...
	Если ИсточникИнформацииОЦенахДляПечати = Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоСебестоимости Тогда
		...
		Если ПредварительныйРасчет Тогда
			...
			|	ВЫРАЗИТЬ(&Организация КАК Справочник.Организации).ИНН КАК ОрганизацияБИН,
			...
			|	ЦеныНоменклатуры.СтоимостьРегл + ЦеныНоменклатуры.ДопРасходыРегл + ЦеныНоменклатуры.ТрудозатратыРегл + ЦеныНоменклатуры.ПостатейныеПостоянныеРегл + ЦеныНоменклатуры.ПостатейныеПеременныеРегл КАК СтоимостьРегл
		...
		Иначе
			...
			|		И ОприходованиеИзлишковТоваровТовары.Ссылка.Проведен
			|		И ОприходованиеИзлишковТоваровТовары.Ссылка.Организация = &Организация
			|		И ОприходованиеИзлишковТоваровТовары.Ссылка = &ДокументСписания
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			|	ВЫБРАТЬ
			...
			|			ТОГДА 0
			|		ИНАЧЕ (СебестоимостьТоваровОстатки.СтоимостьРеглОстаток + СебестоимостьТоваровОстатки.ДопРасходыРеглОстаток + СебестоимостьТоваровОстатки.ТрудозатратыРеглОстаток + СебестоимостьТоваровОстатки.ПостатейныеПостоянныеРеглОстаток + СебестоимостьТоваровОстатки.ПостатейныеПеременныеРеглОстаток) / СебестоимостьТоваровОстатки.КоличествоОстаток
			|	КОНЕЦ КАК Цена
			...
			|	СтоимостьТоваровСрезПоследних.СтоимостьРегл + СтоимостьТоваровСрезПоследних.ДопРасходыРегл + СтоимостьТоваровСрезПоследних.ТрудозатратыРегл + СтоимостьТоваровСрезПоследних.ПостатейныеПостоянныеРегл + СтоимостьТоваровСрезПоследних.ПостатейныеПеременныеРегл КАК Цена
	...
	ИначеЕсли ИсточникИнформацииОЦенахДляПечати = Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоВидуЦен Тогда 
		...
		|	СУММА(ПересчетТоваровТовары.КоличествоУпаковокФакт) КАК КоличествоФактическое,
		|	СУММА(ПересчетТоваровТовары.КоличествоУпаковок) КАК КоличествоПоУчету
		|ПОМЕСТИТЬ ТаблицаИнвентаризации
		|ИЗ
		|	Документ.ПересчетТоваров.Товары КАК ПересчетТоваровТовары
		|ГДЕ
		...
		|					ИЗ
		|						ТаблицаИнвентаризации КАК Таблица)
		|				И Цена <> 0) КАК ЦеныНоменклатуры
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
// Функция получает данные для формирования печатной формы ИНВ19 Айнура 210422
//
Функция ПолучитьДанныеДляПечатнойФормыИНВ19(ПараметрыПечати, ДокументОснование) Экспорт
	...
	//Начало: МК Баубериков А - 06.08.2020
	Запрос.УстановитьПараметр("ДокументСписания", ДокументОснование.ДокументСписания);
	Запрос.УстановитьПараметр("ДокументОприходования", ДокументОснование.ДокументОприходования);
	Запрос.УстановитьПараметр("ДокументПересортицы", ДокументОснование.ДокументПересортицы); 
	//Конец: МК Баубериков А - 06.08.2020
	...
	Если ПредварительныеДанныеРезультат.ИсточникИнформацииОЦенахДляПечати = Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоСебестоимости Тогда
		...
		Если ПредварительныеДанныеРезультат.ПредварительныйРасчет Тогда
			...
			|ГДЕ
			|	Товары.Ссылка.Склад = &Склад
			|	И Товары.Ссылка.Организация = &Организация
			|	И Товары.Ссылка.Проведен
			|	И Товары.Ссылка = &ДокументОприходования
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			...
			|	И ВидыЗапасов.Ссылка.Организация = &Организация
			|	И ВидыЗапасов.Ссылка.Проведен
			|	И ВидыЗапасов.Ссылка = &ДокументПересортицы
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			...
			|	И Товары.Ссылка.Организация = &Организация
			|	И Товары.Ссылка.Проведен
			|	И Товары.Ссылка = &ДокументПересортицы
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
		...
		Иначе
			...
			|			ТОГДА 0
			|		ИНАЧЕ СУММА(ЕСТЬNULL(СебестоимостьТоваровПоРегистратору.СтоимостьРегл, 0) + ЕСТЬNULL(СебестоимостьТоваровПоРегистратору.ДопРасходыРегл, 0) + ЕСТЬNULL(СебестоимостьТоваровПоРегистратору.ТрудозатратыРегл, 0) + ЕСТЬNULL(СебестоимостьТоваровПоРегистратору.ПостатейныеПостоянныеРегл, 0) + ЕСТЬNULL(СебестоимостьТоваровПоРегистратору.ПостатейныеПеременныеРегл, 0)) / СУММА(СебестоимостьТоваровПоРегистратору.Количество)
			|	КОНЕЦ КАК Цена
			|ПОМЕСТИТЬ ТаблицаЦен
			|ИЗ
			|	ТаблицаИнвентаризации КАК ТаблицаИнвентаризации
	...
	ИначеЕсли ПредварительныеДанныеРезультат.ИсточникИнформацииОЦенахДляПечати = Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоВидуЦен Тогда 
		...
		|	ПересчетТоваровТовары.Характеристика КАК Характеристика,
		|	ПересчетТоваровТовары.Серия КАК Серия,
		|	ЕСТЬNULL(ПересчетТоваровТовары.КоличествоУпаковок, 0) - ЕСТЬNULL(ПересчетТоваровТовары.КоличествоУпаковокФакт, 0) КАК Количество,
		|	""1_Списание"" КАК Операция,
		...
		|	И ПересчетТоваровТовары.Ссылка.Проведен
		|	И ПересчетТоваровТовары.Ссылка.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПересчетовТоваров.Выполнено)
		|	И ЕСТЬNULL(ПересчетТоваровТовары.КоличествоУпаковок, 0) - ЕСТЬNULL(ПересчетТоваровТовары.КоличествоУпаковокФакт, 0) > 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ПересчетТоваровТовары.Характеристика,
		|	ПересчетТоваровТовары.Серия,
		|	-(ЕСТЬNULL(ПересчетТоваровТовары.КоличествоУпаковок, 0) - ЕСТЬNULL(ПересчетТоваровТовары.КоличествоУпаковокФакт, 0)),
		|	""2_Оприходование"",
		|	NULL,
		|	NULL,
		|	NULL
		...
		|	ВложенныйЗапрос.СвязаннаяНоменклатура КАК СвязаннаяНоменклатура,
		|	ВложенныйЗапрос.СвязаннаяХарактеристика КАК СвязаннаяХарактеристика,
		|	ВложенныйЗапрос.СвязаннаяСерия КАК СвязаннаяСерия,
		|	&ВидЦены КАК ВидЦен
		...
		|		ИНАЧЕ ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0)
		|	КОНЕЦ КАК Стоимость,
		|	ТаблицаТовары.Операция КАК Операция,
		|	ТаблицаТовары.СвязаннаяНоменклатура КАК СвязаннаяНоменклатура,
		|	ТаблицаТовары.СвязаннаяХарактеристика КАК СвязаннаяХарактеристика,
		|	ТаблицаТовары.СвязаннаяСерия КАК СвязаннаяСерия,
