...
Функция ТекстЗапросаДляРасчетаНеобходимостиСчетаФактуры() Экспорт
	...
	|						ТОГДА ИСТИНА
	|					КОГДА НЕ УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС И НЕ УслугиСНДС.Ссылка ЕСТЬ NULL
	|						ТОГДА ИСТИНА
	|					ИНАЧЕ ЛОЖЬ
	|				КОНЕЦ
	|		ИНАЧЕ ЛОЖЬ
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ДанныеДокумента.Контрагент КАК Контрагент,
    |	ВЫБОР
    |       КОГДА НЕ ДанныеДокумента.СпособВыпискиАктовВыполненныхРабот = ЗНАЧЕНИЕ(Перечисление.СпособыВыпискиАктовВыполненныхРабот.ВБумажномВиде) 
    |           И НЕ ДанныеДокумента.ДатаПодписанияГЗ = ДАТАВРЕМЯ(1,1,1)
    |           ТОГДА ДанныеДокумента.ДатаПодписанияГЗ
    |       ИНАЧЕ ДанныеДокумента.Дата
    |   КОНЕЦ КАК Период,
    |	ВЫБОР
    |       КОГДА (НЕ ДанныеДокумента.СпособВыпискиАктовВыполненныхРабот = ЗНАЧЕНИЕ(Перечисление.СпособыВыпискиАктовВыполненныхРабот.ВБумажномВиде) 
    |           И НЕ ДанныеДокумента.ДатаПодписанияГЗ = ДАТАВРЕМЯ(1,1,1))
    |               ИЛИ ДанныеДокумента.СпособВыпискиАктовВыполненныхРабот = ЗНАЧЕНИЕ(Перечисление.СпособыВыпискиАктовВыполненныхРабот.ВБумажномВиде)
    |           ТОГДА ИСТИНА
    |       ИНАЧЕ ЛОЖЬ
    |   КОНЕЦ КАК Подписан,
    ...
    Запрос.УстановитьПараметр("Подписан",                      Реквизиты.Подписан);
...
Функция ТекстЗапросаВтРаботыУслуги(Запрос, ТекстыЗапроса)
	...
	|		 ИЛИ ТаблицаУслуги.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК))
...
Функция ТекстЗапросаТаблицаНДСЗаписиКнигиПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС0) КАК СтавкаНДС,
	|	NULL КАК ДатаОплаты,
...
Функция ПолучитьДанныеДляПечатнойФормыАктВыполненныхРаботР1(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ДанныеДокумента.Номер КАК Номер,
	|	ДанныеДокумента.Дата КАК ДатаДокумента,
	|	ДанныеДокумента.ДатаПодписанияГЗ КАК ДатаПринятия,
	|	ДанныеДокумента.Партнер КАК Партнер,
	|	ДанныеДокумента.Контрагент КАК Контрагент,
	|	ДанныеДокумента.Организация КАК Организация,
	|	ДанныеДокумента.Валюта КАК Валюта,
	|	ДанныеДокумента.Организация КАК Поставщик,
	|	ДанныеДокумента.Дата КАК ДатаВыполненияРабот,
	...
	|	ДанныеДокумента.Договор.Номер КАК НомерДоговора,
	|	ДанныеДокумента.Договор.Дата КАК ДатаДоговора,
	...
	|		ПО ДанныеДокумента.Валюта = Валюты.Ссылка
	|ГДЕ
	...
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(ДанныеДокумента.Содержание КАК СТРОКА(250))  <> """" 
	|			ТОГДА ВЫРАЗИТЬ(ДанныеДокумента.Содержание КАК СТРОКА(250))
	|		КОГДА ВЫРАЗИТЬ(ДанныеДокумента.Номенклатура.НаименованиеПолное КАК Строка(400)) <> """"
	|			ТОГДА ВЫРАЗИТЬ(ДанныеДокумента.Номенклатура.НаименованиеПолное КАК СТРОКА(400))
	|		ИНАЧЕ ДанныеДокумента.Номенклатура.Наименование
	...
	|	ДанныеДокумента.Номенклатура.ЕдиницаИзмерения.КодЭСФ КАК ЕдиницаИзмеренияКод,
	|	ДанныеДокумента.Номенклатура.ЕдиницаИзмерения.Наименование КАК ЕдиницаИзмеренияНаименование,
	...
	|	ДанныеДокумента.СуммаБезНДС КАК СтоимостьБезУчетаКосвенныхНалогов,
	|	ДанныеДокумента.СуммаБезНДС + ДанныеДокумента.СуммаНДС КАК СтоимостьСУчетомКосвенныхНалогов,
	...
	|	ДанныеДокумента.СуммаБезНДС / ДанныеДокумента.Количество КАК Цена,
	|	ДанныеДокумента.Количество КАК Количество,
	|	ДанныеДокумента.СуммаБезНДС КАК РазмерОборота,
	...
	|ИЗ
	|	АктВыполненныхРаботТаблицаТоваров КАК ДанныеДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтавкиНДС КАК СтавкиНДС
	|		ПО ДанныеДокумента.СтавкаНДС = СтавкиНДС.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка,
	|	НомерСтроки
...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	|	ЕСТЬNULL(СуммыДокументовВВалютеРегл.БазаНДСРегл, 0)     КАК БазаНДСРегл,
	|	ТаблицаДокумента.СтавкаНДС                              КАК СтавкаНДС,
...
Функция КомплектПечатныхФорм() Экспорт
	...
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "Р1", НСтр("ru = 'Акт выполненных работ (Р-1)'"), 1);
