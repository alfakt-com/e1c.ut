...
Процедура ПриИзмененииДоговораСервер()
	...
	Объект.НалогообложениеНДС = ЗначениеНастроекПовтИсп.НалогообложениеНДС(Объект.Организация, , Объект.Договор, Объект.НаправлениеДеятельности, Объект.Дата);
	ПриИзмененииНалогообложенияНДССервер();
    ...
    Если ТипДоговора = Перечисления.ТипыДоговоров.СПокупателем 
        И ЗначениеЗаполнено(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Договор, "СпособВыпискиАктовВыполненныхРабот")) Тогда
...
Функция ЗаполнитьЦеныПоСоглашениюСервер(ПоВсемСтрокам = Ложь)
	...
	ЦеныРассчитаны = ПродажиСервер.ЗаполнитьЦены(
		...
		Новый Структура( // Параметры заполнения
			"Дата, Валюта, Соглашение, Организация, РасчитыватьНаборы, НалогообложениеНДС, ПоляЗаполнения, КолонкиПоЗначению",
			Объект.Дата,
			Объект.Валюта,
			Объект.Соглашение,
			Объект.Организация,
