...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Если Объект.Ссылка.Проведен И Не (РольДоступна("ПолныеПрава") Или РольДоступна("swРазрешитьПерепроведениеЗаказовКлиентов")) Тогда
...
Процедура ДоговорПриИзмененииСервер()
	...
	Объект.БанковскийСчет = Справочники.БанковскиеСчетаОрганизаций.НайтиПоНаименованию("0EXX в Сбербанк АО ДБ, ТОО ""Альфа-КТ""");
...
Процедура ТоварыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	ПроверкаНаСпецЦену(ТекущиеДанные);
...
Функция ПолучитьЖанну()
	
	Возврат Справочники.Пользователи.НайтиПоНаименованию("Фёдорова Жанна Юрьевна");	
...
Процедура ТоварыВидЦеныПриИзменении(Элемент)
	...
	//ЦС Владимир
	//Если ПолучитьТекПользователя() <> ПолучитьЖанну() Тогда
...
Процедура ТоварыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	ПроверитьТовары();
	Если ПроверитьТовары() = Ложь тогда
...
&НаКлиенте
Процедура ТоварыВидЦеныНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	Если Элемент.ВидЦены = ПредопределенноеЗначение("Справочник.ВидыЦен.ПустаяСсылка") тогда
		Элемент.ВидЦены = Неопределено;
		Сообщить("Произвольный тип цен можно использовать только при ручной скидке!");
	КонецЕсли;
...
#Область Установка_Ключа_строки
...
&НаСервере
Процедура swУсловияОплатыПриИзмененииНаСервере()
