...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	УчетНДСУТ.ЗаполнитьСписокВыбораНалогообложенияПродажи(Элементы.НалогообложениеНДС, Объект.ХозяйственнаяОперация, Объект.НалогообложениеНДС);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	// СНТ
	СНТСерверПереопределяемый.ОбновитьРеквизитыСНТ(ЭтаФорма,,,"ГруппаСчетФактураТранспортныеНакладные");
	// Конец СНТ
...
Процедура ОбновитьТекстСчетаФактуры()
	...
	ТекстСчетФактура = ЗакупкиСервер.ПредставлениеСчетаФактурыВДокументеЗакупки(
...
&НаКлиенте
Процедура ПослеЗакрытияВопросаЗаписьПриСозданииСНТ(Результат, Параметры) Экспорт
