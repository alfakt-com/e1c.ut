...
Функция ТекстЗапросаВтВидыЗапасов(Запрос, ТекстыЗапроса)
	...
	|	, ТаблицаВидыЗапасов.ДокументПоступления.ЗакупкаПодДеятельность КАК ВидДеятельностиНДС 
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	ТекстЗапроса =
		...
		|					ТОГДА ВЫРАЗИТЬ((ВидыЗапасов.СуммаСНДС) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15,2))
...
Функция ТекстЗапросаТаблицаНДСПредъявленный(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИНАЧЕ ТаблицаВидыЗапасов.БазаНДСРегл КОНЕЦ) КАК СуммаБезНДС,
	|	СУММА(ЕСТЬNULL(ПересчитаннаяТаблицаДокумента.СуммаНДСРегл,ТаблицаВидыЗапасов.СуммаНДС)) КАК НДС,
	|	СУММА(ЕСТЬNULL(ПересчитаннаяТаблицаДокумента.СуммаНДСУпр, ТаблицаВидыЗапасов.СуммаНДС)) КАК НДСУпр,
	...
	|
	|ГДЕ
	|	ТаблицаВидыЗапасов.ВидДеятельностиНДС В (&ТипыНалогообложенияНДСПоступления)
	|	И &Организация <> ЗНАЧЕНИЕ(Справочник.Организации.УправленческаяОрганизация)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаВидыЗапасов.ВидЦенности,
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(ЕСТЬNULL(ПересчитаннаяТаблицаДокумента.СуммаНДСРегл,ТаблицаВидыЗапасов.СуммаНДС)) <> 0
	|";
...
Функция СформироватьПечатнуюФормуВозвратПоставщику(МассивОбъектов, ОбъектыПечати)
	...
	//Ахметов Ержан задача № 6476  08.12.2022 ++
	|	ВозвратТоваровПоставщику.ДокументПоступления КАК ДокументПоступления
	//Ахметов Ержан задача № 6476  08.12.2022 --
	|ИЗ
	|	Документ.ВозвратТоваровПоставщику КАК ВозвратТоваровПоставщику
	|ГДЕ
	...
	|	ВЫБОР
	|		КОГДА ВложенныйЗапрос.Ссылка.ВозвратПринятойМногооборотнойТары
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		ПредставлениеПоставщика                         = ФормированиеПечатныхФорм.ОписаниеОрганизации(ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Получатель, ДанныеПечати.Дата), "ПолноеНаименование");
		...
		//Ахметов Ержан задача № 6476  08.12.2022 ++
		Если ЗначениеЗаполнено(ДанныеПечати.ДокументПоступления)Тогда
			ТекстЭСФПриобретенияТоваров = ПолучитьЭСФПриобретенияТоваров(ДанныеПечати.ДокументПоступления);
			Если ТекстЭСФПриобретенияТоваров <> Неопределено Тогда
				ОбластьМакета                                   = Макет.ПолучитьОбласть("ДокументОснования");
				ОбластьМакета.Параметры.ДокументОснования 		= ТекстЭСФПриобретенияТоваров; 
				ТабличныйДокумент.Вывести(ОбластьМакета);
			КонецЕсли;
		КонецЕсли;
		//Ахметов Ержан задача № 6476  08.12.2022 --
...
//Ахметов Ержан задача № 6476  08.12.2022 ++
Функция ПолучитьЭСФПриобретенияТоваров(Ссылка)
	
	НомерИДата=Неопределено;
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.НомерВходящегоДокумента) И ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ДатаВходящегоДокумента) Тогда
			НомерИДата = "ЭСФ № " + ВыборкаДетальныеЗаписи.НомерВходящегоДокумента + " от "+Формат(ВыборкаДетальныеЗаписи.ДатаВходящегоДокумента,"ДФ=dd.MM.yyyy; ДЛФ=D")+"г.";
		Иначе
			НомерИДата = Неопределено;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ12(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ВозвратТоваровПоставщику.Менеджер.ФизическоеЛицо КАК Ответственный,
