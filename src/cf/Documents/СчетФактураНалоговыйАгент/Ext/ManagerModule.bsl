...
Процедура ЗаполнитьОплатыПоставщикам(ОтборРасчетов, ОплатыПоставщику) Экспорт
	...
	СтавкаНДС = ?(ЗначениеЗаполнено(ОтборРасчетов.СтавкаНДС), ОтборРасчетов.СтавкаНДС, Справочники.СтавкиНДС.НДС12);
	...
	Пока Выборка.Следующий() Цикл
		...
		// в УПК не вводим проверку на документ-основание, тк не контролируем заполнение документа оплаты
		Если ИСТИНА ИЛИ ВозможныйТипДокументаОснования.СодержитТип(ТипЗнч(Выборка.ДокументОснование)) Тогда
...
Функция ТекстЗапросаТаблицаЖурналУчетаСчетовФактур(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ПО ПодтверждениеОплатыНДСВБюджет.СчетФактура = ТаблицаРасшифровка.Ссылка
	|		И ПодтверждениеОплатыНДСВБюджет.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияОплатыНДСВБюджет.ПолученоПодтверждение)
	|ГДЕ
...
Процедура ИнициализироватьВтДокументыПоступления(Запрос)
	...
	|		Расчеты.ДолгРегл КАК СуммаОплаты
	|	ИЗ
...
Процедура ИнициализироватьВтПартииПрочихРасходов(Запрос)
	...
	//|	ПО
	//|		ДокументыПоступления.СтатьяРасходов =  СтатьиРасходов.Ссылка
...
Процедура СформироватьДвиженияВЖурналУчетаСчетовФактур(МассивРегистраторов, ПризнакОтменаПроведения = Ложь) Экспорт
	...
	Запрос.Текст = 	
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СчетФактурыНалоговыйАгент.Ссылка КАК СчетФактура,
	|	СчетФактурыНалоговыйАгент.Организация КАК Организация,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СчетФактураНалоговыйАгент КАК СчетФактурыНалоговыйАгент
	|		ПО ДанныеОплатыВБюджет.СчетФактура = СчетФактурыНалоговыйАгент.Ссылка
	|ГДЕ
	...
	|	И СчетФактурыНалоговыйАгент.Проведен
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СчетФактурыНалоговыйАгент.Ссылка,
	|	СчетФактурыНалоговыйАгент.Организация,
	|	НАЧАЛОПЕРИОДА(ЖурналУчетаСчетовФактур.ДатаСчетаФактуры, МЕСЯЦ),
	|	ЖурналУчетаСчетовФактур.ДатаСчетаФактуры
	|ИЗ
	|	Документ.СчетФактураНалоговыйАгент КАК СчетФактурыНалоговыйАгент
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЖурналУчетаСчетовФактур КАК ЖурналУчетаСчетовФактур
	|		ПО ЖурналУчетаСчетовФактур.СчетФактура = СчетФактурыНалоговыйАгент.Ссылка
	|ГДЕ
	|	СчетФактурыНалоговыйАгент.Ссылка В(&МассивРегистраторов)
	|	И СчетФактурыНалоговыйАгент.Проведен
	|ИТОГИ ПО
	|	СчетФактура";
...
Функция ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, МассивОбъектов) Экспорт 
	...
	|	ЗНАЧЕНИЕ(Справочник.ОтветственныеЛицаОрганизаций.ПустаяСсылка) КАК Кладовщик,
	|	ЗНАЧЕНИЕ(Справочник.ОтветственныеЛицаОрганизаций.ПустаяСсылка) КАК ДолжностьКладовщика,
	|	ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка) КАК БанковскийСчетКонтрагента,
	|	СчетФактура.Дата КАК ДатаОборотаПоРеализации,
...
Функция РезультатЗапросаПоОплатамНалоговыйАгент(НачалоПериода, КонецПериода, Организация, РасчетныйДокумент)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НАЧАЛОПЕРИОДА(Расчеты.Период) КАК Период,
	...
	|	СУММА(Расчеты.СуммаРегл) КАК Сумма
	|ПОМЕСТИТЬ Расчеты
	|ИЗ
	...
	// используем данные только двух документов ПриобретениеТоваровУслуг, ПриобретениеУслугПрочихАктивов
	|	И (Расчеты.Регистратор ССЫЛКА Документ.ПриобретениеТоваровУслуг
	|		ИЛИ Расчеты.Регистратор ССЫЛКА Документ.ПриобретениеУслугПрочихАктивов 
	|		И Расчеты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход))
	|	ИЛИ (Расчеты.Регистратор ССЫЛКА Документ.ВводОстатков и Расчеты.Регистратор.ВидОперации <> ЗНАЧЕНИЕ(Перечисление.ТипыОперацийВводаОстатков.ОстаткиПоАвансамКлиентов)
	|		И Расчеты.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход))
	|	И АналитикаУчетаПоПартнерамДанные.Организация В (&МассивОрганизаций)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	АналитикаУчетаПоПартнерамДанные.Договор,
	|	Расчеты.Валюта
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументОснование,
	...
	|	Расчеты.Договор КАК Договор,
	|	ЕСТЬNULL(ДоговорыКонтрагентов.ВидАгентскогоДоговора, ЗНАЧЕНИЕ(Перечисление.ВидыАгентскихДоговоров.Нерезидент)) КАК ВидАгентскогоДоговора,
	|	Расчеты.Валюта КАК Валюта,
	|	Расчеты.СуммаОплаты КАК СуммаОплаты,
	|	Расчеты.Сумма КАК Сумма,
	...
	|,	ВЫБОР 
	|		КОГДА Расчеты.ДокументОснование ССЫЛКА Документ.ПриобретениеТоваровУслуг 
	|			ТОГДА ВЫРАЗИТЬ(Расчеты.ДокументОснование КАК Документ.ПриобретениеТоваровУслуг).ВидПоступленияНДСНалоговогоАгента 
	|		КОГДА Расчеты.ДокументОснование ССЫЛКА Документ.ПриобретениеУслугПрочихАктивов 
	|			ТОГДА ВЫРАЗИТЬ(Расчеты.ДокументОснование КАК Документ.ПриобретениеУслугПрочихАктивов).ВидПоступленияНДСНалоговогоАгента
	|		ИНАЧЕ
	|			 """" 
	...
	|			И (ВЫБОР 
	|				КОГДА Расчеты.ДокументОснование ССЫЛКА Документ.ПриобретениеТоваровУслуг 
	|					ТОГДА ВЫРАЗИТЬ(Расчеты.ДокументОснование КАК Документ.ПриобретениеТоваровУслуг).ВидПоступленияНДСНалоговогоАгента = СчетФактураНалоговыйАгент.ВидОперацииПоступления 
	|				КОГДА Расчеты.ДокументОснование ССЫЛКА Документ.ПриобретениеУслугПрочихАктивов 
	|					ТОГДА ВЫРАЗИТЬ(Расчеты.ДокументОснование КАК Документ.ПриобретениеУслугПрочихАктивов).ВидПоступленияНДСНалоговогоАгента = СчетФактураНалоговыйАгент.ВидОперацииПоступления
	|				ИНАЧЕ ИСТИНА 
	|			КОНЕЦ)
