...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	КонецЕсли;
	Если Разделы.Итог("ТаможенныйСбор") = 0 Тогда
		НепроверяемыеРеквизиты.Добавить("СтатьяРасходовСбор");
    КонецЕсли;
    Если ВариантОтражения = 0 Тогда
        НепроверяемыеРеквизиты.Добавить("СчетРасчетовСБюджетом");
    КонецЕсли;
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		ЗаполнитьНДС = НЕ (Раздел.СтавкаНДС.Ставка = 0 ИЛИ Раздел.СтавкаНДС.ДляОсвобожденногоОборота ИЛИ Раздел.СтавкаНДС.МестоРеализацииНеРК);
    ...
    Если Разделы.Итог("ТаможенныйСбор") <>  ТаможенныйСбор Тогда
        ...
        ТекстОшибки = НСтр("ru='Не совпадает сумма таможенного сбора по документу и суммы таможенных сборов по разделам.'");
...
Функция ВременныеТаблицы()
	...
	|///////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаРазделов.ВидСтавкиПошлины КАК ВидСтавкиПошлины,
	...
	|	Товары.ПризнакПроисхождения КАК ПризнакПроисхождения,
	|	Разделы.ВидСтавкиПошлины КАК СпособПроисхождения,
	...
	|	ПО
	|		Товары.Номенклатура = ДанныеНоменклатуры.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|				Разделы.НомерДляСФ
	|			КОНЕЦ)
	|		И ПодобранныеГТД.НомерСтрокиГТД = (
	|			ВЫБОР КОГДА &НумероватьПоТоварам ТОГДА
	|				Товары.НомерСтроки
	|			ИНАЧЕ
	|				Разделы.НомерРаздела
	|			КОНЕЦ)
	|		И ПодобранныеГТД.КодТНВЭД = ЕСТЬNULL(ДанныеНоменклатуры.КодТНВЭД.Код, """")
	|		И ПодобранныеГТД.НаименованиеТовара = Товары.НаименованиеТовара
	|		И ПодобранныеГТД.СпособПроисхождения = Разделы.ВидСтавкиПошлины
	...
	Запрос.УстановитьПараметр("Товары",
		Товары.Выгрузить(, "НомерСтроки, АналитикаУчетаНоменклатуры, ВидЗапасов, НомерРаздела, СтранаПроисхождения, НомерДляСФ, НомерГТД, ХозяйственнаяОперация, НаименованиеТовара, Номенклатура, ПризнакПроисхождения"));
	Запрос.УстановитьПараметр("ТаблицаРазделов", Разделы.Выгрузить(, "НомерРаздела, НомерДляСФ, ВидСтавкиПошлины"));
...
Процедура ЗаполнитьНомераГТД(ВременныеТаблицы)
	...
	|	ГенерацияГТД.СпособПроисхождения,
	|	ГенерацияГТД.КодТНВЭД,
	|	ГенерацияГТД.НомерСтрокиГТД,
	...
	|			КОНЕЦ)
	|		И (ПодобранныеГТД.КодТНВЭД = ЕСТЬNULL(ДанныеНоменклатуры.КодТНВЭД.Код, """"))
	|		И (ПодобранныеГТД.НаименованиеТовара = (
	|   		ВЫБОР 
	| 				КОГДА  
	|					Товары.НаименованиеТовара = """"  
	|				ТОГДА  Товары.Номенклатура.Наименование 
