...
//Начало: МК Баубериков - 15.09.2020
&НаКлиенте
Процедура ЗакончитьВыборФайла(Результат, Адрес, Имя, ДопПараметры) Экспорт 
	Если Результат Тогда 
        ...
        РезультатЗагрузки	= ВыполнитьЗагрузкуНаСервере(АдресХранения, 2);
		
	КонецЕсли;
КонецПроцедуры   

&НаКлиенте
Процедура РаспределитьпоРазделам(Команда)
...
&НаСервере
Функция ВыполнитьЗагрузкуНаСервере(АдресХранения, ПерваяСтрока)

	Путь	= КаталогВременныхФайлов() + "temp.xlsx";
	...
	Для сч=ПерваяСтрока по ТабДок.ВысотаТаблицы цикл
		ТекстКодНоменклатуры                      = ТабДок.Область("R"+СтрЗаменить(Строка(сч),Символы.НПП,"")+"C2").Текст;
		ТекстСуммаПошлины                         = ТабДок.Область("R"+СтрЗаменить(Строка(сч),Символы.НПП,"")+"C3").Текст;
		ТекстТаможенныйСбор                       = ТабДок.Область("R"+СтрЗаменить(Строка(сч),Символы.НПП,"")+"C4").Текст;
		ТекстТаможеннаяСтоимость                  = ТабДок.Область("R"+СтрЗаменить(Строка(сч),Символы.НПП,"")+"C5").Текст;
		ТекстСуммаНДС                             = ТабДок.Область("R"+СтрЗаменить(Строка(сч),Символы.НПП,"")+"C6").Текст;
		ТекстНомерРаздела                         = ТабДок.Область("R"+СтрЗаменить(Строка(сч),Символы.НПП,"")+"C7").Текст;
		ТекстНаименование                         = ТабДок.Область("R"+СтрЗаменить(Строка(сч),Символы.НПП,"")+"C8").Текст;
		ТекстВидСтавкиПошлины                     = ТабДок.Область("R"+СтрЗаменить(Строка(сч),Символы.НПП,"")+"C9").Текст;
		ТекстСтавкаНДС                            = ТабДок.Область("R"+СтрЗаменить(Строка(сч),Символы.НПП,"")+"C10").Текст;

		//Период                            = Дата(СтрЗаменить(Прав(ТекстПериод,4)+Лев(ТекстПериод,5),".",""));
	    Номенклатура                            = Справочники.Номенклатура.НайтиПоКоду(ТекстКодНоменклатуры);
		СуммаПошлины                            = ?(НЕ ПустаяСтрока(ТекстСуммаПошлины),Число(ТекстСуммаПошлины),0);
		ТаможенныйСбор                          = ?(НЕ ПустаяСтрока(ТекстТаможенныйСбор),Число(ТекстТаможенныйСбор),0);
		ТаможеннаяСтоимость                     = ?(НЕ ПустаяСтрока(ТекстТаможеннаяСтоимость),Число(ТекстТаможеннаяСтоимость),0);
		СуммаНДС                                = ?(НЕ ПустаяСтрока(ТекстСуммаНДС),Число(ТекстСуммаНДС),0);
		НомерРаздела                            = ?(НЕ ПустаяСтрока(ТекстНомерРаздела),Число(ТекстНомерРаздела),0);
		Наименование                            = СокрЛП(ТекстНаименование);
		
		Отбор = Новый Структура;
		Отбор.Вставить("Номенклатура", Номенклатура);
		Отбор.Вставить("НомерСтроки", НомерРаздела);
		ИскСтроки = Объект.Товары.НайтиСтроки(Отбор);
		
		Для каждого ИскСтр из ИскСтроки цикл
			
			ИскСтр.НомерРаздела = НомерРаздела;
			ИскСтр.СуммаПошлины = СуммаПошлины;
			ИскСтр.ТаможенныйСбор = ТаможенныйСбор;
			ИскСтр.ТаможеннаяСтоимость = ТаможеннаяСтоимость;
			ИскСтр.СуммаНДС = СуммаНДС;
			ИскСтр.НаименованиеТовара = Наименование;
			
		КонецЦикла;
		
		тзРазделы = Объект.Товары.Выгрузить();
		тзРазделы.Свернуть("НомерРаздела","СуммаПошлины,ТаможенныйСбор,ТаможеннаяСтоимость,СуммаНДС");
		
		Объект.Разделы.Очистить();
		
		Для каждого стрРаздел из тзРазделы цикл
			
			НоваяСтрокаРаздел = Объект.Разделы.Добавить();
			НоваяСтрокаРаздел.НомерРаздела        = стрРаздел.НомерРаздела;
            НоваяСтрокаРаздел.СуммаПошлины        = стрРаздел.СуммаПошлины;
            НоваяСтрокаРаздел.ТаможенныйСбор      = стрРаздел.ТаможенныйСбор;
            НоваяСтрокаРаздел.СуммаНДС            = стрРаздел.СуммаНДС;
            НоваяСтрокаРаздел.ТаможеннаяСтоимость = стрРаздел.ТаможеннаяСтоимость;

		КонецЦикла;

			
		
		//Если Номенклатура<>Неопределено и НЕ Номенклатура.Пустая() тогда
		//	НоваяСтрока = Объект.Список.Добавить();
		//	НоваяСтрока.Номенклатура                            = Номенклатура;
		//	НоваяСтрока.СуммаПошлины                            = СуммаПошлины;
		//	НоваяСтрока.ТаможенныйСбор                          = ТаможенныйСбор;
		//	НоваяСтрока.ТаможеннаяСтоимость                     = ТаможеннаяСтоимость;
		//	НоваяСтрока.СуммаНДС                                = СуммаНДС;
		//	НоваяСтрока.НомерРаздела                            = НомерРаздела;

		//Иначе
		//	Сообщить("Ошибка загрузки строки: "+ Строка(сч)+ " . Проверьте поля: период, подразделение, должность, вид бонуса");
		//КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьВидСтавкиПошлины(Команда)
	
	Если Элементы.Разделы.ВыделенныеСтроки.Количество()>0 Тогда
		ВидСтавкиПошлины = Неопределено;
		
		ОткрытьФорму("Перечисление.СпособыПроисхожденияТоваров.ФормаВыбора",,,,,, Новый ОписаниеОповещения("УстановитьВидСтавкиПошлиныЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;   	
	
КонецПроцедуры

&НаКлиенте
Процедура УстановитьВидСтавкиПошлиныЗавершение(Результат, ДополнительныеПараметры) Экспорт
    
	ВидСтавкиПошлины = Результат;
	Если ЗначениеЗаполнено(ВидСтавкиПошлины) Тогда
		УстановитьВидСтавкиПошлиныСервер(ВидСтавкиПошлины);
		
		Для каждого стр из Элементы.Разделы.ВыделенныеСтроки цикл
		
			Раздел = Объект.Разделы.Получить(стр);
...
&НаСервере
Процедура УстановитьВидСтавкиПошлиныСервер(ВидСтавкиПошлины)
	
	ЗаполнитьПоляТабчасти("Разделы", Новый Структура("ВидСтавкиПошлины", ВидСтавкиПошлины));
...
Процедура ЗаполнитьСтавкуНДС(Команда)
	...
	Если Элементы.Разделы.ВыделенныеСтроки.Количество()>0 Тогда
		...
		ОткрытьФорму("Справочник.СтавкиНДС.ФормаВыбора",,,,,, Новый ОписаниеОповещения("УстановитьСтавкуНДСЗавершение", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли; 
...
Процедура УстановитьСтавкуНДСЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЗначениеЗаполнено(СтавкаНДС) Тогда
		УстановитьСтавкуНДССервер(СтавкаНДС);
...
&НаСервере
Процедура УстановитьСтавкуНДССервер(СтавкаНДС)
	
	ЗаполнитьПоляТабчасти("Разделы", Новый Структура("СтавкаНДС", СтавкаНДС));
		
КонецПроцедуры


//Конец: МК Баубериков - 15.09.2020
...
&НаКлиенте
Процедура РазделыСпециальнаяПошлинаПриИзменении(Элемент)
	Раздел = Элементы.Разделы.ТекущиеДанные;
	ПересчитатьПоля(Раздел, "СуммаНДС", Раздел);
	ЗаполнитьЗначенияСвойств(ЭтаФорма, РазностиИтогов(Объект.Разделы, Объект.Товары));
КонецПроцедуры
...
Процедура РазделыТаможенныйСборПриИзменении(Элемент)
	Раздел = Элементы.Разделы.ТекущиеДанные;
	ПересчитатьПоля(Раздел, "ТаможенныйСбор, СуммаНДС", Раздел);
	ЗаполнитьЗначенияСвойств(ЭтаФорма, РазностиИтогов(Объект.Разделы, Объект.Товары));
	//ПересчитатьТаможеннуюСтоимостьТоваров(Раздел,Объект.Товары);
КонецПроцедуры
...
&НаКлиенте
Процедура ТоварыСпециальнаяПошлинаПриИзменении(Элемент)
	Товар = Элементы.Товары.ТекущиеДанные;
	ПересчитатьПоля(Товар, "СуммаНДС", ДанныеТекущегоРаздела(Элементы.Разделы));
	ЗаполнитьЗначенияСвойств(ЭтаФорма, РазностиИтогов(Объект.Разделы, Объект.Товары));
КонецПроцедуры
...
&НаКлиенте
Процедура ТоварыТаможенныйСборПриИзменении(Элемент)
	Товар = Элементы.Товары.ТекущиеДанные;
	ПересчитатьПоля(Товар, "СуммаНДС", ДанныеТекущегоРаздела(Элементы.Разделы));
	ЗаполнитьЗначенияСвойств(ЭтаФорма, РазностиИтогов(Объект.Разделы, Объект.Товары));
КонецПроцедуры
...
Процедура ТоварыПризнакПроисхожденияПриИзменении(Элемент)
	...
	Если Объект.Проведен Тогда
		МодификацияКонфигурацииКлиентПереопределяемый.КлючевоеПолеИсточникаПроисхожденияПриИзменении(Элементы.Товары.ТекущиеДанные);
	КонецЕсли;
...
Процедура РаспределитьНаТовары(Команда)
    РаспределитьНаТоварыРазделы();
КонецПроцедуры    

&НаКлиенте
Процедура РаспределитьВсе(Команда)
    РаспределитьНаТоварыРазделы(Истина);
КонецПроцедуры

&НаКлиенте
Процедура РаспределитьНаТоварыРазделы(ВсеРазделы = Ложь)
    ...
    КонецЕсли;
	РаспределитьНаТоварыСервер(ВсеРазделы);
...
Процедура ПересчетКурсаВалютыСервер()
	...
	Запрос.УстановитьПараметр("КодВалюты", "398");
...
&НаСервере
Процедура РаспределитьНаТоварыСервер(ВсеРазделы = Ложь)
	Отбор = Новый Структура("НомерРаздела");
    Если ВсеРазделы Тогда
        Разделы = Объект.Разделы;
    Иначе
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Если ТоварыРаздела.Количество() > 0 Тогда
			...
			КонецЕсли;
			НовыеСуммы = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(Раздел.СпециальнаяПошлина, Коэффициенты);
			Если НовыеСуммы <> Неопределено Тогда
				ТоварыРаздела.ЗагрузитьКолонку(НовыеСуммы, "СпециальнаяПошлина");
			КонецЕсли;
			...
			КонецЕсли;
			НовыеСуммы = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(Раздел.ТаможенныйСбор, Коэффициенты);
			Если НовыеСуммы <> Неопределено Тогда
				ТоварыРаздела.ЗагрузитьКолонку(НовыеСуммы, "ТаможенныйСбор");
			КонецЕсли;
		КонецЕсли;
		...
		Для каждого Товар Из ТоварыРаздела Цикл
			...
			Объект.Товары[Товар.НомерСтроки-1].СпециальнаяПошлина = ?(Раздел.СпециальнаяПошлина > 0., Товар.СпециальнаяПошлина, 0.);
			Объект.Товары[Товар.НомерСтроки-1].СуммаНДС = ?(Раздел.СуммаНДС > 0., Товар.СуммаНДС, 0.);
			Объект.Товары[Товар.НомерСтроки-1].ТаможенныйСбор = ?(Раздел.ТаможенныйСбор > 0., Товар.ТаможенныйСбор, 0.);
		КонецЦикла;
	КонецЦикла;
...
Процедура РаспределитьТаможенныйСбор()
	...
	Если ЗначениеЗаполнено(Объект.ТаможенныйСбор) Тогда
        ...
        КонецЦикла;
        НовыеСуммы = ОбщегоНазначения.РаспределитьСуммуПропорциональноКоэффициентам(Объект.ТаможенныйСбор, Коэффициенты);
        Если НовыеСуммы = Неопределено Тогда
			Возврат;
        КонецЕсли;
        Для каждого ТекРаздел Из Разделы Цикл
            ТекРаздел.ТаможенныйСбор = НовыеСуммы[ТекРаздел.НомерСтроки-1];       
        КонецЦикла;
	КонецЕсли;
...
Процедура РассчитатьПоТоварамСервер()
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Если ТоварыРаздела.Количество() > 0 Тогда
			...
			Раздел.ТаможенныйСбор = 0.;
			Раздел.СпециальнаяПошлина = 0.;
			ПересчитатьПоля(Раздел, "СуммаПошлины, СуммаНДС", Раздел);
			Для Каждого Товар Из ТоварыРаздела Цикл
				...
				Раздел.СпециальнаяПошлина = Раздел.СпециальнаяПошлина + Товар.СпециальнаяПошлина;
				Раздел.СуммаНДС = Раздел.СуммаНДС + Товар.СуммаНДС;
				Раздел.ТаможенныйСбор = Раздел.ТаможенныйСбор + Товар.ТаможенныйСбор;
			КонецЦикла;
	...
	Если СтрНайти(ПоляПересчета, "суммандс")>0 Тогда
		ТекущиеДанные.СуммаНДС =
			ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(
				ТекущиеДанные.ТаможеннаяСтоимость + ТекущиеДанные.СуммаПошлины + ТекущиеДанные.СпециальнаяПошлина + ТекущиеДанные.ТаможенныйСбор,
...
Процедура НастроитьЭлементыФормы()
    ...
    Если Объект.ВариантОтражения > 0 Тогда
        Элементы.СчетРасчетовСБюджетом.Видимость = Истина;
        Элементы.ЗачетОплатыФорма.Видимость      = Ложь;
    Иначе
        Элементы.СчетРасчетовСБюджетом.Видимость = Ложь;
        Элементы.ЗачетОплатыФорма.Видимость      = Истина;
    КонецЕсли;
...
//кт Начало Апаев_О [21.09.2022] - Задача № 5944
&НаСервере
Функция кт_ОпределитьКоличествоСтрокРаздела(ОтборТабчасти)
	
	ТоварыРаздела = Объект.Товары.Выгрузить(ОтборТабчасти); 
	
	Возврат ТоварыРаздела.Количество();
	
КонецФункции

&НаКлиенте
Процедура кт_ЗагрузитьExcel(Команда)
	
	Если Элементы.ТоварыПоказатьВсеТовары.Пометка = Истина Тогда
		Возврат;
	КонецЕсли;
	
	РазделОтбора  	  = ДанныеТекущегоРаздела(Элементы.Разделы).НомерРаздела;
	ОтборТабчасти 	  = Новый Структура("НомерРаздела", РазделОтбора);
	КоличествоТоваров = кт_ОпределитьКоличествоСтрокРаздела(ОтборТабчасти);
	...
	Исключение
		...
		Сообщить("Второй лист не найден!");
		Возврат;
	КонецПопытки;
	...
	НачатьПрисвоение = Ложь;
	НомерСтроки	     = 1;
	
	Для СтрокаОтсчета = 1 По КоличествоСтрок Цикл
		
		ТекущееЗначение = Excel.Cells(СтрокаОтсчета, 2).Text;
		
		Если НачатьПрисвоение = Истина Тогда
			
			ТекущееЗначение = СтрЗаменить(ТекущееЗначение, Символы.ПС,"");
			ТекущееЗначение = СтрЗаменить(ТекущееЗначение, Символы.ВК,"");
			
			кт_ПрисвоитьНаименованиеНоменклатурыТовара(ОтборТабчасти, НомерСтроки, ТекущееЗначение);	
			
			НомерСтроки = НомерСтроки + 1;	
			
		КонецЕсли;
		
		Если ТекущееЗначение = "Наименование товара" Тогда
			НачатьПрисвоение = Истина;	
		КонецЕсли;
		
		Если НомерСтроки > КоличествоТоваров Тогда
			Прервать;
		КонецЕсли;
...
&НаСервере
Процедура кт_ПрисвоитьНаименованиеНоменклатурыТовара(ОтборТабчасти, НомерСтроки, НаименованиеТовара)
	
	Строки = Объект["Товары"].НайтиСтроки(ОтборТабчасти);	
	Объект.Товары[Объект.Товары.Индекс(Строки[НомерСтроки - 1])].НаименованиеТовара = НаименованиеТовара;
	
КонецПроцедуры
//кт Окончание Апаев_О [21.09.2022]
