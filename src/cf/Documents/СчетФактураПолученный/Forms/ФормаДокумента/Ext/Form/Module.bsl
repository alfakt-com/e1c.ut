...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ОбновитьСостояниеЭД" ИЛИ 
		ИмяСобытия = ЭСФКлиентСервер.ИмяСобытияЗаписьЭСФ() Тогда
		УстановитьТекстСостоянияЭДНаСервере();
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	// ЭСФ
	ПараметрыЗаписи.Вставить("АктуальныйЭСФ", АктуальныйЭСФ);
	// Конец ЭСФ
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.ЭСФ") Тогда	
...
Процедура НомерИсправленияПриИзменении(Элемент)
	...
	Если Объект.Исправление И ЗначениеЗаполнено(Объект.НомерИсправления) Тогда
		Объект.Номер = Объект.НомерИсправления;
	КонецЕсли;
...
Процедура ОткрытьИсходныеСчетаФактуры()
	...
	СтруктураПараметров.Вставить("Заголовок", НСтр("ru='Дополнительный к счетам-фактурам (%КоличествоДокументов%)'"));
...
Процедура ОбработатьИзменениеСчетФактурыОснования(ВыбранныйСчетФактура)
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Для выбранного документа уже введен дополнительный счет-фактура.'");
...
Процедура НастроитьВидимостьВалютыКодаОперации()
	...
	Элементы.ДекорацияКодВидаОперации.Видимость = ОтключенныйФункционал И НЕ КодыВидовОперацийНаУвеличениеУменьшение;
...
Процедура ЗаполнитьСписокВыборкаДатыЗаписиКнигиПокупок(СписокВыбора, Знач ДатаПолучения, ДатаСоставления = Неопределено)
	...
	КонецЕсли;
	СтрокаШаблон = НСтр("ru = 'к %Квартал% кварталу %Год% г.'");
