...
Функция ИсправляемыйСчетФактураПоОснованию(Ссылка, Основание, Организация = Неопределено) Экспорт
	...
	|ИЗ
	|	Документ.ВозвратТоваровПоставщику.Товары КАК ДанныеВозврата
	|ГДЕ
	...
	|ИЗ
	|	Документ.ВозвратТоваровПоставщику.Товары КАК ДанныеОснования
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.ВозвратТоваровПоставщику.Товары КАК ДанныеВозврата
	|	ПО
	...
	|	И НЕ СчетФактураПолученный.Корректировочный
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция ТребуетсяСозданиеКорректировочного(СчетФактура, Исходный = Истина) Экспорт
	...
	Если Исходный Тогда
		...
		|		ПО ОснованияСчетаФактуры.ДокументОснование = ВозвратТоваровПоставщику.ДокументПоступления
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТребуетсяОформлениеСчетаФактуры КАК ТребуетсяСчетФактура
		|		ПО (ВозвратТоваровПоставщику.Ссылка = ТребуетсяСчетФактура.Основание)
		|ГДЕ
		|	ОснованияСчетаФактуры.Ссылка = &Ссылка";
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	ПараметрыУчетнойПолитики = РегистрыСведений.УчетнаяПолитикаОрганизаций.ПараметрыУчетнойПолитики(Реквизиты.Организация, Реквизиты.ДатаПолучения);
	...
	Запрос.УстановитьПараметр("ПрименяетсяУчетНДСПоФактическомуИспользованию", ?(ПараметрыУчетнойПолитики = Неопределено, Ложь, ПараметрыУчетнойПолитики.ПрименяетсяУчетНДСПоФактическомуИспользованию));
	...
	Если ЗначениеЗаполнено(Реквизиты.ДатаИсправления)
		...
		Если Реквизиты.Корректировочный Тогда
			ИнформацияПоИсправлению = НСтр("ru='Исправление %НомерИсправления% от %ДатаИсправления% (дополнительный)'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		КонецЕсли;
...
#Область ТекстВозвратТоваровПоставщику
	...
	|
	|ГДЕ
	|	ТаблицаВидыЗапасов.Ссылка.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратТоваровПоставщику),
...
#Область ТекстОтчетКомиссионераВознаграждение
	...
	|	ЛОЖЬ КАК ПрошлыеКорректировки,
	|	ВЫБОР
	|		КОГДА (ОтчетКомиссионера.СтавкаНДСВознаграждения = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС) 
	|			ИЛИ ОтчетКомиссионера.СтавкаНДСВознаграждения = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК)) 
...
#Область ТекстВозвратТоваровПоставщику
	...
	|			ТОГДА ВЫРАЗИТЬ(ТаблицаВидыЗапасов.СуммаСНДС * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ СуммыДокументовВВалютеРегл.СуммаБезНДСРегл + СуммыДокументовВВалютеРегл.СуммаНДСРегл
	|	КОНЕЦ * (-1) КАК СуммаПоСчетуФактуреЖурнал,
	|	ВЫБОР
	|		КОГДА &ВалютаСчетаФактуры = &ВалютаОснования
	|			ТОГДА ТаблицаВидыЗапасов.СуммаНДС
	|		КОГДА ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаБезНДСРегл,0) = 0
	...
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ВозвратКомиссионногоТовара,
	|	"""" КАК КодВидаОперации
	|ИЗ
	|	Документ.ВозвратТоваровПоставщику.ВидыЗапасов КАК ТаблицаВидыЗапасов
	|
	...
	|	ПО 
	|		ТаблицаВидыЗапасов.Ссылка = ВтДокументыОснования.ДокументОснование
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РегистрСведений.СуммыДокументовВВалютеРегл КАК СуммыДокументовВВалютеРегл
	|	ПО 
...
Функция ТекстЗапросаВтТаблицаДанныхДляЖурнала(Запрос, ТекстыЗапроса)
	...
	|   ТаблицаДанныхДляЖурнала.СтавкаНДС,
	|   NULL КАК СтранаНазначения,
	|   &СчетФактураОснование КАК ИсходныйСчетФактура,
	|   ТаблицаДанныхДляЖурнала.СуммаРегл,
	|   ТаблицаДанныхДляЖурнала.СуммаНДСРегл
	|
	...
	|		ТаблицаДанных.СуммаРегл,
	|		ТаблицаДанных.СуммаНДСРегл
	|
	|	ИЗ
	|		(ВЫБРАТЬ
	|			МАКСИМУМ(ТаблицаДокументов.НалогообложениеНДС) КАК НалогообложениеНДС,
	|			МАКСИМУМ(ТаблицаДокументов.СтавкаНДС) КАК СтавкаНДС,
	...
	|   	ЕСТЬNULL(ВЫРАЗИТЬ(ТаблицаДанныхИсправление.Основание КАК Документ.СчетФактураПолученный).Номер, &Номер) КАК НомерСчетаФактуры,
	...
	|			ИНАЧЕ """"
	|		КОНЕЦ КАК КодВидаОперацииКомиссия,
	|		ТаблицаДанныхИсправление.СтавкаНДС,
	|		ТаблицаДанныхИсправление.СуммаРегл,
	|		ТаблицаДанныхИсправление.СуммаНДСРегл
	|		
	|	ИЗ
	|		(ВЫБРАТЬ
	...
	|					КОНЕЦ) КАК КодВидаСделки,
	|			ТаблицаДокументов.СчетФактураВыданный
	|			,&СчетФактураОснование КАК Основание
	|		ИЗ
	|			(ВЫБРАТЬ
	...
	|			ИНАЧЕ """"
	|		КОНЕЦ КАК КодВидаОперацииКомиссия,
	|		ТаблицаДанныхКорректировка.СтавкаНДС,
	|		ТаблицаДанныхКорректировка.СуммаРегл,
	|		ТаблицаДанныхКорректировка.СуммаНДСРегл
	|
	|	ИЗ
	|		(ВЫБРАТЬ
	...
	|			МАКСИМУМ(ТаблицаДокументов.НалогообложениеНДС) КАК НалогообложениеНДС,
	|		
	|			МАКСИМУМ(ТаблицаДокументов.СтавкаНДС),
	|			СУММА(ТаблицаДокументов.СуммаБезНДС + ТаблицаДокументов.СуммаНДС) КАК СуммаРегл,
	|			СУММА(ТаблицаДокументов.СуммаНДС) КАК СуммаНДСРегл,
	|		
...
Функция ТекстЗапросаТаблицаЖурналУчетаСчетовФактур(Запрос, ТекстыЗапроса, Регистры)
	...
	|,	ТаблицаДанныхДляЖурнала.СтавкаНДС					                    	КАК СтавкаНДС,
	|	ТаблицаДанныхДляЖурнала.СтранаНазначения			                    	КАК СтранаНазначения,
	|	ТаблицаДанныхДляЖурнала.ИсходныйСчетФактура			                    	КАК ИсходныйСчетФактура,
	|	ТаблицаДанныхДляЖурнала.СуммаРегл					                    	КАК СуммаРегл,
	|	ТаблицаДанныхДляЖурнала.СуммаНДСРегл				                    	КАК СуммаНДСРегл
	|
	|ИЗ 
