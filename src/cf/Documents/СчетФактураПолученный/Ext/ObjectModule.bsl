...
#Область КорректировкаПриобретения
	...
	Для Каждого СтрТЧ Из ДокументыОснования Цикл
		...
		Если ТипЗнч(СтрТЧ.ДокументОснование) = Тип("ДокументСсылка.КорректировкаПриобретения")
			ИЛИ ТипЗнч(СтрТЧ.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровМеждуОрганизациями")
			ИЛИ ТипЗнч(СтрТЧ.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровПоставщику") Тогда
			...
			Иначе
				ОперацияОснования = Неопределено;
			КонецЕсли;
				
			ЕстьКорректировки = ЕстьКорректировки Или ОперацияОснования = ХозОперации.КорректировкаПоСогласованиюСторон Или (ТипЗнч(СтрТЧ.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровМеждуОрганизациями")
									ИЛИ ТипЗнч(СтрТЧ.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровПоставщику"));
		...
		Если НЕ ЕстьДополнения 
			И (ТипЗнч(СтрТЧ.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровПоставщику")
				ИЛИ ТипЗнч(СтрТЧ.ДокументОснование) = Тип("ДокументСсылка.ВозвратТоваровМеждуОрганизациями")) Тогда
	...
	Если НетИсходногоСчетФактуры Тогда
		Если КоличествоОснований > 1 Тогда
			ТекстСообщения = НСтр("ru = 'Перед добавлением дополнительного счета-фактуры создайте исходный счет-фактуру для всех документов списка ""Документы-основания"".'");
		Иначе
			ТекстСообщения = НСтр("ru = 'Перед добавлением дополнительного счета-фактуры создайте исходный счет-фактуру для документа-основания.'");
		КонецЕсли;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "ДокументыОснованияПредставление", , Отказ);
	КонецЕсли;
	...
	ИначеЕсли ЕстьДополнения И НЕ Корректировочный Тогда
		
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Счет-фактура должен быть дополнительным, т.к. среди документов основания есть возврат товаров поставщику'"));
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Документы.СчетФактураПолученный.ИнициализироватьДанныеДокумента(Ссылка, ДополнительныеСвойства, "ОтражениеДокументовВРеглУчете,ЖурналУчетаСчетовФактур,РеестрДокументов,НДСКВозмещению");
...
Функция ПолучитьПараметрыСчетаФактурыПоОснованиям()
	...
	Запрос.Параметры.Вставить("ДокументОснование_КорректировкаПриобретения", Новый Массив);
	Запрос.Параметры.Вставить("ДокументОснование_ВозвратТоваровПоставщику", Новый Массив);
	...
	Для Каждого ТипОснования Из ТипыОснований Цикл
		...
		Если ИмяОбъекта = "КорректировкаПриобретения"
			...
			|ИЗ
			|	Документ.ВозвратТоваровПоставщику.Товары КАК ТекущийДокумент
			|ГДЕ
			|	ТекущийДокумент.Ссылка В(&ДокументОснование_ВозвратТоваровПоставщику)
			|	И ТекущийДокумент.Ссылка.ХозяйственнаяОперация В (
			...
			|ИЗ
			|	ВТ_ОснованияИсходных КАК ОснованияИсходных
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВозвратТоваровПоставщику.Товары КАК ВозвратТоваровПоставщику
				...
				Пока ВыборкаПоИсходнымСчетамФактуры.Следующий() Цикл
					...
					Пока ВыборкаПоДокументам.Следующий() Цикл
						...
						Если РеквизитыОснований.Найти(ВыборкаПоДокументам.ДокументОснование) = Неопределено 
							...
							Если ВыборкаПоДокументам.ИсправлениеИсходногоДокумента Тогда
