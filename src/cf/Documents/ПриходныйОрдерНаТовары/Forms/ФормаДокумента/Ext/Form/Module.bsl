...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Ахметов Ержан Задача № 6486 07.12.2022 ++ 
	Если ПроверитьНаличиеПоступленияТовара(Объект.Распоряжение) = Ложь  Тогда
		Отказ=Истина;
		Возврат;
	КонецЕсли;
	//Ахметов Ержан Задача № 6486 07.12.2022 --
...
//Ахметов Ержан Задача № 6486 07.12.2022 ++ 
&НаСервере
Функция ПроверитьНаличиеПоступленияТовара(Распоряжение)
	...
	Если ТипЗнч(Распоряжение) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		...
		Запрос.УстановитьПараметр("ЗаказПоставщику", Распоряжение);
		...
		КонецЦикла;
		Проверка = Ложь;
		Сообщить("Доступ закрыт! У документа "+Распоряжение+" отсутствует документ Приобретение товаров и услуг!");
	КонецЕсли;
