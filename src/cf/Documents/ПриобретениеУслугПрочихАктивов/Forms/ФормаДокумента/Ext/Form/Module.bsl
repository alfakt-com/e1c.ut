...
Процедура НастроитьПредставлениеСчетаФактуры(НеТребуетсяВводСчетаФактуры = Неопределено)
	...
	Если НеТребуетсяВводСчетаФактуры = Неопределено ИЛИ (НеТребуетсяВводСчетаФактуры = Ложь И ЕстьЗаписьКнигиПокупок) Тогда
		НеТребуетсяВводСчетаФактуры = НеТребуетсяВводСчетаФактуры(Объект.НалогообложениеНДС) ИЛИ ЕстьЗаписьКнигиПокупок;
	КонецЕсли;
...
Процедура АктуализироватьЗакупкуПодДеятельность(Заполнить = Истина)
	...
	Если Заполнить Тогда
		Объект.ЗакупкаПодДеятельность = ЗначениеНастроекПовтИсп.ЗакупкаСПотреблениемПодДеятельность(
											Объект.Организация, 
											, // Склад
											Объект.Договор, 
...
Функция ПоместитьВыделенныеСтрокиВХранилище(Знач ВыделенныеСтроки)
	...
	ТаблицаСтрок.Колонки.Добавить("Сумма", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,2)));
	ТаблицаСтрок.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,3)));
	ТаблицаСтрок.Колонки.Добавить("СтавкаНДС", Справочники.ТипВсеСсылки());
