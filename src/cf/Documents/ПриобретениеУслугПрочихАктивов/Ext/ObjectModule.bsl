...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для ТекИндекс = 0 По Расходы.Количество()-1 Цикл
		...
		Если ПартионныйУчетВключен
			И ПолучитьФункциональнуюОпцию("ОтключенныйФункционал")
...
Процедура ЗаполнитьУсловияЗакупок(Знач УсловияЗакупок) Экспорт
	...
	ЗакупкаПодДеятельность = ЗначениеНастроекПовтИсп.ЗакупкаСПотреблениемПодДеятельность(
								Организация, 
								, // Склад 
								Договор,
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаписьКнигиПокупок") Тогда
