...
Функция ДанныеДляЕГАИС(Объект) Экспорт
	...
	|			ТОГДА ПРЕДСТАВЛЕНИЕ(Товары.Номенклатура.ЕдиницаИзмерения.Код)
	...
	|	,Товары.НаименованиеЕдиницыИзмерения,
	|	Товары.КодЕдиницыИзмерения
	|ИЗ
	|	Товары КАК Товары
	|		
	...
	Для Каждого СтрокаТЧ Из ТаблицаТоваровЧека Цикл
		...
		НоваяСтрока.Вставить("НаименованиеЕдиницыИзмерения", СтрокаТЧ.НаименованиеЕдиницыИзмерения);
		НоваяСтрока.Вставить("КодЕдиницыИзмерения", СтроковыеФункцииКлиентСервер.СтрокаВЧисло(СтрокаТЧ.КодЕдиницыИзмерения));
...
Функция ЧекЕГАИСXML(ДокументСсылка, ДополнительныеПараметры)
	...
	ПараметрыФормированияТекстаЗапроса = ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеЕГАИС_ПараметрыФормированияТекстаЗапросаВложенныхШтрихкодов();
	ПараметрыФормированияТекстаЗапроса.ДокументСсылка                  = Неопределено;
	ТекстыЗапроса.Добавить(
		ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеЕГАИС_ТекстЗапросаВложенныхШтрихкодовПоДокументу(ПараметрыФормированияТекстаЗапроса),
		"ВложенныеШтрихкоды");
	...
	РезультатыЗапроса = ИСМПТВыбытиеКодовМаркировкиСервер.ИнтеграцияИС_ВыполнитьПакетЗапросов(Запрос, ТекстыЗапроса);
	
	Возврат ИСМПТВыбытиеКодовМаркировкиСервер.ЧекиЕГАИС_ЧекXML(ДокументСсылка, РезультатыЗапроса, МенеджерВременныхТаблиц);
...
Функция ЧекЕГАИСОтменаXML(ДокументСсылка, ДополнительныеПараметры)
	...
	Возврат ИСМПТВыбытиеКодовМаркировкиСервер.ЧекиЕГАИС_ЧекЕГАИСXML(ДокументСсылка, РезультатыЗапроса,
		МенеджерВременныхТаблиц, Перечисления.ВидыДокументовЕГАИС.ЧекККМ);
...
Функция ДанныеДляИСМП(Объект, МенеджерВременныхТаблиц) Экспорт
	...
	ПараметрыСканирования = ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ПараметрыСканирования(Объект.Ссылка);
	ШтрихкодыУпаковок = ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ВложенныеШтрихкодыУпаковок(МассивУпаковок, ПараметрыСканирования);
...
Функция КоличествоМаркируемойПродукцииПоДаннымШтрихкода(ДанныеШтрихкода)
	...
	Если ТипЗнч(ДанныеШтрихкода) = Тип("Структура") Тогда
		Если ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ПрисутствуетТабачнаяПродукция(ДанныеШтрихкода.ВидыПродукции) Тогда
			КоличествоМаркируемойПродукции = ДанныеШтрихкода.КоличествоПачек;
		ИначеЕсли ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ПрисутствуетОбувнаяПродукция(ДанныеШтрихкода.ВидыПродукции) Тогда
	...
	Если Не ЗначениеЗаполнено(КоличествоМаркируемойПродукции) Тогда 
		КоличествоМаркируемойПродукции = 1;
	КонецЕсли;
...
Функция ОбработатьДанныеШтрихкода(Форма, ДанныеШтрихкода, ПараметрыСканирования, ВложенныеШтрихкоды = Неопределено) Экспорт
	...
	ИСМПТВыбытиеКодовМаркировкиСервер.ПроверкаИПодборПродукцииИС_ЗаполнитьКешШтрихкодовУпаковок(Форма, "Объект", "АкцизныеМарки", "АкцизнаяМарка");
	ИСМПТВыбытиеКодовМаркировкиСервер.ПроверкаИПодборПродукцииИС_ПрименитьКешШтрихкодовУпаковок(Форма, ИСМПТВыбытиеКодовМаркировкиСервер.ПроверкаИПодборПродукцииИСМПУТ_НастройкиИсточникаКешаЧека());
	ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ОбновитьКэшМаркируемойПродукции(Форма);
...
Функция ОбработатьДанныеШтрихкодаЕдиницыАлкогольнойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	...
	Результат = ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ИнициализироватьРезультатОбработкиШтрихкода(Неопределено, ДанныеШтрихкода);
...
Функция ОбработатьДанныеШтрихкодаЕдиницыСНомеромИСерией(Форма, ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	...
	Результат = ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ИнициализироватьРезультатОбработкиШтрихкода(ПараметрыСканирования, ДанныеШтрихкода);
	Результат.ТекстОшибки = НСтр("ru = 'Обработка кодов DataMatrix в документе Чек ККМ не поддерживается. Считайте код акцизной марки.';
...
Функция ОбработатьДанныеШтрихкодаМаркируемойПродукции(Форма, ДанныеШтрихкода, ПараметрыСканирования) Экспорт
	...
	Если ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ПрисутствуетТабачнаяПродукция(ДанныеШтрихкода.ВидыПродукции)
		Или ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ПрисутствуетОбувнаяПродукция(ДанныеШтрихкода.ВидыПродукции) Тогда
...
Функция ОбработатьДанныеШтрихкодаПотребительскойУпаковки(Форма, ДанныеШтрихкода, ПараметрыСканирования, ПараметрыЗаполнения = Неопределено)
	...
	Если ПараметрыЗаполнения = Неопределено Тогда
		...
		РезультатОбработки     = ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ИнициализироватьРезультатОбработкиШтрихкода(Неопределено, ДанныеШтрихкода);
	...
	ПараметрыПоиска = ИСМПТВыбытиеКодовМаркировкиСервер.ИнтеграцияИС_ПоляДляПоискаМаркируемойПродукции();
	ЗаполнитьЗначенияСвойств(ПараметрыПоиска, ДанныеШтрихкода,,
		?(ЗначениеЗаполнено(ДанныеШтрихкода.Серия), "", "Серия"));
	ПараметрыПоиска.Вставить("МаркируемаяПродукция", МаркируемаяПродукция);
	...
	ИначеЕсли ПараметрыПоиска.Свойство("Серия") Тогда
		
		ПараметрыПоискаБезУчетаСерий = ИСМПТВыбытиеКодовМаркировкиСервер.ОбщегоНазначения_СкопироватьРекурсивно(ПараметрыПоиска);
		ПараметрыПоискаБезУчетаСерий.Удалить("Серия");
		НайденныеСтрокиТовары = Объект.Товары.НайтиСтроки(ПараметрыПоискаБезУчетаСерий);
		
		Если НайденныеСтрокиТовары.Количество() > 0 
			И НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(
			НайденныеСтрокиТовары[0].СтатусУказанияСерий, ПараметрыУказанияСерий) Тогда
		...
		Иначе
			...
			ДобавитьНовуюСтрокуВТаблицуТовары(Форма, ПараметрыПоиска, ДанныеШтрихкода,
				ПараметрыУказанияСерий, РезультатОбработки);
...
Процедура ДобавитьНовуюСтрокуВТаблицуТовары(Форма, ПараметрыПоискаТовары, ДанныеШтрихкода, ПараметрыУказанияСерий, РезультатОбработки)
	...
	ПараметрыПоискаТовары.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
	НайденныеСтрокиТовары       = Объект.Товары.НайтиСтроки(ПараметрыПоискаТовары);
	Если НайденныеСтрокиТовары.Количество() > 0 Тогда
		
		СерииВТЧТовары = НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(
			НайденныеСтрокиТовары[0].СтатусУказанияСерий, ПараметрыУказанияСерий);
		...
		Иначе
			...
			СтрокаТЧСерии.Количество = КоличествоМаркируемойПродукции;
	...
	Иначе
		...
		НоменклатураКлиентСервер.ОбновитьКешированныеЗначенияДляУчетаСерий(
			НоваяСтрокаТовары,КэшированныеЗначения, ПараметрыУказанияСерий);
		
		Если ЗначениеЗаполнено(ДанныеШтрихкода.Серия)
			И НЕ НоменклатураКлиентСервер.ВЭтомСтатусеСерииНеУказываются(НоваяСтрокаТовары.СтатусУказанияСерий, ПараметрыУказанияСерий) Тогда
			
			Если НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(
				НоваяСтрокаТовары.СтатусУказанияСерий, ПараметрыУказанияСерий) Тогда
				НоваяСтрокаТовары.Серия = ДанныеШтрихкода.Серия;
			Иначе
				СтрокаТЧСерии = Объект.Серии.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаТЧСерии, ДанныеШтрихкода);
		...
		НоменклатураСервер.ЗаполнитьСтатусыУказанияСерийПриОкончанииРедактированияСтрокиТЧ(Объект,
			ПараметрыУказанияСерий, НоваяСтрокаТовары.ПолучитьИдентификатор(), КэшированныеЗначения);
...
Процедура ЗаполнитьТоварыИАкцизныеМаркиМаркируемойПродукции(Форма, ДеревоУпаковок, ПараметрыЗаполнения, УпаковкаВерхнегоУровня = Неопределено) Экспорт
	...
	Для Каждого СтрокаДерева Из ДеревоУпаковок.Строки Цикл
		...
		Если СтрокаДерева.ВидПродукции <> Неопределено
			...
			Если СтрокаДерева.ВидПродукции = Перечисления.ВидыПродукцииИС.Табачная
				Или СтрокаДерева.ВидПродукции = Перечисления.ВидыПродукцииИС.Обувная Тогда
...
Процедура ОбработатьНайденныеСтрокиВТаблицеТовары(Форма, НайденныеСтрокиТоваров, ДанныеШтрихкода,
	...
	Если ТоварыСоответствуютМаркам Тогда
		...
		СерииВТЧТовары = НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(
			СтрокаТоварыДляИзмененияКоличества.СтатусУказанияСерий, ПараметрыУказанияСерий);
		...
		Если Не СерииВТЧТовары Тогда
			...
			СтрокаТЧСерии.Количество = КоличествоМаркируемойПродукции;
		Иначе
			Если Не ЗначениеЗаполнено(СтрокаТоварыДляИзмененияКоличества.Серия)
				И ЗначениеЗаполнено(ДанныеШтрихкода.Серия) Тогда
			СтрокаТоварыДляИзмененияКоличества.Серия = ДанныеШтрихкода.Серия;
			КонецЕсли;
		КонецЕсли;
	...
	КэшированныеЗначения = ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	НоменклатураКлиентСервер.ОбновитьКешированныеЗначенияДляУчетаСерий(
		СтрокаТоварыДляИзмененияКоличества, КэшированныеЗначения, ПараметрыУказанияСерий);
	НоменклатураСервер.ЗаполнитьСтатусыУказанияСерийПриОкончанииРедактированияСтрокиТЧ(Объект,
		ПараметрыУказанияСерий, СтрокаТоварыДляИзмененияКоличества.ПолучитьИдентификатор(), КэшированныеЗначения);
