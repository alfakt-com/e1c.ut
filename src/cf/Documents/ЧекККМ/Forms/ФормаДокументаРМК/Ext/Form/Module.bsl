...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИСМПТВыбытиеКодовМаркировкиСервер.СобытияФорм_ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка);
	
	ОбщегоНазначенияУТ.ИнициализироватьКешТекущейСтроки(ЭтотОбъект, "Товары");
	
	ПриСозданииЧека();
	УчетМаркируемойПродукции = ИСМПТВыбытиеКодовМаркировкиСервер.ИнтеграцияИС_ИспользуетсяМаркируемаяПродукция();
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если НоменклатураКлиент.ЭтоУказаниеСерий(ИсточникВыбора) Тогда
		...
		ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормКлиент_ПриИзмененииЭлемента(ЭтотОбъект, "Товары", ДополнительныеПараметры);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормИСКлиент_ОбработкаОповещения(ЭтотОбъект, ИмяСобытия, Параметр, Источник, ДополнительныеПараметры);
	...
	Если Источник = "ПодключаемоеОборудование" И ВводДоступен()
		...
		Если ИмяСобытия = "ScanData" И ИСМПТВыбытиеКодовМаркировкиКлиент.МенеджерОборудованияУТКлиент_ЕстьНеобработанноеСобытие() Тогда
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ ПараметрыЗаписи.Свойство("РежимТранзакции") ИЛИ НЕ ПараметрыЗаписи.РежимТранзакции Тогда
		ИСМПТВыбытиеКодовМаркировкиСервер.СобытияФорм_ПослеЗаписиНаСервере(ЭтотОбъект, ТекущийОбъект, ПараметрыЗаписи);
	КонецЕсли;
...
Процедура ТоварыПриАктивизацииЯчейки(Элемент)
	...
	ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормКлиент_ПриАктивизацииЯчейки(ЭтотОбъект, Элемент);
...
Процедура ТоварыПередУдалением(Элемент, Отказ)
	...
	ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормКлиент_ПриИзмененииЭлемента(ЭтотОбъект, "Товары", ДополнительныеРеквизиты);
...
Процедура ТоварыНоменклатураПриИзмененииСервер(ИдентификаторТекущейСтроки)
	...
	ИСМПТВыбытиеКодовМаркировкиКлиентСервер.ОбработкаТабличнойЧастиКлиентСерверЛокализация_ДополнитьСтруктуруДействийПриИзмененииЭлемента(ЭтотОбъект, "Номенклатура", СтруктураДействий);
...
Процедура ТоварыХарактеристикаПриИзмененииСервер(ИдентификаторТекущейСтроки)
	...
	ИСМПТВыбытиеКодовМаркировкиКлиентСервер.ОбработкаТабличнойЧастиКлиентСерверЛокализация_ДополнитьСтруктуруДействийПриИзмененииЭлемента(ЭтотОбъект, "Характеристика", СтруктураДействий);
	ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, Неопределено);
...
Процедура ТоварыСерияПриИзменении(Элемент)
	...
	СтруктураДействий = Новый Структура;
	ИСМПТВыбытиеКодовМаркировкиКлиентСервер.ОбработкаТабличнойЧастиКлиентСерверЛокализация_ДополнитьСтруктуруДействийПриИзмененииЭлемента(ЭтотОбъект, "Серия", СтруктураДействий);
...
Процедура Подключаемый_ВыполнитьПереопределяемуюКоманду(Команда)
	...
	ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормИСКлиент_ВыполнитьПереопределяемуюКоманду(ЭтотОбъект, Команда, Неопределено);
...
Функция ПараметрыОперацииФискализацииЧека()
	...
	//Добавлено для поддержания нового формата структуры:
	...
	ПараметрыОперацииФискализацииЧека.СерияСвидетельстваПоНДС = СтруктураСостояниеКассовойСмены.СведенияООрганизации.СерияСвидетельстваПоНДС;
	ПараметрыОперацииФискализацииЧека.НомерСвидетельстваПоНДС = СтруктураСостояниеКассовойСмены.СведенияООрганизации.НомерСвидетельстваПоНДС;
...
Процедура ПробитьЧекПослеПроведения(Проведен, ПараметрыОперацииФискализацииЧека) Экспорт
	...
	Если Результат Или ПараметрыКассыККМ.ИспользоватьБезПодключенияОборудования Тогда
		...
		Если Не ПараметрыКассыККМ.ИспользоватьБезПодключенияОборудования Тогда
			...
			ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормКлиент_ПриИзмененииЭлемента(ЭтотОбъект, "ПробитьЧекПослеПроведения", ДополнительныеПараметры);
...
Процедура ПечатьЧека_ПослеОткрытияЧека(ПараметрыВыполнения, ДополнительныеПараметры) Экспорт
	...
	ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормКлиент_ПриИзмененииЭлемента(ЭтотОбъект, "ПечатьЧека_ПослеОткрытияЧека", ПараметрыЛокализации);
...
Процедура ПечатьЧека_ПослеОшибкиПечатиЧека(ПараметрыВыполнения, ДополнительныеПараметры) Экспорт
	...
	ПараметрыЛокализации.Вставить("ДополнительныеПараметры", ДополнительныеПараметры);
	ПараметрыЛокализации.Вставить("СтандартнаяОбработка",    Истина);
	ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормКлиент_ПриИзмененииЭлемента(ЭтотОбъект, "ПечатьЧека_ПослеОшибкиПечатиЧека", ПараметрыЛокализации);
...
Процедура ПечатьЧека_Завершение(РезультатВыполнения, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Если РезультатВыполнения.Свойство("ВыходныеПараметры")
			...
			Если ПараметрыФискализации = Неопределено И Не ПараметрыКассыККМ.ИспользоватьБезПодключенияОборудования Тогда
				...
				ПараметрыФискализации = РеквизитыФискальнойОперацииКассовогоУзла;
			КонецЕсли;
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов, ЗагрузкаИзТСД = Ложь)
	...
	ИСМПТВыбытиеКодовМаркировкиКлиентСервер.ОбработкаТабличнойЧастиКлиентСерверЛокализация_ДополнитьСтруктуруДействийПриИзмененииЭлемента(ЭтотОбъект, "Номенклатура", СтруктураДействийСДобавленнымиСтроками);
	...
	//В рос.библиотеке маркировки, на основе которой делается выбытие, параметр МаркируемаяАлкогольнаяПродукцияВТЧ заменена на МаркируемаяПродукцияВТЧ
	//Т.к. в нашей конфигурации библиотеки не обновлены, имя параметра оставлено старое, в модулях из маркировки также протянуто старое название, 
	//чтобы не возникало конфликтов при обращении к форме из других мест конфигцрации, не относящихся к маркировке.
	СтруктураДействий.МаркируемаяАлкогольнаяПродукцияВТЧ     = УчетМаркируемойПродукции;    
...
Процедура ПоискПоШтрихкодуЗавершение(ИсходныеДанные, ДополнительныеПараметры) Экспорт
	...
	ПараметрыСканирования = ИСМПТВыбытиеКодовМаркировкиКлиент.ШтрихкодированиеИСКлиент_ПараметрыСканирования(ЭтотОбъект);
	
	ШтрихкодКоличество = ИСМПТВыбытиеКодовМаркировкиКлиент.ОбщегоНазначенияКлиент_СкопироватьРекурсивно(ИсходныеДанные);
	Если Не ШтрихкодКоличество.Свойство("ШтрихкодыПреобразованы") Тогда
		ШтрихкодКоличество.Штрихкод = ИСМПТВыбытиеКодовМаркировкиКлиентСервер.ШтрихкодированиеИСКлиентСервер_ШтрихкодВBase64(ИсходныеДанные.Штрихкод);
	КонецЕсли;
	
	РезультатОбработки = Подключаемый_ОбработатьВводШтрихкода(ШтрихкодКоличество, Неопределено, ПараметрыСканирования);
	...
	ИСМПТВыбытиеКодовМаркировкиКлиент.ШтрихкодированиеИСКлиент_ЗавершитьОбработкуШтрихкода(ПараметрыЗавершенияВводаШтрихкода);
	
	Элементы.Товары.ТекущаяСтрока = Объект.Товары.Количество()-1;
	
	ОбновитьДанныеИнформационнойПанели(ЭтотОбъект, ТолькоПросмотр = Истина);
	ПодключитьОбработчикОжидания("ВывестиИнформациюНаДисплейПокупателя", 0.1, Истина);
...
Процедура Подключаемый_ПослеОбработкиШтрихкодов()
	...
	ДополнительныеПараметры.Вставить("КэшированныеЗначения", КэшированныеЗначения);
	ДополнительныеПараметры.Вставить("СтандартнаяОбработка", Истина);
	ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормКлиент_ПриИзмененииЭлемента(ЭтотОбъект, "Штрихкоды", ДополнительныеПараметры);
	КэшированныеЗначения = ДополнительныеПараметры.КэшированныеЗначения;
...
Процедура ОбработатьСтрокиТЧ(ДобавленныеСтроки, ИзмененныеСтроки, КэшированныеЗначения = Неопределено) Экспорт
	...
	СтруктураДействийСИзмененнымиСтроками = Новый Структура;
	ИСМПТВыбытиеКодовМаркировкиКлиентСервер.ОбработкаТабличнойЧастиКлиентСерверЛокализация_ДополнитьСтруктуруДействийПриИзмененииЭлемента(
	...
	Если ДобавленныеСтроки.Количество()>0 Или ИзмененныеСтроки.Количество()>0 Тогда
		ИСМПТВыбытиеКодовМаркировкиСервер.СобытияФорм_ПриИзмененииЭлемента(ЭтотОбъект, "Товары");
	КонецЕсли;
...
Процедура ОбработатьУказаниеСерийСервер(ПараметрыФормыУказанияСерий)
	...
	ИСМПТВыбытиеКодовМаркировкиСервер.СобытияФорм_ПриИзмененииЭлемента(ЭтотОбъект, "Серии");
...
Процедура ПересчитатьДокументНаКлиенте()
	...
	ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормКлиент_ПриИзмененииЭлемента(ЭтотОбъект, "ПраваДоступа");
...
Процедура ОткрытьФормуОплатыНаличными(ИзмененныеДанныеЗаписаны, ДополнительныеПараметры) Экспорт
	...
	ПараметрыОткрытияФормы.Вставить("ОтправлятьТолькоEmail",  ПараметрыКассыККМ.ОтправлятьТолькоEmail);
...
Процедура ЗавершениеОплаты(РезультатОплаты, ДополнительныеПараметры) Экспорт
	...
	Если РезультатОплаты <> Неопределено Тогда
		...
		ПолучательИИН            = РезультатОплаты.ПолучательИИН;
...
Процедура ЗаполнитьСлужебныеРеквизитыПоНоменклатуре()
	...
	ИСМПТВыбытиеКодовМаркировкиКлиентСервер.ОбработкаТабличнойЧастиКлиентСерверЛокализация_ДополнитьСтруктуруДействийПриИзмененииЭлемента(ЭтотОбъект, "НоменклатураПризнаки", СтруктураДействий);
...
Процедура ПроверитьКоличествоВДокументе(Результат, ДополнительныеПараметры) Экспорт
	...
	ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормКлиент_ПриИзмененииЭлемента(ЭтотОбъект, "ПроверитьКоличествоВДокументе", ПараметрыОткрытия);
...
Функция ДанныеДляПробитияЧека()
	...
	Для каждого СтрокаТЧ из ДанныеПоТоварам Цикл
		...
		Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "ОсобенностьУчета")
			...
			КоличествоКодовМаркировки = МассивСтрокДляДобавленияВЧек.Количество();
			
			//Если кодов маркировки меньше чем номенклатурных позиций
			Если СтрокаТЧ.КоличествоУпаковок > КоличествоКодовМаркировки
				И Не (МассивСтрокДляДобавленияВЧек[0].КоличествоУпаковок = СтрокаТЧ.КоличествоУпаковок) Тогда
				Для Каждого СтрокаМаркировки Из МассивСтрокДляДобавленияВЧек Цикл 
					СтрокаМаркировки.Сумма 		= СтрокаТЧ.Сумма / СтрокаТЧ.КоличествоУпаковок;
					СтрокаМаркировки.СуммаНДС 	= ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(СтрокаМаркировки.Сумма, ЦенообразованиеКлиентСервер.ПолучитьСтавкуНДСЧислом(СтрокаТЧ.СтавкаНДС), Истина);
				КонецЦикла;
				
				СтрокаТЧ.Сумма 		= СтрокаТЧ.Сумма / СтрокаТЧ.КоличествоУпаковок * (СтрокаТЧ.КоличествоУпаковок - МассивСтрокДляДобавленияВЧек.Количество());
				СтрокаТЧ.СуммаНДС 	= ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(СтрокаТЧ.Сумма, ЦенообразованиеКлиентСервер.ПолучитьСтавкуНДСЧислом(СтрокаТЧ.СтавкаНДС), Истина);
				СтрокаТЧ.КоличествоУпаковок = СтрокаТЧ.КоличествоУпаковок - МассивСтрокДляДобавленияВЧек.Количество();
		...
		Для каждого СтрокаДляДобавленияВЧек из МассивСтрокДляДобавленияВЧек Цикл
			...
			СтрокаПозицииЧека.КодЕдиницыИзмерения    = СтрокаТЧ.КодЕдиницыИзмерения;
			...
			Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "ОсобенностьУчета")
				И (СтрокаТЧ.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ТабачнаяПродукция
				Или СтрокаТЧ.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ОбувнаяПродукция)
				И ЗначениеЗаполнено(СтрокаДляДобавленияВЧек.Штрихкод) Тогда
				
				ТабачнаяПродукция = СтрокаТЧ.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ТабачнаяПродукция;
				
				//Если ТабачнаяПродукция Тогда
					СоставКодаМаркировки = ШтрихкодированиеМОТПКлиентСервер.СоставКодаМаркировки(СтрокаДляДобавленияВЧек.Штрихкод);
				//Иначе
				//	СоставКодаМаркировки = ШтрихкодированиеИСМПКлиентСервер.СоставКодаМаркировки(СтрокаДляДобавленияВЧек.Штрихкод);
				//КонецЕсли;
				
				Если СоставКодаМаркировки <> Неопределено Тогда
					СтрокаПозицииЧека.Штрихкод                                                              = СтрокаДляДобавленияВЧек.Штрихкод;
					СтрокаПозицииЧека.ДанныеКодаТоварнойНоменклатуры.ТипМаркировки                          = ?(ТабачнаяПродукция, Перечисления.ТипыМаркировкиККТ.ТабачнаяПродукция, Перечисления.ТипыМаркировкиККТ.ОбувныеТовары);
					СтрокаПозицииЧека.ДанныеКодаТоварнойНоменклатуры.ГлобальныйИдентификаторТорговойЕдиницы = СоставКодаМаркировки.GTIN;
					СтрокаПозицииЧека.ДанныеКодаТоварнойНоменклатуры.СерийныйНомер                          = СоставКодаМаркировки.СерийныйНомер;
					СтрокаПозицииЧека.ДанныеКодаТоварнойНоменклатуры.КодМаркировки                          = СтрокаДляДобавленияВЧек.Штрихкод;
				КонецЕсли;
...
&НаСервере
Функция Подключаемый_ОбработатьВводШтрихкода(ШтрихкодКоличество, КэшированныеЗначения, ПараметрыСканирования = Неопределено)
	
	РезультатОбработкиШтрихкода = ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ОбработатьВводШтрихкода(ЭтотОбъект, ШтрихкодКоличество, КэшированныеЗначения, ПараметрыСканирования);
...
Функция ОбработкаКодаМаркировкиВыполнитьДействиеСервер(Действие, РезультатВыбора, РезультатОбработкиШтрихкода, ПараметрыСканирования, КэшированныеЗначения)
	...
	ПараметрыОбработкиВыбора    = ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ИнициализироватьПараметрыОбработкиВыбора(РезультатВыбора, РезультатОбработкиШтрихкода, КэшированныеЗначения);
	РезультатОбработкиШтрихкода = ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ВыполнитьДействие(ЭтотОбъект, Действие, ПараметрыОбработкиВыбора);
...
Функция ПараметрыЗавершенияВводаШтрихкода(ИсходныеДанные, РезультатОбработки, ПараметрыСканирования)
	...
	ПараметрыЗавершенияВводаШтрихкода = ИСМПТВыбытиеКодовМаркировкиКлиент.ШтрихкодированиеИСКлиент_ПараметрыЗавершенияОбработкиШтрихкода();
...
Функция ПослеОбработкиШтрихкодовСервер(РезультатОбработкиШтрихкода, КэшированныеЗначения)
	...
	Если РезультатОбработкиШтрихкода.ТребуетсяОбработкаШтрихкода Тогда
		...
		ДанныеДляОбработки.ИзменятьКоличество                     = ИзменятьКоличество;
		ДанныеДляОбработки.РассчитыватьНаборы                     = Истина;
		ДанныеДляОбработки.МаркируемаяАлкогольнаяПродукцияВТЧ     = УчетМаркируемойПродукции;
		ДанныеДляОбработки.ШтрихкодыВТЧ                           = Истина;
		ДанныеДляОбработки.ЗагрузкаИзТСД                          = ЗагрузкаИзТСД;
...
Процедура ОбработкаОповещенияНаСервере(ИмяСобытия, Параметр, Источник)
	...
	ИСМПТВыбытиеКодовМаркировкиСервер.СобытияФорм_ПриИзмененииЭлемента(ЭтотОбъект, "Событие", ДополнительныеПараметры);
...
Процедура Подключаемый_ОткрытьФормуУточненияДанных()
	...
	ИСМПТВыбытиеКодовМаркировкиКлиент.ШтрихкодированиеИСКлиент_Подключаемый_ОткрытьФормуУточненияДанных(ЭтотОбъект, ОписаниеОповещенияОбработкиКодаМаркировки());
