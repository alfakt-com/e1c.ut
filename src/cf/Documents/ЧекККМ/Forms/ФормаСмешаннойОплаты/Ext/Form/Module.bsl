...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ОтправлятьТолькоEmail Тогда
		Элементы.КомандаОтправитьSMS.Видимость = Ложь;
		Элементы.ГруппаОтправкаЭлектронногоЧекаКонтактнаяИнформацияТелефон.Видимость = Ложь;
	КонецЕсли;
	Элементы.ГруппаОтправкаИИН.Видимость = ПолучитьФункциональнуюОпцию("УказыватьИИНПриРозничныхПродажах");
...
Процедура КомандаСтереть(Команда)
	...
	ИначеЕсли АктивноеПоле = Элементы.ПолучательИИН.Имя Тогда
...
Процедура КомандаПробитьЧек(Команда)
	...
	РезультатОплаты.Вставить("ПолучательИИН", ПолучательИИН);
...
&НаКлиенте
Процедура КомандаУказатьИИН(Команда)
	
	Если АктивноеПоле = Элементы.ПолучательИИН.Имя Тогда
		РозничныеПродажиКлиентСервер.УстановитьАктивныйЭлемент(ЭтотОбъект, Элементы.ПолеПолученоНаличными);
	Иначе
		РозничныеПродажиКлиентСервер.УстановитьАктивныйЭлемент(ЭтотОбъект, Элементы.ПолучательИИН);
	КонецЕсли;
...
Процедура ДобавитьЦифру(ВведеннаяЦифраСтрокой)
	...
	ИначеЕсли АктивноеПоле = Элементы.ПолучательИИН.Имя Тогда
		...
		ПолучательИИН = СокрЛП(ПолучательИИН) + ВведеннаяЦифраСтрокой;
		ПроверитьВведенныйИИН();
...
&НаКлиенте
Процедура ПроверитьВведенныйИИН()
	
	ИИНЗаполненКорректно = Истина;
	
	ТекстСообщения = "";
	Если ПустаяСтрока(ПолучательИИН) Тогда
		ТекстСообщения = НСтр("ru = 'Для передачи в чеке введите ИИН / БИН покупателя.'");
		Элементы.ДекорацияТекстИИН.Заголовок = ТекстСообщения;
		Возврат;
	...
	Если ИИНЗаполненКорректно Тогда
		ТекстСообщения = НСтр("ru = 'ИИН / БИН введен корректно.'");
		Элементы.ДекорацияТекстИИН.Заголовок = ТекстСообщения;
	Иначе
		Элементы.ДекорацияТекстИИН.Заголовок = ТекстСообщения;
	КонецЕсли;
