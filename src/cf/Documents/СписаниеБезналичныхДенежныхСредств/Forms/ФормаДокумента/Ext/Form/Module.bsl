...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если Объект.ХозяйственнаяОперация <> ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыПоЗарплатномуПроекту")
			И Объект.ХозяйственнаяОперация <> ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПеречислениеВзносов")
...
Процедура КодНазначенияПлатежаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТекущийКодСтроки = ?(НЕ ЗначениеЗаполнено(Объект.КодНазначенияПлатежа), Неопределено, СокрЛП(Объект.КодНазначенияПлатежа));
...
Процедура ВыгрузитьМТ102(Команда)
	...
	Иначе
		ВыгрузитьВыгрузитьМТ102Фрагмент();
	КонецЕсли;
...
&НаКлиенте
Процедура ВыгрузитьВыгрузитьМТ102Фрагмент()
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета") Тогда 
		ВидОперации = "ВыплатаЗарплатыПеречислением";
		Ведомости = ПолучитьВедомостиНаСервере();
	ИначеЕсли Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПеречислениеВзносов") Тогда 
		ВидОперации = "ПеречислениеВзносов";
		Ведомости = ПолучитьВедомостиНаСервере();
	ИначеЕсли ПолучитьФункциональнуюОпциюФормы("УправлениеТорговлей")
			И Объект.ХозяйственнаяОперация = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Перечисление.ХозяйственныеОперации.ПеречислениеДенежныхСредствПодотчетникам") Тогда
		ВидОперации = "ПеречислениеДенежныхСредствПодотчетникам";
		Ведомости = Объект.Ссылка;
	КонецЕсли;
...
Функция ПолучитьВедомостиНаСервере()
	...
	Возврат Объект.РасшифровкаПлатежа.Выгрузить(,"Ведомость").ВыгрузитьКолонку("Ведомость");
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику
		...
		Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета
			И ПеречислениеСпискуФизическихЛиц = 1 Тогда
...
Процедура УправлениеЭлементамиФормы()
	...
	ДенежныеСредстваСервер.УстановитьПараметрыВыбораБанковскогоСчетаКонтрагента(Объект, Элементы.БанковскийСчетКонтрагента, ?(ПеречислениеСпискуФизическихЛиц = 1, Истина, Ложь));
	...
	ДенежныеСредстваСервер.УстановитьПараметрыВыбораТипаНалога(Объект, Элементы.ТипНалога);
...
Функция РеквизитыФормы(Форма)
	...
	// ЗУПК
	РеквизитыФормы.Вставить("ПеречислениеСпискуФизическихЛиц");
	// ЗУПК
...
Процедура УстановитьУсловноеОформление()
    ...
    СписокНалогов.Добавить(Перечисления.ТипыНалогов.НДС_Импорт);
...
Процедура УстановитьНадписьРеквизитыПлательщика()
	...
	Если ЗначениеЗаполнено(Объект.БанковскийСчет) Тогда
		НадписьРеквизитыПлательщика = ДенежныеСредстваСервер.НадписьРеквизитыПлательщика(
			Объект.ИННПлательщика, Объект.РегистрацияВНалоговомОргане, Объект.ТекстПлательщика, ЮрФизЛицо, Объект.ХозяйственнаяОперация);
	Иначе
...
&НаКлиенте
Процедура РасшифровкаБезРазбиенияТипПлатежаПоЛизингуНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
