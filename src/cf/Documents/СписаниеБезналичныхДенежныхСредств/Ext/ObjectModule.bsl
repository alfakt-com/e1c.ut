...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплатыНаЛицевыеСчета Тогда
		...
		// определим списочный или односотрудниковый документ
		Если ЗначениеЗаполнено(ПодотчетноеЛицо) И НЕ ЗначениеЗаполнено(Контрагент) Тогда
			Если МассивНепроверяемыхРеквизитов.Найти("Контрагент") = Неопределено Тогда
				МассивНепроверяемыхРеквизитов.Добавить("Контрагент");
			КонецЕсли;
		ИначеЕсли НЕ ЗначениеЗаполнено(ПодотчетноеЛицо) И ЗначениеЗаполнено(Контрагент) Тогда
			Если МассивНепроверяемыхРеквизитов.Найти("ПодотчетноеЛицо") = Неопределено Тогда
	...
	Если ИСТИНА ИЛИ ДатаНачалаПримененияПриказа126н <> '00010101' И Дата >= ДатаНачалаПримененияПриказа126н Тогда
...
Процедура ЗаполнитьПоТаможеннойДекларации(Знач ДокументОснование, Знач СуммаКОплате, ДанныеЗаполнения)
    ...
    Если ДокументОснование.ВариантОтражения = 0 Тогда
    	// Заполним данные шапки документа.
    	Запрос = Новый Запрос("
    	|ВЫБРАТЬ
    	|	ВЫБОР КОГДА ДанныеДокумента.БанковскийСчетОрганизации <> ЗНАЧЕНИЕ(Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка) ТОГДА
    ...
    Иначе
        ...
        |ПОМЕСТИТЬ ВТ_ДанныеНДСКУплате
        |ИЗ
        |	Документ.ТаможеннаяДекларацияИмпорт.Товары КАК ТаможеннаяДекларацияИмпортТовары
        |ГДЕ
        |	НЕ ТаможеннаяДекларацияИмпортТовары.ВидОперацииПоступления.ВидНДС = ЗНАЧЕНИЕ(Перечисление.ВидыНДС.НДСУплачиваемыйПриИмпортеМетодомЗачета)
        |
        |СГРУППИРОВАТЬ ПО
        ...
        |	ВТ_ДанныеНДСКУплате.СуммаНДС   КАК СуммаНДС,
        |	&Ссылка                        КАК ДокументОснование,
        ...
        |ИЗ
        |	Документ.ТаможеннаяДекларацияИмпорт КАК ДанныеДокумента
        |   ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДанныеНДСКУплате КАК ВТ_ДанныеНДСКУплате
        |       ПО ДанныеДокумента.Ссылка = ВТ_ДанныеНДСКУплате.Ссылка
        |ГДЕ
