...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//++ Тишбаев О.М.
	ИндексЭлемента = МассивНепроверяемыхРеквизитов.Найти("Подразделение");
	Если ИндексЭлемента <> Неопределено Тогда
		МассивНепроверяемыхРеквизитов.Удалить(ИндексЭлемента);
	КонецЕсли;
	//
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	//++Аян_К++ 231221
	Если НЕ Альфа_КТ.ПроверитьДоступ("ЭквайринговаяОперацияПКО", ПараметрыСеанса.ТекущийПользователь, ТекущаяДата(), ТекущаяДата()) Тогда 
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ДенежныеСредстваСервер.ОтразитьДетализацияПлатежа(ДополнительныеСвойства, Движения, Отказ);
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	Если ЗначениеЗаполнено(ЭквайринговыйТерминал) И Не ЗначениеЗаполнено(Организация) Тогда
