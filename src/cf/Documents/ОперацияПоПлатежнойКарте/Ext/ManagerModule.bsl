...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ДанныеДокумента.ЭквайринговыйТерминал.БанковскийСчет КАК БанковскийСчет,
...
Функция ТекстЗапросаДетализацияПлатежа(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               |	0 КАК НомерСтроки,
	               |	&Период КАК Период,
	               |	Документ.ВидОплаты КАК ВидОплаты,
	               |	Документ.Банк КАК Банк,
	               |	Документ.ТипОплаты КАК ТипОплаты,
	               |	ВЫБОР
	               |		КОГДА Документ.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента)
	               |			ТОГДА Документ.СуммаДокумента
	               |		ИНАЧЕ -Документ.СуммаДокумента
	               |	КОНЕЦ КАК СуммаОперации,
	               |	ВЫБОР
	               |		КОГДА Документ.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента)
	               |			ТОГДА ВЫРАЗИТЬ(-Документ.СуммаДокумента * ЕСТЬNULL(КомиссииПоОплатамСрезПоследних.ОсновнойПроцент, 0) / 100 КАК ЧИСЛО(15, 2))
	               |		ИНАЧЕ ВЫРАЗИТЬ(Документ.СуммаДокумента * ЕСТЬNULL(КомиссииПоОплатамСрезПоследних.ОсновнойПроцент, 0) / 100 КАК ЧИСЛО(15, 2))
	               |	КОНЕЦ КАК КомиссияПоОсновномуПроценту,
	               |	ВЫБОР
	               |		КОГДА Документ.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПоступлениеОплатыОтКлиента)
	               |			ТОГДА ВЫРАЗИТЬ(-Документ.СуммаДокумента * ЕСТЬNULL(КомиссииПоОплатамСрезПоследних.ДополнительныйПроцент, 0) / 100 КАК ЧИСЛО(15, 2))
	               |		ИНАЧЕ ВЫРАЗИТЬ(Документ.СуммаДокумента * ЕСТЬNULL(КомиссииПоОплатамСрезПоследних.ДополнительныйПроцент, 0) / 100 КАК ЧИСЛО(15, 2))
	               |	КОНЕЦ КАК КомиссияПоДопПроценту,
	               |	Документ.ОнлайнМагазин КАК ОнлайнПокупка,
	               |	Документ.ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	               |	ОперацияПоПлатежнойКартеРасшифровкаПлатежа.ОснованиеПлатежа КАК ДокументОснование,
	               |	Документ.Контрагент КАК Контрагент,
	               |	Документ.БанковскаяКарта КАК БанковскаяКарта
	               |ИЗ
	               |	Документ.ОперацияПоПлатежнойКарте КАК Документ
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КомиссииПоОплатам.СрезПоследних КАК КомиссииПоОплатамСрезПоследних
	               |		ПО Документ.ТипОплаты = КомиссииПоОплатамСрезПоследних.ТипыОплат
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОперацияПоПлатежнойКарте.РасшифровкаПлатежа КАК ОперацияПоПлатежнойКартеРасшифровкаПлатежа
	               |		ПО Документ.Ссылка = ОперацияПоПлатежнойКартеРасшифровкаПлатежа.Ссылка
	               |ГДЕ
	               |	Документ.Ссылка = &Ссылка
	               |
	               |УПОРЯДОЧИТЬ ПО
