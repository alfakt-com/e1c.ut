...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	ВЫБОР
	|		КОГДА ЗаявлениеОВвозеТоваров.Договор = ЗНАЧЕНИЕ(Справочник.ДоговорыКонтрагентов.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		КОГДА НЕ ЗаявлениеОВвозеТоваров.Договор.ВариантОформленияЗакупок ЕСТЬ NULL
	|				И ЗаявлениеОВвозеТоваров.Договор.ВариантОформленияЗакупок <> ЗНАЧЕНИЕ(Перечисление.ВариантыОформленияЗакупок.НеРазделять)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
...
Функция ТекстЗапросаТаблицаТоварыОрганизаций(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&Период 												КАК Период,
	|	&Организация 											КАК Организация,
	|	ЗаявлениеТаблицаТовары.АналитикаУчетаНоменклатуры.Склад КАК Склад,
	...
	|	ЗаявлениеТаблицаТовары.Количество 						КАК Количество,
	|	ЗаявлениеТаблицаТовары.АналитикаУчетаНоменклатуры 		КАК АналитикаУчетаНоменклатуры,
	|	&ХозяйственнаяОперация 									КАК ХозяйственнаяОперация,
	...
	|
	|ГДЕ
	|	ЗаявлениеТаблицаТовары.Ссылка = &Ссылка
...
Функция ТекстЗапросаТаблицаДатыПоступленияТоваровОрганизаций(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ПО
	|		ЗаявлениеТаблицаТовары.ВидЗапасов = ПоступленияТоваров.ВидЗапасов
...
Функция ПодготовитьТаблицыЗаявленияОВвозеТоваровДляВыгрузкиПечати(МассивДокументов) Экспорт
	...
	|	ЕСТЬNULL(Реквизиты.Контрагент.СтранаРегистрации.Код, """") КАК КодСтраныПродавец,
	...
	|	0 КАК НомерСпецификации_05_1,
	|	0 КАК НомерСпецификации_05_2,
	|	0 КАК НомерСпецификации_05_3,
	|	0 КАК НомерСпецификации_05_4,
	|	0 КАК НомерСпецификации_05_5,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаСпецификации_05_1,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаСпецификации_05_2,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаСпецификации_05_3,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаСпецификации_05_4,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаСпецификации_05_5,
	...
	|	0 КАК НомерСпецификации_07_1,
	|	0 КАК НомерСпецификации_07_2,
	|	0 КАК НомерСпецификации_07_3,
	|	0 КАК НомерСпецификации_07_4,
	|	0 КАК НомерСпецификации_07_5,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаСпецификации_07_1,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаСпецификации_07_2,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаСпецификации_07_3,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаСпецификации_07_4,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаСпецификации_07_5,
	...
	|,  ЕСТЬNULL(Реквизиты.Организация.КодНалоговогоОргана, """") КАК КодНалоговогоОргана
	|ИЗ
	...
	|	Товары.Ссылка.НалоговыйПериод КАК НалоговыйПериод,
	...
	|	Товары.СтавкаНДС.Ставка КАК П000010000118,
	...
	|	ВЫБОР
	|		КОГДА Товары.СтавкаНДС.ДляОсвобожденногоОборота = ИСТИНА ИЛИ Товары.СтавкаНДС.МестоРеализацииНеРК
	|			ТОГДА ""Х""
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК Льгота,
	...
	|	ЕСТЬNULL(УчастникиСделки.Продавец.СтранаРегистрации.Код, """") КАК КодСтраныПродавец,
	|	УчастникиСделки.Продавец.СтранаРегистрации.НаименованиеПолное КАК НаименованиеСтраныПродавец,
	|	УчастникиСделки.Покупатель,
	|	ЕСТЬNULL(УчастникиСделки.Покупатель.СтранаРегистрации.Код, """") КАК КодСтраныПокупатель,
	...
	|	УчастникиСделки.НомерСпецификации КАК НомерСпецификации_05_1,
	|	УчастникиСделки.ДатаСпецификации КАК ДатаСпецификации_05_1,
	|	НЕОПРЕДЕЛЕНО КАК ПродавецИНН_КПП,
	...
	Для каждого СтрокаШапки Из Шапка Цикл
		...
		ИначеЕсли СтрокаШапки.Дата < '20210101' Тогда
			Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЗаявлениеОВвозеТоваров.ЗаявлениеОВвозе2015кв1");
		Иначе
			Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЗаявлениеОВвозеТоваров.ЗаявлениеОВвозе2021кв1");
		КонецЕсли;
		...
		РеквизитыРаздел1.Вставить("НалоговыйПериодМесяц", Формат(СтрокаШапки.НалоговыйПериод, "ДФ=""ММ"""));
		РеквизитыРаздел1.Вставить("НалоговыйПериодГод", Формат(СтрокаШапки.НалоговыйПериод, "ДФ=""гггг""")); 
		РеквизитыРаздел2 = Новый Структура("НомерЗаяв,Число,Месяц,Год,СтрВсего,Стр,КодНалоговогоОргана");
		РеквизитыРаздел3 = Новый Структура("НомерЗаяв,Число,Месяц,Год,СтрВсего,Стр,ТипПрод_3,
			...
			|НомерСпецификации_12_1,ДатаСпецификации_12_1,
			|НомерСпецификации_12_2,ДатаСпецификации_12_2,
			|НомерСпецификации_12_3,ДатаСпецификации_12_3,
			|НомерСпецификации_12_4,ДатаСпецификации_12_4,
			|НомерСпецификации_12_5,ДатаСпецификации_12_5,
			|ТипПок_3,ТипПокупателя,ПокупательИНН_КПП,
		...
		РеквизитыРаздел1.Вставить("ПокупательАдресСтрока", ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОПокупателе, "ЮридическийАдрес,"));
		...
		РеквизитыРаздел2.КодНалоговогоОргана = СтрокаШапки.КодНалоговогоОргана;
		...
		Если НайденныеСтрокиУчастникиСделки.Количество() > 0 Тогда
			...
			Если СтрокаУчастникиСделки <> Неопределено Тогда
				...
				РеквизитыРаздел1.Вставить("ПродавецАдресСтрока", ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОПродавце, "ЮридическийАдрес,"));
				...
				РеквизитыРаздел1.НомерСпецификации_05_1 = СтрокаУчастникиСделки.НомерСпецификации_05_1;
				РеквизитыРаздел1.ДатаСпецификации_05_1  = Формат(СтрокаУчастникиСделки.ДатаСпецификации_05_1, "ДЛФ=D");
				
				Если ЗначениеЗаполнено(СтрокаУчастникиСделки.НомерСпецификации_05_1) 
					И ЗначениеЗаполнено(СтрокаУчастникиСделки.ДатаСпецификации_05_1) Тогда
					СтрокаСпецификации = ТаблицаСпецификацииДляВыгрузки.Добавить();
					СтрокаСпецификации.Номер = СтрокаУчастникиСделки.НомерСпецификации_05_1;
					СтрокаСпецификации.Дата =  Формат(СтрокаУчастникиСделки.ДатаСпецификации_05_1, "ДЛФ=D");
				КонецЕсли;
				...
				Если НайденныеСтрокиУчастникиСделки.Количество() = 1 Тогда
					...
					РеквизитыРаздел3.НомерСпецификации_12_1 = СтрокаУчастникиСделки.НомерСпецификации_05_1;
					РеквизитыРаздел3.ДатаСпецификации_12_1  = СтрокаУчастникиСделки.ДатаСпецификации_05_1;
		...
		ТаблицаЗначенийСпецификаций = Новый ТаблицаЗначений;
		ТаблицаЗначенийСпецификаций.Колонки.Добавить("НомерСпецификации");
		ТаблицаЗначенийСпецификаций.Колонки.Добавить("ДатаСпецификации");
		
		Для Счетчик = 1 по 5 Цикл
			Если ТаблицаСпецификацииКонтрагента.НайтиСледующий(СтруктураПоиска) Тогда
				СтрокаТаблицыЗначенийСпецификаций = ТаблицаЗначенийСпецификаций.Добавить();
				СтрокаТаблицыЗначенийСпецификаций.НомерСпецификации = ТаблицаСпецификацииКонтрагента.Номер;
				СтрокаТаблицыЗначенийСпецификаций.ДатаСпецификации = Формат(ТаблицаСпецификацииКонтрагента.Дата, "ДЛФ=D");
			...
			Иначе
				СтрокаТаблицыЗначенийСпецификаций = ТаблицаЗначенийСпецификаций.Добавить();
				СтрокаТаблицыЗначенийСпецификаций.НомерСпецификации = 0;
				СтрокаТаблицыЗначенийСпецификаций.ДатаСпецификации = ДАТА(1,1,1);
			КонецЕсли;
		КонецЦикла;
		...
		Если СтрокаУчастникиСделки <> Неопределено Тогда
			...
			Для Счетчик = 1 по 5 Цикл
				РеквизитыРаздел1["НомерСпецификации_07_"+Строка(Счетчик)] = ТаблицаЗначенийСпецификаций.Получить(Счетчик-1).НомерСпецификации;
				РеквизитыРаздел1["ДатаСпецификации_07_"+Строка(Счетчик)] = ТаблицаЗначенийСпецификаций.Получить(Счетчик-1).ДатаСпецификации;
			КонецЦикла;			
			...
			РеквизитыРаздел1.Вставить("ПродавецАдресСтрока", ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОКонтрагенте, "ЮридическийАдрес,"));
		...
		Иначе
			...
			Для Счетчик = 1 по 5 Цикл
				РеквизитыРаздел1["НомерСпецификации_05_"+Строка(Счетчик)] = ТаблицаЗначенийСпецификаций.Получить(Счетчик-1).НомерСпецификации;
				РеквизитыРаздел1["ДатаСпецификации_05_"+Строка(Счетчик)] = ТаблицаЗначенийСпецификаций.Получить(Счетчик-1).ДатаСпецификации;
			КонецЦикла;
		...
		ПрисутствуютНесоответствияНалоговомуПериоду = Ложь;
		...
		Если НайденныеСтрокиТовары.Количество() > 0 Тогда
			...
			Для каждого СтрокаТоваров Из НайденныеСтрокиТовары Цикл
				...
				СтрокаТоваров.НомерСтроки = Формат(СтрокаТоваров.П000010000101,"ЧЦ=4; ЧВН=; ЧГ=0");
				Если ЗначениеЗаполнено(СтрокаТоваров.НалоговыйПериод) Тогда
					Если НЕ ((СтрокаТоваров.ДатаПринятияНаУчет >= НачалоМесяца(СтрокаТоваров.НалоговыйПериод)) и 
						(СтрокаТоваров.ДатаПринятияНаУчет <= КонецМесяца(СтрокаТоваров.НалоговыйПериод))) Тогда
						Если НЕ ПрисутствуютНесоответствияНалоговомуПериоду Тогда
							ТекстСообщения = СтрШаблон(НСтр("ru = 'В документе %1'"), СтрокаТоваров.ДокументСсылка);
							ТекстСообщения = ТекстСообщения + Символы.ПС
								+ СтрШаблон(НСтр("ru = 'выявлены несоответствия ""Даты принятия к учету"" налоговому периоду документа ""%1"":'"),
							                Формат(СтрокаТоваров.НалоговыйПериод, "ДФ='MMMM yyyy'"));
							
							ПрисутствуютНесоответствияНалоговомуПериоду = Истина;
						КонецЕсли;
						ТекстСообщения = ТекстСообщения + Символы.ПС
						   + СтрШаблон(НСтр("ru = '- ""%1"": строка %2 по документу %3 не относится к выбранному налоговому периоду'"),
						               СтрокаТоваров.П000010000102, СтрокаТоваров.П000010000101, СтрокаТоваров.ДокументПоступления);
					КонецЕсли;
				КонецЕсли;		   
				
				СтрокаТоваров.П000010000102 = НоменклатураКлиентСервер.ПредставлениеНоменклатурыДляПечати(
					?(ЗначениеЗаполнено(СтрокаТоваров.НаименованиеТовара), СтрокаТоваров.НаименованиеТовара, СтрокаТоваров.П000010000102), СтрокаТоваров.ХарактеристикаНаименование);
			...
			Если ПрисутствуютНесоответствияНалоговомуПериоду Тогда
		...
		РеквизитыРаздел1.Вставить("П000010000120", Окр(ТаблицаТоварыРаздел1.Итог("П000010000120"),0,1));
		РеквизитыРаздел1.Вставить("П000010000121", Окр(ТаблицаТоварыРаздел1.Итог("П000010000121"),0,1));
		...
		Если СтрокаШапки.Дата >= '20210101' Тогда
			ПодвалТаблицаТовары.Параметры.П000010000121 = РеквизитыРаздел1.П000010000121;
		КонецЕсли;
...
Функция СформироватьПредставлениеМестонахожденияКонтрагента(КодСтраны, ПредставлениеКонтактнойИнформации)
	...
	Если КодСтраны = "398" ИЛИ НЕ ЗначениеЗаполнено(КодСтраны) Тогда
...
Функция ТекстЗапросаТаблицаНДСПредъявленный(Запрос, ТекстыЗапроса, Регистры)
	...
	// в УПК необходимо для корректного деления сумм по виду операции поступления (подстановка счета в проводках)
...
Функция ТекстЗапросаТоварыКОформлениюЗаявленийОВвозе(ЗаполнениеНаОсновании = Ложь)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТоварыКОформлению.НомерСтроки КАК НомерСтроки,
	|	ТоварыКОформлению.ДокументПоступления КАК ДокументПоступления,
	|	ТоварыКОформлению.Номенклатура КАК Номенклатура,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОстаткиКОформлению КАК ОстаткиКОформлению
	|		ПО ТоварыКОформлению.ДокументПоступления = ОстаткиКОформлению.ДокументПоступления
