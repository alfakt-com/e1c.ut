...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//КТ Начало Апаев_О [28.09.2022] - Задача № 6148
	НепроверяемыеРеквизиты = Новый Массив;
	
	СтруктураТипов = ПолучитьСтруктураТиповДокументовОснования();
	
	Если СтруктураТипов["ЕстьРеализация"]   = Истина
		И СтруктураТипов["ЕстьПеремещение"] = Ложь Тогда
		НепроверяемыеРеквизиты.Добавить("кт_Склад");
		НепроверяемыеРеквизиты.Добавить("кт_СкладОтправитель");
		НепроверяемыеРеквизиты.Добавить("кт_СкладПолучателя");
	КонецЕсли;
	
	ОбщегоНазначения.УдалитьНепроверяемыеРеквизитыИзМассива(ПроверяемыеРеквизиты, НепроверяемыеРеквизиты);
	//КТ Окончание Апаев_О [02.10.2022]
...
//КТ Начало Апаев_О [02.10.2022] - Задача № 6148
Функция ПолучитьСтруктураТиповДокументовОснования()
	
	СтрукутраТипов = Новый Структура;
	СтрукутраТипов.Вставить("ЕстьРеализация",  Ложь);
	СтрукутраТипов.Вставить("ЕстьПеремещение", Ложь);
	...
	|ИЗ
	|	&ДокументыОснования КАК ВТ_ДокументыОснования
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА ВТ_ДокументыОснования.ДокументОснование ССЫЛКА Документ.РеализацияТоваровУслуг
	|			ТОГДА ""ЕстьРеализация""
	|		КОГДА ВТ_ДокументыОснования.ДокументОснование ССЫЛКА Документ.ПеремещениеТоваров
	|			ТОГДА ""ЕстьПеремещение""
	|	КОНЕЦ КАК ТипДокумента
	|ИЗ
	|	ВТ_ДокументыОснования КАК ВТ_ДокументыОснования
	|ГДЕ
	|	(ТИПЗНАЧЕНИЯ(ВТ_ДокументыОснования.ДокументОснование) = ТИП(Документ.РеализацияТоваровУслуг)
	|			ИЛИ ТИПЗНАЧЕНИЯ(ВТ_ДокументыОснования.ДокументОснование) = ТИП(Документ.ПеремещениеТоваров))";
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			СтрукутраТипов[Выборка.ТипДокумента] = Истина;	
		КонецЦикла;
		
	КонецЕсли;
	
	Возврат СтрукутраТипов;
