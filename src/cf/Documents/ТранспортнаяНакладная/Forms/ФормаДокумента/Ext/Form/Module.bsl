...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	//КТ Окончание Апаев_О [28.09.2022]
...
&НаСервере
Процедура кт_СкладОтправительПриИзмененииНаСервере()
	
	Если ЗначениеЗаполнено(Объект.кт_СкладОтправитель) Тогда		
		ТаблицаАдресов = УправлениеКонтактнойИнформацией.ЗначенияКонтактнойИнформацииОбъекта(Объект.кт_СкладОтправитель, Перечисления.ТипыКонтактнойИнформации.Адрес);		
		Если ТаблицаАдресов.Количество() > 0 Тогда
			Объект.АдресПогрузки = ТаблицаАдресов[0].Значение;	
		КонецЕсли;		
	КонецЕсли;
...
&НаКлиенте
Процедура кт_СкладОтправительПриИзменении(Элемент)
	
	кт_СкладОтправительПриИзмененииНаСервере();
...
&НаСервере
Процедура кт_СкладПолучателяПриИзмененииНаСервере()
	
	Если ЗначениеЗаполнено(Объект.кт_СкладПолучателя) Тогда		
		ТаблицаАдресов = УправлениеКонтактнойИнформацией.ЗначенияКонтактнойИнформацииОбъекта(Объект.кт_СкладПолучателя, Перечисления.ТипыКонтактнойИнформации.Адрес);		
		Если ТаблицаАдресов.Количество() > 0 Тогда
			Объект.АдресПогрузки = ТаблицаАдресов[0].Значение;	
		КонецЕсли;		
	КонецЕсли;
...
&НаКлиенте
Процедура кт_СкладПолучателяПриИзменении(Элемент)
	
	кт_СкладПолучателяПриИзмененииНаСервере();
