...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		...
		Если НЕ Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратВзносов") Тогда
			Отказ = Истина;
...
&НаКлиенте
Процедура РасшифровкаПлатежаВедомостьПриИзменении(Элемент)
	
	
КонецПроцедуры

&НаКлиенте
Процедура РасшифровкаБезРазбиенияВедомостьПриИзменении(Элемент)
...
Процедура ФискальнаяОперацияОбновитьСтатус()
	...
	//Кт Начало [18.08.2022] - Скрытие кнопки
	Элементы.ФискальнаяОперацияСтатус.Видимость = Ложь;
	//Кт Окончание [18.08.2022]
...
&НаСервереБезКонтекста
Функция ПолучитьСуммуВедомостейВозврата(СсылкиНаВедомости) Экспорт
	
	Возврат Документы.ПоступлениеБезналичныхДенежныхСредств.ПолучитьСуммуВедомостейВозврата(СсылкиНаВедомости);
	
КонецФункции

&НаКлиенте
Процедура ВедомостьПриИзменении()
	
	ТекущиеДанные = Элементы.РасшифровкаПлатежа.ТекущиеДанные;
	
	Если ЗначениеЗаполнено(ТекущиеДанные.Ведомость) Тогда
		МассивВедомостейВозвратВзносовОтчислений = Новый Массив;
		МассивВедомостейВозвратВзносовОтчислений.Добавить(ТекущиеДанные.Ведомость);
		ТекущиеДанные.Сумма = ПолучитьСуммуВедомостейВозврата(МассивВедомостейВозвратВзносовОтчислений);
	КонецЕсли;
