...
Процедура ЗаполнитьИменаРеквизитовПоХозяйственнойОперации(ХозяйственнаяОперация, МассивВсехРеквизитов, МассивРеквизитовОперации) Экспорт
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратВзносов Тогда
		МассивРеквизитовОперации.Добавить("БанковскийСчетКонтрагента");
...
Функция ЗначенияПараметровПроведения(Реквизиты = Неопределено)
	...
	Значения.Вставить("Ответственный",                                   Справочники.Пользователи.ПустаяСсылка());
...
Функция ТекстЗапросаТаблицаДенежныеСредстваБезналичные(Запрос, ТекстыЗапроса, Регистры)
	...
	|,		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратВзносов)
	|		)
	|";
...
Функция ПолучитьСуммуВедомостейВозврата(СсылкиНаВедомости) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	СУММА(-НалогиВзносыОтчисленияРасчетыСБюджетомИФондами.НалогВзнос) КАК СуммаНалоговВзносовПоВедомостям
		|ИЗ
		|	РегистрНакопления.НалогиВзносыОтчисленияРасчетыСБюджетомИФондами КАК НалогиВзносыОтчисленияРасчетыСБюджетомИФондами
		|ГДЕ
		|	НалогиВзносыОтчисленияРасчетыСБюджетомИФондами.Регистратор В (&СсылкиНаВедомости)";
	
	Запрос.УстановитьПараметр("СсылкиНаВедомости", СсылкиНаВедомости);
	
	РезультатЗапроса = Запрос.Выполнить();
	
	СуммаНалоговВзносов = РезультатЗапроса.Выбрать();
	
	Если СуммаНалоговВзносов.Следующий() Тогда
		Возврат СуммаНалоговВзносов.СуммаНалоговВзносовПоВедомостям;
	КонецЕсли;
