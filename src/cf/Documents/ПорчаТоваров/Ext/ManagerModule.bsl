...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Изменение", Метаданные.Документы.ПорчаТоваров) Тогда
		...
		КомандаПечати.Идентификатор = "АктСписанияЗапасовЗ6";
		КомандаПечати.Представление = НСтр("ru = 'Акт списания запасов (З-6)'");
...
Функция ПолучитьДанныеДляПечатнойФормыЗ6(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Запрос.УстановитьПараметр("МассивОбъектов",                              МассивОбъектов);
	Запрос.УстановитьПараметр("ПоСебестоимости",                             Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоСебестоимости);
	Запрос.УстановитьПараметр("ПоВидуЦен",                                   Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоВидуЦен);
	...
	|ИЗ
	|	ДанныеДокументов КАК Документ
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ИнвентаризационнаяОпись КАК ИнвентаризационнаяОпись
	|		ПО (Документ.ДатаДокумента МЕЖДУ ИнвентаризационнаяОпись.ДатаНачала И ИнвентаризационнаяОпись.ДатаОкончания)
	|			И Документ.Склад = ИнвентаризационнаяОпись.Склад
	|			И Документ.Организация = ИнвентаризационнаяОпись.Организация
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Документ.ПредварительныйРасчет КАК ПредварительныйРасчет,
	|	ТаблицаОтветственныеЛица.РуководительНаименование КАК Руководитель,
	|	ТаблицаОтветственныеЛица.РуководительДолжность КАК ДолжностьРуководителя,
	|	ТаблицаОтветственныеЛица.ГлавныйБухгалтерНаименование КАК ГлавныйБухгалтер,
	|	Документ.СкладПредставление КАК СкладПредставление,
	|	Документ.Ссылка КАК Ссылка,
	|	Документ.Номер КАК Номер,
	|	Документ.ДатаДокумента КАК ДатаДокумента,
	|	Документ.Организация КАК Организация,
	|	Документ.Кладовщик КАК Кладовщик,
	|	Документ.ДолжностьКладовщика КАК ДолжностьКладовщика,
	|	Документ.Ответственный КАК Ответственный,
	|	Документ.ИсточникИнформацииОЦенахДляПечати КАК ИсточникИнформацииОЦенахДляПечати,
	|	Документ.ВидЦены КАК ВидЦены,
	|	Документ.Склад КАК Склад,
	|	ДокументОснование.Основание.Дата КАК ОснованиеДата,
	|	ДокументОснование.Основание.Номер КАК НомерОснования,
	|	ДокументОснование.Основание КАК ДокументОснование
	|ИЗ
	|	ДанныеДокументов КАК Документ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДокументОснование КАК ДокументОснование
	|		ПО ДокументОснование.Ссылка = Документ.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаОтветственныеЛица КАК ТаблицаОтветственныеЛица
	|		ПО Документ.Ссылка = ТаблицаОтветственныеЛица.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	...
	|ВЫБРАТЬ
	|	Документ.Ссылка КАК Ссылка,
	|	Документ.ДатаДокумента КАК Дата,
	|	Документ.ВидЦены КАК ВидЦены,
	|	Документ.ПриходоватьТоварыПоСебестоимостиСписания,
	...
	|ИЗ
	|	Документ.ПорчаТоваров.Товары КАК ПорчаТоваровТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеДокументов КАК Документ
	|		ПО Документ.Ссылка = ПорчаТоваровТовары.Ссылка
	|			И Документ.ИсточникИнформацииОЦенахДляПечати = &ПоВидуЦен
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ПорчаТоваровТовары.ВидЦены КАК ВидЦены,
	|	МАКСИМУМ(ЦеныНоменклатуры.Период) КАК Период
	|ПОМЕСТИТЬ ТаблицаЦенПоВидуЦенТовары
	|ИЗ
	|	ПорчаТоваровТовары КАК ПорчаТоваровТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры КАК ЦеныНоменклатуры
	|		ПО ПорчаТоваровТовары.Номенклатура = ЦеныНоменклатуры.Номенклатура
	|			И ПорчаТоваровТовары.Характеристика = ЦеныНоменклатуры.Характеристика
	|			И ПорчаТоваровТовары.ВидЦены = ЦеныНоменклатуры.ВидЦены
	|			И ПорчаТоваровТовары.Дата >= ЦеныНоменклатуры.Период
	|
	|СГРУППИРОВАТЬ ПО
	|	ПорчаТоваровТовары.Характеристика,
	...
	|	ПорчаТоваровТовары.Цена КАК Цена,
	|	МАКСИМУМ(ЦеныНоменклатуры.Период) КАК Период
	|ПОМЕСТИТЬ ТаблицаЦенПоВидуЦенУтиль
	|ИЗ
	|	ПорчаТоваровТовары КАК ПорчаТоваровТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры КАК ЦеныНоменклатуры
	|		ПО ПорчаТоваровТовары.НоменклатураОприходование = ЦеныНоменклатуры.Номенклатура
	|			И ПорчаТоваровТовары.ХарактеристикаОприходование = ЦеныНоменклатуры.Характеристика
	|			И ПорчаТоваровТовары.ВидЦены = ЦеныНоменклатуры.ВидЦены
	|			И ПорчаТоваровТовары.Дата >= ЦеныНоменклатуры.Период
	|ГДЕ
	|	ПорчаТоваровТовары.НоменклатураОприходование.Качество = ЗНАЧЕНИЕ(Перечисление.ГрадацииКачества.НеГоден)
	|	
	|СГРУППИРОВАТЬ ПО
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеДокументов КАК Документ
	|	ПО Документ.Ссылка = ПорчаТоваровОбщая.Ссылка
	|		И Документ.ИсточникИнформацииОЦенахДляПечати = &ПоСебестоимости
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПорчаТоваровСКлючамиАналитики.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	|	ПорчаТоваровСКлючамиАналитики.ВидЗапасов КАК ВидЗапасов,
	|	ПорчаТоваровСКлючамиАналитики.Организация КАК Организация,
	|	ПорчаТоваровСКлючамиАналитики.Характеристика КАК Характеристика,
	|	ПорчаТоваровСКлючамиАналитики.Количество КАК Количество,
	|	ПорчаТоваровСКлючамиАналитики.НомерСтроки КАК НомерСтроки,
	|	ПорчаТоваровСКлючамиАналитики.Номенклатура КАК Номенклатура,
	|	ПорчаТоваровСКлючамиАналитики.Ссылка КАК Ссылка,
	|	ПорчаТоваровСКлючамиАналитики.НоменклатураОприходование   КАК НоменклатураОприходование,
	|	ПорчаТоваровСКлючамиАналитики.ХарактеристикаОприходование КАК ХарактеристикаОприходование,
	|	ПорчаТоваровСКлючамиАналитики.Цена                        КАК Цена,
	|	МАКСИМУМ(ЦеныНоменклатуры.Период) КАК Период
	|ПОМЕСТИТЬ ТаблицаЦенПоПредварительнойСебестоимости
	|ИЗ
	|	ПорчаТоваровСКлючамиАналитики КАК ПорчаТоваровСКлючамиАналитики
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтоимостьТоваров КАК ЦеныНоменклатуры
	|		ПО ПорчаТоваровСКлючамиАналитики.АналитикаУчетаНоменклатуры = ЦеныНоменклатуры.АналитикаУчетаНоменклатуры
	|			И (ПорчаТоваровСКлючамиАналитики.ВидЗапасов = ЦеныНоменклатуры.ВидЗапасов
	|					И &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	|				ИЛИ НЕ &УчитыватьСебестоимостьТоваровПоВидамЗапасов)
	|			И (ЦеныНоменклатуры.Организация = ПорчаТоваровСКлючамиАналитики.Организация)
	|			И ПорчаТоваровСКлючамиАналитики.Дата >= ЦеныНоменклатуры.Период
	|ГДЕ
	|	ЕСТЬNULL(ПорчаТоваровСКлючамиАналитики.ПредварительныйРасчет, ЛОЖЬ) = ИСТИНА
	|	
	|СГРУППИРОВАТЬ ПО
	|	ПорчаТоваровСКлючамиАналитики.АналитикаУчетаНоменклатуры,
	|	ПорчаТоваровСКлючамиАналитики.ВидЗапасов,
	|	ПорчаТоваровСКлючамиАналитики.Организация,
	|	ПорчаТоваровСКлючамиАналитики.Характеристика,
	|	ПорчаТоваровСКлючамиАналитики.Количество,
	|	ПорчаТоваровСКлючамиАналитики.НомерСтроки,
	|	ПорчаТоваровСКлючамиАналитики.Номенклатура,
	|	ПорчаТоваровСКлючамиАналитики.Ссылка,
	|	ПорчаТоваровСКлючамиАналитики.НоменклатураОприходование,
	|	ПорчаТоваровСКлючамиАналитики.ХарактеристикаОприходование,
	|	ПорчаТоваровСКлючамиАналитики.Цена
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			ТОГДА 0
	|		ИНАЧЕ ВЫБОР
	|				КОГДА СебестоимостьТоваровПоДокументуДвижения.АналитикаУчетаНоменклатуры ЕСТЬ NULL 
	|					ТОГДА СУММА(ЕСТЬNULL(СебестоимостьТоваровПоРегистратору.СтоимостьРегл, 0)) / СУММА(СебестоимостьТоваровПоРегистратору.Количество)
	|				ИНАЧЕ СУММА(ЕСТЬNULL(СебестоимостьТоваровПоДокументуДвижения.СтоимостьРегл, 0)) / СУММА(СебестоимостьТоваровПоРегистратору.Количество)
	|			КОНЕЦ
	|	КОНЕЦ КАК Цена,
	|	ПорчаТоваровСКлючамиАналитики.Ссылка
	|ПОМЕСТИТЬ ТаблицаЦенПоФактическойСебестоимости
	|ИЗ
	|	ПорчаТоваровСКлючамиАналитики КАК ПорчаТоваровСКлючамиАналитики
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров КАК СебестоимостьТоваровПоДокументуДвижения
	|		ПО ПорчаТоваровСКлючамиАналитики.АналитикаУчетаНоменклатуры = СебестоимостьТоваровПоДокументуДвижения.АналитикаУчетаНоменклатуры
	|			И (ПорчаТоваровСКлючамиАналитики.ВидЗапасов = СебестоимостьТоваровПоДокументуДвижения.ВидЗапасов
	|					И &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	|				ИЛИ НЕ &УчитыватьСебестоимостьТоваровПоВидамЗапасов)
	|			И (СебестоимостьТоваровПоДокументуДвижения.ДокументДвижения = ПорчаТоваровСКлючамиАналитики.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.СебестоимостьТоваров КАК СебестоимостьТоваровПоРегистратору
	|		ПО ПорчаТоваровСКлючамиАналитики.АналитикаУчетаНоменклатуры = СебестоимостьТоваровПоРегистратору.АналитикаУчетаНоменклатуры
	|			И (ПорчаТоваровСКлючамиАналитики.ВидЗапасов = СебестоимостьТоваровПоРегистратору.ВидЗапасов
	|					И &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	|				ИЛИ НЕ &УчитыватьСебестоимостьТоваровПоВидамЗапасов)
	|			И (СебестоимостьТоваровПоРегистратору.Регистратор = ПорчаТоваровСКлючамиАналитики.Ссылка)
	|ГДЕ
	|	ЕСТЬNULL(ПорчаТоваровСКлючамиАналитики.ПредварительныйРасчет, ИСТИНА) = ЛОЖЬ
	|	
	|СГРУППИРОВАТЬ ПО
	|	ПорчаТоваровСКлючамиАналитики.АналитикаУчетаНоменклатуры,
	|	СебестоимостьТоваровПоДокументуДвижения.АналитикаУчетаНоменклатуры,
	|	ПорчаТоваровСКлючамиАналитики.ВидЗапасов,
	|	ПорчаТоваровСКлючамиАналитики.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаЦенНоменклатуры.Характеристика.НаименованиеПолное КАК ХарактеристикаНаименование,
	|	ПРЕДСТАВЛЕНИЕ(ТаблицаЦенНоменклатуры.Номенклатура.ЕдиницаИзмерения) КАК ЕдиницаИзмерения,
	|	ТаблицаЦенНоменклатуры.Номенклатура." + ?(ИспользуетсяДопКолонка, ДопКолонка, "Код") + " КАК НоменклатурныйНомер,
	|	ТаблицаЦенНоменклатуры.Количество КАК Количество,
	|	ТаблицаЦенНоменклатуры.НомерСтроки КАК НомерСтроки,
	|	ВЫБОР
	...
	|		КОГДА НЕ &ТекстЗапросаКоэффициентУпаковки ЕСТЬ NULL 
	|				И &ТекстЗапросаКоэффициентУпаковки <> 0
	|			ТОГДА ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) / &ТекстЗапросаКоэффициентУпаковки * ТаблицаЦенНоменклатуры.Количество
	|		ИНАЧЕ ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) * ТаблицаЦенНоменклатуры.Количество
	|	КОНЕЦ КАК СуммаСНДС,
	|	ТаблицаЦенНоменклатуры.ВидЦены.ВалютаЦены КАК Валюта,
	|	ТаблицаЦенНоменклатуры.Номенклатура.НаименованиеПолное КАК НоменклатураНаименование,
	|	ТаблицаЦенНоменклатуры.Номенклатура КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА ТаблицаЦенНоменклатуры.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = &ВозвратнаяТара
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоВозвратнаяТара,
	|	ТаблицаЦенНоменклатуры.Номенклатура.СрокГодности КАК СрокСлужбы,
	|	ТаблицаЦенНоменклатуры.Ссылка КАК Ссылка
	|ИЗ
	|	ТаблицаЦенПоВидуЦенТовары КАК ТаблицаЦенНоменклатуры
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры КАК ЦеныНоменклатуры
	|		ПО ТаблицаЦенНоменклатуры.Номенклатура = ЦеныНоменклатуры.Номенклатура
	|			И ТаблицаЦенНоменклатуры.Характеристика = ЦеныНоменклатуры.Характеристика
	|			И ТаблицаЦенНоменклатуры.ВидЦены = ЦеныНоменклатуры.ВидЦены
	|			И ТаблицаЦенНоменклатуры.Период = ЦеныНоменклатуры.Период
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ТаблицаПорчаТоваров.Характеристика.НаименованиеПолное,
	|	ПРЕДСТАВЛЕНИЕ(ТаблицаПорчаТоваров.Номенклатура.ЕдиницаИзмерения),
	|	ТаблицаПорчаТоваров.Номенклатура." + ?(ИспользуетсяДопКолонка, ДопКолонка, "Код") + ",
	|	ТаблицаПорчаТоваров.Количество,
	|	ТаблицаПорчаТоваров.НомерСтроки,
	|	ЕСТЬNULL(ЦеныНоменклатуры.СтоимостьРегл, 0),
	|	ЕСТЬNULL(ЦеныНоменклатуры.СтоимостьРегл, 0) * ТаблицаПорчаТоваров.Количество,
	|	&ВалютаРегламентированногоУчета КАК Валюта,
	|	ТаблицаПорчаТоваров.Номенклатура.НаименованиеПолное,
	|	ТаблицаПорчаТоваров.Номенклатура,
	|	ВЫБОР
	|		КОГДА ТаблицаПорчаТоваров.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = &ВозвратнаяТара
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	ТаблицаПорчаТоваров.Номенклатура.СрокГодности,
	|	ТаблицаПорчаТоваров.Ссылка
	|ИЗ
	|	ТаблицаЦенПоПредварительнойСебестоимости КАК ТаблицаПорчаТоваров
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтоимостьТоваров КАК ЦеныНоменклатуры
	|		ПО ТаблицаПорчаТоваров.АналитикаУчетаНоменклатуры = ЦеныНоменклатуры.АналитикаУчетаНоменклатуры
	|			И (ТаблицаПорчаТоваров.ВидЗапасов = ЦеныНоменклатуры.ВидЗапасов
	|					И &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	|				ИЛИ НЕ &УчитыватьСебестоимостьТоваровПоВидамЗапасов)
	|			И (ЦеныНоменклатуры.Организация = ТаблицаПорчаТоваров.Организация)
	|			И ЦеныНоменклатуры.Период = ТаблицаПорчаТоваров.Период
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ПорчаТоваровСКлючамиАналитики.Характеристика КАК Справочник.ХарактеристикиНоменклатуры).НаименованиеПолное,
	|	ПРЕДСТАВЛЕНИЕ(ВЫРАЗИТЬ(ПорчаТоваровСКлючамиАналитики.Номенклатура КАК Справочник.Номенклатура).ЕдиницаИзмерения),
	|	ПорчаТоваровСКлючамиАналитики.Номенклатура." + ?(ИспользуетсяДопКолонка, ДопКолонка, "Код") + ",
	|	ПорчаТоваровСКлючамиАналитики.Количество,
	|	ПорчаТоваровСКлючамиАналитики.НомерСтроки,
	|	ТаблицаЦен.Цена,
	|	ТаблицаЦен.Цена * ПорчаТоваровСКлючамиАналитики.Количество,
	|	&ВалютаРегламентированногоУчета КАК Валюта,
	|	ПорчаТоваровСКлючамиАналитики.Номенклатура.НаименованиеПолное,
	|	ПорчаТоваровСКлючамиАналитики.Номенклатура,
	|	ВЫБОР
	|		КОГДА ПорчаТоваровСКлючамиАналитики.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = &ВозвратнаяТара
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	ПорчаТоваровСКлючамиАналитики.Номенклатура.СрокГодности,
	|	ПорчаТоваровСКлючамиАналитики.Ссылка
	|ИЗ
	|	ПорчаТоваровСКлючамиАналитики КАК ПорчаТоваровСКлючамиАналитики
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаЦенПоФактическойСебестоимости КАК ТаблицаЦен
	|		ПО ПорчаТоваровСКлючамиАналитики.ВидЗапасов = ТаблицаЦен.ВидЗапасов
	|			И ПорчаТоваровСКлючамиАналитики.АналитикаУчетаНоменклатуры = ТаблицаЦен.АналитикаУчетаНоменклатуры
	|ГДЕ
	|	ЕСТЬNULL(ПорчаТоваровСКлючамиАналитики.ПредварительныйРасчет, ИСТИНА) = ЛОЖЬ
	|
	|УПОРЯДОЧИТЬ ПО
	...
	|ВЫБРАТЬ
	|	ТаблицаЦенНоменклатуры.Характеристика.НаименованиеПолное КАК ХарактеристикаНаименование,
	|	ПРЕДСТАВЛЕНИЕ(ТаблицаЦенНоменклатуры.Номенклатура.ЕдиницаИзмерения) КАК УтильЕдиницаИзмерения,
	|	ТаблицаЦенНоменклатуры.Номенклатура." + ?(ИспользуетсяДопКолонка, ДопКолонка, "Код") + " КАК НоменклатураКод,
	|	ТаблицаЦенНоменклатуры.Количество КАК Количество,
	|	ТаблицаЦенНоменклатуры.НомерСтроки КАК НомерСтроки,
	|	ВЫБОР
	...
	|				ИНАЧЕ ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0)
	|			КОНЕЦ
	|		ИНАЧЕ ТаблицаЦенНоменклатуры.Цена
	|	КОНЕЦ КАК Цена,
	|	ВЫБОР
	|		КОГДА Документ.ПриходоватьТоварыПоСебестоимостиСписания
	|			ТОГДА ВЫБОР 
	...
	|				ИНАЧЕ ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) * ТаблицаЦенНоменклатуры.Количество
	|			КОНЕЦ
	|		ИНАЧЕ ТаблицаЦенНоменклатуры.Цена * ТаблицаЦенНоменклатуры.Количество
	|	КОНЕЦ КАК Сумма,
	|	ВЫБОР
	|		КОГДА Документ.ПриходоватьТоварыПоСебестоимостиСписания
	|			ТОГДА ТаблицаЦенНоменклатуры.ВидЦены.ВалютаЦены 
	|		ИНАЧЕ Документ.ВалютаДокумента
	|	КОНЕЦ КАК Валюта,
	|	ТаблицаЦенНоменклатуры.Номенклатура.НаименованиеПолное КАК НоменклатураНаименование,
	|	ТаблицаЦенНоменклатуры.Номенклатура КАК Номенклатура,
	|	ВЫБОР
	...
	|	ТаблицаЦенНоменклатуры.Ссылка КАК Ссылка
	|ИЗ
	|	ТаблицаЦенПоВидуЦенУтиль КАК ТаблицаЦенНоменклатуры
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеДокументов КАК Документ
	|		ПО Документ.Ссылка = ТаблицаЦенНоменклатуры.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры КАК ЦеныНоменклатуры
	|		ПО ТаблицаЦенНоменклатуры.Номенклатура = ЦеныНоменклатуры.Номенклатура
	|			И ТаблицаЦенНоменклатуры.Характеристика = ЦеныНоменклатуры.Характеристика
	...
	|
	|ВЫБРАТЬ
	|	ТаблицаПорчаТоваров.ХарактеристикаОприходование.НаименованиеПолное,
	|	ПРЕДСТАВЛЕНИЕ(ТаблицаПорчаТоваров.НоменклатураОприходование.ЕдиницаИзмерения),
	|	ТаблицаПорчаТоваров.НоменклатураОприходование." + ?(ИспользуетсяДопКолонка, ДопКолонка, "Код") + ",
	|	ТаблицаПорчаТоваров.Количество,
	|	ТаблицаПорчаТоваров.НомерСтроки,
	|	ВЫБОР
	|		КОГДА Документ.ПриходоватьТоварыПоСебестоимостиСписания
	|			ТОГДА ЕСТЬNULL(ЦеныНоменклатуры.СтоимостьРегл, 0)
	|		ИНАЧЕ ТаблицаПорчаТоваров.Цена
	|	КОНЕЦ КАК Цена,
	|	ВЫБОР
	|		КОГДА Документ.ПриходоватьТоварыПоСебестоимостиСписания
	|			ТОГДА ЕСТЬNULL(ЦеныНоменклатуры.СтоимостьРегл, 0) * ТаблицаПорчаТоваров.Количество
	|		ИНАЧЕ ТаблицаПорчаТоваров.Цена * ТаблицаПорчаТоваров.Количество
	|	КОНЕЦ КАК Сумма,
	|	&ВалютаРегламентированногоУчета КАК Валюта,
	|	ТаблицаПорчаТоваров.НоменклатураОприходование.НаименованиеПолное,
	|	ТаблицаПорчаТоваров.НоменклатураОприходование,
	|	ВЫБОР
	|		КОГДА ТаблицаПорчаТоваров.НоменклатураОприходование.ВидНоменклатуры.ТипНоменклатуры = &ВозвратнаяТара
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	ТаблицаПорчаТоваров.НоменклатураОприходование.СрокГодности,
	|	ТаблицаПорчаТоваров.Ссылка
	|ИЗ
	|	ТаблицаЦенПоПредварительнойСебестоимости КАК ТаблицаПорчаТоваров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеДокументов КАК Документ
	|		ПО Документ.Ссылка = ТаблицаПорчаТоваров.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтоимостьТоваров КАК ЦеныНоменклатуры
	|		ПО ТаблицаПорчаТоваров.АналитикаУчетаНоменклатуры = ЦеныНоменклатуры.АналитикаУчетаНоменклатуры
	|			И (ТаблицаПорчаТоваров.ВидЗапасов = ЦеныНоменклатуры.ВидЗапасов
	...
	|			И ЦеныНоменклатуры.Период = ТаблицаПорчаТоваров.Период
	|ГДЕ
	|	ТаблицаПорчаТоваров.НоменклатураОприходование.Качество = ЗНАЧЕНИЕ(Перечисление.ГрадацииКачества.НеГоден)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ПорчаТоваровСКлючамиАналитики.ХарактеристикаОприходование КАК Справочник.ХарактеристикиНоменклатуры).НаименованиеПолное,
	|	ПРЕДСТАВЛЕНИЕ(ВЫРАЗИТЬ(ПорчаТоваровСКлючамиАналитики.НоменклатураОприходование КАК Справочник.Номенклатура).ЕдиницаИзмерения),
	|	ПорчаТоваровСКлючамиАналитики.НоменклатураОприходование." + ?(ИспользуетсяДопКолонка, ДопКолонка, "Код") + ",
	|	ПорчаТоваровСКлючамиАналитики.Количество,
	|	ПорчаТоваровСКлючамиАналитики.НомерСтроки,
	|	ВЫБОР
	|		КОГДА Документ.ПриходоватьТоварыПоСебестоимостиСписания
	|			ТОГДА ЕСТЬNULL(ТаблицаЦен.Цена, 0)
	|		ИНАЧЕ ПорчаТоваровСКлючамиАналитики.Цена
	|	КОНЕЦ КАК Цена,
	|	ВЫБОР
	|		КОГДА Документ.ПриходоватьТоварыПоСебестоимостиСписания
	|			ТОГДА ЕСТЬNULL(ТаблицаЦен.Цена, 0) * ПорчаТоваровСКлючамиАналитики.Количество
	|		ИНАЧЕ ПорчаТоваровСКлючамиАналитики.Цена * ПорчаТоваровСКлючамиАналитики.Количество
	|	КОНЕЦ КАК Сумма,
	|	&ВалютаРегламентированногоУчета КАК Валюта,
	|	ПорчаТоваровСКлючамиАналитики.НоменклатураОприходование.НаименованиеПолное,
	|	ПорчаТоваровСКлючамиАналитики.НоменклатураОприходование,
	|	ВЫБОР
	|		КОГДА ПорчаТоваровСКлючамиАналитики.НоменклатураОприходование.ВидНоменклатуры.ТипНоменклатуры = &ВозвратнаяТара
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	ПорчаТоваровСКлючамиАналитики.НоменклатураОприходование.СрокГодности,
	|	ПорчаТоваровСКлючамиАналитики.Ссылка
	|ИЗ
	|	ПорчаТоваровСКлючамиАналитики КАК ПорчаТоваровСКлючамиАналитики
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеДокументов КАК Документ
	|		ПО Документ.Ссылка = ПорчаТоваровСКлючамиАналитики.Ссылка
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаЦенПоФактическойСебестоимости КАК ТаблицаЦен
	|		ПО ПорчаТоваровСКлючамиАналитики.ВидЗапасов = ТаблицаЦен.ВидЗапасов
	|			И ПорчаТоваровСКлючамиАналитики.АналитикаУчетаНоменклатуры = ТаблицаЦен.АналитикаУчетаНоменклатуры
	|ГДЕ
	|	ПорчаТоваровСКлючамиАналитики.НоменклатураОприходование.Качество = ЗНАЧЕНИЕ(Перечисление.ГрадацииКачества.НеГоден)
	|	И ЕСТЬNULL(ПорчаТоваровСКлючамиАналитики.ПредварительныйРасчет, ИСТИНА) = ЛОЖЬ
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка,
	|	НомерСтроки
	...
	Возврат Новый Структура(
		"РезультатПоШапке, РезультатПоТоварам, РезультатУтиль",
		РезультатПакетаЗапросов[3],
		РезультатПакетаЗапросов[10],
		РезультатПакетаЗапросов[11]);
