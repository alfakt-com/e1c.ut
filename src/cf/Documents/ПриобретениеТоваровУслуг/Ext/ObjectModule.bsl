...
Процедура ЗаполнитьУсловияЗакупок(Знач УсловияЗакупок) Экспорт
	...
	Если УсловияЗакупок.ИспользуютсяДоговорыКонтрагентов <> Неопределено И УсловияЗакупок.ИспользуютсяДоговорыКонтрагентов Тогда
		...
		Если ЗначениеЗаполнено(Договор) Тогда
			НалогообложениеНДС = ЗначениеНастроекПовтИсп.НалогообложениеНДС(, , Договор);
		КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ПолучитьФункциональнуюОпцию("ЗапретитьПоступлениеТоваровБезНомеровГТД")
		И ХозяйственныеОперацииИмпорта.Найти(ХозяйственнаяОперация) = Неопределено
		И ХозяйственныеОперацииВвозаИзЕАЭС.Найти(ХозяйственнаяОперация) = Неопределено Тогда
	...
	КонецЕсли;
		...
		МассивНепроверяемыхРеквизитов.Добавить("ВидПоступленияНДСНалоговогоАгента");
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.ЗаписьКнигиПокупок") Тогда
		ЗаписьКнигиПокупок = ДанныеЗаполнения;
	КонецЕсли;
...
Процедура ЗаполнитьЗакупкуПодДеятельность()
	...
	ЗакупкаПодДеятельность = ЗначениеНастроекПовтИсп.ЗакупкаПодДеятельность(
								Организация, 
								Склад, 
								Договор, 
