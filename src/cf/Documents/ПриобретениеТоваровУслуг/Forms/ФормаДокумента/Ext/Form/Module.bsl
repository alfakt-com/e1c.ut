...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СкладыСервер.ЗаполнитьСкладыВТабличнойЧасти(Объект.Склад, Ложь, Объект.Товары, Истина);
...
Процедура ОбработкаВыбора(РезультатВыбора, ИсточникВыбора)
	...
	// СНТ	
	ИначеЕсли ИсточникВыбора.ИмяФормы = "Документ.СНТ.ФормаВыбора" Тогда
		Если РезультатВыбора <> Неопределено Тогда
			СНТКлиентПереопределяемый.ОбработкаВыбораСНТ(Объект.Ссылка, РезультатВыбора);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_СчетФактура" Тогда   //При создании СФ из СНТ
		НастроитьПредставлениеСчетаФактуры();
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	ПровернаНаДоступностьКДоверенности(); ///Сергей 24.09.2020 +++ (По Доверенностям)
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	///Сергей 24.09.2020 +++ (По Доверенностям)
	Если ЗначениеЗаполнено(Объект.new_Доверенность) Тогда
		ДокДов = Объект.new_Доверенность.ПолучитьОбъект();
		
		ЕстьСовпад = Ложь;
		Для Каждого СтрДов Из ДокДов.new_Поступление Цикл 
			Если СтрДов.ПриобретениеТоваровУслуг = Объект.Ссылка Тогда
				ЕстьСовпад = Истина;	
			КонецЕсли; 
		КонецЦикла;
		
		Если ЕстьСовпад = Ложь Тогда 
			НовСТР = ДокДов.new_Поступление.Добавить();
			НовСТР.ПриобретениеТоваровУслуг = Объект.Ссылка;
		КонецЕсли;
		ДокДов.Статус = Перечисления.СтатусыДоверенностей.Использована;
		ДокДов.Записать(РежимЗаписиДокумента.Запись);
	Иначе 
		Если Объект.new_НужнаДоверенность = "По Доверенности" Тогда
			Сообщить("Вы не привязали Доверенность к поступлению!!!");
			Отказ = Истина;
			Возврат;
		КонецЕсли;		
	КонецЕсли;
...
Процедура РеквизитыПечатиАвансовогоОтчетаНажатие(Элемент)
	...
	ДанныеПечати.Вставить("Контрагент",             	  Объект.Контрагент);
...
Процедура ПерезаполнитьПоПриемке(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ПерезаполнитьПоПриемкеЗавершение", ЭтотОбъект, Новый Структура("ЕстьПринятыеТовары", ЕстьПринятыеТовары)), НСтр("ru='Перенести товары по факту в товары по данным поставщика перед заполнением по фактической приемке?'"), СписокКнопок);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьПоЗаказу(Команда)
	...
	Если Объект.Товары.Количество() > 0 Тогда
		...
		ПоказатьВопрос(Новый ОписаниеОповещения("ЗаполнитьПоЗаказуЗавершение", ЭтотОбъект), НСтр("ru='Список ""Товары"" будет перезаполнен. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
        Возврат;
...
Процедура УстановитьУсловноеОформление()
	...
	ХозяйственныеОперацииИмпорта = ХозяйственныеОперацииИмпорта();
	ХозяйственныеОперацииИмпорта.Добавить(ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС"));
		...
		Если ПересчитыватьЦены Тогда
			...
			ЦеныРассчитаны = ЗакупкиСервер.ЗаполнитьЦены(
				...
				Новый Структура( // Параметры заполнения
					"ПоляЗаполнения, Дата, Валюта, Соглашение, НалогообложениеНДС",
...
Процедура ПриИзмененииДоговораСервер()
	...
	Если ЗначениеЗаполнено(Объект.Договор) Тогда
		...
		Если ХозОперацииЗакупкаПоИмпорту.Найти(Объект.ХозяйственнаяОперация) = Неопределено
			И ХозОперацииЗакупкаВСтранахЕАЭС.Найти(Объект.ХозяйственнаяОперация) = Неопределено Тогда
			Объект.НалогообложениеНДС = ЗначениеНастроекПовтИсп.НалогообложениеНДС(, , Объект.Договор);
		Иначе
...
Функция ЗаполнитьЦеныПоСоглашениюСервер(ПоВсемСтрокам = Ложь)
	...
	ПараметрыЗаполнения.Вставить("ПоляЗаполнения", "Цена, СтавкаНДС, ВидЦеныПоставщика, НалогообложениеНДС");
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	// СНТ
	СНТСерверПереопределяемый.ОбновитьРеквизитыСНТ(ЭтаФорма,,,"ГруппаИнтеграция");
	// Конец СНТ
...
Процедура УстановитьВидимостьЭлементовПоОперацииСервер()
	...
	ВидимостьЭлементов = (ХозяйственныеОперацииИмпорта.Найти(Объект.ХозяйственнаяОперация) = Неопределено
		И Объект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ЗакупкаВСтранахЕАЭС);
...
Процедура АктуализироватьЗакупкуПодДеятельность(Заполнить = Истина)
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ЗакупкаУПоставщика
		...
		Если Заполнить Тогда
			Объект.ЗакупкаПодДеятельность = ЗначениеНастроекПовтИсп.ЗакупкаПодДеятельность(
												Объект.Организация, 
												Объект.Склад, 
												Объект.Договор, 
...
Процедура НастроитьПредставлениеСчетаФактуры(НеТребуетсяВводСчетаФактуры = Неопределено)
	...
	ЕстьЗаписьКнигиПокупок = ЗначениеЗаполнено(Объект.ЗаписьКнигиПокупок);
	...
	Иначе
		...
		Если НеТребуетсяВводСчетаФактуры = Неопределено Тогда
			НеТребуетсяВводСчетаФактуры = НеТребуетсяВводСчетаФактурыДляОперации(Объект.ХозяйственнаяОперация, Объект.НалогообложениеНДС) ИЛИ ЕстьЗаписьКнигиПокупок;
		КонецЕсли;
	...
	Элементы.ТекстЗаписьКнигиПокупок.Видимость = ЕстьЗаписьКнигиПокупок;
	
	Если ЕстьЗаписьКнигиПокупок Тогда
		ЭтаФорма.ТекстЗаписьКнигиПокупок = Новый ФорматированнаяСтрока(Строка(Объект.ЗаписьКнигиПокупок),,,, ПолучитьНавигационнуюСсылку(Объект.ЗаписьКнигиПокупок));
	КонецЕсли;
...
Процедура СсылкаНаЭлектронныеАВРНажатие(Элемент)
	...
	Если ЗначениеЗаполнено(ЭлектронныйАВР) Тогда
		ПоказатьЗначение( ,ЭлектронныйАВР);
	КонецЕсли;
...
&НаСервере
Процедура УстановитьЦену()
	Если Объект.ЦС_УстановкаЦен = Истина тогда
		ТипЦенДляУстановки = Константы.ЦС_ТипЦенДляУстановки.Получить();
		Коэффициент = Константы.ЦС_КоэффициентПриходная.Получить();
		ТекВалюта = Объект.Валюта;
		ДатаДок = Объект.Дата;
		Если  ТипЦенДляУстановки.ВалютаЦены <> Справочники.Валюты.НайтиПоКоду("398") тогда
		Сообщить("Валюта вида цен для установки должна быть тенге") иначе
			Для Каждого стр из Объект.Товары цикл
				Если ТекВалюта = ТипЦенДляУстановки.ВалютаЦены тогда
					Цена = окр(стр.Цена + (стр.Цена * Коэффициент/100), 0) 
				иначе
					Цена = стр.Цена + (стр.Цена * Коэффициент/100) * ПОлучитьКурс(ТекВалюта, ДатаДок);	
				КонецЕсли;		
				Если Цена <> 0 тогда	
					...
					НоваяЗапись.ВидЦены = ТипЦенДляУстановки;
					НоваяЗапись.Номенклатура = стр.Номенклатура;
					НоваяЗапись.Период = Объект.Дата;
					НоваяЗапись.Валюта = ТипЦенДляУстановки.ВалютаЦены;
...
Функция ПолучитьКурс(ТекВалюта, ДатаДок);	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
...
&НаКлиенте
Процедура ПровернаНаДоступностьКДоверенности()
	
	Если Объект.new_Доверенность.Пустая() и Объект.new_НужнаДоверенность = "" тогда
		Если Строка(Объект.ХозяйственнаяОперация) = "Закупка у поставщика" Тогда
			Элементы.ГруппаДоверенность.Видимость = Истина;	
			Элементы.new_Доверенность.Видимость = Ложь;
			Объект.new_Доверенность = "";
			Объект.new_НужнаДоверенность = "По Доверенности";
			ПриИзмененииnew_НужнаДовереннос();
		Иначе 
			Элементы.ГруппаДоверенность.Видимость = Ложь;
			Элементы.new_Доверенность.Видимость = Ложь;
			Объект.new_Доверенность.Пустая();                                     
			Объект.new_НужнаДоверенность = "";
		КонецЕсли;
	ИначеЕсли Объект.new_НужнаДоверенность = "Без Доверенности" тогда
		Если Строка(Объект.ХозяйственнаяОперация) = "Закупка у поставщика" Тогда
			Элементы.ГруппаДоверенность.Видимость = Истина;
		Иначе 
			Элементы.ГруппаДоверенность.Видимость = Ложь;	
	...
	Иначе 
		Элементы.ГруппаДоверенность.Видимость = Истина;
		Элементы.new_Доверенность.Видимость = Истина;
		Элементы.new_Доверенность.ТолькоПросмотр = Истина;
		Элементы.new_НужнаДоверенность.ТолькоПросмотр = Истина;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура new_НужнаДоверенностьПриИзменении(Элемент)
...
&НаКлиенте
Процедура ПриИзмененииnew_НужнаДовереннос()
...
Процедура СоздатьНоменклатуруПоставщикаНаСервере()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЭСФТовары.ТоварНаименование КАК ТоварНаименование,
	               ...
	               |		ПО ЭСФТовары.Ссылка.СчетФактура = СчетФактураПолученныйДокументыОснования.Ссылка
	               |ГДЕ
	...
	Для Каждого текстрока из Объект.Товары Цикл
		нашел = ТЗ.Найти(текСтрока.Номенклатура);
		Если НЕ ЗначениеЗаполнено(текСтрока.НоменклатураПоставщика) и Не нашел = Неопределено Тогда
			НоваяНоменклатураПоставщика = Справочники.НоменклатураПоставщиков.СоздатьЭлемент();
			НоваяНоменклатураПоставщика.Наименование = нашел.ТоварНаименование;
			НоваяНоменклатураПоставщика.Владелец = Объект.Контрагент.Партнер;
			НоваяНоменклатураПоставщика.Номенклатура = текСтрока.Номенклатура;
