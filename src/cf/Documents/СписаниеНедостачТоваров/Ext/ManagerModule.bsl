...
Функция ПолучитьДанныеДляПечатнойФормыЗ6(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|ВЫБРАТЬ
	|	СписаниеНедостачТоваров.Ссылка КАК Ссылка,
	|	СписаниеНедостачТоваров.ДатаДокумента КАК Дата,
	...
	|ИЗ
	|	ДанныеДокументов КАК СписаниеНедостачТоваров
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ИнвентаризационнаяОпись КАК ИнвентаризационнаяОпись
	|		ПО (СписаниеНедостачТоваров.ДатаДокумента МЕЖДУ ИнвентаризационнаяОпись.ДатаНачала И ИнвентаризационнаяОпись.ДатаОкончания)
	...
	|	СписаниеНедостачТоваров.ДатаДокумента,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ИнвентаризационнаяОпись КАК ИнвентаризационнаяОпись
	|		ПО (КоличествоИнвентаризационныхОписей.Дата МЕЖДУ ИнвентаризационнаяОпись.ДатаНачала И ИнвентаризационнаяОпись.ДатаОкончания)
	|			И КоличествоИнвентаризационныхОписей.Склад = ИнвентаризационнаяОпись.Склад
	|			И КоличествоИнвентаризационныхОписей.Организация = ИнвентаризационнаяОпись.Организация
	|			И (ИнвентаризационнаяОпись.Проведен)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	СписаниеНедостачТоваровТовары.Количество КАК Количество,
	|	СписаниеНедостачТоваровТовары.НомерСтроки КАК НомерСтроки
	|ПОМЕСТИТЬ СписаниеНедостачТоваровТовары
	|ИЗ
	|	Документ.СписаниеНедостачТоваров.Товары КАК СписаниеНедостачТоваровТовары
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеДокументов КАК Документ
	|		ПО Документ.Ссылка = СписаниеНедостачТоваровТовары.Ссылка
	...
	|	СписаниеНедостачТоваровТовары.Номенклатура КАК Номенклатура,
	|	СписаниеНедостачТоваровТовары.Ссылка КАК Ссылка,
	|	СписаниеНедостачТоваровТовары.ВидЦены КАК ВидЦены,
	|	МАКСИМУМ(ЦеныНоменклатуры.Период) КАК Период
	|ПОМЕСТИТЬ ТаблицаЦенПоВидуЦен
	|ИЗ
	|	СписаниеНедостачТоваровТовары КАК СписаниеНедостачТоваровТовары
	...
	|			И СписаниеНедостачТоваровТовары.ВидЦены = ЦеныНоменклатуры.ВидЦены
	|			И СписаниеНедостачТоваровТовары.Дата >= ЦеныНоменклатуры.Период
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	СписаниеНедостачТоваровТовары.ВидЦены
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеДокументов КАК Документ
	|		ПО Документ.Ссылка = СписаниеНедостачТоваровВидЗапасов.Ссылка
	|			И Документ.ИсточникИнформацииОЦенахДляПечати = &ПоСебестоимости
	|			И ЕСТЬNULL(Документ.ПредварительныйРасчет, ЛОЖЬ) = ИСТИНА
	...
	|			И (СправочникКлючейАналитики.Склад = СписаниеНедостачТоваровВидЗапасов.Ссылка.Склад)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаКлючейАналитики.КлючАналитикиУчетаНоменклатуры КАК КлючАналитикиУчетаНоменклатуры,
	|	ТаблицаКлючейАналитики.ВидЗапасов КАК ВидЗапасов,
	|	ТаблицаКлючейАналитики.Организация КАК Организация,
	|	ТаблицаКлючейАналитики.Характеристика КАК Характеристика,
	|	ТаблицаКлючейАналитики.Количество КАК Количество,
	|	ТаблицаКлючейАналитики.НомерСтроки КАК НомерСтроки,
	|	ТаблицаКлючейАналитики.Номенклатура КАК Номенклатура,
	|	ТаблицаКлючейАналитики.Ссылка КАК Ссылка,
	|	МАКСИМУМ(ЦеныНоменклатуры.Период) КАК Период
	|ПОМЕСТИТЬ ТаблицаЦенПоПредварительнойСебестоимости
	|ИЗ
	|	ТаблицаКлючейАналитики КАК ТаблицаКлючейАналитики
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтоимостьТоваров КАК ЦеныНоменклатуры
	|		ПО ТаблицаКлючейАналитики.КлючАналитикиУчетаНоменклатуры = ЦеныНоменклатуры.АналитикаУчетаНоменклатуры
	|			И (ТаблицаКлючейАналитики.ВидЗапасов = ЦеныНоменклатуры.ВидЗапасов
	|					И &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	|				ИЛИ НЕ &УчитыватьСебестоимостьТоваровПоВидамЗапасов)
	|			И (ЦеныНоменклатуры.Организация = ТаблицаКлючейАналитики.Организация)
	|			И ТаблицаКлючейАналитики.Дата >= ЦеныНоменклатуры.Период
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаКлючейАналитики.КлючАналитикиУчетаНоменклатуры,
	|	ТаблицаКлючейАналитики.ВидЗапасов,
	|	ТаблицаКлючейАналитики.Организация,
	|	ТаблицаКлючейАналитики.Характеристика,
	|	ТаблицаКлючейАналитики.Количество,
	|	ТаблицаКлючейАналитики.НомерСтроки,
	|	ТаблицаКлючейАналитики.Номенклатура,
	|	ТаблицаКлючейАналитики.Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ПО Документ.Ссылка = СписаниеНедостачТоваровВидЗапасов.Ссылка
	|			И Документ.ИсточникИнформацииОЦенахДляПечати = &ПоСебестоимости
	|			И ЕСТЬNULL(Документ.ПредварительныйРасчет, ИСТИНА) = ЛОЖЬ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ТоварыИнвентаризации.Ссылка КАК Ссылка
	...
	|			И (ВидыЗапасовИАналитикиУчетаНоменклатурыКомиссионныхТоваров.Ссылка = ТоварыИнвентаризации.Ссылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|					И &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	|				ИЛИ НЕ &УчитыватьСебестоимостьТоваровПоВидамЗапасов)
	|			И (СебестоимостьТоваровПоРегистратору.Регистратор = ТаблицаИнвентаризации.Ссылка)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СписаниеНедостачТоваровТовары.Характеристика.НаименованиеПолное КАК ХарактеристикаНаименование,
	|	ПРЕДСТАВЛЕНИЕ(СписаниеНедостачТоваровТовары.Номенклатура.ЕдиницаИзмерения) КАК ЕдиницаИзмерения,
	|	СписаниеНедостачТоваровТовары.Номенклатура." + ?(ИспользуетсяДопКолонка, ДопКолонка, "Код") + " КАК НоменклатурныйНомер,
	|	СписаниеНедостачТоваровТовары.Количество КАК Количество,
	|	СписаниеНедостачТоваровТовары.НомерСтроки КАК НомерСтроки,
	|	ВЫБОР
	...
	|		КОГДА НЕ &ТекстЗапросаКоэффициентУпаковки ЕСТЬ NULL 
	|				И &ТекстЗапросаКоэффициентУпаковки <> 0
	|			ТОГДА ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) / &ТекстЗапросаКоэффициентУпаковки * СписаниеНедостачТоваровТовары.Количество
	|		ИНАЧЕ ЕСТЬNULL(ЦеныНоменклатуры.Цена, 0) * СписаниеНедостачТоваровТовары.Количество
	|	КОНЕЦ КАК СуммаСНДС,
	|	СписаниеНедостачТоваровТовары.ВидЦены.ВалютаЦены КАК Валюта,
	|	СписаниеНедостачТоваровТовары.Номенклатура.НаименованиеПолное КАК НоменклатураНаименование,
	|	СписаниеНедостачТоваровТовары.Номенклатура КАК Номенклатура,
	|	ВЫБОР
	|		КОГДА СписаниеНедостачТоваровТовары.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = &ВозвратнаяТара
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоВозвратнаяТара,
	|	СписаниеНедостачТоваровТовары.Номенклатура.СрокГодности КАК СрокСлужбы,
	|	СписаниеНедостачТоваровТовары.Ссылка КАК Ссылка
	|ИЗ
	|	ТаблицаЦенПоВидуЦен КАК СписаниеНедостачТоваровТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры КАК ЦеныНоменклатуры
	|		ПО СписаниеНедостачТоваровТовары.Номенклатура = ЦеныНоменклатуры.Номенклатура
	|			И СписаниеНедостачТоваровТовары.Характеристика = ЦеныНоменклатуры.Характеристика
	|			И СписаниеНедостачТоваровТовары.ВидЦены = ЦеныНоменклатуры.ВидЦены
	|			И СписаниеНедостачТоваровТовары.Период = ЦеныНоменклатуры.Период
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ТаблицаСписаниеНедостачТоваров.Характеристика.НаименованиеПолное,
	|	ПРЕДСТАВЛЕНИЕ(ТаблицаСписаниеНедостачТоваров.Номенклатура.ЕдиницаИзмерения),
	|	ТаблицаСписаниеНедостачТоваров.Номенклатура." + ?(ИспользуетсяДопКолонка, ДопКолонка, "Код") + ",
	|	ТаблицаСписаниеНедостачТоваров.Количество,
	|	ТаблицаСписаниеНедостачТоваров.НомерСтроки,
	|	ЕСТЬNULL(ЦеныНоменклатуры.СтоимостьРегл, 0),
	|	ЕСТЬNULL(ЦеныНоменклатуры.СтоимостьРегл, 0) * ТаблицаСписаниеНедостачТоваров.Количество,
	|	&ВалютаРегламентированногоУчета,
	|	ТаблицаСписаниеНедостачТоваров.Номенклатура.НаименованиеПолное,
	|	ТаблицаСписаниеНедостачТоваров.Номенклатура,
	|	ВЫБОР
	|		КОГДА ТаблицаСписаниеНедостачТоваров.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = &ВозвратнаяТара
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	ТаблицаСписаниеНедостачТоваров.Номенклатура.СрокГодности,
	|	ТаблицаСписаниеНедостачТоваров.Ссылка
	|ИЗ
	|	ТаблицаЦенПоПредварительнойСебестоимости КАК ТаблицаСписаниеНедостачТоваров
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтоимостьТоваров КАК ЦеныНоменклатуры
	|		ПО ТаблицаСписаниеНедостачТоваров.КлючАналитикиУчетаНоменклатуры = ЦеныНоменклатуры.АналитикаУчетаНоменклатуры
	|			И (ТаблицаСписаниеНедостачТоваров.ВидЗапасов = ЦеныНоменклатуры.ВидЗапасов
	|					И &УчитыватьСебестоимостьТоваровПоВидамЗапасов
	|				ИЛИ НЕ &УчитыватьСебестоимостьТоваровПоВидамЗапасов)
	|			И (ЦеныНоменклатуры.Организация = ТаблицаСписаниеНедостачТоваров.Организация)
	|			И (ЦеныНоменклатуры.Период = ТаблицаСписаниеНедостачТоваров.Период)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВЫРАЗИТЬ(ТаблицаИнвентаризации.Характеристика КАК Справочник.ХарактеристикиНоменклатуры).НаименованиеПолное,
	|	ПРЕДСТАВЛЕНИЕ(ВЫРАЗИТЬ(ТаблицаИнвентаризации.Номенклатура КАК Справочник.Номенклатура).ЕдиницаИзмерения),
	|	ТаблицаИнвентаризации.Номенклатура." + ?(ИспользуетсяДопКолонка, ДопКолонка, "Код") + ",
	|	ТаблицаИнвентаризации.Количество,
	|	ТаблицаИнвентаризации.НомерСтроки,
	|	ТаблицаЦен.Цена,
	|	ТаблицаЦен.Цена * ТаблицаИнвентаризации.Количество,
	|	&ВалютаРегламентированногоУчета,
	|	ТаблицаИнвентаризации.Номенклатура.НаименованиеПолное,
	|	ТаблицаИнвентаризации.Номенклатура,
	|	ВЫБОР
	|		КОГДА ТаблицаИнвентаризации.Номенклатура.ВидНоменклатуры.ТипНоменклатуры = &ВозвратнаяТара
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ,
	|	ТаблицаИнвентаризации.Номенклатура.СрокГодности,
	|	ТаблицаИнвентаризации.Ссылка
	|ИЗ
	|	ТаблицаИнвентаризации КАК ТаблицаИнвентаризации
	...
	|			И ТаблицаИнвентаризации.Ссылка = ТаблицаЦен.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка,
	|	НомерСтроки
