...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("ДокументСсылка.ЭСФ") Тогда	
		...
		Если ЗначениеЗаполнено(ЭтаФорма.АктуальныйЭСФ) 
			И НЕ Объект.ПолученВЭлектронномВиде Тогда
			
			Объект.ПолученВЭлектронномВиде = Истина
...
Процедура СостояниеЭДНажатие(Элемент, СтандартнаяОбработка)
	...
	ЭСФКлиентУТ.СостояниеЭДНажатие(ЭтаФорма, ПараметрыОткрытия, ТекстСостоянияЭД);
...
Процедура ЦенностиСтавкаНДСПриИзменении(Элемент)
	...
	ОбработкаТабличнойЧастиКлиент.ОбработатьСтрокуТЧ(ТекущаяСтрока, СтруктураДействий, КэшированныеЗначения);
	ТекущаяСтрока.УплаченныйНДС = ТекущаяСтрока.СуммаНДС;
...
Процедура УстановитьУсловноеОформление()
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ЦенностиДатаОборота.Имя);

	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ДатаОборотаВТабличнойЧасти");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
...
Процедура ПриИзмененииВалютыСервер(НоваяВалюта)
	...
	Для Каждого ТекСтрока Из Объект.Ценности Цикл
		...
		ТекСтрока.УплаченныйНДС = ТекСтрока.СуммаНДС;
...
Процедура УстановитьДоступностьРеквизитовПолучения(Форма)
	...
	Форма.Элементы.СостояниеЭД.Доступность				= Форма.Получен;
