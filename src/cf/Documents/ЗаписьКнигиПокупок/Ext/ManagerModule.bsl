...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|,	ДанныеДокумента.ДатаОборотаВТабличнойЧасти КАК ДатаОборотаВТабличнойЧасти
	|ИЗ
	...
	Запрос.УстановитьПараметр("ДатаОборотаВТабличнойЧасти", 	 Реквизиты.ДатаОборотаВТабличнойЧасти);
...
Функция ТекстЗапросаТаблицаЖурналУчетаСчетовФактур(Запрос, ТекстыЗапроса, Регистры)
	...
	//в журнал записываем УплаченныйНДС, т.к. это сумма НДС указанный в СФ.
	|	ВЫБОР
	|		КОГДА &СпособКорректировкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыКорректировкиНДС.Скорректировать)
	|				И СУММА(ТаблицаЦенности.УплаченныйНДС) < 0
	|			ТОГДА -СУММА(ТаблицаЦенности.УплаченныйНДС) * &КоэффициентПересчетаВВалютуРегл
	|		ИНАЧЕ СУММА(ТаблицаЦенности.УплаченныйНДС) * &КоэффициентПересчетаВВалютуРегл
	|	КОНЕЦ                                                                                       КАК СуммаНДС,
	|	ВЫБОР
	|		КОГДА &СпособКорректировкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыКорректировкиНДС.Скорректировать)
	|				И СУММА(ТаблицаЦенности.Сумма + ТаблицаЦенности.УплаченныйНДС) < 0
	|			ТОГДА -СУММА(ТаблицаЦенности.Сумма + ТаблицаЦенности.УплаченныйНДС) * &КоэффициентПересчетаВВалютуРегл
	|		ИНАЧЕ СУММА(ТаблицаЦенности.Сумма + ТаблицаЦенности.УплаченныйНДС) * &КоэффициентПересчетаВВалютуРегл
	|	КОНЕЦ                                                                                       КАК СуммаРегл,
	//в журнал записываем УплаченныйНДС, т.к. это сумма НДС указанный в СФ. 
	|	ВЫБОР
	|		КОГДА &СпособКорректировкиНДС = ЗНАЧЕНИЕ(Перечисление.СпособыКорректировкиНДС.Скорректировать)
