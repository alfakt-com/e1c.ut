...
// Переформировывает движения счетов-фактур по регистрам НДСЗаписиКнигиПокупок и НДСЗаписиКнигиПродаж без перепроведения
// документа. Сбрасывает статус отражения в регл. учете.
//
// Параметры:
...
Функция ТоварыЗаказовКлиентов(ПлатежныйДокумент) Экспорт
	...
	|ГДЕ
	|	Товары.СтавкаНДС В (ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС12), ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС18_118), ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС10), ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС10_110))
	|
	|СГРУППИРОВАТЬ ПО
	|	Товары.Ссылка,
...
Процедура ИнициализироватьВтТаблицаНДСЗаписиКнигиПокупок(Запрос)
	...
	|	СУММА(ВЫБОР 
	|		КОГДА НДСЗаписиКнигиПродаж.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
...
Функция РезультатЗапросаПоАвансам(НачалоПериода, КонецПериода, Организация, РасчетныйДокумент, ПравилоОтбораАвансов)
	...
	|					И ПКО_РасшифровкаПлатежа.СтавкаНДС В (&СтавкиНДС12)
	...
	|	КОНЕЦ КАК Контрагент,
	|	СУММА(ВЫБОР
	|			КОГДА ТаблицаАвансы.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС12)
	...
	СтавкиНДС12 = Новый Массив;
	СтавкиНДС12.Добавить(Справочники.СтавкиНДС.НДС12);
	Запрос.УстановитьПараметр("СтавкиНДС12", СтавкиНДС12);
	...
	СтавкиБезНДС.Добавить(Справочники.СтавкиНДС.НДС0);
	СтавкиБезНДС.Добавить(Справочники.СтавкиНДС.БезНДС);
