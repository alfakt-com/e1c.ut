...
Процедура ОбработкаЗаписиНового(НовыйОбъект, Источник, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.Номенклатура") Тогда
		...
		Если ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.Номенклатура")
			ИЛИ ТипЗнч(НовыйОбъект) = Тип("СправочникСсылка.ОбъектыЭксплуатации")
...
Процедура ТоварыНайтиСоздатьОбновитьНомераГТДНаСервере(МассивИдентификаторовСтрок)
	...
	ТаблицаНомеровГТД = Новый ТаблицаЗначений; //Справочники.НомераГТД.ПустаяТаблицаНомеровГТД(); Для УПК
	...
	//Справочники.НомераГТД.НайтиСоздатьОбновитьНомераГТД(ТаблицаНомеровГТД, "Товары", Объект.Ссылка); Для УПК
...
Процедура УправлениеФормой()
	...
	Если ИспользуетсяСНТ И Объект.Направление = Перечисления.НаправленияЭСФ.Входящий 
		И СтрНайти(Объект.ДокументПодтверждающийПоставкуНомер, "SNT") > 0 
		Тогда
		//если ЭСФ получен по СНТ, то источники не показываем, т.к. они создаюстся в СНТ.
		ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыИсточникПроисхождения", "Видимость", Ложь);
		ЭСФКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ТоварыИдентификаторТовара", "Видимость", Ложь);
