...
Функция ТоварыПоДокументыОснованию(ДокументОснование) Экспорт
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		|	ВЫБОР
		|		КОГДА ВидыЗапасов.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС0) 
		...
		|	ВЫБОР
		|		КОГДА Товары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС0) 
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.АктВыполненныхРабот") Тогда
		...
		|	ВЫБОР
		|		КОГДА Услуги.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС0) 
...
Функция ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, МассивОбъектов) Экспорт 
	...
	|	РеализацияТоваровУслуг.Дата                           КАК ДатаОборотаПоРеализации,
	...
	|	СчетаФактуры.Организация                              КАК Поставщик,
	|	СчетаФактуры.Контрагент                               КАК Покупатель,
	|	РеализацияТоваровУслуг.СпособДоставки                 КАК СпособДоставки,
	|	РеализацияТоваровУслуг.Договор.НаименованиеДляПечати  КАК Договор,
	...
	|	АктВыполненныхРабот.Дата       КАК ДатаОборотаПоРеализации,
	...
	|	АктВыполненныхРабот.Договор.НаименованиеДляПечати  КАК Договор,
...
Функция ТекстЗапросаТаблицаЖурналУчетаСчетовФактур(Запрос, ТекстыЗапроса, Регистры)
	...
	|,	СУММА(Товары.СуммаСНДС) КАК СуммаРегл,
	|	СУММА(Товары.СуммаНДС) КАК СуммаНДСРегл
	|
	|ИЗ
