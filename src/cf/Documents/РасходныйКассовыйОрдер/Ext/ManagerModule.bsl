...
Функция ТекстЗапросаДетализацияПлатежа(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	-РасходныйКассовыйОрдер.СуммаДокумента КАК СуммаОперации,
	               ...
	               |	РасходныйКассовыйОрдерРасшифровкаПлатежа.Заказ КАК ДокументОснование,
...
Функция СформироватьПечатнуюФормуКО2(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьМакета.Параметры.ФИОГлавногоБухгалтера = ?(НЕ ЗначениеЗаполнено(Выборка.ГлавныйБухгалтер), "Не предусмотрен", Выборка.ГлавныйБухгалтер);
		ОбластьМакета.Параметры.ФИОКассира = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(Выборка.Кассир, Выборка.ДатаДокумента);
		
		ОбластьМакета.Параметры.Получатель    = Выборка.ФИОПолучателя;
		
		Если Выборка.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику Тогда
			ОбластьМакета.Параметры.ФИОПолучателя    = Выборка.ПодотчетноеЛицо;			
		КонецЕсли;
...
Функция СформироватьПечатнуюФормуОбъявлениеНаВзносНаличными(МассивОбъектов, ОбъектыПечати)
	...
	|	ДанныеДокумента.Организация.КБЕ КАК КБЕ,
	|	ВЫБОР
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			ОбластьМакета.Параметры.НазваниеОрганизации = Выборка.НаименованиеОрганизацииСокращенное;
			ОбластьМакета.Параметры.НазваниеПолучателя  = Выборка.НаименованиеОрганизацииСокращенное;
			ОбластьМакета.Параметры.БИНОрганизации  = Выборка.ИНН;
			ОбластьМакета.Параметры.БИНПолучателя   = Выборка.ИНН;
			ОбластьМакета.Параметры.Дата                = Выборка.Дата;
			ОбластьМакета.Параметры.Номер		        = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(
																									Выборка.Номер,
																									Ложь,
																									Истина
																								);
			ОбластьМакета.Параметры.Сумма = Формат(Выборка.Сумма, "ЧЦ=15; ЧДЦ=2; ЧРД='-'");
			...
			ОбластьМакета.Параметры.ФИОКассира = ФизическиеЛицаУТ.ФамилияИнициалыФизЛица(Выборка.Кассир);
			ОбластьМакета.Параметры.КОдОрганизации = Выборка.Кбе;
			ОбластьМакета.Параметры.КБеПолучателя  = Выборка.Кбе;	
			ОбластьМакета.Параметры.БанкПолучателя = Выборка.НаименованиеБанка;
			ОбластьМакета.Параметры.СчетЗачисления = Выборка.НомерСчета;
