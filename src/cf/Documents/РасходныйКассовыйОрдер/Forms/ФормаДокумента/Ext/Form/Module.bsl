...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратОплатыКлиенту") Тогда 
		Если ПроверитьЗаполнениеРеквизитов("Выдать") Тогда 
			Отказ = Истина;
			Возврат
		КонецЕсли;
		Если ПроверитьЗаполнениеРеквизитов("Основание") Тогда 
			Отказ = Истина;
			Возврат
		КонецЕсли;
		Если ПроверитьЗаполнениеРеквизитов("Приложение") Тогда 
			Отказ = Истина;
			Возврат
		КонецЕсли;
		Если ПроверитьЗаполнениеРеквизитов("ПоДокументу") Тогда 
			Отказ = Истина;
			Возврат
		КонецЕсли;
	КонецЕсли;
...
Процедура ПодотчетноеЛицоПриИзмененииСервер()
	...
	Если ЗначениеЗаполнено(Объект.ПодотчетноеЛицо) Тогда
		...
		ПодотчетноеЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ПодотчетноеЛицо, "Наименование");
		Объект.Выдать = СклонениеПредставленийОбъектов.ПросклонятьФИО(ПодотчетноеЛицо,3,Объект.ПодотчетноеЛицо);
