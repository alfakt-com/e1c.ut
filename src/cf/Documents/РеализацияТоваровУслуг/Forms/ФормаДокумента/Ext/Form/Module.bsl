...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Конец СНТ
	 ...
	 //Начало: МК Баубериков А - 21.05.2020
	 Если Параметры.Ключ.Пустая() 
		 И ТипЗнч(Объект.ЗаказКлиента) = Тип("ДокументСсылка.ЗаказКлиента")
		 И ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(Объект.ЗаказКлиента, "Менеджер") Тогда
				 Объект.Менеджер = Объект.ЗаказКлиента.Менеджер;
				 Объект.Подразделение = Объект.ЗаказКлиента.Подразделение;
	 КонецЕсли;
	 //Окончание: МК Баубериков А - 21.05.2020
	 
	 //++Аян_К++ 12.01.2022 страт
	Элементы.Номер.Доступность = Альфа_КТ.ПроверитьДоступ("РеализацияНомер", ПараметрыСеанса.ТекущийПользователь, ТекущаяДата(), ТекущаяДата());
	//++Аян_К++ 12.01.2022 финиш
	
	//Ербол-старт
	//Объект.swУсловияОплаты = Альфа_КТ.ПолучитьУсловиеОплаты(Объект.ЗаказКлиента);  	
	//Ербол-финиш
...
//Кт Начало Апаев_О [26.09.2022] - Задача № 6060
&НаСервере
Функция ПроверитьРольНаСервере(ИмяРоли)
	Возврат РольДоступна(ИмяРоли);
КонецФункции
//Кт Окончание Апаев_О [26.09.2022]
...
Процедура ПриОткрытии(Отказ)
	...
	//Кт Начало Апаев_О [26.09.2022] - Задача № 6060
	Если ПроверитьРольНаСервере("кт_ДоступРеализацииТолькоТекущегоДня") Тогда
		Если НЕ (НачалоДня(ТекущаяДата()) = НачалоДня(Объект.Дата)) Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не Объект.РеализацияПоЗаказам Тогда
		ВызватьИсключение("Запрещена реализация без заказа!");
	КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если (ИмяСобытия = "Запись_СчетФактураВыданный"
		И ((Параметр.Свойство("ФормаВладелец") И Параметр.ФормаВладелец = УникальныйИдентификатор)
			Или (Параметр.Свойство("СписокОснований") И Параметр.СписокОснований.НайтиПоЗначению(Объект.Ссылка) <> Неопределено)))
...
Процедура ТоварыНоменклатураПриИзмененииВопросПользователюЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	СкидкиНаценкиКлиент.СброситьФлагСкидкиРассчитаны(ЭтаФорма);
	// ВС
	СтруктураЗаполнения = Новый Структура("Номенклатура,ПризнакУчетаНаВиртуальномСкладе",ТекущаяСтрока.Номенклатура,Ложь);
	ВСВызовСервера.УстановитьПризнакВиртуальногоСкладаДляТабличнойЧасти(Объект.Товары,СтруктураЗаполнения,Объект.Дата);
	ТекущаяСтрока.ПризнакУчетаНаВиртуальномСкладе = СтруктураЗаполнения.ПризнакУчетаНаВиртуальномСкладе;
	// Конец ВС
КонецПроцедуры
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	// Конец СНТ
	// ВС
	УстановитьВидимостьПризнакВиртуальногоСклада();
	// Конец ВС
...
Процедура ПриИзмененииХозяйственнойОперацииСервер()
	...
	УчетНДСУТ.ПроверитьКорректностьНалогообложенияНДСПродажи(
...
Процедура ЗаполнитьСлужебныеРеквизитыПоНоменклатуре()
	...
	// ВС
	ВССерверПереопределяемый.УстановитьПризнакВиртуальногоСкладаДляТабличнойЧасти(Объект.Товары, , Объект.Дата);
	// Конец ВС
КонецПроцедуры
...
Процедура ИзменитьТабличнуюЧастьПоРезультатамПроверки(ВозвращаемыеПараметры, КэшированныеЗначения)
	...
	Для Каждого СтрокаИсточник Из ТаблицаТовары Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаТЧ, СтруктураДействий, КэшированныеЗначения);
			// ВС
			ВССерверПереопределяемый.УстановитьПризнакВиртуальногоСкладаДляТабличнойЧасти(Объект.Товары,СтрокаТЧ,Объект.Дата);
			// Конец ВС
		КонецЕсли;
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	// ВС
	ВСВызовСервера.УстановитьПризнакВиртуальногоСкладаДляТабличнойЧасти(Объект.Товары, , Объект.Дата);
	// Конец ВС
КонецПроцедуры
...
&НаСервереБезКонтекста
Процедура ПроверитьНаличиеЭСФ(Ссылка, Организация, ЕстьЭСФ)
	
	ЭСФСерверПереопределяемый.ПроверитьНаличиеЭСФ(Ссылка, Организация, ЕстьЭСФ); 
...
Процедура УстановитьЗаголовокРеквизитовПечати()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьАктыНаПередачуПрав") Тогда
		Элементы.РеквизитыПечатиТОРГ12.Заголовок = 
			НСтр("ru = 'Реквизиты печати накладной / Акта на передачу прав'");
	КонецЕсли;
...
Функция ВыполнитьПроверкиПередИзменениемСтатуса()
	...
	Перем ЕстьКорректировки, ЕстьЭСФ;
	...
	ПроверитьНаличиеКорректировок(Объект.Ссылка, ЕстьКорректировки);
	ПроверитьНаличиеЭСФ(Объект.Ссылка, Объект.Организация, ЕстьЭСФ);
	...
	ИначеЕсли  ЕстьЭСФ Тогда
		...
		ПоказатьВопрос(
			Оповещение,
			НСтр("ru='По связанному с документом счете-фактуре выписан ЭСФ.
				|Изменение исходного документа запрещено.'"),
			СписокКнопок,
			,
			"ВвестиКорректировку");
...
// Швецов Тимур, 03.05.2019
&НаКлиенте
Процедура swУсловияОтгрузкиПриИзменении(Элемент)
	swУстановитьВидимостьЭлементов();
...
Процедура swУстановитьВидимостьЭлементов()
	//Если Объект.swУсловияОтгрузки = Перечисления.swУсловияОтгрузки.Доставка Тогда
...
&НаСервере
Процедура УстановитьВидимостьПризнакВиртуальногоСклада()
	
	ДатаПроверки = ?(Объект.Дата = Дата(1,1,1), ТекущаяДатаСеанса(), Объект.Дата);
	
	Видимость = ЭСФСервер.ВедетсяУчетПоТоварамНаВС(ДатаПроверки);
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Элементы,
		"ТоварыПризнакУчетаНаВиртуальномСкладе",
		"Видимость",
		Видимость);
