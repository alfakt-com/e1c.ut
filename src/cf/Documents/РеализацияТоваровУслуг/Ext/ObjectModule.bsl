		...
		Если УсловияПродажПоУмолчанию <> Неопределено Тогда
			...
			Если НЕ ИспользоватьСоглашенияСКлиентами ИЛИ 
				...
				Если ИспользоватьСоглашенияСКлиентами Тогда
					...
					Если Контрагент.ЮрФизЛицо = ПредопределенноеЗначение("Перечисление.ЮрФизЛицо.ФизЛицо") тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЭтоНовый() И НЕ ЗначениеЗаполнено(Номер) Тогда
		...
		Если СокрЛП(Номер) = "02-02-99999"  Тогда 
			   ПолучитьПоследнийНомер();
		КонецЕсли;	
	КонецЕсли;
	...
	// sw end
	// sw begin
	// Швецов Тимур, 03.05.2019
	Если swУсловияОтгрузки = Перечисления.swУсловияОтгрузки.Самовывоз Тогда
		swВодительЭкспедитор = Справочники.Пользователи.ПустаяСсылка();
	КонецЕсли;
	// sw end
	
	//+ТТ
	//Если СНТ уже создан и отправлен, менять нельзя
	ПользТек = ПользователиКлиентСервер.ТекущийПользователь();
	Доступность = bon_БонуснаяПрограммаСервер.ПроверкаДоступа(ПользТек, "Доступ к изменению реал и возврат (Роль)");
	Если Не Отказ и ЗначениеЗаполнено(Ссылка) И НЕ Доступность Тогда
		Отказ = НЕ Документы.РеализацияТоваровУслуг.ПроверкаСтатусаСНТ(Отказ,Ссылка);
		Если Отказ Тогда
			Сообщить("СНТ уже отправлен! Редактирование запрещено!");
		КонецЕсли;
	КонецЕсли;
	//-ТТ
	...
	Если ТипДанныхЗаполнения = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("ДокументОснование")
			...
			Если ЗначениеЗаполнено(ЗаказКлиента) Тогда
				//ТТ+  Если не выгружен в ВМС, создавать запрещено
				Если НЕ Альфа_КТ.ПроверитьДоступ("СозданиеРеалкиНаОснованииЗКБезВыгрузкиВВМС", ПользователиКлиентСервер.ТекущийПользователь(), ТекущаяДата(), ТекущаяДата()) И ЗаказКлиента.Склад.ВидСклада = Перечисления.ВидыСкладов.Основной и НЕ Infor_Интеграция._ОбъектВыгружен(ДанныеЗаполнения.ДокументОснование) Тогда
					ВызватьИсключение("Объект не был выгружен в WMS. Отказано!");
				ИначеЕсли ЗаказКлиента.Склад.ВидСклада = Перечисления.ВидыСкладов.Основной Тогда
					Если Константы.ИспользоватьКонтрольЗадолженностиКонтрагентов.Получить() и Константы.ПроверятьЗадолженностьПередСозданиемРеализации.Получить() Тогда
						Отказ = Ложь;
						swПроверитьЗадолженностьКонтрагента(Отказ);
						Если Отказ Тогда
							ВызватьИсключение("Уже есть не проведенные документы по клиенту. Отказано!");
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				//ТТ-
				
				bon_БонусныйСчет = ЗаказКлиента.bon_БонусныйСчет;
				bon_ДатаПроведенияПервДляБонИзЗаказа = ЗаказКлиента.bon_ДатаПроведенияПервДляБон;
				//Начисленные бонусы
				Для каждого СтрокаТаблицыИсточника Из ЗаказКлиента.bon_БонусПартНачис Цикл
					СтрокаТаблицыПриемника = bon_БонусПартНачис.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриемника,СтрокаТаблицыИсточника);
				КонецЦикла;
				
				//Начисленные бонусы по Реферальной системе
				Если ЗаказКлиента.bon_БонусПартНачисРеф.Количество() > 0 Тогда
					Для каждого СтрокаТаблицыИсточника Из ЗаказКлиента.bon_БонусПартНачисРеф Цикл
						СтрокаТаблицыПриемника = bon_БонусПартНачис.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриемника,СтрокаТаблицыИсточника);
					КонецЦикла;
				КонецЕсли;
				...
				Для каждого СтрокаТаблицыИсточника Из ЗаказКлиента.bon_ПартииБонусовИст Цикл
					СтрокаТаблицыПриемника = bon_ПартииБонусовИст.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриемника,СтрокаТаблицыИсточника);
				КонецЦикла;
				
				//Подарки Выданные
				Для каждого СтрокаТаблицыИсточника Из ЗаказКлиента.bon_ПодаркиВыданные Цикл
					СтрокаТаблицыПриемника = bon_ПодаркиВыданные.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриемника,СтрокаТаблицыИсточника);
				КонецЦикла;
				
				//Срабатывшие по счетчику
				Для каждого СтрокаТаблицыИсточника Из ЗаказКлиента.bon_ТаблицаВыдАкцийпоСчетчику Цикл
					СтрокаТаблицыПриемника = bon_ТаблицаВыдАкцийпоСчетчику.Добавить();
					ЗаполнитьЗначенияСвойств(СтрокаТаблицыПриемника,СтрокаТаблицыИсточника);
				КонецЦикла;
	            //Даулет №5972 
				Комментарий = ЗаказКлиента.ДополнительнаяИнформация;
			КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//ЦС_Владимир
	Если Контрагент.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо и
		(ЭтотОбъект.БанковскийСчетОрганизации = ПредопределенноеЗначение("Справочник.БанковскиеСчетаОрганизаций.ПустаяСсылка") или	
		ЭтотОбъект.БанковскийСчетГрузоотправителя = ПредопределенноеЗначение("Справочник.БанковскиеСчетаКонтрагентов.ПустаяСсылка") или
		ЭтотОбъект.Грузоотправитель	= ПредопределенноеЗначение("Справочник.Контрагенты.ПустаяСсылка")) тогда
		Сообщить("Заполните обязательные реквизиты печати");
		Отказ = Истина;
		Возврат;	
	КонецЕсли;
	//
	...
	Если константы.ИспользоватьКонтрольЗадолженностиКонтрагентов.Получить() тогда
		swПроверитьЗадолженностьКонтрагента(Отказ);
	КонецЕсли;
	...
	Если Не Отказ Тогда
		Если ЗначениеЗаполнено(bon_БонусныйСчет) Тогда
			bon_БонуснаяПрограммаСервер.ДвиженияБонусовПоПартиямПриход(ЭтотОбъект, Товары,Отказ);
		КонецЕсли;
	КонецЕсли;
	///Сергей 07.08.2020 ---
...
Процедура ЗаполнитьДокументНаОснованииАктаОРасхожденияхПослеПриемки(Знач ДанныеЗаполнения)
	...
	|	ВозвратТоваровОтКлиента.УдалитьРуководитель                 КАК Руководитель,
	|	ВозвратТоваровОтКлиента.ВозвратПереданнойМногооборотнойТары КАК ВернутьМногооборотнуюТару,
	|	ВозвратТоваровОтКлиента.ПредусмотренЗалогЗаТару             КАК ТребуетсяЗалогЗаТару,
...
Функция ПараметрыЗаполненияВидовЗапасов(ЭтоПроведение)
	...
	УчетНДСУТ.ПараметрыЗаполненияВидовЗапасовПоНалогообложению(ПараметрыЗаполнения.ОтборыВидовЗапасов, Организация, Дата);
...
Процедура swПроверитьЗадолженностьКонтрагента(Отказ)
	СуммаВзаиморасчетов = ПолучитьСуммуВзаиморасчетовПоТЧ();
	НастройкиКонтроляЗадолженности = swОбщегоНазначения.ПолучитьНастройкиКонтроляЗадолженности(Партнер, ВалютаВзаиморасчетов, Дата);
	...
	Если Константы.ПроверятьЗадолженностьПередСозданиемРеализации.Получить() Тогда
		Текст = СтрЗаменить(Текст,"###Заменить###","ОБЪЕДИНИТЬ ВСЕ
	        |	
	        |	ВЫБРАТЬ
	...
	Иначе
		Текст = СтрЗаменить(Текст,"###Заменить###","");
	КонецЕсли;
	Запрос.Текст = Текст;
	...
	Если СуммаДолга + СуммаВзаиморасчетов > НастройкиКонтроляЗадолженности.РазрешеннаяСуммаЗадолженности Тогда
		ТекстОшибки = НСтр("ru='Долг клиента в валюте " + ВалютаВзаиморасчетов + " составляет " + Формат(СуммаДолга, "ЧЦ=18; ЧДЦ=2; ЧН=0,00") + ", по текущему документу он увеличится на " + Формат(СуммаВзаиморасчетов, "ЧЦ=18; ЧДЦ=2; ЧН=0,00") + ", в сумме это больше, чем разрешенная задолженность " + Формат(НастройкиКонтроляЗадолженности.РазрешеннаяСуммаЗадолженности, "ЧЦ=18; ЧДЦ=2; ЧН=0,00") + "'");
...
Процедура ПолучитьПоследнийНомер()
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
		|ГДЕ
		|	РеализацияТоваровУслуг.Дата >= НАЧАЛОПЕРИОДА(&Дата, ГОД)
		|	И НЕ РеализацияТоваровУслуг.ПометкаУдаления
		|
		|УПОРЯДОЧИТЬ ПО
		|	Дата УБЫВ";
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда 
		 НомерДок  = Число(Сокрлп(Прав(ВыборкаДетальныеЗаписи.Номер,7)))+1;
		 Номер = Строка(Лев(ВыборкаДетальныеЗаписи.Номер,6) + СтрЗаменить(СТРОКА(НомерДок),Символ(160),""));
	КонецЕсли;
