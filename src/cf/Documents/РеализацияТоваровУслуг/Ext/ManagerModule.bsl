...
Процедура ЗаполнитьПоОстаткамЗаказов(ДанныеОтбора,
	...
	Если МассивЗаказов <> Неопределено Тогда
		ДанныеОтбора.МаксимальныйКлючСтроки = МассивЗаказов[0].МаксимальныйКлючСтроки;	
	КонецЕсли;
	///Сергей 03.08.2020 ---
...
Функция ТекстЗапросаДляРасчетаНеобходимостиСчетаФактуры() Экспорт
	...
	|			ТОГДА 
	|				ВЫБОР 
	|					КОГДА ЛОЖЬ И УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС //НЕ РК
	|							И ДанныеДокумента.НалогообложениеНДС <> &ПродажаОблагаетсяЕНВД
	|							И ДанныеДокумента.НалогообложениеНДС <> &ПродажаНаЭкспорт
	|						ТОГДА ИСТИНА
	|					КОГДА НЕ УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС И НЕ ТоварыСНДС.Ссылка ЕСТЬ NULL
	|						ТОГДА ИСТИНА
	|					ИНАЧЕ ЛОЖЬ
	|				КОНЕЦ
	|		ИНАЧЕ ЛОЖЬ
...
Функция ТекстЗапросаТаблицаТоварыОрганизаций(Запрос, ТекстыЗапроса, Регистры)
	...
	|			ИЛИ ТаблицаВидыЗапасов.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК))
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|		 ИЛИ ВидыЗапасов.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК))
	|		 И &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|		ИНАЧЕ &НалогообложениеНДС
...
Функция ТекстЗапросаТаблицаНДСЗаписиКнигиПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС0),
	|	NULL,
	|	NULL,
	...
	|				КОГДА ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС0)
	|					ТОГДА 0
	|				КОГДА ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|					ТОГДА 0
	|				КОГДА ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК)
	|					ТОГДА 0
	...
	|	И ЛОЖЬ //Отключенный функционал для УПК
	|	
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ТекстЗапросаТаблицаСуммыДокументовВВалютеРегл(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИНАЧЕ ВЫРАЗИТЬ((ТаблицаТовары.СуммаСНДС - ТаблицаТовары.СуммаНДС) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15, 2)) КОНЕЦ) КАК СуммаБезНДСРегл,
	|	ЕСТЬNULL(Суммы.СуммаНДСРегл, ВЫБОР
	|		КОГДА &Валюта = &ВалютаРегламентированногоУчета
	|		ТОГДА ТаблицаТовары.СуммаНДС
	|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаТовары.СуммаНДС * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15, 2)) КОНЕЦ) КАК СуммаНДСРегл,
	|	ВЫБОР
	...
	|						ТаблицаТовары.СуммаВзаиморасчетов * ТаблицаТовары.СуммаНДС / ТаблицаТовары.СуммаСНДС
	|						ИНАЧЕ 0 КОНЕЦ КАК ЧИСЛО(15, 2))
	|			ИНАЧЕ ВЫРАЗИТЬ((ТаблицаТовары.СуммаСНДС - ТаблицаТовары.СуммаНДС) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15, 2))  КОНЕЦ)
	...
	|						ТаблицаТовары.СуммаВзаиморасчетов * ТаблицаТовары.СуммаНДС / ТаблицаТовары.СуммаСНДС
	|						ИНАЧЕ 0 КОНЕЦ КАК ЧИСЛО(15, 2))
	|			ИНАЧЕ ВЫРАЗИТЬ((ТаблицаТовары.СуммаСНДС - ТаблицаТовары.СуммаНДС) * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(15, 2)) КОНЕЦ)
	...
	|					ТаблицаТовары.СуммаВзаиморасчетов * ТаблицаТовары.СуммаНДС / ТаблицаТовары.СуммаСНДС
	|					ИНАЧЕ 0 КОНЕЦ КАК ЧИСЛО(15, 2))
	|		ИНАЧЕ (ТаблицаТовары.СуммаСНДС - ТаблицаТовары.СуммаНДС) КОНЕЦ) * &КоэффициентПересчетаВВалютуУпр КАК СуммаБезНДСУпр,
	|	ЕСТЬNULL(Суммы.СуммаНДСУпр, ВЫБОР
	|		КОГДА &Валюта = &ВалютаУправленческогоУчета
	|		ТОГДА ТаблицаТовары.СуммаНДС
	|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаТовары.СуммаНДС * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(15, 2)) КОНЕЦ) КАК СуммаНДСУпр,
	|
	...
	|		ИНАЧЕ ВЫРАЗИТЬ((ТаблицаВидыЗапасов.СуммаСНДС - ТаблицаВидыЗапасов.СуммаНДС) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15, 2)) КОНЕЦ) КАК СуммаБезНДСРегл,
	|	ЕСТЬNULL(Суммы.СуммаНДСРегл, ВЫБОР
	|		КОГДА &Валюта = &ВалютаРегламентированногоУчета
	|		ТОГДА ТаблицаВидыЗапасов.СуммаНДС
	|		КОГДА &ВалютаВзаиморасчетов = &ВалютаРегламентированногоУчета
	|		ТОГДА ТаблицаВидыЗапасов.СуммаВзаиморасчетовНДС
	|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаВидыЗапасов.СуммаНДС * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15, 2)) КОНЕЦ) КАК СуммаНДСРегл,
	|	ЕСТЬNULL(Суммы.БазаНДСРегл, ВЫБОР
	|		КОГДА &Валюта = &ВалютаРегламентированногоУчета
	|		ТОГДА ТаблицаВидыЗапасов.СуммаСНДС - ТаблицаВидыЗапасов.СуммаНДС
	|		КОГДА &ВалютаВзаиморасчетов = &ВалютаРегламентированногоУчета
	|		ТОГДА ТаблицаВидыЗапасов.СуммаВзаиморасчетов - ТаблицаВидыЗапасов.СуммаВзаиморасчетовНДС
	|		ИНАЧЕ ВЫРАЗИТЬ((ТаблицаВидыЗапасов.СуммаСНДС - ТаблицаВидыЗапасов.СуммаНДС) * &КоэффициентПересчетаВВалютуРегл КАК ЧИСЛО(15, 2)) КОНЕЦ) КАК БазаНДСРегл,
	|	ЕСТЬNULL(Суммы.БазаНДСУпр, ВЫБОР
	|		КОГДА &Валюта = &ВалютаУправленческогоУчета
	|		ТОГДА ТаблицаВидыЗапасов.СуммаСНДС - ТаблицаВидыЗапасов.СуммаНДС
	|		КОГДА &ВалютаВзаиморасчетов = &ВалютаУправленческогоУчета
	|		ТОГДА ТаблицаВидыЗапасов.СуммаВзаиморасчетов - ТаблицаВидыЗапасов.СуммаВзаиморасчетовНДС
	|		ИНАЧЕ ВЫРАЗИТЬ((ТаблицаВидыЗапасов.СуммаСНДС - ТаблицаВидыЗапасов.СуммаНДС) * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(15, 2)) КОНЕЦ) КАК БазаНДСУпр,
	|
	|	ЕСТЬNULL(Суммы.СуммаБезНДСУпр, ВЫБОР
	|		КОГДА &Валюта = &ВалютаУправленческогоУчета
	|		ТОГДА ТаблицаВидыЗапасов.СуммаСНДС - ТаблицаВидыЗапасов.СуммаНДС
	|		КОГДА &ВалютаВзаиморасчетов = &ВалютаУправленческогоУчета
	|		ТОГДА ТаблицаВидыЗапасов.СуммаВзаиморасчетов - ТаблицаВидыЗапасов.СуммаВзаиморасчетовНДС
	|		ИНАЧЕ ВЫРАЗИТЬ((ТаблицаВидыЗапасов.СуммаСНДС - ТаблицаВидыЗапасов.СуммаНДС) * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(15, 2)) КОНЕЦ) КАК СуммаБезНДСУпр,
	|	ЕСТЬNULL(Суммы.СуммаНДСУпр, ВЫБОР
	|		КОГДА &Валюта = &ВалютаУправленческогоУчета
	|		ТОГДА ТаблицаВидыЗапасов.СуммаНДС
	|		КОГДА &ВалютаВзаиморасчетов = &ВалютаУправленческогоУчета
	|		ТОГДА ТаблицаВидыЗапасов.СуммаВзаиморасчетовНДС
	|		ИНАЧЕ ВЫРАЗИТЬ(ТаблицаВидыЗапасов.СуммаНДС * &КоэффициентПересчетаВВалютуУпр КАК ЧИСЛО(15, 2)) КОНЕЦ) КАК СуммаНДСУпр,
	|
	|	ЗНАЧЕНИЕ(Перечисление.ТипыРасчетовСПартнерами.РасчетыСКлиентом) КАК ТипРасчетов,
	|	
...
Функция ТекстЗапросаТаблицаМатериалыИРаботыВПроизводстве(Запрос, ТекстыЗапроса, Регистры)
	...
	|			ИЛИ ТаблицаТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК)) 
	|				И &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС) 
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|		КОГДА &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЭкспортСырьевыхТоваровУслуг)
...
Процедура УстановитьПараметрЗапросаТипыНалогообложенияНДСОблагаемые(Запрос) Экспорт
	...
	Запрос.УстановитьПараметр("ТипыНалогообложенияНДСОблагаемые", УчетНДСУТ.ТипыНалогообложенияНДСОблагаемые());
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если НЕ ЭтоПартнер Тогда
		...
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.МенеджерПечати = "Обработка.ПечатьАктОбОказанииУслуг";
		КомандаПечати.Идентификатор = "Р1";
		КомандаПечати.Представление = НСтр("ru = 'Акт выполненных работ (Р-1)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		КомандаПечати.Порядок = 16;
		
		// Накладная на отпуск запасов (З-2)
		КомандаПечати = КомандыПечати.Добавить();
	...
	Если НЕ ЭтоПартнер Тогда
		// Накладная на отпуск запасов без услуг (З-2)
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.МенеджерПечати = "Обработка.ПечатьОбщихФорм";
		КомандаПечати.Идентификатор = "ТОРГ12";
		КомандаПечати.Представление = НСтр("ru = 'Накладная на отпуск запасов без услуг (З-2)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		КомандаПечати.ДополнительныеПараметры.Вставить("ВыводитьУслуги", Ложь);
		КомандаПечати.Порядок = 18;

		// Товарная накладная с номерами ГТД (З-2)
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.МенеджерПечати = "Обработка.ПечатьОбщихФорм";
		КомандаПечати.Идентификатор = "ТОРГ12";
		КомандаПечати.Представление = НСтр("ru = 'Накладная на отпуск запасов с номерами ГТД (З-2)'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		КомандаПечати.ДополнительныеПараметры.Вставить("ВыводитьУслуги", Ложь);
...
Функция СформироватьКомплектПечатныхФорм(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати) Экспорт
	...
	Для Каждого КомплектПечати Из КомплектыПечатиПоОбъектам Цикл
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			КомплектПечати.Имя,
			КомплектПечати.Представление,
			Обработки.ПечатьАктОбОказанииУслуг.СформироватьПечатнуюФормуАктВыполненныхРаботР1(СтруктураТипов, ОбъектыПечати, ПараметрыПечати));
	КонецЦикла;
...
Функция КомплектПечатныхФорм() Экспорт
	...
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "Р1", НСтр("ru = 'Акт выполненных работ (Р-1)'"),                         2);
	...
	КонецЕсли;
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ТОРГ12", НСтр("ru = 'Накладная на отпуск запасов (З-2)'"),                    0);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ТОРГ12БезУслуг", НСтр("ru = 'Накладная на отпуск запасов без услуг (З-2)'"),  2);
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ТОРГ12_ГТД", НСтр("ru = 'Накладная на отпуск запасов с номерами ГТД (З-2)'"), 0);
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьТТН") Тогда
		РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ТТН", НСтр("ru = 'Товарно-транспортная накладная'"),               		0);
...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ12(ПараметрыПечати, МассивОбъектов) Экспорт 
	...
	//ТТ
	Результат = Документы.РеализацияТоваровУслуг.ПроверкаСтатусаСНТ(Результат,МассивОбъектов.Получить(0));
	Если НЕ Результат Тогда
		ВызватьИсключение "Отказ! СНТ еще не создан. Обратитесь в бухгалтерию!";	
	КонецЕсли;
	...
	|	РеализацияТоваровУслуг.Менеджер.ФизическоеЛицо КАК Ответственный,
	|	РеализацияТоваровУслуг.ПеревозчикПартнер КАК ТранспортнаяОрганизация,
	...
	|	СУММА(ТаблицаТоваров.Цена)           КАК Цена,
	...
	|	ВЫБОР
	|		КОГДА НЕ &ВыводитьБазовыеЕдиницыИзмерения
	|			ТОГДА (ВЫБОР КОГДА ТаблицаТоваров.Ссылка.ЦенаВключаетНДС ТОГДА ТаблицаТоваров.СуммаБезНДС + ТаблицаТоваров.СуммаНДС
	|						ИНАЧЕ ТаблицаТоваров.СуммаБезНДС КОНЕЦ) / ТаблицаТоваров.КоличествоУпаковок
	|		ИНАЧЕ (ВЫБОР КОГДА ТаблицаТоваров.Ссылка.ЦенаВключаетНДС ТОГДА ТаблицаТоваров.СуммаБезНДС + ТаблицаТоваров.СуммаНДС
	|						ИНАЧЕ ТаблицаТоваров.СуммаБезНДС КОНЕЦ) / ТаблицаТоваров.Количество
	|	КОНЕЦ КАК Цена,
...
Функция ТекстЗапросаДанныхОснованияДляПечатнойФормыСчетФактура() Экспорт
	...
	|,	ДанныеДокументов.СпособДоставки                           КАК СпособДоставки,
	|	ДанныеДокументов.АдресДоставки                            КАК АдресДоставки,
	|	ДанныеДокументов.Договор                                  КАК Договор,
	|	ДанныеДокументов.ФормаОплаты                              КАК ФормаОплаты,
	|	ДанныеДокументов.УсловияПоставки                          КАК УсловияПоставки,
	|	ДанныеДокументов.СпособОтправленияПоКлассификатору        КАК СпособОтправленияПоКлассификатору,
	|	ДанныеДокументов.НомерДокументаГЗ                         КАК НомерДокументаГЗ,
	|	ДанныеДокументов.ДатаДокументаГЗ                          КАК ДатаДокументаГЗ,
	|	ДанныеДокументов.ДатаПодписанияГЗ                         КАК ДатаПодписанияГЗ
	|
	|//ОператорПОМЕСТИТЬ
	|ИЗ
	|	Документ.РеализацияТоваровУслуг КАК ДанныеДокументов
...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	|		И ВариантыКомплектацииНоменклатуры.Основной
	|ГДЕ ТаблицаДокумента.Номенклатура.ТипНоменклатуры НЕ В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга), ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа))
	|	И ДанныеДокументов.Валюта = &ВалютаРегламентированногоУчета
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаДокумента.Ссылка,
	|	ТаблицаДокумента.АналитикаУчетаНоменклатуры,
	|	ТаблицаДокумента.АналитикаУчетаНаборов,
	...
	|	АналитикаНаборов.КлючАналитики                                 КАК АналитикаУчетаНаборов,
	|	ЕСТЬNULL(ВариантыКомплектацииНоменклатуры.Ссылка, ЗНАЧЕНИЕ(Справочник.ВариантыКомплектацииНоменклатуры.ПустаяСсылка)) КАК ВариантКомплектацииНоменклатуры,
	|	ЕСТЬNULL(ВариантыКомплектацииНоменклатуры.ВариантПредставленияНабораВПечатныхФормах, ЗНАЧЕНИЕ(Перечисление.ВариантыПредставленияНаборовВПечатныхФормах.ПустаяСсылка)) КАК ВариантПредставленияНабораВПечатныхФормах,
	|	ЕСТЬNULL(ВариантыКомплектацииНоменклатуры.ВариантРасчетаЦеныНабора, ЗНАЧЕНИЕ(Перечисление.ВариантыРасчетаЦенНаборов.ПустаяСсылка)) КАК ВариантРасчетаЦеныНабора,
	...
	|
	|ГДЕ
	|	НЕ ДанныеДокументов.Валюта = &ВалютаРегламентированногоУчета
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаДокумента.Ссылка,
	|	Аналитика.КлючАналитики,
	|	АналитикаНаборов.КлючАналитики,
	...
	|		ТаблицаДанныхДокументов КАК ДанныеДокументов
	|	ПО
	|		ВидыЗапасов.Ссылка = ДанныеДокументов.Ссылка
	|ГДЕ
	|	ДанныеДокументов.Валюта = &ВалютаРегламентированногоУчета";
	
	Запрос.Выполнить();
	
	ТаблицаВидовЗапаса = Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("ВидыЗапасов").ПолучитьДанные().Выгрузить();
	ТаблицаТоваров = Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("РеализацияТоваровУслугТаблицаТоваров").ПолучитьДанные().Выгрузить();
	
	Если ПараметрыЗаполнения.ВключаяНомераГТД Тогда
		
		ИтоговаяТаблица = ТаблицаТоваров.СкопироватьКолонки();
		
		Для Каждого СтрокаТоваров Из ТаблицаТоваров Цикл 
			
			СтрокиЗапасов = ТаблицаВидовЗапаса.НайтиСтроки(Новый Структура("Ссылка, АналитикаУчетаНоменклатуры, АналитикаУчетаНаборов", СтрокаТоваров.Ссылка, СтрокаТоваров.АналитикаУчетаНоменклатуры, СтрокаТоваров.АналитикаУчетаНаборов ));
			КоличествоТовараВстроке = СтрокаТоваров.Количество;
			
			//Если запасов не обнаружено, включаем строку в итоговую таблицу целиком
			Если СтрокиЗапасов.Количество() = 0 Тогда
				...
				НоваяСтрока.СуммаБезНДС 		= СтрокаТоваров.СуммаБезНДС;
				НоваяСтрока.БазаНДСРегл 		= СтрокаТоваров.БазаНДСРегл;
				НоваяСтрока.СуммаНДС 			= СтрокаТоваров.СуммаНДС;
				СтрокаТоваров.Количество = 0;
				Продолжить;
			КонецЕсли;
			
			СуммаБезНДССписанное	= 0;
			БазаНДСРеглСписанное	= 0;
			СуммаНДССписанное 		= 0;
			
			Для Каждого НайденнаяСтрока Из СтрокиЗапасов Цикл 
				...
				НоваяСтрока = ИтоговаяТаблица.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТоваров);
				НоваяСтрока.НомерГТД = НайденнаяСтрока.НомерГТД;
				
				Если СтрокаТоваров.Количество >= НайденнаяСтрока.Количество Тогда 
					
					СтрокаТоваров.Количество 		= СтрокаТоваров.Количество - НайденнаяСтрока.Количество;
					НоваяСтрока.Количество 			= НайденнаяСтрока.Количество;
					НоваяСтрока.КоличествоУпаковок 	= НайденнаяСтрока.Количество;
					НоваяСтрока.СуммаБезНДС 		= Окр(СтрокаТоваров.СуммаБезНДС / КоличествоТовараВстроке * НайденнаяСтрока.Количество, 2);
					НоваяСтрока.БазаНДСРегл 		= Окр(СтрокаТоваров.БазаНДСРегл / КоличествоТовараВстроке * НайденнаяСтрока.Количество, 2);
					НоваяСтрока.СуммаНДС 			= Окр(СтрокаТоваров.СуммаНДС / КоличествоТовараВстроке * НайденнаяСтрока.Количество, 2);
					ТаблицаВидовЗапаса.Удалить(НайденнаяСтрока);
					
				Иначе 
					
					НоваяСтрока.Количество 			= СтрокаТоваров.Количество;
					НоваяСтрока.КоличествоУпаковок 	= СтрокаТоваров.Количество;
					НоваяСтрока.СуммаБезНДС 		= Окр(СтрокаТоваров.СуммаБезНДС / КоличествоТовараВстроке * СтрокаТоваров.Количество, 2);
					НоваяСтрока.БазаНДСРегл 		= Окр(СтрокаТоваров.БазаНДСРегл / КоличествоТовараВстроке * СтрокаТоваров.Количество, 2);
					НоваяСтрока.СуммаНДС 			= Окр(СтрокаТоваров.СуммаНДС / КоличествоТовараВстроке * СтрокаТоваров.Количество, 2);
					НайденнаяСтрока.Количество 		= НайденнаяСтрока.Количество - СтрокаТоваров.Количество;
					СтрокаТоваров.Количество = 0;
					
				КонецЕсли;
				
				СуммаБезНДССписанное	= СуммаБезНДССписанное + НоваяСтрока.СуммаБезНДС;
				БазаНДСРеглСписанное	= БазаНДСРеглСписанное + НоваяСтрока.БазаНДСРегл;
				СуммаНДССписанное 		= СуммаНДССписанное + НоваяСтрока.СуммаНДС;
				
			КонецЦикла;
			
			//Проверим корректность округления рассчитанных значений
			НоваяСтрока.СуммаБезНДС 	= НоваяСтрока.СуммаБезНДС + (СтрокаТоваров.СуммаБезНДС - СуммаБезНДССписанное);
			НоваяСтрока.БазаНДСРегл 	= НоваяСтрока.БазаНДСРегл + (СтрокаТоваров.БазаНДСРегл - БазаНДСРеглСписанное);
			НоваяСтрока.СуммаНДС		= НоваяСтрока.СуммаНДС + (СтрокаТоваров.СуммаНДС - СуммаНДССписанное);
			
		КонецЦикла;
	Иначе
		
		ИтоговаяТаблица = ТаблицаТоваров.Скопировать();
		
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ТаблицаТоваров", ИтоговаяТаблица);
	
	Запрос.Текст = "
	|УНИЧТОЖИТЬ РеализацияТоваровУслугТаблицаТоваров;
	|/////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТаблицаТоваров.Ссылка                                     КАК Ссылка,
	|	ТаблицаТоваров.АналитикаУчетаНоменклатуры                 КАК АналитикаУчетаНоменклатуры,
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора,
	...
	Иначе
		...
		ТоварыРеализации.Колонки.Добавить("КлючСтроки", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(5, 0))); ///Сергей 03.08.2020 +++
		ТоварыРеализации.Колонки.Добавить("bon_СуммаБонусовВыдано", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(10, 0)));  ///Сергей 02.09.2020 +++
		ТоварыРеализации.Колонки.Добавить("bon_СуммаБонусовПринято", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(10, 0))); ///Сергей 02.09.2020 +++
	...
	Запрос.Текст =
		...
		|	ТоварыРеализации.КлючСтроки КАК КлючСтроки,
		|	ТоварыРеализации.bon_СуммаБонусовВыдано КАК bon_СуммаБонусовВыдано,
		|	ТоварыРеализации.bon_СуммаБонусовПринято КАК bon_СуммаБонусовПринято
		|ПОМЕСТИТЬ
		...
		|	ЗаказТовары.КлючСтроки КАК КлючСтроки,
		|	ЗаказТовары.bon_СуммаБонусовВыдано КАК bon_СуммаБонусовВыдано,
		|	ЗаказТовары.bon_СуммаБонусовПринято КАК bon_СуммаБонусовПринято
		|ИЗ
...
Функция ТекстНДССТоваровПереданныхНаКомиссию()
	...
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСПриПередачеНаКомиссию) КАК СчетДт,
	|
	...
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
	|	""НДС с товаров, переданных на комиссию"" КАК Содержание
	|
	|ИЗ
...
Функция ПолучитьДанныеДляПечатнойФормыАктВыполненныхРаботР1(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ДанныеДокумента.Номер КАК Номер,
	|	ДанныеДокумента.Дата КАК ДатаДокумента,
	|	ДанныеДокумента.Дата КАК ДатаПринятия,
	|	ДанныеДокумента.Партнер КАК Партнер,
	|	ДанныеДокумента.Контрагент КАК Контрагент,
	|	ДанныеДокумента.Организация КАК Организация,
	|	ДанныеДокумента.Валюта КАК Валюта,
	|	ДанныеДокумента.ЦенаВключаетНДС КАК ЦенаВключаетНДС,
	|	ВЫБОР
	|		КОГДА ДанныеДокумента.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК УчитыватьНДС,
	|	ДанныеДокумента.Договор,
	|	ДанныеДокумента.Договор.НаименованиеДляПечати КАК ПредставлениеДоговора,
	|	ТаблицаОтветственныеЛица.РуководительНаименование КАК Отпустил,
	|	ТаблицаОтветственныеЛица.РуководительДолжность КАК ДолжностьОтветственного
	|ИЗ
	|	Документ.РеализацияТоваровУслуг КАК ДанныеДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаОтветственныеЛица КАК ТаблицаОтветственныеЛица
	|		ПО ДанныеДокумента.Ссылка = ТаблицаОтветственныеЛица.Ссылка
	...
	|	ДанныеДокумента.Номенклатура.НаименованиеПолное КАК УслугаНаименованиеПолное,
	|	&ТекстЗапросаЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	ДанныеДокумента.Характеристика КАК Характеристика,
	|	ДанныеДокумента.Характеристика.НаименованиеПолное КАК ХарактеристикаНаименованиеПолное,
	|	ДанныеДокумента.СуммаБезНДС / ДанныеДокумента.КоличествоУпаковок КАК Цена,
	|	ДанныеДокумента.КоличествоУпаковок КАК Количество,
	|	ДанныеДокумента.СуммаБезНДС КАК Сумма,
	|	ДанныеДокумента.СуммаНДС КАК СуммаНДС,
	|	ДанныеДокумента.СуммаБезНДС + ДанныеДокумента.СуммаНДС КАК СуммаСНДС,
	|	ДанныеДокумента.НомерСтроки КАК НомерСтроки
	|ИЗ
	|	РеализацияТоваровУслугТаблицаТоваров КАК ДанныеДокумента
	|ГДЕ
	|	(ДанныеДокумента.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТекстЗапросаЕдиницаИзмерения",
		Справочники.УпаковкиЕдиницыИзмерения.ТекстЗапросаЗначениеРеквизитаЕдиницыИзмерения(
			"Ссылка", "ДанныеДокумента.Упаковка", "ДанныеДокумента.Номенклатура"));
...
//ТТ
//Возврат разрешения на операции с документом реализации
Функция ПроверкаСтатусаСНТ(Отказ,СсылкаНаДокумент) Экспорт
	
	Если СсылкаНаДокумент.Дата <= Дата(2021,1,17,23,59,59) Тогда
		Возврат Истина;
	КонецЕсли;
	
	СпСостоянииСНТ = Новый СписокЗначений;
	СпСостоянииСНТ.Добавить(Перечисления.СостоянияСНТ.ПринятСервером);
	СпСостоянииСНТ.Добавить(Перечисления.СостоянияСНТ.ДоставленПолучателю);
	СпСостоянииСНТ.Добавить(Перечисления.СостоянияСНТ.ПодтвержденПолучателем);
	СпСостоянииСНТ.Добавить(Перечисления.СостоянияСНТ.ПодтвержденОГД);
	СпСостоянииСНТ.Добавить(Перечисления.СостоянияСНТ.ОбрабатываетсяСервером);
	
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      |	СНТ.Ссылка КАК Ссылка,
	                      |	СНТ.Дата КАК Дата,
	                      |	СНТ.Состояние КАК Состояние,
	                      |	ЕСТЬNULL(ПартнерыДополнительныеРеквизиты.Значение, ИСТИНА) КАК ВыписыватьСНТ
	                      |ИЗ
	                      |	Документ.СНТ КАК СНТ
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Партнеры.ДополнительныеРеквизиты КАК ПартнерыДополнительныеРеквизиты
	                      |		ПО СНТ.ДокументОснование.Партнер = ПартнерыДополнительныеРеквизиты.Ссылка
	                      |			И (ПартнерыДополнительныеРеквизиты.Свойство.Имя = ""ВыпискаСНТ"")
	                      |ГДЕ
	                      |	СНТ.ДокументОснование = &ДокументОснование
	                      |	И НЕ СНТ.ПометкаУдаления
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	Дата УБЫВ");
	...
	Пока Выборка.Следующий() Цикл
		Если СокрЛП(Выборка.ВыписыватьСНТ) = "Не готов" Тогда
			Возврат Истина;
		КонецЕсли;
		Если СпСостоянииСНТ.НайтиПоЗначению(Выборка.Состояние) <> Неопределено Тогда  //В списке запрещенных статусов
			Возврат Истина;
		Иначе
			Возврат Ложь;
		КонецЕсли;
