...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИСМПТВыбытиеКодовМаркировкиСервер.СобытияФорм_ПриСозданииНаСервере(ЭтотОбъект, Отказ, СтандартнаяОбработка);
...
Процедура ПоискПоШтрихкодуЗавершение(ДанныеШтрихкода, ДополнительныеПараметры) Экспорт
	...
	ДополнительныеПараметры.Вставить("СтандартнаяОбработка", Истина);
	ДополнительныеПараметры.Вставить("КэшированныеЗначения", КэшированныеЗначения);
	ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормКлиент_ПриИзмененииЭлемента(ЭтотОбъект, "ТоварыПоискПоШтрихкоду", ДополнительныеПараметры);
	КэшированныеЗначения = ДополнительныеПараметры.КэшированныеЗначения;
	Если Не ДополнительныеПараметры.СтандартнаяОбработка Тогда
		Возврат;
	КонецЕсли;
...
Функция Подключаемый_ОбработатьВводШтрихкода(ДанныеШтрихкода, КэшированныеЗначения, ПараметрыСканирования = Неопределено)
	...
	РезультатОбработкиШтрихкода = ИСМПТВыбытиеКодовМаркировкиСервер.ШтрихкодированиеИС_ОбработатьВводШтрихкода(
...
Функция Подключаемый_ВыполнитьДействие(Действие, РезультатВыбора, РезультатОбработкиШтрихкода, КэшированныеЗначения)
	...
	ДополнительныеПараметры.Вставить("КэшированныеЗначения"        , КэшированныеЗначения);
	ИСМПТВыбытиеКодовМаркировкиСервер.СобытияФорм_ПриИзмененииЭлемента(ЭтотОбъект, "Подключаемый_ВыполнитьДействие", ДополнительныеПараметры);
...
Процедура Подключаемый_ПослеОбработкиШтрихкодов()
	...
	//ИСМПТВыбытиеКодовМаркировкиКлиент.СобытияФормКлиент_ПриИзмененииЭлемента(ЭтотОбъект, "Товары");
...
Функция ДанныеДляПробитияЧека()
	...
	Для каждого СтрокаТЧ из ДанныеПоТоварам Цикл
		...
		Для каждого СтрокаДляДобавленияВЧек из МассивСтрокДляДобавленияВЧек Цикл
			...
			ИначеЕсли ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаТЧ, "ОсобенностьУчета")
				И СтрокаТЧ.ОсобенностьУчета = Перечисления.ОсобенностиУчетаНоменклатуры.ОбувнаяПродукция
				И ЗначениеЗаполнено(СтрокаДляДобавленияВЧек.Штрихкод) Тогда
...
Процедура ИзменитьТабличнуюЧастьПоРезультатамПроверки(ВозвращаемыеПараметры, КэшированныеЗначения)
	...
	ИСМПТВыбытиеКодовМаркировкиСервер.СобытияФорм_ПриИзмененииЭлемента(ЭтотОбъект, "ЗавершенаПроверкаКоличества", ВозвращаемыеПараметры);
