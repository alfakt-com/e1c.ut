...
Функция ТекстЗапросаТаблицаДвиженияДенежныеСредстваКонтрагент(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ДанныеДокумента.Ссылка = &Ссылка
	|	И &ПолноеПроведение
	|	И ТаблицаТовары.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.НДС12)
	|СГРУППИРОВАТЬ ПО
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'Справка отчет кассира-операциониста'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;

	// Акт возврата денежных средств покупателю
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Обработчик = "УправлениеПечатьюУТКлиент.ПечатьКМ3";
	КомандаПечати.МенеджерПечати = "";
	КомандаПечати.Идентификатор = "КМ3";
	КомандаПечати.Представление = НСтр("ru = 'Акт возврата денежных средств покупателю'");
...
Функция ПолучитьДанныеДляПечатнойФормыКМ3(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	СтруктураДанныхДляПечати    = Новый Структура("РезультатЗапроса, ЗаголовокДокумента",
	                                               РезультатЗапроса, НСтр("ru = 'Акт возврата денежных средств покупателю'"));
...
Функция ТекстЗапросаДляРасчетаНеобходимостиСчетаФактуры() Экспорт
	...
	|			И ТаблицаСчетовФактуры.Контрагент = ЗНАЧЕНИЕ(Справочник.Партнеры.РозничныйПокупатель)
	|ГДЕ
...
Функция ТекстЗапросаДанныхОснованияДляПечатнойФормыСчетФактура() Экспорт
	...
	|//ОператорПОМЕСТИТЬ
	|ИЗ
	|	Документ.ОтчетОРозничныхПродажах КАК ДанныеДокументов
	|ГДЕ
	|	ДанныеДокументов.Ссылка В (&ДокументОснование_ОтчетОРозничныхПродажах)";
...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	|	СУММА(ТаблицаТоваров.Количество) КАК КоличествоУпаковок,
	|	СУММА(ТаблицаТоваров.Сумма - ТаблицаТоваров.СуммаНДС) КАК СуммаБезНДС,
	|	ТаблицаТоваров.СтавкаНДС КАК СтавкаНДС,
	|	СУММА(ТаблицаТоваров.СуммаНДС) КАК СуммаНДС,
	|	Аналитика.Номенклатура.ТипНоменклатуры = Значение(Перечисление.ТипыНоменклатуры.Товар) КАК ЭтоТовар,
	...
	|		ПО ТаблицаТоваров.АналитикаУчетаНаборов = АналитикаНаборов.КлючАналитики
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютеРегл КАК СуммыДокументовВВалютеРегл
	|		ПО ТаблицаТоваров.Ссылка = СуммыДокументовВВалютеРегл.Регистратор
	|			И ТаблицаТоваров.ИдентификаторСтроки = СуммыДокументовВВалютеРегл.ИдентификаторСтроки
	|			И (СуммыДокументовВВалютеРегл.Активность)
	|			И (&ПересчитыватьВВалютуРегл)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВариантыКомплектацииНоменклатуры КАК ВариантыКомплектацииНоменклатуры
	|		ПО (ВариантыКомплектацииНоменклатуры.Владелец = ТаблицаТоваров.АналитикаУчетаНаборов.НоменклатураНабора)
	|			И (ВариантыКомплектацииНоменклатуры.Характеристика = ТаблицаТоваров.АналитикаУчетаНаборов.ХарактеристикаНабора)
	|			И (ВариантыКомплектацииНоменклатуры.Основной)
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтчетОРозничныхПродажахВидыЗапасов.Ссылка КАК Ссылка,
	|	ОтчетОРозничныхПродажахВидыЗапасов.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	|	ОтчетОРозничныхПродажахВидыЗапасов.АналитикаУчетаНаборов КАК АналитикаУчетаНаборов,
	|	ОтчетОРозничныхПродажахВидыЗапасов.ВидЗапасов КАК ВидЗапасов,
	|	ОтчетОРозничныхПродажахВидыЗапасов.НомерГТД КАК НомерГТД,
	|	ОтчетОРозничныхПродажахВидыЗапасов.Количество КАК Количество
	|ПОМЕСТИТЬ ВидыЗапасов
	|ИЗ
	|	ТаблицаДанныхДокументов КАК ДанныеДокументов
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтчетОРозничныхПродажах.ВидыЗапасов КАК ОтчетОРозничныхПродажахВидыЗапасов
	|		ПО ОтчетОРозничныхПродажахВидыЗапасов.Ссылка = ДанныеДокументов.Ссылка";
	
	Запрос.Выполнить();
	
	ТаблицаВидовЗапаса = Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("ВидыЗапасов").ПолучитьДанные().Выгрузить();
	ТаблицаТоваров = Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("СтрокиТоваров").ПолучитьДанные().Выгрузить();
	...
	Для Каждого СтрокаТоваров Из ТаблицаТоваров Цикл 
		...
		//Если запасов не обнаружено то это услуга, включаем её в итоговую таблицу целиком
		Если СтрокиЗапасов.Количество() = 0 Тогда
			...
			НоваяСтрока.СуммаБезНДС 		= СтрокаТоваров.СуммаБезНДС / КоличествоТовараВстроке * СтрокаТоваров.Количество;
			НоваяСтрока.СуммаНДС 			= СтрокаТоваров.СуммаНДС / КоличествоТовараВстроке * СтрокаТоваров.Количество;
		...
		Для Каждого НайденнаяСтрока Из СтрокиЗапасов Цикл 
			...
			Если СтрокаТоваров.Количество >= НайденнаяСтрока.Количество Тогда 
				...
				НоваяСтрока.СуммаБезНДС 		= СтрокаТоваров.СуммаБезНДС / КоличествоТовараВстроке * НайденнаяСтрока.Количество;
				НоваяСтрока.СуммаНДС 			= СтрокаТоваров.СуммаНДС / КоличествоТовараВстроке * НайденнаяСтрока.Количество;
	...
	|ПОМЕСТИТЬ ОтчетОРозничныхПродажахТаблицаТоваров
	|ИЗ
