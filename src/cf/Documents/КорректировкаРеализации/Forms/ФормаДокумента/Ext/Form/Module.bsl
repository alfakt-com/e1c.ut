...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ОтметкаНезаполненного", КорректировкаУслугПрочихАктивов);
	
	Элемент = УсловноеОформление.Элементы.Добавить();

	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.СтатьяДоходовДополнительно.Имя);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	// СНТ
	СНТСерверПереопределяемый.ОбновитьРеквизитыСНТ(ЭтаФорма,,,"ГруппаПодвалЛево");
	// Конец СНТ
...
&НаСервере
Функция ПолучитьПервичнуюСНТ() Экспорт
	
	РТУСсылка = Объект.ДокументОснование;
	ДанныеСНТ = СНТСерверПереопределяемый.НайтиПервичныйСНТ(РТУСсылка);
	
	Если ДанныеСНТ.Количество() > 0 Тогда
		ДанныеПервичнойСНТ = Новый Структура();
		ДанныеПервичнойСНТ.Вставить("Ссылка", ДанныеСНТ[0].Ссылка);
		ДанныеПервичнойСНТ.Вставить("РегНомерИСЭСФ", ДанныеСНТ[0].РегНомерИСЭСФ);
		ДанныеСНТ = Неопределено;
		Возврат ДанныеПервичнойСНТ; 
	Иначе
