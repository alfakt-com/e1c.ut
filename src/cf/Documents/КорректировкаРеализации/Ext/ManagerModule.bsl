...
Функция ТекстЗапросаДляРасчетаНеобходимостиСчетаФактуры() Экспорт
	...
	|							ТОГДА ИСТИНА
	|						КОГДА НЕ УчетнаяПолитика.ПрименяетсяОсвобождениеОтУплатыНДС // И тчТовары.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|							ТОГДА ИСТИНА
	|						ИНАЧЕ ЛОЖЬ
	|					КОНЕЦ
	|			ИНАЧЕ ЛОЖЬ
...
Функция ТекстЗапросаВтРасхожденияРаботыУслуги(Запрос, ТекстыЗапроса)
	...
	|			ИЛИ ТаблицаРасхождения.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК))
	|		 И &НалогообложениеНДС В (
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС), 
	|				ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПроизводствоСДЦ))
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
...
Функция ТекстЗапросаВтТаблицаКорректировкаРеализацииБезНДС(Запрос, ТекстыЗапроса)
	...
	|				ТОГДА ИСТИНА
	|			КОГДА тчТовары.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|				И тчТовары.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК)
	|				ТОГДА ИСТИНА
	|			КОГДА тчРасхождения.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|				И тчРасхождения.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДСНеРК)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
...
Функция ТекстЗапросаТаблицаНДСПредъявленный(Запрос, ТекстыЗапроса, Регистры)
	...
	|	И &НалогообложениеНДС В (&ТипыНалогообложенияНДСПоступления)
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если НЕ ПраваПользователяПовтИсп.ЭтоПартнер() Тогда
		...
		// Накладная на отпуск запасов с номерами ГТД (З-2)
		КомандаПечати = КомандыПечати.Добавить();
		КомандаПечати.МенеджерПечати = "Обработка.ПечатьОбщихФорм";
		КомандаПечати.Идентификатор = "ТОРГ12";
...
Функция ТекстЗапросаВтВидыЗапасов() Экспорт
	...
	|	СУММА(ВсеВидыЗапасов.БазаНДСРегл)                            КАК БазаНДСРегл,
...
Функция ПолучитьДанныеДляПечатнойФормыТОРГ12(ПараметрыПечати, МассивОбъектов) Экспорт 
	...
	|	КорректировкаРеализации.Менеджер.ФизическоеЛицо КАК Ответственный,
	...
	|		ПО КорректировкаРеализации.Ссылка = ДанныеДокументов.Ссылка
	|ГДЕ
	|	КорректировкаРеализации.Валюта <> &ВалютаРегламентированногоУчета";
	...
	ТаблицаВалют              = МассивРезультатов[2].Выгрузить();
...
Функция ТекстЗапросаДанныхОснованияДляПечатнойФормыСчетФактура() Экспорт
	...
	|	ДанныеДокументов.ДокументОснование.СпособДоставки         КАК СпособДоставки,
...
Процедура ЗаполнитьРасхожденияПоТоварам(Объект)
	...
	|	ТаблицаТоваров.ВидОперацииРеализации КАК ВидОперацииРеализации,
	|	
	...
	|	ДанныеРасхождений.ВидОперацииРеализации,
...
Процедура ПоместитьВременнуюТаблицуТоваров(МенеджерВременныхТаблиц, ПараметрыЗаполнения = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ПечатьСчетаФактуры",             ПараметрыЗаполнения.Свойство("ПечатьСчетаФактуры"));
	...
	|	ЕСТЬNULL(СуммыРегл.БазаНДСРегл, Товары.СуммаСНДС - Товары.СуммаНДС)     КАК БазаНДСРегл,
	|	
	...
	|	ВЫБОР КОГДА Товары.Количество < 0 И ЕСТЬNULL(СуммыРегл.БазаНДСРегл, Товары.СуммаСНДС - Товары.СуммаНДС) > 0
	|		ТОГДА -ЕСТЬNULL(СуммыРегл.БазаНДСРегл, Товары.СуммаСНДС - Товары.СуммаНДС)
	|		ИНАЧЕ ЕСТЬNULL(СуммыРегл.БазаНДСРегл, Товары.СуммаСНДС - Товары.СуммаНДС)
	|	КОНЕЦ КАК БазаНДСРегл,
	|	
	|	Товары.СтавкаНДС КАК СтавкаНДС,
	|	
	...
	|	ЕСТЬNULL(СуммыРегл.БазаНДСРегл, Услуги.СуммаСНДС - Услуги.СуммаНДС)     КАК БазаНДСРегл,
	|	
	...
	|	ЕСТЬNULL(СуммыРегл.БазаНДСРегл, Доходы.СуммаСНДС - Доходы.СуммаНДС)     КАК БазаНДСРегл,
	|	
	...
	|		-СУММА(ТаблицаДокумента.Количество)
	|	КОНЕЦ * ВЫБОР
	|		КОГДА &ПечатьСчетаФактуры
	|				И ДанныеДокументов.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратНедопоставленногоТовара)
	|			ТОГДА -1
	|		ИНАЧЕ 1
	|		КОНЕЦ КАК Количество,
	|	
	...
	|		КОНЕЦ КАК СуммаБезНДС,
	|
	|	ВЫБОР КОГДА СУММА(ТаблицаДокумента.БазаНДСРегл) > 0 ТОГДА
	|		СУММА(ТаблицаДокумента.БазаНДСРегл)
	|	ИНАЧЕ
	|		-СУММА(ТаблицаДокумента.БазаНДСРегл)
	|	КОНЕЦ * ВЫБОР
	|		КОГДА &ПечатьСчетаФактуры
	...
	|		КОНЕЦ  КАК СуммаБезНДС,
	|
	|	ВЫБОР КОГДА СУММА(Услуги.БазаНДСРегл) > 0 ТОГДА
	|		СУММА(Услуги.БазаНДСРегл)
	|	ИНАЧЕ
	|		-СУММА(Услуги.БазаНДСРегл)
	|	КОНЕЦ * ВЫБОР
	|		КОГДА &ПечатьСчетаФактуры
	...
	|		ТаблицаДанныхДокументов КАК ДанныеДокументов
	|	ПО
	|		Услуги.Ссылка = ДанныеДокументов.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	Услуги.Ссылка,
...
Процедура ПреобразоватьВременнуюТаблицуТоваровДляСчетаФактуры(МенеджерВременныхТаблиц) Экспорт
	...
	|	ЕСТЬNULL(ТаблицаТоваровПредыдущие.БазаНДСРегл, 0)        КАК БазаНДСРеглДо,
	|	ТаблицаДокумента.СуммаНДС                                КАК СуммаНДС,
	|	ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаНДС, 0)           КАК СуммаНДСДо,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СуммаБезНДС >= ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаБезНДС, 0)
	...
	|	КОНЕЦ                                                    КАК РазницаБезНДСУменьшение,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.БазаНДСРегл >= ЕСТЬNULL(ТаблицаТоваровПредыдущие.БазаНДСРегл, 0)
	|			ТОГДА ТаблицаДокумента.БазаНДСРегл - ЕСТЬNULL(ТаблицаТоваровПредыдущие.БазаНДСРегл, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ                                                    КАК РазницаБазаНДСУвеличение,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.БазаНДСРегл < ЕСТЬNULL(ТаблицаТоваровПредыдущие.БазаНДСРегл, 0)
	|			ТОГДА -(ТаблицаДокумента.БазаНДСРегл - ЕСТЬNULL(ТаблицаТоваровПредыдущие.БазаНДСРегл, 0))
	|		ИНАЧЕ 0
	|	КОНЕЦ                                                    КАК РазницаБазаНДСУменьшение,
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.СуммаНДС >= ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаНДС, 0)
	|			ТОГДА ТаблицаДокумента.СуммаНДС - ЕСТЬNULL(ТаблицаТоваровПредыдущие.СуммаНДС, 0)
	|			ИНАЧЕ 0
	...
	|	0                                                                        КАК БазаНДСРегл,
	|	ТаблицаТоваровПредыдущие.БазаНДСРегл                                     КАК БазаНДСРеглДо,
	|	0                                                                        КАК СуммаНДС,
	...
	|	ТаблицаТоваровПредыдущие.БазаНДСРегл                                     КАК РазницаБазаНДСУменьшение,
...
Функция ПолучитьДанныеДляПечатнойФормыАктВыполненныхРаботР1(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	ДанныеДокументов.ВидКорректировки КАК ХозяйственнаяОперация,
	|	
	|	ВЫБОР КОГДА ДанныеДокументов.ВидКорректировки В(
	|		ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияПерепоставленногоТовара),
	...
	|ИЗ
	|	ТаблицаТоваровКорректировкаРеализации КАК ДанныеДокумента
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеКорректировки КАК ДанныеКорректировки
	|		ПО ДанныеДокумента.Ссылка = ДанныеКорректировки.Ссылка
	|ГДЕ
	...
	|	НЕОПРЕДЕЛЕНО,
	|	"""",
	|	ДанныеДокумента.СуммаБезНДС / ДанныеДокумента.КоличествоУпаковок,
	|	ДанныеДокумента.Количество,
	|	ДанныеДокумента.СуммаБезНДС,
	|	ДанныеДокумента.СуммаНДС,
	|	ДанныеДокумента.СуммаНДС + ДанныеДокумента.СуммаБезНДС,
	...
	|		ПО ДанныеДокумента.Ссылка = ДанныеКорректировки.Ссылка
	|ГДЕ
	|	ДанныеКорректировки.КорректировкаПрочейРеализации
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки
	|ИТОГИ
	...
	ТаблицаВалют = МассивРезультатов[3].Выгрузить();
