...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, ДополнительныеСвойства, Регистры = Неопределено) Экспорт
	...
	ТекстЗапросаТаблицаВыданныеПодарки(Запрос,ТекстыЗапроса,Регистры);
...
//++Тишбаев О.М. Вынести в расширение не забудь
Функция ПроверитьЗадолженностьКонтрагентаПоЗаказам(Источник, РежимЗаписи) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ 1
	               ...
	               |ГДЕ
	               |	Infor_ИсторияСтатусовВМС.Объект = &ЭтотОбъект
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Infor_ИсторияСтатусовВМС.ДатаИзменения";

	Запрос.УстановитьПараметр("ЭтотОбъект", Источник);
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Если Выборка.Следующий() 
		Или Источник.СуммаДокумента = 0 
		Или РежимЗаписи <> РежимЗаписиДокумента.Проведение Тогда
		Возврат Ложь;	
	КонецЕсли;
	
	ВыборкаЛимита = РегистрыСведений.swНастройкиКонтроляЗадолженностиКонтрагентов.Выбрать(,, Новый Структура("Партнер", Источник.Партнер));
	ВходитВЛимит  = ВыборкаЛимита.Следующий();
	
	//Сверка взаиморасчетов на 31.12.2019 23:59:59
	ДолгСверки      = 0;
	ДанныеДокумента = Новый Структура;
	ДанныеДокумента.Вставить("КонецПериода",           Дата(2019,12,31));
	ДанныеДокумента.Вставить("Организация",            Источник.Организация);
	ДанныеДокумента.Вставить("Контрагент",             Источник.Контрагент); 
	...
	ТаблицаНашихДанных = Документы.СверкаВзаиморасчетов.ОстаткиРасчетов(ДанныеДокумента);
	
	Если ТаблицаНашихДанных <> Неопределено Тогда 
		ДолгСверки = ТаблицаНашихДанных.Итог("СуммаНашДолг") - ТаблицаНашихДанных.Итог("СуммаДолгПартнера");
	КонецЕсли;
	//Сверка взаиморасчетов на 31.12.2019 23:59:59
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	-СУММА(ЕСТЬNULL(РасчетыСКлиентами.КОплатеПриход, 0)) КАК ЗадолженностьКлиента
	               |ПОМЕСТИТЬ ВТ_КОплате
	               |ИЗ
	               ...
	               |ГДЕ
	               |	РасчетыСКлиентами.Регистратор <> &ЭтотЗаказ
	               |
	               |ИМЕЮЩИЕ
	               ...
	               |
	               |ВЫБРАТЬ
	               |	СУММА(ЕСТЬNULL(РасчетыСКлиентами.КОплатеРасход, 0))
	               |ИЗ
	               |	РегистрНакопления.РасчетыСКлиентами.Обороты(
	               |			&НачалоПериода,
	               |			,
	               ...
	               |
	               |ИМЕЮЩИЕ
	               |	СУММА(ЕСТЬNULL(РасчетыСКлиентами.КОплатеРасход, 0)) > 0
	               |
	               |ОБЪЕДИНИТЬ ВСЕ
	               |
	               |ВЫБРАТЬ
	               ...
	               |					АналитикаПоПартнерам.КлючАналитики КАК КлючАналитики
	               |				ИЗ
	               |					АналитикаУчетаПоПартнерам КАК АналитикаПоПартнерам)) КАК РасчетыСПоставщикамиОстатки
	               |
	               |ОБЪЕДИНИТЬ ВСЕ
	               |
	               |ВЫБРАТЬ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	СУММА(ЕСТЬNULL(ВТ_КОплате.ЗадолженностьКлиента, 0)) КАК ЗадолженностьКлиента
	               |ИЗ
	               |	ВТ_КОплате КАК ВТ_КОплате";    
	
	Запрос.УстановитьПараметр("ЭтотЗаказ",      Источник);
	Запрос.УстановитьПараметр("Контрагент",     Источник.Контрагент);
	Запрос.УстановитьПараметр("Организация",    Источник.Организация); 
	Запрос.УстановитьПараметр("НачалоПериода",  Новый Граница(Дата(2020,01,01,00,00,00), ВидГраницы.Включая));
	Запрос.УстановитьПараметр("СуммаДокумента", -Источник.СуммаДокумента + ДолгСверки);
	
	ДанныеЗапроса = Запрос.Выполнить().Выгрузить();
	
	Если ДанныеЗапроса[0].ЗадолженностьКлиента >= 0 Тогда		
		Возврат Ложь;
	Иначе
		Если ВходитВЛимит Тогда 
			Сообщить("Сумма задолженности клиента составляет: " + ДанныеЗапроса[0].ЗадолженностьКлиента + "тенге.");
			Возврат Истина;
		КонецЕсли;
		
		ДостаточноСредств = -ДанныеЗапроса[0].ЗадолженностьКлиента <= (Окр(Источник.СуммаДокумента/2, 2, РежимОкругления.Окр15как20));
		Если ДостаточноСредств Тогда
			Возврат Ложь;
		Иначе			
			Сообщить("Сумма задолженности клиента составляет: " + ДанныеЗапроса[0].ЗадолженностьКлиента + "тенге.");
			Возврат Истина;
...
Функция ТекстЗапросаТаблицаВыданныеПодарки(Запрос, ТекстыЗапроса, Регистры)
	ИмяРегистра = "bon_ВыданныеПодарки";
	ТекстЗапроса =
		"ВЫБРАТЬ
		|	ТаблицаТовары.Номенклатура КАК Номенклатура,
		...
		|	ТаблицаТовары.Ссылка.МаркетинговаяАкция КАК МаркетинговаяАкция,
		|	&Период КАК Период,
		|	ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) КАК ВидДвижения
		|ИЗ
		|	Документ.ЗаказКлиента.Товары КАК ТаблицаТовары
...
Функция ДанныеДляПечатныхФормСчетаНаОплатуИзвещения(ПараметрыПечати, МассивОбъектов)
	...
	|   Документы.КодНазначенияПлатежа КАК КодНазначенияПлатежа,
