...
Процедура ПриОткрытии(Отказ)
	...
	///Сергей 16.10.2020 +++
	Попытка
		РольПров = Строка("New: Разрешение на замен номеклатуры в заказах (Роль)");
		Если ПроверкаДоступа(РольПров) Тогда
			Элементы.ФормаЗаменитьНоменклотуру.Видимость = Истина;	
		КонецЕсли;
	Исключение
	КонецПопытки;
	///Сергей 16.10.2020 +++
...
&НаКлиенте
Процедура ЗаменитьНоменклотуру(Команда)
	
	Форма = ПолучитьФорму("Обработка.new_ЗаменаНоменклотурыВЗаказах.Форма.Форма");
	Форма.Открыть();	
