...
Процедура ПриОткрытии(Отказ)
	...
	ПроверкаНаполученнубСчетФактуру(); ///Сергей 24.09.2020 +++ (По Доверенностям)
	...
	Если Объект.ТипДоверенности = Перечисления.ТипыВыданныхДоверенностей.НаПолучениеДС Тогда
		...
		Иначе
			Если Объект.СуммаОплатПоДокументу < Объект.СуммаДокумента Тогда
				...
				Формат(Объект.СуммаОплатПоДокументу, "ЧЦ=15; ЧДЦ=2; ЧРГ="));
			Иначе
...
#Область Поступлений_и_СчетФактур

&НаСервере
Процедура ПроверкаНаполученнубСчетФактуру()
	
	Для Каждого СтрПост Из Объект.new_Поступление Цикл 
		Если Не ЗначениеЗаполнено(СтрПост.СчетФактура) и ЗначениеЗаполнено(СтрПост.ПриобретениеТоваровУслуг)Тогда
			Если ЗначениеЗаполнено(СтрПост.ПриобретениеТоваровУслуг.НомерВходящегоДокумента) Тогда
				Запрос = Новый Запрос;
				Запрос.Текст = 
				"ВЫБРАТЬ
				...
				|	И НАЧАЛОПЕРИОДА(СчетФактураПолученный.Дата) = &Дата
				|
				|ОБЪЕДИНИТЬ ВСЕ
				|
				|ВЫБРАТЬ
				...
				|ГДЕ
				|	СчетФактураПолученныйАванс.Номер = &Номер
				|	И НАЧАЛОПЕРИОДА(СчетФактураПолученныйАванс.Дата) = &Дата
				|
				|ОБЪЕДИНИТЬ ВСЕ
				|
				|ВЫБРАТЬ
				|	СчетФактураПолученныйНалоговыйАгент.Ссылка
				|ИЗ
				|	Документ.СчетФактураПолученныйНалоговыйАгент КАК СчетФактураПолученныйНалоговыйАгент
				|ГДЕ
				|	СчетФактураПолученныйНалоговыйАгент.Номер = &Номер
				|	И НАЧАЛОПЕРИОДА(СчетФактураПолученныйНалоговыйАгент.Дата) = &Дата";
				Запрос.УстановитьПараметр("Номер", СтрПост.ПриобретениеТоваровУслуг.НомерВходящегоДокумента);
				Запрос.УстановитьПараметр("Дата", СтрПост.ПриобретениеТоваровУслуг.ДатаВходящегоДокумента);
				
				Рез = Запрос.Выполнить().Выгрузить();

				СтрПост.СчетФактура = Рез[0].Ссылка;
				Записать();
			КонецЕсли;
		КонецЕсли;		
	КонецЦикла;
