...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Изменение", Метаданные.Документы.ДоверенностьВыданная) Тогда
		...
		КомандаПечати.Представление = НСтр("ru = 'Доверенность (Д-1)'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "М2") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "М2", НСтр("ru = 'Доверенность на получение активов (Д-1)'"), СформироватьПечатнуюФорму(ПараметрыПечати["Тип"], МассивОбъектов, ОбъектыПечати));
...
Функция СформироватьПечатнуюФорму(Тип, МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|	ВЫБОР
	|		КОГДА Доверенность.БанковскийСчет.РучноеИзменениеРеквизитовБанка
	|			ТОГДА Доверенность.БанковскийСчет.НаименованиеБанка
	|		ИНАЧЕ Доверенность.БанковскийСчет.Банк.Наименование
	|	КОНЕЦ КАК НаименованиеБанка,
	|	ВЫБОР
	|		КОГДА Доверенность.БанковскийСчет.РучноеИзменениеРеквизитовБанка
	|			ТОГДА Доверенность.БанковскийСчет.БИКБанка
	|		ИНАЧЕ Доверенность.БанковскийСчет.Банк.Код
	|	КОНЕЦ КАК БИК,
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		ФИОДоверенногоВДательномПадеже = "";
		Если ДанныеПечати.ФизЛицоПол = Перечисления.ПолФизическогоЛица.Мужской Тогда
			ПолЧислом = 1;
		ИначеЕсли ДанныеПечати.ФизЛицоПол = Перечисления.ПолФизическогоЛица.Женский Тогда
			ПолЧислом = 2;
		Иначе 
			ПолЧислом = Неопределено;
		КонецЕсли;
		ОбщегоНазначения.Просклонять(Строка(ДанныеПечати.ФизЛицо), 3, ФИОДоверенногоВДательномПадеже, ПолЧислом);
		ФамилияИмяОтчествоДоверенного = ?(ЗначениеЗаполнено(ДанныеПечати.Должность), Строка(ДанныеПечати.Должность)+", ", "")+Строка(ФИОДоверенногоВДательномПадеже);
		...
		СтруктураДанныхШапка.Вставить("ФамилияИмяОтчествоДоверенного", СокрЛП(ФамилияИмяОтчествоДоверенного));
		СтруктураДанныхШапка.Вставить("ДолжностьДоверенного", ДанныеПечати.Должность);
		СтруктураДанныхШапка.Вставить("ОрганизацияПредставление", ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование"));
		...
		Если ЗначениеЗаполнено(ДанныеПечати.НаименованиеБанка) Тогда 
			СтруктураДанныхШапка.Вставить("НаименованиеБанка", СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				"%1, БИК %2",
		...
		Пока ВыборкаСтрокТовары.Следующий() Цикл
			...
			ИтогоКоличество = ИтогоКоличество + ВыборкаСтрокТовары.КоличествоУпаковок;
		...
		Пока ВыборкаСтрокМатериальныеЦенности.Следующий() Цикл
			...
			ИтогоКоличество = ИтогоКоличество + ВыборкаСтрокМатериальныеЦенности.Количество;
		КонецЦикла;
		...
		Если НЕ ДанныеПечати.СуммаДокумента = 0 Тогда
			...
			СтруктураДанныхСтрокаТаблицы.Вставить("КоличествоПрописью",
				...
				КоличествоПрописью(ДанныеПечати.СуммаДокумента) + ")"));
			СтруктураДанныхСтрокаТаблицы.Вставить("ЕдиницаИзмеренияПредставление", ДанныеПечати.ВалютаПредставление);
			ОбластьМакета.Параметры.Заполнить(СтруктураДанныхСтрокаТаблицы);
			ТабличныйДокумент.Вывести(ОбластьМакета);
			
			ИтогоКоличество = ИтогоКоличество + ДанныеПечати.СуммаДокумента;
		КонецЕсли;
		
		СтруктураИтогов = Новый Структура;
		СтруктураИтогов.Вставить("ИтогоКоличество", ?(ИтогоКоличество = 0,
			"",
			Строка(ИтогоКоличество) + " (" +
			КоличествоПрописью(ИтогоКоличество) + ")"));
		...
		//Начало: МК Баубериков А - 16.06.2020
		СтруктураДанныхПодвал.Вставить("ДолжностьГлавныйБухгалтер", ДанныеПечати.Ссылка.ГлавныйБухгалтер.Должность);
        //Конец: МК Баубериков А - 16.06.2020		
...
Функция КомплектПечатныхФорм() Экспорт
	...
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "М2", НСтр("ru = 'Доверенность на получение активов (Д-1)'"), 1);
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если ДоступностьРаспоряженийТовары Тогда
		...
		ДелоРодитель.Представление  = НСтр("ru = 'Доверенности на получение активов'");
