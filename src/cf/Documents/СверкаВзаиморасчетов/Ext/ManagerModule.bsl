...
Функция СформироватьПечатнуюФормуАктСДаннымиКонтрагента(МассивОбъектов, ОбъектыПечати)
	...
	Пока ДанныеДокумента.Следующий() Цикл
		...
		Для Каждого СтрокаРасхождений Из ТаблицаРасхождений Цикл
			Если СтрокаРасхождений.СуммаДолгПартнера = 0 И СтрокаРасхождений.СуммаНашДолг = 0  ИЛИ СтрокаРасхождений.СуммаДолгПартнера = СтрокаРасхождений.СуммаНашДолг Тогда
...
Функция ДетальныеРасчеты(ДанныеДокумента)
	...
	|	ВЫБОР КОГДА ДокументыИнтеркампани.Документ ЕСТЬ NULL ТОГДА
	|		ВЫБОР
	|			КОГДА ТИПЗНАЧЕНИЯ(РасчетыСКлиентамиОстаткиИОбороты.Регистратор) = ТИП(Документ.АктВыполненныхРабот)
	|					И ВЫРАЗИТЬ(РасчетыСКлиентамиОстаткиИОбороты.Регистратор КАК Документ.АктВыполненныхРабот).СпособВыпискиАктовВыполненныхРабот В
	|																					(ЗНАЧЕНИЕ(Перечисление.СпособыВыпискиАктовВыполненныхРабот.НаПорталеГосЗакупа),
	|																					ЗНАЧЕНИЕ(Перечисление.СпособыВыпискиАктовВыполненныхРабот.НаПорталеИСЭСФ)) ТОГДА
	|				
	|				ВЫРАЗИТЬ(РасчетыСКлиентамиОстаткиИОбороты.Регистратор КАК Документ.АктВыполненныхРабот).ДатаПодписанияГЗ
	|				
	|			КОГДА ТИПЗНАЧЕНИЯ(РасчетыСКлиентамиОстаткиИОбороты.Регистратор) = ТИП(Документ.РеализацияУслугПрочихАктивов)
	|					И ВЫРАЗИТЬ(РасчетыСКлиентамиОстаткиИОбороты.Регистратор КАК Документ.РеализацияУслугПрочихАктивов).СпособВыпискиАктовВыполненныхРабот В
	|																					(ЗНАЧЕНИЕ(Перечисление.СпособыВыпискиАктовВыполненныхРабот.НаПорталеГосЗакупа),
	|																					ЗНАЧЕНИЕ(Перечисление.СпособыВыпискиАктовВыполненныхРабот.НаПорталеИСЭСФ)) ТОГДА
	|				
	|				ВЫРАЗИТЬ(РасчетыСКлиентамиОстаткиИОбороты.Регистратор КАК Документ.РеализацияУслугПрочихАктивов).ДатаПодписанияГЗ
	|				
	|			ИНАЧЕ
