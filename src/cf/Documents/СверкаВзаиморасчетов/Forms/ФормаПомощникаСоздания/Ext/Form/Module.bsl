...
Процедура РасчетыСПартнерами(Команда)
	...
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СтраницаНастройкаФильтров Тогда
		...
		// ERP К - не переносить при обновлении
		ПоляОтбораОтчета = "Организация, Партнер, Контрагент, Договор, СегментПартнеров";
		Для Каждого ЭлементОтбора Из КомпоновщикОтбор.Настройки.Отбор.Элементы Цикл
			// ERP К - не переносить при обновлении
			Если СтрНайти(ПоляОтбораОтчета, Строка(ЭлементОтбора.ЛевоеЗначение)) <> 0 Тогда
				...
				// ERP К - не переносить при обновлении
				ДобавитьВОтборЭлементОтбора(ФиксированныеНастройки.Отбор, ПользНастройкаОтбора,ЭлементОтбора);
...
&НаКлиенте
Процедура ДобавитьВОтборЭлементОтбора(ОтборПриемник,ПользНастройкаОтбора,ЭлементОтбора)
	НовыйЭлемент = ОтборПриемник.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЗаполнитьЗначенияСвойств(НовыйЭлемент,ПользНастройкаОтбора);
