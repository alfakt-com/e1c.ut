...
Функция НастройкаОтборовНаУзле(ИдентификаторНастройки)
	...
	//Возможно необходимо доработать выбор форматов
	...
	Если ИдентификаторНастройки <> "ОбменУП2ЗУП3" Тогда
		СтруктураНастроек.Вставить("РазделыУчета", ПолучитьРазделыУчета(Истина, ИдентификаторНастройки));
	КонецЕсли;
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если Не ИдентификаторНастройки = "ОбменУП2ЗУП3" Тогда
		...
		КонецЕсли;
		ТекстОписания = ТекстОписания + ";";
		Если НастройкаОтборовНаУзле.РазделыУчета.Выгружать.Найти(Ложь) = Неопределено Тогда
			ОграничениеОтборПоРазделам = Символы.ПС + НСтр("ru = 'Выгружать данные по всем разделам учета;'");
		Иначе
			Индекс = 0;
	...
	КонецЕсли;
	ТекстОписания = ТекстОписания + ОграничениеОтборПоРазделам;
...
Процедура ПриПолученииОписанияВариантаНастройки(ОписаниеВарианта, ИдентификаторНастройки, ПараметрыКонтекста) Экспорт
	...
	Если ИдентификаторНастройки = "ОбменУПБП" 
		...
		ОписаниеВарианта.Вставить("ЗаголовокКомандыДляСозданияНовогоОбменаДанными", НСтр("ru = 'Бухгалтерия для Казахстана, редакция 3.0'"));
		ОписаниеВарианта.Вставить("ЗаголовокПомощникаСозданияОбмена",               НСтр("ru = 'Настройка синхронизации с программой ""Бухгалтерия для Казахстана, редакция 3.0""'"));
		ОписаниеВарианта.Вставить("ЗаголовокУзлаПланаОбмена",                       НСтр("ru = 'Синхронизация с программой ""Бухгалтерия для Казахстана, редакция 3.0""'"));
		ОписаниеВарианта.Вставить("НаименованиеКонфигурацииКорреспондента",         НСтр("ru = 'Бухгалтерия для Казахстана, редакция 3.0'"));
...
Функция ПолучитьПредставлениеКонфигурацииКорреспондента(КраткоеИмяКонфигурации) Экспорт
	...
	ИначеЕсли КраткоеИмяКонфигурации = "УправлениеНебольшойФирмойДляКазахстана" Тогда
		Возврат НСтр("ru = 'Управление небольшой фирмой для Казахстана, ред.1.5'");
	Иначе
		Возврат "";
	КонецЕсли;
...
Функция ПереходНаСинхронизациюЧерезУниверсальныйФорматИнтернет(КодУзла, Ошибка) Экспорт
	...
	Если Метаданные.ПланыОбмена.Найти("ОбменУправлениеТорговлейБухгалтерияПредприятия30") <> Неопределено Тогда
