...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ИСТИНА
	|			ТОГДА Движения.АналитикаУчетаПартийПроизводства
	|		ИНАЧЕ Движения.АналитикаУчетаПартийПроизводства
	|	КОНЕЦ АналитикаУчетаПартийПроизводства,
	|	(ВЫБОР
	...
	ЗапросТипов.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
	
	ЗапросТипов.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТИПЗНАЧЕНИЯ(ВТДляОбработкиСебестоимостьТоваров.Регистратор) КАК ТипРегистратора
		|ИЗ
		|	ВТДляОбработкиСебестоимостьТоваров КАК ВТДляОбработкиСебестоимостьТоваров";
	
	ВыборкаПоТипам = ЗапросТипов.Выполнить().Выбрать();
	
	Пока ВыборкаПоТипам.Следующий() Цикл
		
		ИмяТипаОбъекта = XMLТип(ВыборкаПоТипам.ТипРегистратора).ИмяТипа;
		ПолноеИмяДокумента = "Документ." + Сред(ИмяТипаОбъекта, Найти(ИмяТипаОбъекта, ".") +1);
