...
Функция get_placeПолучитьСклад(Запрос)
	Ответ = Новый HTTPСервисОтвет(200);
	стр = Запрос.ПараметрыЗапроса.Получить("str");
	ID = Запрос.ПараметрыЗапроса.Получить("chat_id");	
	TelegramID = Справочники.ЦС_TelegramID.НайтиПоРеквизиту("TelegramID", ID);
	
	Если TelegramID = Справочники.ЦС_TelegramID.ПустаяСсылка() тогда	
		струк= новый структура;
		струк.Вставить("success", Ложь);
		струк.Вставить("active", Ложь);
		струк.Вставить("manager_phone", Константы.ЦС_ТелефонМенеджера.Получить());
		ТекстОтвета = ЗаписатьJSON1(струк);		
		Ответ.УстановитьТелоИзСтроки(ТекстОтвета);
		Возврат Ответ;
	КонецЕсли;
	
	Если TelegramID.ПоказатьВсеСклады тогда	
		
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	СвободныеОстаткиОстатки.Склад.Ссылка КАК СкладСсылка,
		|	СвободныеОстаткиОстатки.Склад.Наименование КАК place
		|ИЗ
		...
		|						ГДЕ
		|							(Номенклатура.Артикул ПОДОБНО ""%"" + &стр + ""%""
		|								ИЛИ Номенклатура.Код ПОДОБНО ""%"" + &стр + ""%""
		|								ИЛИ Номенклатура.Наименование ПОДОБНО ""%"" + &стр + ""%""
		|								ИЛИ Номенклатура.НаименованиеПолное ПОДОБНО ""%"" + &стр + ""%"")
		|			
		|						ОБЪЕДИНИТЬ ВСЕ
		|			
		|						ВЫБРАТЬ
		...
		|						ГДЕ
		|							ШтрихкодыНоменклатуры.Штрихкод ПОДОБНО ""%"" + &стр + ""%"") КАК ВЗ
		|					СГРУППИРОВАТЬ ПО
		|						ВЗ.Ссылка)
		|				И Склад В
		|					(ВЫБРАТЬ
		|						ЦС_СкладыДляБота.Склады
		|					ИЗ
		|						РегистрСведений.ЦС_СкладыДляБота КАК ЦС_СкладыДляБота)) КАК СвободныеОстаткиОстатки";
	...
	иначе		
		...
		|						ЦС_СкладыДляБота.Склады
		|					ИЗ
		|						РегистрСведений.ЦС_СкладыДляБота КАК ЦС_СкладыДляБота
		|					ГДЕ
		|						ЦС_СкладыДляБота.Местонахождение = &Местонахождение)) КАК СвободныеОстаткиОстатки";
		
		Запрос.УстановитьПараметр("стр", стр);
		Запрос.УстановитьПараметр("Местонахождение", TelegramID.Местонахождение);		
		РезультатЗапроса = Запрос.Выполнить().Выгрузить();
	...
	ТаблицаСкладов.Колонки.Добавить("place");
	ТаблицаСкладов.Колонки.Добавить("uuid");
	
	Для каждого Строка из РезультатЗапроса цикл
		СтрокаТаблицы = ТаблицаСкладов.Добавить();	
		СтрокаТаблицы.place = Строка.place;
		СтрокаТаблицы.uuid = Строка.СкладСсылка.УникальныйИдентификатор();	
	КонецЦикла;
	
	струк= новый структура("data", ТаблицаСкладов) ;
	струк.Вставить("success", Истина);
	струк.Вставить("active", TelegramID.Активен);
	струк.Вставить("manager_phone", TelegramID.ТелефонМенеджера);
...
Функция getПолучитьНомен(Запр)
	Ответ = Новый HTTPСервисОтвет(200);
	стр = Запр.ПараметрыЗапроса.Получить("str");
	склад = Запр.ПараметрыЗапроса.Получить("place");
	ID = Запр.ПараметрыЗапроса.Получить("chat_id");	
	success = 1;
	TelegramID = Справочники.ЦС_TelegramID.НайтиПоРеквизиту("TelegramID", ID);
	...
	НовыйGUID = Новый УникальныйИдентификатор(склад);
	СсылкаСклад = Справочники.Склады.ПолучитьСсылку(НовыйGUID);
	...
	|	вз.Склад КАК Склад,
	|	МАКСИМУМ(вз.Цена) КАК Цена,
	|	вз.Тип КАК Тип,
	|	СУММА(вз.ВРезерве) КАК ВРезерве,
	|	СУММА(вз.КоличествоДляПокупки) КАК КоличествоДляПокупки,
	|	вз.ДокументПоступленияЖелаемаяДатаПоступления КАК ДокументПоступленияЖелаемаяДатаПоступления,
	|	СУММА(вз.КПоступлениюПриход) КАК КПоступлениюПриход,
	|	вз.Номенклатура КАК Номенклатура
	|ИЗ
	|	(ВЫБРАТЬ
	|		СвободныеОстаткиОстатки.Номенклатура.Наименование КАК Товар,
	|		СвободныеОстаткиОстатки.ВНаличииОстаток КАК Количество,
	|		СвободныеОстаткиОстатки.Номенклатура.ЕдиницаИзмерения.Наименование КАК ЕдиницаИзмерения,
	|		СвободныеОстаткиОстатки.Склад.Наименование КАК Склад,
	|		ЦеныНоменклатурыСрезПоследних.Цена КАК Цена,
	|		ЦеныНоменклатурыСрезПоследних.ВидЦены.Наименование КАК Тип,
	|		СвободныеОстаткиОстатки.ВРезервеСоСкладаОстаток КАК ВРезерве,
	|		СвободныеОстаткиОстатки.ВНаличииОстаток - СвободныеОстаткиОстатки.ВРезервеСоСкладаОстаток КАК КоличествоДляПокупки,
	|		НЕОПРЕДЕЛЕНО КАК ДокументПоступленияЖелаемаяДатаПоступления,
	|		0 КАК КПоступлениюПриход,
	...
	|							ШтрихкодыНоменклатуры.Штрихкод ПОДОБНО ""%"" + &стр + ""%"") КАК ВЗ
	|					СГРУППИРОВАТЬ ПО
	|						ВЗ.Ссылка)) КАК СвободныеОстаткиОстатки
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(
	|					&ТекДата,
	|					ВидЦены В
	|						(ВЫБРАТЬ
	|							ЦС_ТипыЦенДляБота.ТипЦен
	|						ИЗ
	|							РегистрСведений.ЦС_ТипыЦенДляБота КАК ЦС_ТипыЦенДляБота)) КАК ЦеныНоменклатурыСрезПоследних
	...
	|		СвободныеОстаткиОстатки.Номенклатура.ЕдиницаИзмерения.Наименование,
	...
	|		СвободныеОстаткиОстатки.ВНаличииОстаток - СвободныеОстаткиОстатки.ВРезервеСоСкладаОстаток,
	|		СвободныеОстаткиОстатки.Номенклатура
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		ТоварыКПоступлениюОстаткиИОбороты.Номенклатура.Наименование,
	|		0,
	|		ТоварыКПоступлениюОстаткиИОбороты.Номенклатура.ЕдиницаИзмерения.Наименование,
	|		ТоварыКПоступлениюОстаткиИОбороты.Склад.Наименование,
	|		ЦеныНоменклатурыСрезПоследних.Цена,
	|		ЦеныНоменклатурыСрезПоследних.ВидЦены.Наименование,
	|		0,
	|		0,
	|		ТоварыКПоступлениюОстаткиИОбороты.ДокументПоступления.ЖелаемаяДатаПоступления,
	|		ТоварыКПоступлениюОстаткиИОбороты.КОформлениюОрдеровПриход,
	...
	|							ШтрихкодыНоменклатуры.Штрихкод ПОДОБНО ""%"" + &стр + ""%"") КАК ВЗ
	|					СГРУППИРОВАТЬ ПО
	|						ВЗ.Ссылка)) КАК ТоварыКПоступлениюОстаткиИОбороты
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(
	|					&ТекДата,
	|					ВидЦены В
	|						(ВЫБРАТЬ
	...
	|						ИЗ
	|							РегистрСведений.ЦС_ТипыЦенДляБота КАК ЦС_ТипыЦенДляБота)) КАК ЦеныНоменклатурыСрезПоследних
	|			ПО ТоварыКПоступлениюОстаткиИОбороты.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура
	|	ГДЕ
	|		ТоварыКПоступлениюОстаткиИОбороты.Склад = &Склад
	|		И ТоварыКПоступлениюОстаткиИОбороты.ДокументПоступления.ЖелаемаяДатаПоступления >= &ЖелаемаяДатаПоступления) КАК вз
	|
	|СГРУППИРОВАТЬ ПО
	|	вз.ЕдиницаИзмерения,
	|	вз.Товар,
	|	вз.Склад,
	|	вз.ДокументПоступленияЖелаемаяДатаПоступления,
	|	вз.Тип,
	|	вз.Номенклатура
	|ИТОГИ
	|	МАКСИМУМ(Количество),
	|	МАКСИМУМ(Цена),
	|	МАКСИМУМ(ВРезерве),
	|	МАКСИМУМ(КоличествоДляПокупки),
	|	МАКСИМУМ(КПоступлениюПриход)
	|ПО
	|	Товар,
	|	ЕдиницаИзмерения,
	|	ДокументПоступленияЖелаемаяДатаПоступления";
	Запрос.УстановитьПараметр("ЖелаемаяДатаПоступления", НачалоДня(ТекущаяДата()));	
	Запрос.УстановитьПараметр("стр", стр);
	Запрос.УстановитьПараметр("Склад", СсылкаСклад);
	Запрос.УстановитьПараметр("ТекДата", КонецДня(ТекущаяДата()));
	РезультатЗапроса = Запрос.Выполнить();	
	ТаблицаМакета = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	
	ТабДок = Новый ТабличныйДокумент;	
	Макет = ПолучитьОбщийМакет("ЦС_Telegram");	
	Склад = СсылкаСклад.Наименование;
	...
	Пока ТаблицаМакета.Следующий() Цикл	
		
		ТаблицаЕд = ТаблицаМакета.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);

		Пока ТаблицаЕд.Следующий() Цикл
			
			
			Детали.Параметры.Заполнить(ТаблицаЕд);
			Табдок.Вывести(Детали);	
			
			ТоварыС = Новый структура();
			ТоварыС.Вставить("Товар", ТаблицаЕд.Товар);
			ТоварыС.Вставить("ЕдиницаИзмерения", ТаблицаЕд.ЕдиницаИзмерения);
			ТоварыС.Вставить("Количество", ТаблицаЕд.Количество);
			ТоварыС.Вставить("Резерв", ТаблицаЕд.ВРезерве);	
			ТоварыС.Вставить("Итого", ТаблицаЕд.КоличествоДляПокупки);
			
						
			ВыборкаА = ТаблицаЕд.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			
			МассивДопДанных = Новый Массив;
			Пока ВыборкаА.Следующий() Цикл
				Строки = Новый структура();
				//Строки.Вставить("ТипЦен", ВыборкаА.Тип);
				//Строки.Вставить("Цена", ВыборкаА.Цена);		
				
			Если ВыборкаА.ДокументПоступленияЖелаемаяДатаПоступления <> НЕОПРЕДЕЛЕНО тогда 
				Строки.Вставить("К_поступлению", ВыборкаА.КПоступлениюПриход);
				Строки.Вставить("Дата_поступления", Формат(ВыборкаА.ДокументПоступленияЖелаемаяДатаПоступления, "ДФ=dd.MM.yyyy"));
				МассивДопДанных.Добавить(Строки);	
				ТоварыС.Вставить("Строки", МассивДопДанных);	
			КонецЕсли;
			
			
			
			ВыборкаБ = ВыборкаА.Выбрать();
			МассивДопДанных2 = Новый Массив;
			Пока ВыборкаБ.Следующий() Цикл
				Строки2 = Новый структура();
				Строки2.Вставить("ТипЦен", ВыборкаБ.Тип);
				
				//Ахметов Ержан 20092022++
				Если ВыборкаБ.Тип = "Розничная" Тогда
					ЦенаТовара=ПолучитьЦены(ВыборкаБ.номенклатура);
					Если ЦенаТовара = Неопределено Тогда
						Строки2.Вставить("Цена", ВыборкаБ.Цена);
					Иначе
						Строки2.Вставить("Цена", ЦенаТовара);
					КонецЕсли;
				...
				МассивДопДанных2.Добавить(Строки2);	
				ТоварыС.Вставить("Строки2", МассивДопДанных2);		
				//для получения картинки последнего товара
				Номенклатура = ВыборкаБ.номенклатура;
			КонецЦикла;	
		...
		МассивТоваров.Добавить(ТоварыС); 
	КонецЦикла;
	...
	ВФ = ПолучитьИмяВременногоФайла("xls");	
	ТабДок.Записать(ВФ,ТипФайлаТабличногоДокумента.XLS);	
	ДД = Новый ДвоичныеДанные(ВФ);	
	Б64 = Base64Строка(ДД);	
	
	струк= новый структура("data", МассивТоваров) ;
	струк.Вставить("success", 1);
	струк.Вставить("active", TelegramID.Активен);
	струк.Вставить("manager_phone", TelegramID.ТелефонМенеджера);
	
	Если  TelegramID.Активен = Истина тогда
		струк.Вставить("excel", Б64);
		
		Если ЗначениеЗаполнено(Номенклатура) Тогда
			СтрРег = РегистрыСведений.ДвоичныеДанныеФайлов.Получить(Новый Структура("Файл",Номенклатура.ФайлКартинки));
			струк.Вставить("image", СтрРег.ДвоичныеДанныеФайла);
		КонецЕсли;
	КонецЕсли;
...
//Олжас Апиев 171022++
Функция ПолучитьЦены(НоменклатураТовар)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	ЦеныНоменклатурыСрезПоследних.Цена КАК Цена,
	                      |	ЕСТЬNULL(ВЫБОР
	                      |			КОГДА bon_СкидкиНаТовар.ВидСкидкиНаценкиТовара = ЗНАЧЕНИЕ(Перечисление.bon_ВидСкидкиНаценкиТовара.Процент)
	                      |				ТОГДА ВЫРАЗИТЬ(ЦеныНоменклатурыСрезПоследних.Цена - ЦеныНоменклатурыСрезПоследних.Цена * bon_СкидкиНаТовар.Процент / 100 КАК ЧИСЛО(10, 0))
	                      |			ИНАЧЕ bon_СкидкиНаТовар.СпецЦена
	                      |		КОНЕЦ, 0) КАК СпецЦена,
	                      |	ЕСТЬNULL(bon_СкидкиНаТовар.Приоритет, 0) КАК Приоритет
	                      |ПОМЕСТИТЬ ЦеныБезОптовых
	                      |ИЗ
	                      |	РегистрСведений.ЦеныНоменклатуры.СрезПоследних(
	                      |			&НаДату,
	                      |			ВидЦены = &ВидЦены
	                      |				И Номенклатура В ИЕРАРХИИ (&Товары)
	                      |				И Цена <> 0
	                      |				И Цена = (ВЫРАЗИТЬ(Цена КАК ЧИСЛО(10, 0)))) КАК ЦеныНоменклатурыСрезПоследних
	                      |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.bon_СкидкиНаТовар КАК bon_СкидкиНаТовар
	                      |		ПО (ВЫБОР
	                      |					КОГДА bon_СкидкиНаТовар.ЭтоНомГруппа
	                      |						ТОГДА bon_СкидкиНаТовар.НомГруппа = (ВЫРАЗИТЬ(ЦеныНоменклатурыСрезПоследних.Номенклатура.Наименование КАК СТРОКА(10)))
	                      |					ИНАЧЕ ЦеныНоменклатурыСрезПоследних.Номенклатура = bon_СкидкиНаТовар.Номенклотура
	                      |				КОНЕЦ
	                      |				ИЛИ ВЫБОР
	                      |					КОГДА bon_СкидкиНаТовар.ЭтоНомГруппа
	                      |						ТОГДА bon_СкидкиНаТовар.НомГруппа = (ВЫРАЗИТЬ(ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Наименование КАК СТРОКА(10)))
	                      |					ИНАЧЕ ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель = bon_СкидкиНаТовар.Номенклотура
	                      |				КОНЕЦ
	                      |				ИЛИ ВЫБОР
	                      |					КОГДА bon_СкидкиНаТовар.ЭтоНомГруппа
	                      |						ТОГДА bon_СкидкиНаТовар.НомГруппа = (ВЫРАЗИТЬ(ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель.Наименование КАК СТРОКА(10)))
	                      |					ИНАЧЕ ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель = bon_СкидкиНаТовар.Номенклотура
	                      |				КОНЕЦ
	                      |				ИЛИ ВЫБОР
	                      |					КОГДА bon_СкидкиНаТовар.ЭтоНомГруппа
	                      |						ТОГДА bon_СкидкиНаТовар.НомГруппа = (ВЫРАЗИТЬ(ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель.Родитель.Наименование КАК СТРОКА(10)))
	                      |					ИНАЧЕ ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель.Родитель = bon_СкидкиНаТовар.Номенклотура
	                      |				КОНЕЦ
	                      |				ИЛИ ВЫБОР
	                      |					КОГДА bon_СкидкиНаТовар.ЭтоНомГруппа
	                      |						ТОГДА bon_СкидкиНаТовар.НомГруппа = (ВЫРАЗИТЬ(ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель.Родитель.Родитель.Наименование КАК СТРОКА(10)))
	                      |					ИНАЧЕ ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель.Родитель.Родитель = bon_СкидкиНаТовар.Номенклотура
	                      |				КОНЕЦ
	                      |				ИЛИ ВЫБОР
	                      |					КОГДА bon_СкидкиНаТовар.ЭтоНомГруппа
	                      |						ТОГДА bon_СкидкиНаТовар.НомГруппа = (ВЫРАЗИТЬ(ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель.Родитель.Родитель.Родитель.Наименование КАК СТРОКА(10)))
	                      |					ИНАЧЕ ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель.Родитель.Родитель.Родитель = bon_СкидкиНаТовар.Номенклотура
	                      |				КОНЕЦ
	                      |				ИЛИ ВЫБОР
	                      |					КОГДА bon_СкидкиНаТовар.ЭтоНомГруппа
	                      |						ТОГДА bon_СкидкиНаТовар.НомГруппа = (ВЫРАЗИТЬ(ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель.Родитель.Родитель.Родитель.Родитель.Наименование КАК СТРОКА(10)))
	                      |					ИНАЧЕ ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель.Родитель.Родитель.Родитель.Родитель = bon_СкидкиНаТовар.Номенклотура
	                      |				КОНЕЦ
	                      |				ИЛИ ВЫБОР
	                      |					КОГДА bon_СкидкиНаТовар.ЭтоНомГруппа
	                      |						ТОГДА bon_СкидкиНаТовар.НомГруппа = (ВЫРАЗИТЬ(ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель.Родитель.Родитель.Родитель.Родитель.Родитель.Наименование КАК СТРОКА(10)))
	                      |					ИНАЧЕ ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель.Родитель.Родитель.Родитель.Родитель.Родитель = bon_СкидкиНаТовар.Номенклотура
	                      |				КОНЕЦ
	                      |				ИЛИ ВЫБОР
	                      |					КОГДА bon_СкидкиНаТовар.ЭтоНомГруппа
	                      |						ТОГДА bon_СкидкиНаТовар.НомГруппа = (ВЫРАЗИТЬ(ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель.Родитель.Родитель.Родитель.Родитель.Родитель.Родитель.Наименование КАК СТРОКА(10)))
	                      |					ИНАЧЕ ЦеныНоменклатурыСрезПоследних.Номенклатура.Родитель.Родитель.Родитель.Родитель.Родитель.Родитель.Родитель.Родитель = bon_СкидкиНаТовар.Номенклотура
	                      |				КОНЕЦ)
	                      |			И (НЕ bon_СкидкиНаТовар.Номенклотура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
	                      |			И (bon_СкидкиНаТовар.ВидЦены = &ВидЦены)
	                      |			И (bon_СкидкиНаТовар.Подразделение = &Подразделение)
	                      |			И (bon_СкидкиНаТовар.ДатаНач <= &НаДату)
	                      |			И (bon_СкидкиНаТовар.ДатаКон >= &НаДату)
	                      |
	                      |ОБЪЕДИНИТЬ
	                      |
	                      |ВЫБРАТЬ
	                      ...
	                      |			ИНАЧЕ bon_СкидкиНаТовар.СпецЦена
	                      |		КОНЕЦ, 0),
	                      |	ЕСТЬNULL(bon_СкидкиНаТовар.Приоритет, 0)
	                      |ИЗ
	                      |	РегистрСведений.ЦеныНоменклатуры.СрезПоследних(
	                      |			&НаДату,
	                      |			ВидЦены = &ВидЦены
	                      ...
	                      |				И Цена <> 0
	                      |				И Цена = (ВЫРАЗИТЬ(Цена КАК ЧИСЛО(10, 0)))) КАК ЦеныНоменклатурыСрезПоследних
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.bon_СкидкиНаТовар КАК bon_СкидкиНаТовар
	                      |		ПО (ВЫБОР
	                      |					КОГДА bon_СкидкиНаТовар.ЭтоНомГруппа
	                      |						ТОГДА bon_СкидкиНаТовар.НомГруппа = (ВЫРАЗИТЬ(ЦеныНоменклатурыСрезПоследних.Номенклатура.Наименование КАК СТРОКА(10)))
	                      |					ИНАЧЕ ЦеныНоменклатурыСрезПоследних.Номенклатура = bon_СкидкиНаТовар.Номенклотура
	                      ...
	                      |////////////////////////////////////////////////////////////////////////////////
	                      |ВЫБРАТЬ
	                      |	ЦеныБезОптовых.Номенклатура КАК Номенклатура,
	                      |	ЦеныБезОптовых.Цена КАК Цена,
	                      |	ЦеныБезОптовых.СпецЦена КАК СпецЦена,
	                      |	ЦеныБезОптовых.Приоритет КАК Приоритет,
	                      |	ЕСТЬNULL(ЦеныНоменклатурыСрезПоследних.Цена, 0) КАК ОптоваяЦена
	                      |ИЗ
	                      |	ЦеныБезОптовых КАК ЦеныБезОптовых
	                      |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(
	                      |				&НаДату,
	                      ...
	                      |					И Цена <> 0
	                      |					И Цена = (ВЫРАЗИТЬ(Цена КАК ЧИСЛО(10, 0)))) КАК ЦеныНоменклатурыСрезПоследних
	                      |		ПО ЦеныБезОптовых.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура
	                      |
	                      |УПОРЯДОЧИТЬ ПО
	                      |	Номенклатура,
	                      |	Приоритет УБЫВ");
	Запрос.УстановитьПараметр("Товары",           	НоменклатураТовар);
	Запрос.УстановитьПараметр("ВидЦены",         	Константы.НормативныйВидЦен.Получить());
	Запрос.УстановитьПараметр("ВидЦеныРозничная", 	Справочники.ВидыЦен.НайтиПоНаименованию("Розничная", Истина));//В момент создание использовалось Розничная цена "Розничная"
	Запрос.УстановитьПараметр("НаДату",     		КонецДня(ТекущаяДата()));  //КонецДня(ТекущаяДата())
	
	Подразделение = Справочники.СтруктураПредприятия.НайтиПоКоду("02-000033"); //временно ориентир на Интернет магазин
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТекТовар = Выборка.Номенклатура Тогда Продолжить; КонецЕсли; //исключаем дубли
		
		текЦена =  ?(Выборка.СпецЦена>0,Выборка.СпецЦена,Неопределено);
		Прервать;
	КонецЦикла;
