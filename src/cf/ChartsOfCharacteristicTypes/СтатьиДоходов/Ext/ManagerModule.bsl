...
Процедура ЗаполнитьПредопределенныеСтатьиДоходов() Экспорт
	...
	МассивСтатейДоходов.Добавить(ПланыВидовХарактеристик.СтатьиДоходов.ДоходыОтВыбытияФА);
	...
	Для каждого Статья Из МассивСтатейДоходов Цикл
		...
		ИначеЕсли Статья = ПланыВидовХарактеристик.СтатьиДоходов.РеализацияОС Тогда
			...
			КонецЕсли;
		ИначеЕсли Статья = ПланыВидовХарактеристик.СтатьиДоходов.ДоходыОтВыбытияФА Тогда
			Если ПолучитьФункциональнуюОпцию("УправлениеТорговлей") Тогда
				СтатьиДоходовОбъект.Наименование = НСтр("ru = 'Доходы от превышения стоимости выбывших активов, над стоимостным балансом группы'");
				СтатьиДоходовОбъект.ТипЗначения = Новый ОписаниеТипов("СправочникСсылка.Организации");
			КонецЕсли;
		КонецЕсли;
